...
#define ELEVEN_GB (11*1024*1024UL)
...
#define ELEVEN_GB_PAGES (ELEVEN_GB >> (PAGE_SHIFT - 10))
...
static bool is_slab_leak(unsigned long *slab_unreclaim)
{
	...
	*slab_unreclaim = global_node_page_state(NR_SLAB_UNRECLAIMABLE);
	/*pages to KB*/
...
#endif
{
	...
	pr_info ("[mm momitor] into monitor_handler");
	...
	pr_info ("[mm momitor] out monitor_handler");
...
static void memory_leak_water_mark(void)
{
	...
	if (totalram_pages > ELEVEN_GB_PAGES)
		slab_unreclaim_limit = 4096*1024; //2.5G
	else if (totalram_pages > SEVEN_GB_PAGES)
		slab_unreclaim_limit = 3096*1024; //2G
	else if (totalram_pages > FIVE_GB_PAGES)
		slab_unreclaim_limit = 2563*1024; //2.5G
	else if (totalram_pages > THREE_GB_PAGES)
		slab_unreclaim_limit = 2048*1024; //2G
	else if (totalram_pages > TWO_GB_PAGES)
		slab_unreclaim_limit = 1200*1024; //1200MB
	else if (totalram_pages > ONE_GB_PAGES)
		slab_unreclaim_limit = 1024*1024; //1024MB

#ifdef CONFIG_DEBUG_KMEMLEAK
	if (kmemleak_skip_disable) {
		pr_info("kmemleak is enabled");
		slab_unreclaim_limit *= 5;
