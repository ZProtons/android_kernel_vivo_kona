...
static int msm_geni_serial_startup(struct uart_port *uport)
{
	...
	//vivo liupeng add for AT_MODE begin // @Mark A
	...
	//vivo liupeng add for AT_MODE end
...
#if defined(CONFIG_SERIAL_CORE_CONSOLE) || defined(CONFIG_CONSOLE_POLL)
//vivo liupeng add for AT_MODE begin // @Mark B
//warning: keeping @Mark A,then @Mark B must be retained !
//recommend: it is recommended to replace "__init" with "init" !
//vivo liupeng add for AT_MODE end
...
static int __init msm_geni_serial_init(void)
{
	...
	//vivo guojunzheng add for AT_MODE begin
	if (console_set_on_cmdline != 1) {
		pr_info("console not set cmdline\n");
		return ret;
	}
	//vivo guojunzheng add for AT_MODE end
