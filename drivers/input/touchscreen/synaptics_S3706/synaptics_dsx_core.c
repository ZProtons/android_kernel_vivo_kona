...
#define DO_FORCE_UPDATE 4

#define FP_DETECT_OPEN		1
#define FP_DETECT_CLOSE		0

//#define SYNA_SCREEN_CLOCK_ADDR 0x044D
...
struct syna_reg_addr {
	unsigned short syna_screen_clock_addr;
	unsigned short syna_scan_rate_addr;
	unsigned short syna_face_detect_addr;
	unsigned short s3706_tsin_ctr_addr;
	unsigned short syna_game_mode_addr;
	unsigned short syna_dclick_mode_addr;
};

struct syna_reg_addr reg_addr = {0x044D, 0x043E, 0x043F, 0x0438, 0x0441, 0x0426};
struct syna_reg_addr reg_addr_mp = {0x044A, 0x043E, 0x043F, 0x0438, 0x0441, 0x0426};
...
static ts_pin_config pin_config;
...
#if defined(CONFIG_SECURE_TOUCH)
#define SECURE_TOUCH_STOP_TIMEOUT		500
#define SECURE_TOUCH_INTERRUPT_TIMEOUT	500
...
static void synaptics_secure_touch_notify(struct synaptics_rmi4_data *rmi4_data)
{
	...
	touchscreen_secure_touch_notify();
}
	...
	if (atomic_read(&rmi4_data->st_enabled)) {
		...
		if (blocking) {
			ret = wait_for_completion_interruptible_timeout(
				&rmi4_data->st_powerdown, msecs_to_jiffies(SECURE_TOUCH_STOP_TIMEOUT));
			if (ret <= 0) {
				VTI("interrupt or timeout, ret = %d, ap need change i2c owner", ret);
	...
	struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (atomic_read(&rmi4_data->st_enabled)) {
		if (atomic_cmpxchg(&rmi4_data->st_pending_irqs, 0, 1) == 0) {
			...
			synaptics_secure_touch_notify(rmi4_data);
			ret = wait_for_completion_interruptible_timeout(
				&rmi4_data->st_irq_processed, msecs_to_jiffies(SECURE_TOUCH_INTERRUPT_TIMEOUT));
			if (ret <= 0) {
				VTI("interrupt or timeout, ret = %d, ap need to stop TUI", ret);
...
/*extern void mdss_dsi_panel_bklt_dcs_send(int level);*/
static int synaptics_rmi4_f12_gesture_point_report(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *fhandler)
{  
	int retval;
	unsigned char touch_count = 0; /* number of touch points */
	...
	static unsigned char objects_already_present_ges;
#endif
	...
	size_of_2d_data = sizeof(struct synaptics_rmi4_f12_finger_data);
 	...
 	if (extra_data->data15_size) {
		...
		/*dev_dbg(rmi4_data->pdev->dev.parent, */
		VTI("Number of fingers to process = %d",
				fingers_to_process);
	}


#ifdef F12_DATA_15_WORKAROUND
	fingers_to_process = max(fingers_to_process, objects_already_present_ges);
#endif
	if (!fingers_to_process) {
		vivoTsReleasePoints();
		return 0;
	}
	x = data_addr + extra_data->data1_offset;
	y = fingers_to_process * size_of_2d_data;
	VTI("addr = 0x%x, len = %d, finger = %d, fingers_to_process = %d", x, y, finger, fingers_to_process);
	...
	for (finger = 0; finger < fingers_to_process; finger++)
	{
...
#ifdef F12_DATA_15_WORKAROUND
		objects_already_present_ges = finger + 1;
#endif
		...
		VTI("gggg:finger = %d finger status = %d pos:x = %d y = %d wx = %d wy = %d", finger, finger_status, x, y, wx, wy);
		start_x= vivoTsGetVtsData()->screenClockRect->startPoint.pointX;
		start_y= vivoTsGetVtsData()->screenClockRect->startPoint.pointY;
		width = vivoTsGetVtsData()->screenClockRect->width;
		height = vivoTsGetVtsData()->screenClockRect->height;
		VTI("gggg:start_x = %d start_y  = %d  width = %d height = %d ", start_x, start_y, width, height);
		if((start_x > x || (x > (start_x + width))) || (start_y > y || (y > (start_y + height))))
		{
			finger_status = 0;//不在设定范围内，直接报一次UP 
		}
		switch (finger_status) {
		case F12_FINGER_STATUS:
			if ((x == 0) && (y == 0)) {
				VTI("x=0 y=0 exception");
				break;
			}
			VTI("report DN !!!!!!!");
			vivoTsInputReport(VTS_TOUCH_DOWN, finger, x, y, (wx + wy) >> 1);
			touch_count++;
			break;
		...
		default:
			VTI("report UP !!!!!!!");
			vivoTsInputReport(VTS_TOUCH_UP, finger, 0, 0, 0);
			break;
		}
	}
...
//extern int dsi_panel_update_hbm_aod_ud_fingerprint_for_tp(void);
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	if (rmi4_data->suspend && rmi4_data->enable_wakeup_gesture) {
		if (vivoTsGetVtsData()->supportsCreenclock) {
			if (vivoTsGetVtsData()->screenClockRect->enable) { //get point data in gesture
			   retval = synaptics_rmi4_f12_gesture_point_report(rmi4_data,fhandler);
			   VTI("gesture point report retval is  %d",retval);
		   } 
		}
		...
		VTI("gesture type:0x%04x", gesture_type);
		if (gesture_type == 0x0b || gesture_type == 0x08 || gesture_type == 0x07) {
			for (j = 0; j < 20; j++) {
				rmi4_data->buf_gesture[j] = 65535;
			}

			/* if (gesture_type == 0x07) { */
			/* 		for (j = 0; j < 13; j++) { */
			/*			retval =  synaptics_rmi4_reg_read(rmi4_data, 0x400i+j*2,&lsb_buf,1); */
			/*			synaptics_rmi4_reg_read(rmi4_data, 0x400i+j*2+1,&lsb_buf,1); */
			/*			VTI("xx point_data[%d] = %d, point_data[%d] = %d", j*2, lsb_buf, j*2+1,  msb_buf); */
			/*		} */
			/* } else { */

			/*read gesture point*/
			retval =  synaptics_rmi4_reg_read(rmi4_data, 0x400, point_data, 25);
			VTI("%d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d %d",
			...
			point_data[18], point_data[19], point_data[20], point_data[21], point_data[22], point_data[23], point_data[24]);

			for (j = 0; j < 13; j++) {
				lsb_buf = point_data[j*2];
				msb_buf = point_data[j*2+1];
				VTI("point_data[%d] = %d, point_data[%d] = %d", j*2, lsb_buf, j*2+1,  msb_buf);
				if (j < 12) {
					rmi4_data->buf_gesture[j] =  ((msb_buf << 8) | lsb_buf);
					if (rmi4_data->buf_gesture[j] == 0) {
						rmi4_data->buf_gesture[j] = 65535;
					}
					VTI("buf_gesture[%d] = %d", j, rmi4_data->buf_gesture[j]);
				} else {
					if (point_data[24] == 16) {
						gtype = 32;
						VTI("o dir is gtype = %d", gtype);
					} else if (point_data[24] == 32) {
						gtype = 16;
						VTI("o dir is gtype = %d", gtype);
					} else {
						gtype = point_data[24];
...
/*	}*/
			if (gesture_type == 0x0b || gesture_type == 0x08) {
				temp = rmi4_data->buf_gesture[10];
				rmi4_data->buf_gesture[10] = rmi4_data->buf_gesture[2];
				rmi4_data->buf_gesture[2] = temp;
				temp = rmi4_data->buf_gesture[11];
				rmi4_data->buf_gesture[11] = rmi4_data->buf_gesture[3];
				rmi4_data->buf_gesture[3] = temp;
			}
		}

		if (0x03 == gesture_type) {	/*dclick*/
			
			if(vivoTsGetVtsData()->hasScreenClock) {
				
				VTI("point_data[1] = %d, point_data[2] = %d, point_data[3] = %d, point_data[4] = %d",
					
				rmi4_data->gesture_detection[1], rmi4_data->gesture_detection[2],
				rmi4_data->gesture_detection[3], rmi4_data->gesture_detection[4]);
				x = (rmi4_data->gesture_detection[2]<<8) | (rmi4_data->gesture_detection[1]);
				y = (rmi4_data->gesture_detection[4]<<8) | (rmi4_data->gesture_detection[3]);
				VTI("X = %d, Y = %d", x, y);
				
				wx = vivoTsGetVtsData()->screenClockRect->startPoint.pointX;
				wy = vivoTsGetVtsData()->screenClockRect->startPoint.pointY;
				width = vivoTsGetVtsData()->screenClockRect->width;
				height = vivoTsGetVtsData()->screenClockRect->height;
				
				if((wx < x && (x < (wx + width))) && (wy < y && (y < (wy + height)))) {
					vivoScreenPointSync(x, y);
					vivoTsInputReport(VTS_GESTURE_EVENT, KEY_SCREEN_CLOCK_WAKE_UP, -1, -1, -1);
				} else {
		...
		if (0x0b == gesture_type) {	/*"c" e w m*/
			
			gesture_type2 = rmi4_data->gesture_detection[2];
			VTI("gesture type2:0x%x", gesture_type2);
		
			if (gesture_type2 == 0x63)
				vivoTsInputReport(VTS_GESTURE_EVENT, KEY_C, -1, -1, -1);
			if (gesture_type2 == 0x65)
				vivoTsInputReport(VTS_GESTURE_EVENT, KEY_E, -1, -1, -1);
			if (gesture_type2 == 0x6d)
				vivoTsInputReport(VTS_GESTURE_EVENT, KEY_M, -1, -1, -1);
			if (gesture_type2 == 0x77)
				vivoTsInputReport(VTS_GESTURE_EVENT, KEY_W, -1, -1, -1);
			if (gesture_type2 == 0x40)
				vivoTsInputReport(VTS_GESTURE_EVENT, KEY_A, -1, -1, -1);
			if (gesture_type2 == 0x66)
				vivoTsInputReport(VTS_GESTURE_EVENT, KEY_F, -1, -1, -1);
		}
		
		if (0x08 == gesture_type) {	/*o*/
			VTI("gesture o dir:0x%x", gtype);
			rmi4_data->buf_gesture[19] = gtype;
			vivoTsInputReport(VTS_GESTURE_EVENT, KEY_O, -1, -1, -1);
		}
		
		if (0x07 == gesture_type) {	/*up down left right*/
			VTI("gesture next type:%d", gtype);
			if (gtype == 0x01) {	/*right*/
		...
		if (0x0f == gesture_type) {	/* finger gesture down */
			if (atomic_read(&vivoTsGetVtsData()->fingerIcoState) && atomic_read(&rmi4_data->fp_report) == 0) {
				vivoTsInputReport(VTS_GESTURE_EVENT, 254, 1, -1, -1);
				/* call lcm HBM function */
				if (vivoTsGetVtsData()->exportSwitch & 0x01) {
					/*mdss_dsi_panel_bklt_dcs_send(1);*/
					//dsi_panel_update_hbm_aod_ud_fingerprint_for_tp();
					VTI("gesture mode gesture DOWN detect");
				}
				atomic_set(&rmi4_data->fp_report, 1);
			}
		}
		if (0x1f == gesture_type) {	/* finger gesture up*/
			if (atomic_read(&vivoTsGetVtsData()->fingerIcoState) && atomic_read(&rmi4_data->fp_report) == 1) {
				/* call lcm HBM function */
				if (vivoTsGetVtsData()->exportSwitch & 0x01) {
					/*mdss_dsi_panel_bklt_dcs_send(0);*/
					VTI("gesture mode gesture UP detect");
				}
				vivoTsInputReport(VTS_GESTURE_EVENT, 254, 0, -1, -1);
				atomic_set(&rmi4_data->fp_report, 0);
			}
		}
/*
...
*/
	...
	/* normal mode to read the finger gesture */
	if (vivoTsGetVtsData()->hasFingerGesture) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
			data_addr + extra_data->data4_offset,
			rmi4_data->gesture_detection,
			sizeof(rmi4_data->gesture_detection));
		if (retval < 0)
			return 0;
		gesture_type = rmi4_data->gesture_detection[0];
		VTD("normal mode gesture type:0x%x", gesture_type);
		if (0x0f == gesture_type) {	/* finger gesture down */
			VTI("normal mode gesture DOWN detect");
			/* call lcm HBM function */
			if (vivoTsGetVtsData()->exportSwitch & 0x01) {
				/*mdss_dsi_panel_bklt_dcs_send(1);*/
			}
		...
		if (0x1f == gesture_type) {	/* finger gesture up*/
			VTI("normal mode gesture UP detect");
			/* call lcm HBM function */
			if (vivoTsGetVtsData()->exportSwitch & 0x01) {
				/*mdss_dsi_panel_bklt_dcs_send(0);*/
			}
	...
	if (extra_data->data15_size) {
		...
		/*dev_dbg(rmi4_data->pdev->dev.parent, */
			VTD("Number of fingers to process = %d",
				fingers_to_process);
	}

	if (1) {
		ret = vivoTsCoverMute(fingers_to_process, 0); 	/*gtype == 0x33*/
		if (ret < 0) {
			if (-2 == ret) {
...
/*	mutex_lock(&(rmi4_data->rmi4_report_mutex)); */
	...
	for (finger = 0; finger < fingers_to_process; finger++) {
		...
		VTD("finger = %d finger status = %d pos:x = %d y = %d wx = %d wy = %d", finger, finger_status, x, y, wx, wy);
...
static int esd_int_flag;
static int last_esd_int_flag;
#define ESD_PROTECT_ADDR 0x0487	/*10 fw is 0x0472*/
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	unsigned short palm_addr = 0x401;
	unsigned short fd_addr = 0x403;
	unsigned char face_status = 0;
	unsigned char palm_status = 0;
#ifdef CONFIG_VIRTUAL_PROX_ON_TOUCH
	int pick[3] = {0};
#endif
	VTD("Function %02x reporting", fhandler->fn_number);

	switch (fhandler->fn_number) {
	case SYNAPTICS_RMI4_F01:
		VTI("f01 int get");
if (0) {
		VTI("f01 get last_flag:%d", last_esd_int_flag);
		if (last_esd_int_flag == 0) {
			ret = synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f51_cmd_base_addr, &device_status, sizeof(device_status));
			if (ret < 0) {
				VTI("0x046F read error");
				return;
			}
			device_status = device_status | 0x02;
			ret = synaptics_rmi4_reg_write(rmi4_data, rmi4_data->f51_cmd_base_addr, &device_status, sizeof(device_status));
			if (ret < 0) {
				VTI("0x046F write error");
				return;
			}
		} else if (last_esd_int_flag == 1) {
			last_esd_int_flag = 0;
		}

		ret = synaptics_rmi4_reg_read(rmi4_data, 0x0004, &device_status, sizeof(device_status));
		if (ret < 0) {
			VTI("read error");
			return;
		}
		if ((device_status&0x0f) == 0x09) {
			VTI("ESD int get, esd_int_flag = %d", esd_int_flag);
			esd_int_flag = 1;
			last_esd_int_flag = 1;
		}
}
	...
	case SYNAPTICS_RMI4_F51:
		VTI("SYNAPTICS_RMI4_F51");
		ret = synaptics_rmi4_reg_read(rmi4_data, palm_addr, &palm_status, sizeof(palm_status));
		if (ret < 0) {
			VTI("palm_addr read error");
			return;
		}
		if (palm_status != 0) {
			VTI("palm mode detect !");
			vivoTsGetVtsData()->largePressNum++;
			vivoTsCoverMute(0, 1);
		}

		#ifdef CONFIG_VIRTUAL_PROX_ON_TOUCH
		/*just for face detection*/
		ret = synaptics_rmi4_reg_read(rmi4_data, fd_addr, &face_status, sizeof(face_status));
		if (ret < 0) {
			VTI("fd_addr read error");
			return;
		}
		VTI("normal mode gesture type:0x%x", face_status);
		if (0x81 == face_status) { /* finger gesture down */
			VTI("--------------------------normal mode FD near is detect");
			pick[0] = 0;
			VTI("algo-prox report near event");
			vivo_ts_prox_report_values(pick);
		} else if (0x80 == face_status && !palm_status) {
			pick[0] = 1;
...
static int syna_mode_change(int which);
		...
		bool report)
{
	...
	if (status.unconfigured && !status.flash_prog) {
		VTI("spontaneous reset detected");
		VTI("spontaneous reset lock");
		mutex_lock(&(rmi4_data->vivo_mutex));
		retval = synaptics_rmi4_reinit_device(rmi4_data);
		if (retval < 0) {
			VTI("Failed to reinit device");
		}
		mutex_unlock(&(rmi4_data->vivo_mutex));
		VTI("spontaneous reset unlock");
		if ((atomic_read(&(vivoTsGetVtsData()->tsState)) != TOUCHSCREEN_NORMAL) && (vivoTsGetVtsData()->lcdRtState == 0)) {
...
extern int i2c_bus_state;

struct synaptics_rmi4_data *irq_rmi4_data;
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	irq_rmi4_data = rmi4_data;
	if (gpio_get_value(bdata->irq_gpio) != bdata->irq_on_state) {
		VTI("irq pull down!");
		/*enable_irq(irq_rmi4_data->irq);*/
		goto exit;
	}
...
static void sync_btm_handler(void *data)
{
	#if defined(CONFIG_SECURE_TOUCH)
		if (IRQ_HANDLED == synaptics_filter_interrupt(irq_rmi4_data))
			return;
	#endif

	synaptics_rmi4_sensor_report(irq_rmi4_data, true);
	/*enable_irq(irq_rmi4_data->irq);*/
}
		...
		bool enable, bool attn_only)
{
	...
	if (1) {
		...
		if (retval < 0) {
			VTE("Failed to read interrupt status");
			goto exit;
		}
	}
#if 0
...
#else
		VTI("int type:%ld", bdata->irq_flags);
		retval = vivoTsInterruptRegister(rmi4_data->irq, NULL, synaptics_rmi4_irq, 0x2002/* 2008 */, rmi4_data);
#endif
		if (retval < 0) {
...
static int synaptics_rmi4_set_gpio(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_gpio_setup(
			...
			true, 0, 0);
	if (retval < 0) {
		VTI("Failed to configure attention GPIO");
		goto err_gpio_irq;
	}

if (0) {
	if (bdata->power_gpio >= 0) {
		VTI("configure power GPIO and power on");
		retval = synaptics_rmi4_gpio_setup(
				bdata->power_gpio,
				true, 1, !bdata->power_on_state);
		if (retval < 0) {
			VTI("Failed to configure power GPIO");
			goto err_gpio_power;
		}
	}
}
	if (bdata->reset_gpio >= 0) {
		...
		if (retval < 0) {
			VTI("Failed to configure reset GPIO");
			goto err_gpio_reset;
		}
	}
...
static int syna_faceDetect_startEnd(int cmd);
static int synaptics_rmi4_reinit_device(struct synaptics_rmi4_data *rmi4_data)
{
	int retval;
	struct synaptics_rmi4_fn *fhandler;
	...
	if (vivo_ts_prox_ear_detect_get()) {
		syna_faceDetect_startEnd(1);
	}

exit:
	VTI("unlock rmi4_reset_mutex");
	mutex_unlock(&(rmi4_data->rmi4_reset_mutex));
	return retval;
}

static int synaptics_rmi4_reset_device_simple(struct synaptics_rmi4_data *rmi4_data,
		bool rebuild)
{
	int retval;
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
int s3706_idleEnableOrDisable(int state)
{
	int ret = 0;
	unsigned char device_ctrl = 0;
	if (g_rmi4_data != NULL) {
		ret = synaptics_rmi4_reg_read(g_rmi4_data,
				g_rmi4_data->f01_ctrl_base_addr,
				&device_ctrl,
				sizeof(device_ctrl));
		if (ret < 0) {
			VTE("get device ctrl bit fail");
			goto ERR;
		}
		if (state) {
			device_ctrl = device_ctrl & ~NO_SLEEP_ON;
			VTI("idle mode is enabled!");
		} else {
			device_ctrl = device_ctrl | NO_SLEEP_ON;
			VTI("idle mode is disabled!");
		}
		ret = synaptics_rmi4_reg_write(g_rmi4_data,
				g_rmi4_data->f01_ctrl_base_addr,
				&device_ctrl,
				sizeof(device_ctrl));
		if (ret < 0) {
			VTE("set device ctrl bit fail");
			goto ERR;
		}
		goto EXIT;
	}
...
void synaptics_3706_rmi4_new_function(struct synaptics_rmi4_exp_fn *exp_fn,
		bool insert)
{
	struct synaptics_rmi4_exp_fhandler *exp_fhandler;
...
EXPORT_SYMBOL(synaptics_3706_rmi4_new_function);

extern struct i2c_client *s3706_syna_client;
static struct platform_device *globe_pdev;

extern int s3706_bbk_syna_fwu_interface(const struct firmware *firmware);
static int bbk_fw_update(const struct firmware *firmware)
{
	int ret = 0;
	/*struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev); */

	/*synaptics_rmi4_irq_enable(rmi4_data, false, false); */
	ret = s3706_bbk_syna_fwu_interface(firmware);
	/*synaptics_rmi4_irq_enable(rmi4_data, true, false); */

	return ret;

}
extern int s3706_bbk_syna_get_config_id(void) ;
static int bbk_syna_fw_version(int which)
{
	unsigned int ver = 0;
	int ret = 0;
	unsigned char bl_info[4] = {0};
	struct synaptics_rmi4_data *rmi4_data;
	...
	if (CONFIG_VERSION == which) {
		if (0) {
			/*show PR id*/
			rmi4_data = platform_get_drvdata(globe_pdev);
			rmi = &(rmi4_data->rmi4_mod_info);

			ret = synaptics_rmi4_reg_read(rmi4_data,
				rmi4_data->f01_query_base_addr + F01_BUID_ID_OFFSET, rmi->build_id,
				sizeof(rmi->build_id));
			if (ret < 0) {
				VTI("read pr version fail.");
				return ret;
			}
			...
			ver = rmi4_data->firmware_id;
			VTI("PR id 0x%x", rmi4_data->firmware_id);
			ver = -500;


		/*read bootloader version as fw config version*/
		rmi4_data = platform_get_drvdata(globe_pdev);
		ret = synaptics_rmi4_reg_read(rmi4_data, 0x0028, bl_info, 4);
		if (ret < 0) {
			VTI("read bootloader version fail.");
			return ret;
		}
		return (unsigned int)((bl_info[0] << 24) | (bl_info[1] << 16) | (bl_info[2] << 8) | bl_info[3]);
				}
		ver = 0;
	}
	if (FW_VERSION == which) {
		ver = s3706_bbk_syna_get_config_id();
		if (ver < 0) {
...
static int syna_S3706_pinctrl_init(struct i2c_client *pdev)
{
	pin_config.pinctrl = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR_OR_NULL(pin_config.pinctrl)) {
		VTE("Failed to get pinctrl");
		return PTR_ERR(pin_config.pinctrl);
	}

	pin_config.pin_active =
		pinctrl_lookup_state(pin_config.pinctrl, "ts_int_active");
	if (IS_ERR_OR_NULL(pin_config.pin_active)) {
			VTE("Failed to look up active state");
		return PTR_ERR(pin_config.pin_active);
	}

	pin_config.pin_suspend =
		pinctrl_lookup_state(pin_config.pinctrl, "ts_int_suspend");
	if (IS_ERR_OR_NULL(pin_config.pin_suspend)) {
		VTE("Failed to look up suspend state");
		return PTR_ERR(pin_config.pin_suspend);
	}

	return 0;
}

static int syna_S3706_pin_ctr(struct pinctrl_state *pin_state)
{
	int rc = -1;
	if (!IS_ERR_OR_NULL(pin_state)) {
		rc = pinctrl_select_state(pin_config.pinctrl, pin_state);
...
static int synaptics_3706_set_power(struct synaptics_rmi4_data *rmi4_data, int on);
int S3706_power_reset(struct synaptics_rmi4_data *rmi4_data) {
	const struct synaptics_dsx_board_data *bdata = rmi4_data->hw_if->board_data;
	VTI("HW reset detected");
	if (hw_reset_count < 10) {
		if (bdata->reset_gpio >= 0) {
			gpio_set_value(bdata->reset_gpio, bdata->reset_on_state);
			msleep(bdata->reset_active_ms);
			gpio_set_value(bdata->reset_gpio, !bdata->reset_on_state);
...
int syna_screen_clock_rect_read(void)
{
	char tmpbuf[5];
	int ret ;
	
	ret = synaptics_rmi4_reg_read(g_rmi4_data, g_rmi4_data->reg_addr_ptr->syna_screen_clock_addr, &tmpbuf[0], 5);
	VTI("syna_screen_clock_addr data is :0x%2x, 0x%2x, 0x%2x, 0x%2x, 0x%2x", tmpbuf[0], tmpbuf[1], tmpbuf[2], tmpbuf[3], tmpbuf[4]);
	
    return 0;
}

static void syna_set_different_gesture(unsigned int data, enum gesture_class type, unsigned char *gesture_switch)
{
	unsigned char temp;
	temp = *gesture_switch;
...
extern int test_do_command(unsigned char command);

static syna_set_gesture_switch(void)
{
	int ret = 0;
	unsigned char gesture_switch;
	unsigned short addr = 0x1d;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	ret = synaptics_rmi4_reg_read(rmi4_data, addr, &gesture_switch, 1);
	if (ret < 0) {
		VTI("failed to read");
		goto END;
	}

	VTI("before write the gesture switch state is %d", gesture_switch);

	/*1 double click gesture*/
	if ((vivoTsGetVtsData()->hasScreenClock) || (vivoTsGetVtsData()->dClickSwitch)) {
		VTI("ScreenClock or DoubleClick is open");
		syna_set_different_gesture(1, double_click_gesture, &gesture_switch);
		
	} else {
		VTI("both ScreenCLock and DoubleClick closed");
		syna_set_different_gesture(0, double_click_gesture, &gesture_switch);
	}

	/*2 letter gesture*/
	if (!(vivoTsGetVtsData()->gestureSwitch & 0x04) && !(vivoTsGetVtsData()->gestureSwitch & 0x20) && !(vivoTsGetVtsData()->gestureSwitch & 0x10) && !(vivoTsGetVtsData()->gestureSwitch & 0x08)
				&& !(vivoTsGetVtsData()->gestureSwitch & 0x40) && !(vivoTsGetVtsData()->exportSwitch & 0x02) && !(vivoTsGetVtsData()->exportSwitch & 0x04)) {
		VTI("all letter gesture closed!!");
		syna_set_different_gesture(0, letter_gesture, &gesture_switch);
		
	} else {
		VTI("at least one letter is open!!!");
		syna_set_different_gesture(1, letter_gesture, &gesture_switch);
	}

	/*3 swipe gesture*/
	if ((vivoTsGetVtsData()->gestureSwitch & 0x01) == 0 && (vivoTsGetVtsData()->gestureSwitch & 0x02) == 0 && (vivoTsGetVtsData()->swipeSwitch) == 0) {
		VTI("all swipe gesture closed");
		syna_set_different_gesture(0, swipe_gesture, &gesture_switch);
	} 

	ret = synaptics_rmi4_reg_write(rmi4_data, addr, &gesture_switch, 1);
	if (ret < 0) {
		VTI("Failed to write to ic edge bit  %d", ret);
		goto END;
	}
	ret = test_do_command(DO_FORCE_UPDATE);
	if (ret < 0) {
		VTE("Failed to do force update");
		goto END;
	}
	ret = synaptics_rmi4_reg_read(rmi4_data, addr, &gesture_switch, 1);
	if (ret < 0) {
	...
	VTI("after write the gesture switch state is %d", gesture_switch);
END:	
	return ret;

}

static int syna_get_double_tap_area() {
	int ret;
	char data_base[8] = {};
	unsigned short addr = 0x19;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	u16 x1, y1, x2, y2;
	
	ret = synaptics_rmi4_reg_read(rmi4_data, addr, &data_base[0], 8);
	if (ret < 0) {
		VTI("failed to read");
		goto END;
	}
	x1 = data_base[0] | (u16)(data_base[1] << 8);
	y1 = data_base[2] | (u16)(data_base[3] << 8);
	x2 = data_base[4] | (u16)(data_base[5] << 8);
	y2 = data_base[6] | (u16)(data_base[7] << 8);
	VTI(" read info-----x1 = %d, y1 = %d, x2 = %d, y2 = %d", x1, y1, x2, y2);
END:
	return ret;

}

static int syna_dclick_longpress_mode_change(int dl_switch)
{
	unsigned char tmp;
	//unsigned short switch_addr = 0x426;
	int ret = 0;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);

	if (dl_switch == FP_DETECT_OPEN) {
		tmp = 129;
		ret = synaptics_rmi4_reg_write(rmi4_data, g_rmi4_data->reg_addr_ptr->syna_dclick_mode_addr, &tmp, sizeof(tmp));
		if (ret < 0) {
			VTE("Failed to write 129 to ic change_dclick_longpress_mode bit  %d", ret);
			return ret;
		}
	} else {
		tmp = 0;
		ret = synaptics_rmi4_reg_write(rmi4_data, g_rmi4_data->reg_addr_ptr->syna_dclick_mode_addr, &tmp, sizeof(tmp));
		if (ret < 0) {
			VTE("Failed to write 0 to ic change_dclick_longpress_mode bit  %d", ret);
			return ret;
		}
	}
...
static int syna_scan_rate_change (int rate);

static int syna_enter_game_mode(int mode);
static int syna_set_screen_clock_report_double_area(void);
...
static int syna_mode_change(int which)
{
	...
	if (which == TOUCHSCREEN_NORMAL) {
		...
		VTI("mode_change lock");
		mutex_lock(&(rmi4_data->vivo_mutex));

		if (last_ts_state == TOUCHSCREEN_SLEEP) {
			synaptics_3706_set_power(rmi4_data, 1);
			msleep(50);
		}
		...
		if (!vivo_ts_prox_ear_detect_get()) {
			synaptics_rmi4_reset_device_simple(rmi4_data, false);
		} else {
			VTI("FD is open, no reset just set configure");
			synaptics_rmi4_set_configured(rmi4_data);
			syna_dclick_longpress_mode_change(FP_DETECT_OPEN);
		}

		rmi4_data->fb_ready = true;
		rmi4_data->sensor_sleep = 0;
		if (vivo_ts_prox_ear_detect_get()) {
			syna_faceDetect_startEnd(1);
		}
		syna_enter_game_mode(-1);
		mutex_unlock(&(rmi4_data->vivo_mutex));
		VTI("mode_change unlock");
		VTI("change to normal mode end");
	}
	if (which == TOUCHSCREEN_GESTURE) {
		VTI("change to gesture mode");
		VTI("gesture_change lock");
		mutex_lock(&(rmi4_data->geture_mutex));
		if (last_ts_state == TOUCHSCREEN_SLEEP) {
			synaptics_3706_set_power(rmi4_data, 1);
			msleep(50);
			synaptics_rmi4_reset_device_simple(rmi4_data, false);
		}
		if (rmi4_data->fd_enable) {
			syna_faceDetect_startEnd(0);
		}
		...
		rmi4_data->sensor_sleep = 0;
		mutex_unlock(&(rmi4_data->geture_mutex));
		VTI("gesture_change unlock");
		atomic_set(&rmi4_data->fp_report, 0);
		/* close or open longpress gesture, avoid dclick no func*/
		if (0 == (vivoTsGetVtsData()->exportSwitch & 0x01)) {
			VTI("The fingerprint is closed, change scannning inerval to 30ms");
			ret = syna_scan_rate_change(3);
			if (ret < 0) {
				VTI("Failed to write change idle scannning inerval to 30ms  %d", ret);
				return ret;
			}
		} else {
			ret = syna_dclick_longpress_mode_change(FP_DETECT_OPEN);
			if (ret < 0) {
				return ret;
			}
		}

		syna_set_gesture_switch();
		
		if(vivoTsGetVtsData()->hasScreenClock)
			syna_set_screen_clock_report_double_area();
	...
	if (which == TOUCHSCREEN_SLEEP) {
		...
		if (last_ts_state == TOUCHSCREEN_SLEEP) {
			VTI("ts state has changed to sleep, do nothing!");
			return 0;
		}
		...
		rmi4_data->enable_wakeup_gesture = false;
		rmi4_data->suspend = false;
		ret = synaptics_rmi4_suspend(&rmi4_data->pdev->dev);
		rmi4_data->fb_ready = false;
		if (!ret)
			synaptics_3706_set_power(rmi4_data, 0);
		msleep(50);
		rmi4_data->sensor_sleep = 1;
...
static int syna_module_id(void)
{
...
static int syna_enter_game_mode(int mode)
{
	...
	ret = synaptics_rmi4_reg_read(rmi4_data, g_rmi4_data->reg_addr_ptr->syna_game_mode_addr, &temp, 1);
		if (ret < 0) {
		VTI("failed to read addr 0x%x", g_rmi4_data->reg_addr_ptr->syna_game_mode_addr);
		ret = -EINVAL;
		goto exit;
	}
	VTI("before write:addr[0x%x] data[0x%x]", g_rmi4_data->reg_addr_ptr->syna_game_mode_addr, temp);
		
	if (mode >= 0) {
		rmi4_data->game_mode = mode;
	}
	if (rmi4_data->game_mode == 0) {
		temp = temp & 0x7f;
	} else {
		temp = temp | 0x80;
	}
	
	ret = synaptics_rmi4_reg_write(rmi4_data, g_rmi4_data->reg_addr_ptr->syna_game_mode_addr, &temp, 1);
	if (ret < 0) {
		VTI("failed to read addr 0x%x, data 0x%x", g_rmi4_data->reg_addr_ptr->syna_game_mode_addr, temp);
		ret = -EINVAL;
		goto exit;
	}
	/*do force update*/
	/*VTI("after write do command DO_FORCE_UPDATE to execute scanning change!");
	ret = test_do_command(DO_FORCE_UPDATE);
	if (ret < 0) {
		VTE("Failed to do force update");
		ret = -EINVAL;
	...
	VTI("after write:addr[0x%x] data[0x%x]", g_rmi4_data->reg_addr_ptr->syna_game_mode_addr, temp);
	ret = 0;
...
static int syna_send_screen_report_cmd()
{
	char temp;
	int retval;
	temp = vivoTsGetVtsData()->screenClockRect->enable;
	retval = synaptics_rmi4_reg_write(g_rmi4_data, g_rmi4_data->reg_addr_ptr->syna_screen_clock_addr , &temp, 1);
	if(retval < 0){
		VTI("failed to write SCREEN REPORT TO IC");
		return -EINVAL;
	}
	return retval;
}

static int syna_send_screen_clock_edge()
{
	char tmpbuf[10] = {};
	int retval = 0;
	int y_max = 0;

	VTI("enter  write screenClockRect to IC ");
	
    if (1) {
		
		mutex_lock(&vivoTsGetVtsData()->screenClockRect->ClockRectMutex);
		tmpbuf[0] = vivoTsGetVtsData()->screenClockRect->enable;
		y_max = vivoTsGetVtsData()->screenClockRect->startPoint.pointY + vivoTsGetVtsData()->screenClockRect->height;
	
		if (y_max > vivoTsGetVtsData()->tsDimensionY) {
			y_max =  vivoTsGetVtsData()->tsDimensionY;
		}

		if (!vivoTsGetVtsData()->screenClockRect->width ||  !vivoTsGetVtsData()->screenClockRect->height){
			mutex_unlock(&vivoTsGetVtsData()->screenClockRect->ClockRectMutex);
			return retval;
		}

		tmpbuf[1] = y_max >> 8;
		tmpbuf[2] = (y_max & 0X000000FF);
		tmpbuf[3] = vivoTsGetVtsData()->screenClockRect->startPoint.pointY >> 8;
		tmpbuf[4] = vivoTsGetVtsData()->screenClockRect->startPoint.pointY & 0x00ff;
		mutex_unlock(&vivoTsGetVtsData()->screenClockRect->ClockRectMutex);
		
		retval = synaptics_rmi4_reg_write(g_rmi4_data, g_rmi4_data->reg_addr_ptr->syna_screen_clock_addr, &tmpbuf[0], 5);
		
	    if (retval < 0) {
			VTI("failed to write screenClockRect");
			return -EINVAL;
		}
   	}

	VTI("EXIT  write screenClockRect to IC ");
...
{
	int ret = 0;
	unsigned char clock_area[8] = {};

	if((vivoTsGetVtsData()->hasScreenClock) && (!vivoTsGetVtsData()->dClickSwitch)){

		mutex_lock(&vivoTsGetVtsData()->screenClockRect->ClockRectMutex);		
		VTI(" write info------pointX1:%d, pointY1:%d, pointX2:%d, pointY2:%d, witdh:%d, height:%d", vivoTsGetVtsData()->screenClockRect->startPoint.pointX, vivoTsGetVtsData()->screenClockRect->startPoint.pointY,
			vivoTsGetVtsData()->screenClockRect->startPoint.pointX + vivoTsGetVtsData()->screenClockRect->width, vivoTsGetVtsData()->screenClockRect->startPoint.pointY + vivoTsGetVtsData()->screenClockRect->height,
			vivoTsGetVtsData()->screenClockRect->width, vivoTsGetVtsData()->screenClockRect->height);
		...
		clock_area[0] = (vivoTsGetVtsData()->screenClockRect->startPoint.pointX & 0x00ff);
		clock_area[1] = (vivoTsGetVtsData()->screenClockRect->startPoint.pointX >> 8) & (0x00ff);
		clock_area[2] = (vivoTsGetVtsData()->screenClockRect->startPoint.pointY & 0x00ff);
		clock_area[3] = (vivoTsGetVtsData()->screenClockRect->startPoint.pointY >> 8) & (0x00ff);

		clock_area[4] = (vivoTsGetVtsData()->screenClockRect->startPoint.pointX + vivoTsGetVtsData()->screenClockRect->width) & 0x00ff;
		clock_area[5] = ((vivoTsGetVtsData()->screenClockRect->startPoint.pointX + vivoTsGetVtsData()->screenClockRect->width) >> 8) & (0x00ff);
		clock_area[6] = (vivoTsGetVtsData()->screenClockRect->startPoint.pointY + vivoTsGetVtsData()->screenClockRect->height) & (0x00ff);
		clock_area[7] = ((vivoTsGetVtsData()->screenClockRect->startPoint.pointY + vivoTsGetVtsData()->screenClockRect->height) >> 8) & (0x00ff);
		mutex_unlock(&vivoTsGetVtsData()->screenClockRect->ClockRectMutex);
		
		ret = synaptics_rmi4_reg_write(g_rmi4_data, 0x19, &clock_area[0], 8);
		if (ret < 0) {
			VTI("failed to write reg");
		}	
	}
	...
	syna_get_double_tap_area();
	return ret;
}


static int syna_process_by_package(unsigned char *package_name, int type)
{
	VTI("get app name:%s, type:%d", package_name, type);
	if (0 == type) {
		if (strcmp(package_name, "GAME_IN") == 0) {
			VTI("enter game mode");
			syna_enter_game_mode(1);
			s3706_idleEnableOrDisable(0);
		} else if (strcmp(package_name, "GAME_OUT") == 0) {
			VTI("out game mode");
			syna_enter_game_mode(0);
			s3706_idleEnableOrDisable(1);
		} else {
			VTI("invalid app name");
		}
	} else if (VTS_SCREEN_SET_EDGE_AND_DCLICK_AREA == type) {
		syna_send_screen_clock_edge();
		VTI("vts_screen_set_edge_and_dclick_area has set");
		syna_set_screen_clock_report_double_area();
	} else if (VTS_SCREEN_SET_REPORT == type) {
		syna_send_screen_report_cmd();
	} else if (VTS_SCREEN_SET_DCLICK_AREA == type) {
		VTI("vts_screen_set_dclick_area has set");
		syna_set_screen_clock_report_double_area();
	} else {
		  VTI("type=%d,do nothing", type);
	}
...
static int syna_scan_rate_change (int rate)
{
	int ret = 0;
	unsigned char temp;
	//unsigned short scan_addr = 0x43E;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);

	
	VTI("---------------The input arg is %d",rate);
	if (rate != 0 && rate != 1 && rate != 2 && rate != 3 && rate != 4) {
		VTI("The input arg is invalid!");
		return 0;
	}
	VTI("gesture_change lock");
	mutex_lock(&(rmi4_data->geture_mutex));
	if (atomic_read(&(vivoTsGetVtsData()->tsState)) != TOUCHSCREEN_GESTURE) {
		VTI("tsState is not gesture mode, no need to change scan rate");
		ret = 0;
		goto END;
	}
	if (rate == 1 || rate == 4) {
		...
		ret = syna_dclick_longpress_mode_change(FP_DETECT_CLOSE);
		if (ret < 0) {
			goto END;
		}
	}
	ret = synaptics_rmi4_reg_read(rmi4_data, g_rmi4_data->reg_addr_ptr->syna_scan_rate_addr, &temp, 1);
		if (ret < 0) {
		VTI("failed to read");
		goto END;
	}
	VTI("before change, read from scan_addr is :0x%x", temp);
	if (temp == rate) {
		VTI("The state is already expected");
		goto END;
	}
	...
	temp = (unsigned char)rate;
	ret = synaptics_rmi4_reg_write(rmi4_data, g_rmi4_data->reg_addr_ptr->syna_scan_rate_addr, &temp, 1);
	if (ret < 0) {
		VTI("Failed to write to ic scanningRate bit  %d", ret);
		goto END;
	}
	...
	/*do force update when the fingerprint is light */
	if (rate == 1 || rate == 4) {
		VTI("after write do command DO_FORCE_UPDATE to execute scanning change!");
		ret = test_do_command(DO_FORCE_UPDATE);
		if (ret < 0) {
	...
	VTI("after change, read from scan_addr is :0x%x", temp);
	VTI("------------------change scanning rate sucess!");
END:
	mutex_unlock(&(rmi4_data->geture_mutex));
	VTI("gesture_change unlock");
	return ret;
...
static int syna_set_edge_bit(int state)
{
	int retval = 0;
	unsigned char tmp, tmp1, temp, temp1 ;
	unsigned short config_addr, edge_base;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);

#if defined(CONFIG_SECURE_TOUCH)
	if (atomic_read(&rmi4_data->st_enabled) == 1) {
		VTI("secure touch enable, do not set charge bit");
		return 0;
	}
#endif

	if (atomic_read(&(vivoTsGetVtsData()->tsState)) != TOUCHSCREEN_NORMAL) {
		VTI("not in normal mode, not to set edge bit");
		goto END;
	}
	config_addr = 0x40F;
	edge_base = 0x415;

	retval = synaptics_rmi4_reg_read(rmi4_data, config_addr, &tmp, 1);
	if (retval < 0) {
		VTI("failed to read\n");
		goto END;
	}
	VTI("read from config_addr is :0x%x", tmp);


	retval = synaptics_rmi4_reg_read(rmi4_data, edge_base, &temp, 1);
	if (retval < 0) {
		VTI("failed to read\n");
		goto END;
	}
	VTI("read from edge_base is :0x%x", temp);


    if (state == 1) {
		tmp1 = tmp & 0xf7;
		retval = synaptics_rmi4_reg_write(rmi4_data, config_addr, &tmp1, 1);
		if (retval < 0) {
			VTI("Failed to write to ic edge bit  %d", retval);
			goto END;
		}
		...
		temp1 = 0x30;
		retval = synaptics_rmi4_reg_write(rmi4_data, edge_base, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to ic edge bit  %d", retval);
			goto END;
		}
		/*for the  x,y of the coner size */
		temp1 = 0x20;
		retval = synaptics_rmi4_reg_write(rmi4_data, edge_base + 1, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of edge_base + 1 edge bit  %d", retval);
			goto END;
		}
		temp1 = 0x60;
		retval = synaptics_rmi4_reg_write(rmi4_data, edge_base + 2, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of edge_base + 2 edge bit  %d", retval);
			goto END;
		}
		temp1 = 0x3c;
		retval = synaptics_rmi4_reg_write(rmi4_data, edge_base + 3, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of edge_base + 3 edge bit  %d", retval);
			goto END;
		}
		temp1 = 0xfa;
		retval = synaptics_rmi4_reg_write(rmi4_data, edge_base + 4, &temp1, 1);
		if (retval < 0) {
			VTI("Failed to write to the size of edge_base + 4 edge bit  %d", retval);
			goto END;
		}	
	}  else if (state == 2) {
		tmp1 = tmp | 0x08;
	...
	retval = synaptics_rmi4_reg_read(rmi4_data, config_addr, &tmp1, 1);
	if (retval < 0) {
		VTI("failed to read config_addr");
		goto END;
	}
	VTI("after edge setbit read from config_addr is :0x%x", tmp1);
	retval = synaptics_rmi4_reg_read(rmi4_data, edge_base, &temp1, 1);
	if (retval < 0) {
		VTI("failed to read edge_base");
		goto END;
	}
	VTI("after edge setbit read from edge_base is :0x%x", temp1);
	END:
		return retval;
}

static int syna_set_charger_bit(int state)
{
	int retval;
	unsigned char tmp ;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	...
	if (atomic_read(&(vivoTsGetVtsData()->tsState)) != TOUCHSCREEN_NORMAL) {
		VTI("not in normal mode, not to write charger");
		return 0;
	}
	VTI("charge_connected = %d", state);
	VTI("rmi4_data->f01_ctrl_base_addr = 0x%x", rmi4_data->f01_ctrl_base_addr);

	retval = synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_ctrl_base_addr, &tmp, 1);
	if (retval < 0) {
		VTI("failed to read\n");
		return retval;
	}
	VTI("read from add:0x%x is :0x%x", rmi4_data->f01_ctrl_base_addr, tmp);
	if (state)
		tmp = tmp | 0x20;
	else
		tmp = tmp & 0xdf;

	/*syna 4322 must, avoid work mode bit[bit1] change to 1(1 means sleep mode), cause tp no function.*/
	tmp = tmp & 0xfc;

	VTI("write to ic is :0x%0x", tmp);
	retval = synaptics_rmi4_reg_write(rmi4_data, rmi4_data->f01_ctrl_base_addr, &tmp, 	sizeof(tmp));
	if (retval < 0) {
		VTI("Failed to write to ic charge bit  %d", retval);
		return retval;
	}
	retval = synaptics_rmi4_reg_read(rmi4_data, rmi4_data->f01_ctrl_base_addr, &tmp, 1);
	if (retval < 0) {
		VTI("failed to read from ic after write");
		return retval;
	}
	VTI("after write :read from ic is :0x%x", tmp);
	return retval;
}

int s3706_syna_gesture_point_get(u16 *data)
{
	int j = 0;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);

	for (j = 0; j < 20; j++) {
		data[j] = rmi4_data->buf_gesture[j];
	}
...
#if defined(CONFIG_SECURE_TOUCH)
static int synaptics_secure_touch_enable_get(void)
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	return atomic_read(&rmi4_data->st_enabled);
}
/*
 ...
 */
static int synaptics_secure_touch_enable_set(int state)
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	int err = 1;
	...
	VTI("secure touch enable = %d", state);
 ...
 */
static int synaptics_secure_touch_get(void)
{
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	int val = 0;
	if (atomic_read(&rmi4_data->st_enabled) == 0) {
		VTE("secure touch is disable");
		return -EBADF;
	}

	if (atomic_cmpxchg(&rmi4_data->st_pending_irqs, -1, 0) == -1) {
		VTE("pending irq is -1");
		return -EINVAL;
	}
...
extern int s3706_syna_get_rawordiff_data(int which, int *data);
extern int s3706_syna_sensor_test(char *buf, void *pdata, int tmp);
static int get_iic_bus_state(void)
{
	VTD("iic suspend state = %d", i2c_bus_state);
	return i2c_bus_state;
}

static int syna_get_i2c_mode(void)
{
	int retval;
	unsigned char offset;
	...
	if (fhandler->fn_number != SYNAPTICS_RMI4_F12) {
		VTI("fhandler fn_number is not SYNAPTICS_RMI4_F12!");
		return 0;
	}

	extra_data = (struct synaptics_rmi4_f12_extra_data *)fhandler->extra;
	if (extra_data == NULL) {
		VTI("extra_data is NULL !!!!!!!!!");
		return 0;
	}
	...
	if (reporting_control[2] == F12_WAKEUP_GESTURE_MODE)
		return 1;

	return 0;
}

int s3706_tsin_pin_ctr(int ctrl)
{
	int retval;
	unsigned char temp;
	//unsigned short f51_ctrl20_addr = 0x0438;
	unsigned char data = 0;

	if ((ctrl != 1) && (ctrl != 0)) {
		VTI("The input tsin state is invalid!");
		return MINUS_ONE;
	}
	...
	if (g_rmi4_data != NULL) {
	  if (0) {
		/*control 0x000D*/
		retval = synaptics_rmi4_reg_read(g_rmi4_data,
				0x000D,
				&temp,
				sizeof(temp));
		if (retval < 0) {
			dev_err(g_rmi4_data->pdev->dev.parent,
					"%s: Failed to read 0x000D register\n",
					__func__);
			return MINUS_ONE;
		}
		temp |= 4;
		...
		if (retval < 0) {
			dev_err(g_rmi4_data->pdev->dev.parent,
					"%s: Failed to write 0x000D register\n",
					__func__);
			return MINUS_ONE;
		}
		...
		if (retval < 0) {
			dev_err(g_rmi4_data->pdev->dev.parent,
					"%s: Failed to read 0x0141 register\n",
					__func__);
			return MINUS_ONE;
		}
		temp |= 4;
		...
		if (retval < 0) {
			dev_err(g_rmi4_data->pdev->dev.parent,
					"%s: Failed to write 0x0141 register\n",
					__func__);
			return MINUS_ONE;
		}
	  }
		...
		retval = synaptics_rmi4_reg_write(g_rmi4_data,
				g_rmi4_data->reg_addr_ptr->s3706_tsin_ctr_addr,
				&data,
				sizeof(data));
		if (retval < 0) {
			dev_err(g_rmi4_data->pdev->dev.parent,
					"%s: Failed to write f51_ctrl20 register\n",
					__func__);
			return MINUS_ONE;
		}
	} else {
		VTE("The g_rmi4_data is NULL!");
		return MINUS_ONE;
	}
...
extern int bbk_factory_mode(void);
...
static unsigned int report_lcm_id(void)
{
	...
	if (lcm_id == 0x20 || lcm_id == 0x21 || lcm_id == 0x22 || lcm_id == 0x23) {
		lcm_id = 0;
	}
	return lcm_id;
}
...
static int getTsPinValue(void)
{
	int ret = 0;
	
	ret = bbk_factory_mode();
	if (ret == 1) 
		g_rmi4_data->reg_addr_ptr = &reg_addr;
	 else 
		g_rmi4_data->reg_addr_ptr = &reg_addr_mp;
	 
	return ret;
}
	
static int synaptics_3706_configure_power(struct synaptics_rmi4_data *rmi4_data)
{
	int rc;
	const struct synaptics_dsx_board_data *bdata = rmi4_data->hw_if->board_data;
	...
	if (gpio_is_valid(bdata->power_gpio)) {
		...
		rc = gpio_request(bdata->power_gpio, I2C_DRIVER_NAME);
		if (rc) {
		...
		VTI("power_gpio not valid");
	}
	if (gpio_is_valid(bdata->iovcc_gpio)) {

		rc = gpio_request(bdata->iovcc_gpio, "IOVCC"I2C_DRIVER_NAME);
		if (rc) {
			VTE("iovcc gpio request fail! %d", bdata->iovcc_gpio);
			return rc;
		}

	} else {
		VTI("iovcc_gpio not valid");
	}

	return 0;
}

//static int has_power_on;
static int synaptics_3706_set_power(struct synaptics_rmi4_data *rmi4_data, int on)
{
	int rc = 0;
	const struct synaptics_dsx_board_data *bdata = rmi4_data->hw_if->board_data;

	VTI("power change to:%d", on);
	VTI("lock lcmResumeAndProximityMutex");

	mutex_lock(&(vivoTsGetVtsData()->lcmResumeAndProximityMutex));
	if (on == 1 && rmi4_data->has_power_on == 0) {
#if 0
		if (gpio_is_valid(bdata->power_gpio)) {
			rc = gpio_direction_output(bdata->power_gpio, 1);	/* 3V */
			if (rc) {
...
#else
			regulator_set_load(rmi4_data->pwr_reg, 50000);
			rc = regulator_enable(rmi4_data->pwr_reg);	/* 3v */
			if (rc < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
						"%s: Failed to enable pwr_3V regulator\n",
						__func__);
				goto exit;
		}
#endif
		...
		if (1) {
			rc = syna_S3706_pin_ctr(pin_config.pin_active);
			if (rc > 0) {
				VTI("Pinctrl active failed");
			}
		}
#if 1
			regulator_set_load(rmi4_data->bus_reg, 30000);
			rc = regulator_enable(rmi4_data->bus_reg);	/* 1.8v */
			if (rc < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
...
#else
			if (gpio_is_valid(bdata->iovcc_gpio)) {
				rc = gpio_direction_output(bdata->iovcc_gpio, 1);	/* 1.8V */
				if (rc) {
					VIVO_TS_LOG_ERR("unable to set direction out to 1 for gpio [%d]\n", bdata->iovcc_gpio);
					VTI("chenpeng unlock lcmResumeAndProximityMutex");
		...
		rmi4_data->has_power_on = 1;
	} else if (on == 0 && rmi4_data->has_power_on == 1) {
		gpio_set_value(bdata->reset_gpio, 0);
		msleep(5);

#if 1
			regulator_set_load(rmi4_data->bus_reg, 0);
			rc = regulator_disable(rmi4_data->bus_reg);	/* 1.8v */
			if (rc < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
...
#else
			if (gpio_is_valid(bdata->iovcc_gpio)) {
				rc = gpio_direction_output(bdata->iovcc_gpio, 0);/*1.8V*/
				if (rc) {
		...
		if (1) {
			/*gpio_set_value(bdata->reset_gpio, 0); */
			rc = syna_S3706_pin_ctr(pin_config.pin_suspend);
			if (rc > 0) {
				VTI("Pinctrl suspend failed");
			}
		}
		...
		if (gpio_is_valid(bdata->power_gpio)) {
			rc = gpio_direction_output(bdata->power_gpio, 0);/*3.3V*/
			if (rc) {
				VIVO_TS_LOG_ERR("unable to set direction out to 0 for gpio [%d]\n", bdata->power_gpio);
				goto exit;
			}
		...
		rc = regulator_disable(rmi4_data->pwr_reg);	/* 3v */
			if (rc < 0) {
				dev_err(rmi4_data->pdev->dev.parent,
						"%s: Failed to disable pwr_3V regulator\n",
						__func__);
				goto exit;
		}
#endif
		rmi4_data->has_power_on = 0;
	}
	VTI("power state:%d", rmi4_data->has_power_on);

	exit:
	VTI("unlock lcmResumeAndProximityMutex");
	mutex_unlock(&(vivoTsGetVtsData()->lcmResumeAndProximityMutex));
	return rc;
}

static void synaptics_rmi4_power_reset(struct synaptics_rmi4_data *rmi4_data)
{
	const struct synaptics_dsx_board_data *bdata  =
			rmi4_data->hw_if->board_data;
...
int s3706_bbk_syna_imei_read(unsigned char *imei);
int s3706_bbk_syna_imei_write(unsigned char *imei);
#include "pd1832_fw.h"
#include "pd1948_fw.h"
#include "pd1955_fw.h"
#include "pd1955_fw_mp.h"


#include "PD1728a_limit.h"
int syna_bbk_regulator_init(struct synaptics_rmi4_data *rmi4_data)
{
#if 1
	int retval = 0;
		rmi4_data->pwr_reg = regulator_get(rmi4_data->pdev->dev.parent,
			"avdd_pwr");
	if (IS_ERR(rmi4_data->pwr_reg)) {
		VTI("Failed to get power_3V regulator");
		retval = PTR_ERR(rmi4_data->pwr_reg);
		goto regulator_put1;
	}
	retval = regulator_set_voltage(rmi4_data->pwr_reg, 3000000,
								3000000);
	if (retval) {
	...
	rmi4_data->bus_reg = regulator_get(rmi4_data->pdev->dev.parent,
			"vcc_i2c");
	if (IS_ERR(rmi4_data->bus_reg)) {
		VTI("Failed to get i2c regulator");
		retval = PTR_ERR(rmi4_data->bus_reg);
		goto regulator_put2;
	}
...
int bbk_add_syna_tool_flag;
int vivo_ts_get_info(unsigned char *buf)
{
	int retval;
	unsigned char modInfo[8] = {0};
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
if (0) {
	bbk_add_syna_tool_flag = 1;
	VTI("set bbk_add_syna_tool_flag to 1");
	buf[0] = 's';
}
	retval = synaptics_rmi4_reg_read(rmi4_data, 0x402, modInfo, 8);
	if (retval < 0) {
		VTI("failed to read modInfo from 0x402");
		retval = snprintf(buf, 255, "read fail\n");
		return retval;
	}
	VTI("get modInfo:0x%x", modInfo[0]);
	retval = snprintf(buf, 255, "Freq Number Gear-%2d,active-%2d,gesture-%2d,idle-%2d,charger-%2d,water-%2d\n", 
		(int)((modInfo[0]>>5) & 0x07), (int)((modInfo[0]>>4) & 0x01), (int)((modInfo[0]>>3) & 0x01), (int)((modInfo[0]>>2) & 0x01), (int)((modInfo[0]>>1) & 0x01), (int)((modInfo[0]) & 0x01));
	return retval;
}
...
static int syna_faceDetect_startEnd(int cmd)
{
	int ret;
	unsigned char temp ;
	unsigned short fdpri_addr = 0x20;
	//unsigned short fdswitch_addr = 0x43F;
	struct synaptics_rmi4_data *rmi4_data = platform_get_drvdata(globe_pdev);
	VTI("---------------------cmd is %d",cmd);
	if (cmd == 1) {
		ret = s3706_idleEnableOrDisable(0);
		if (ret < 0) {
			VTI("disable idle failed!");
			goto END;
		}
		rmi4_data->fd_enable = true;
		VTI("The fd_enable flag has been set %d", rmi4_data->fd_enable);
		ret = synaptics_rmi4_reg_read(rmi4_data, fdpri_addr, &temp, 1);
		if (ret < 0) {
			VTI("failed to read from 0x20");
			goto END;
		}
		temp |= 0x07;
		ret = synaptics_rmi4_reg_write(rmi4_data, fdpri_addr, &temp, 1);
		if (ret < 0) {
			VTI("Failed to write the reg of 0x20  ret = %d", ret);
			goto END;
		}
		temp = 0x11;
		ret = synaptics_rmi4_reg_write(rmi4_data, g_rmi4_data->reg_addr_ptr->syna_face_detect_addr, &temp, 1);
		if (ret < 0) {
			VTI("Failed to write the reg of 0x43F  ret = %d", ret);
			goto END;
		}
		...
		rmi4_data->fd_enable = false;
		VTI("The fd_enable flag has been set %d", rmi4_data->fd_enable);
		ret = s3706_idleEnableOrDisable(1);
		if (ret < 0) {
...
static int synaptics_rmi4_probe(struct platform_device *pdev)
{
	...
	VTI("platform driver and device match, main probe enter");
	...
	vts_data->client = s3706_syna_client;
	if (!strcmp(productName, "PD1955")) {
		vivoTsSetTxRxNum(15, 34);
		vts_data->tsDimensionX = 1080; /*pdata->x_max; */
	    vts_data->tsDimensionY = 2400; /*pdata->y_max; */
	} else {
		vivoTsSetTxRxNum(16, 34);
		vts_data->tsDimensionX = 1080; /*pdata->x_max; */
		vts_data->tsDimensionY = 2340; /*pdata->y_max; */
	}
	...
	vts_data->hasFingerGesture = 1;


	vts_data->irqBtmType = VTS_IRQ_BTM_TYPE_RT_THREAD;
	vts_data->irqBtmThreadHandler = sync_btm_handler;

	vts_data->icNum = VTS_IC_S3706;
	vts_data->mtkOrQcom = VTS_PLATFORM_QCOM;

    vts_data->updateFirmware = bbk_fw_update;
    vts_data->icModeChange = syna_mode_change;
	vts_data->getIcMode = syna_get_i2c_mode;
	vts_data->setGestureSwitch = NULL; 		/*if all gusture switch default on, this function no use*/
	vts_data->getRawOrDiffData = s3706_syna_get_rawordiff_data;
	vts_data->getIcFirmwareOrConfigVersion = bbk_syna_fw_version;
	vts_data->getModuleId = syna_module_id;
	vts_data->getGesturePointData = s3706_syna_gesture_point_get;
	vts_data->setChargerFlagSwitch = syna_set_charger_bit;
	vts_data->getTsPinValue = getTsPinValue;
	vts_data->sensorTest = s3706_syna_sensor_test;
	vts_data->setEdgeRestainSwitch = syna_set_edge_bit;
	vts_data->getLcmId = get_lcm_id;
	vts_data->getI2cBusState = get_iic_bus_state;
	vts_data->writeImei = s3706_bbk_syna_imei_write;
	vts_data->readImei = s3706_bbk_syna_imei_read;
	vts_data->idleEnableOrDisable = s3706_idleEnableOrDisable;
	vts_data->tsInCtl = s3706_tsin_pin_ctr;
	vts_data->otherInfo = vivo_ts_get_info;
	vts_data->callbackType = 1;
	vts_data->scanRateChange = syna_scan_rate_change;
	vts_data->faceStartEnd = syna_faceDetect_startEnd;
	vts_data->processByPackage = syna_process_by_package;
	vts_data->imeiWriteSupport = SUPPORT;
	vts_data->get_st_enable = synaptics_secure_touch_enable_get;
	vts_data->set_st_enable = synaptics_secure_touch_enable_set;
	vts_data->get_st_state = synaptics_secure_touch_get;
	
	vts_data->sensorTestKey = "com.vivotouchscreen.sensortestsyna3706:ActivityVIVOProduction:com.vivotouchscreen.sensortestsyna3706:0:com.vivotouchscreen.sensortestsyna3706";
	vts_data->lcmNoiseTestKey = "com.vivotouchscreen.synadeltadiff:MainActivity:null:null:null";
	vts_data->RFTestKey = "com.vivotouchscreen.synadeltadiff:SynaBspTest:vivotouchscreen.SynaBspTest:0:vivotouchscreen.SynaBspTest";
	vts_data->rawdataTestKey = "com.vivotouchscreen.sensortestsyna3706:MainActivity:null:null:null";

	vts_data->readScrenClockEdge = syna_screen_clock_rect_read;
	vts_data->sendScreenReportCmd = syna_send_screen_report_cmd;
		
	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1832_fw, 	sizeof(pd1832_fw), 	"PD1832", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_LIMIT, 	(const unsigned char *)pd1728a_limit, 	sizeof(pd1728a_limit), 	"PD1832", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1948_fw, 	sizeof(pd1948_fw), 	"PD1948", 		VTS_FW_NOPIN_VALUE, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1955_fw, 	sizeof(pd1955_fw), 	"PD1955", 		0, VTS_MVC_TRY, 0);
	vivoTsFwAdd(VTS_FW_TYPE_FW, 	pd1955_fw_mp,	sizeof(pd1955_fw_mp),	"PD1955",		1, VTS_MVC_TRY, 0);
	
	retval = vivoTsInit(s3706_syna_client, NULL, -1);
	if (retval < 0) {
		vivoTsFree();
		return MINUS_ONE;
	}
	
	/*add for PD1619A incell need this leavel*/
	/*vts_data->resumeEventBlank = FB_EVENT_BLANK; */
	/*vts_data->suspendEventBlank = FB_EVENT_BLANK; */

	hw_if = pdev->dev.platform_data;
	if (!hw_if) {
		VTE("No hardware interface found");
		return -EINVAL;
	}

	bdata = hw_if->board_data;
	if (!bdata) {
		VTE("No board data found");
		return -EINVAL;
	}

	rmi4_data = kzalloc(sizeof(*rmi4_data), GFP_KERNEL);
	if (!rmi4_data) {
		VTE("Failed to alloc mem for rmi4_data");
		retval = -ENOMEM;
		goto err_zalloc_rmi4_data;
	}
	...
	rmi4_data->reset_device_for_vivo = synaptics_rmi4_reset_device_simple;
	...
	mutex_init(&(rmi4_data->vivo_mutex));
	mutex_init(&(rmi4_data->geture_mutex));

	ret = getTsPinValue();
	
#if 0
	rmi4_data->irq_err_workqueue = create_singlethread_workqueue("synaptics_wq");
	if (!rmi4_data->irq_err_workqueue) {
		VTE("can't create irq err worqueue");
		retval = -ENOMEM;
		goto err_get_reg;
	}
	INIT_WORK(&rmi4_data->irq_err_work, rmi4_irq_err_work_func);
#endif
...
if (1) {
	...
	retval = syna_S3706_pinctrl_init(s3706_syna_client);
	if (retval < 0) {
		VTI("Pinctrl init failed");
		goto err_init_reg;
	}

	retval = syna_S3706_pin_ctr(pin_config.pin_suspend);
	if (retval > 0) {
		VTI("Pinctrl suspend failed");
		goto err_init_reg;
	}
}
	retval = syna_bbk_regulator_init(rmi4_data);
	if (retval < 0) {
		VTI("Regulator init failed");
		goto err_init_reg;
	}

	/*power set*/
	synaptics_3706_configure_power(rmi4_data);
	/*power on*/
	synaptics_3706_set_power(rmi4_data, 1);
	VTI("SET POWER ON END");
if (0) {
	retval = synaptics_rmi4_get_reg(rmi4_data, true);
	if (retval < 0) {
		VTI("Failed to get regulators");
		goto err_get_reg;
	}

	retval = synaptics_rmi4_enable_reg(rmi4_data, true);
	if (retval < 0) {
		VTI("Failed to enable regulators");
		goto err_enable_reg;
	}
}
	...
	if (hw_if->ui_hw_init) {
		...
		if (retval < 0) {
			VTI("Failed to initialize hardware interface");
			goto err_ui_hw_init;
		}
	}

	rmi4_data->data_buf = (unsigned char *)kzalloc(2048, GFP_KERNEL);
	if (!rmi4_data->data_buf) {
		VTE("Buffer memory allocation failed");
		goto err_alloc_buffer;
	}

/*	s3706_syna_client->addr = 0x01; */
/*	while(s3706_syna_client->addr <= 127) {*/
		retval = synaptics_rmi4_set_input_dev(rmi4_data);
		if (retval < 0) {
			VTE("Failed to set up input device:addr = 0x%x", s3706_syna_client->addr);
			synaptics_rmi4_power_reset(rmi4_data);
			retval = synaptics_rmi4_set_input_dev(rmi4_data);
			if (retval < 0) {
				VTE("Failed to setup input device after power reset");
				goto err_set_input_dev;
			}
/*		} else {*/
/*			VTI("success set up input device:addr = 0x%x", s3706_syna_client->addr); */
/*			break; */
		}
/*		s3706_syna_client->addr++; */
/*	}*/
/*	if (s3706_syna_client->addr > 127)*/
/*		goto err_set_input_dev; */
	...
	if (0) {
		...
		if (retval < 0) {
			VTE("Failed to register fb notifier client");
		}
	}
#endif
	...
	if (1) {
		...
		if (retval < 0) {
			VTE("Failed to enable attention interrupt");
			goto err_enable_irq;
		}
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		retval = sysfs_create_file(&rmi4_data->input_dev->dev.kobj,
				&attrs[attr_count].attr);
		if (retval < 0) {
			VTE("Failed to create sysfs attributes");
			goto err_sysfs;
		}
	}
...
#ifdef CONFIG_VIRTUAL_PROX_ON_TOUCH
	retval = vivo_ts_prox_init(s3706_syna_client);
	if (retval < 0) {
		VTE("Failed to enable attention interrupt");
		goto err_sysfs;
	}
	...
	vivoTsAfterProbeCompleteCall(s3706_syna_client, NULL, -1);
...
static int __init synaptics_rmi4_init(void)
{
	...
	VTI("The productName is:%s", productName);
	if ((strcmp(productName, "PD1838")) && (strcmp(productName, "PD1948")) && (strcmp(productName, "PD1955"))) {     
    	VTI("The product name is not PD1838/PD1948/PD1955");
    	return 0;                              
	}  
	...
	retval = s3706_synaptics_rmi4_bus_init();
	if (retval)
		return retval;
...
static void __exit synaptics_rmi4_exit(void)
{
	...
	VTI("s3706_synaptics_rmi4_bus_exit");
	s3706_synaptics_rmi4_bus_exit();
