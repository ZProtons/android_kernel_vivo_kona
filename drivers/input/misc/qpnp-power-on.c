...
static bool pmic_wdt_setup_done;

#define VIVO_PMIC_WDT_PET_TIME 100
#define VIVO_PMIC_WDT_BARK_TIME 120

extern int vivo_pmic_wdt_pet(void);
extern int vivo_pmic_wdt_stop(void);
extern int vivo_pmic_wdt_start_kernel(long timeout);
extern void vivo_pmic_wdt_setup(bool g_force_offline_init);
extern char *vivo_get_pmic_status(void);

extern unsigned int power_off_charging_mode;
static void vivo_pmic_wdt_work_handler(struct work_struct *work);
DECLARE_DELAYED_WORK(vivo_pmic_wdt_work, vivo_pmic_wdt_work_handler);
static void vivo_pmic_wdt_work_handler(struct work_struct *work)
{
	//vivo_pmic_wdt_pet();
	schedule_delayed_work(&vivo_pmic_wdt_work, VIVO_PMIC_WDT_PET_TIME*HZ);
				 ...
				 enum pon_power_off_type type)
{
	...
	dev_err(pon->dev, "ps_hold power off type = 0x%02X\n", type);
				       ...
				       enum pon_power_off_type type)
{
	...
	dev_err(pon->dev, "resin power off type = 0x%02X\n", type);
...
void vivo_pmic_wdt_prepare_and_start(void)
{
	bool g_force_offline_init = 0;
	int wdt_force_offline_value = 0;
	int wdt_test_mode_ctl_value = 0;

	if (pmic_wdt_setup_done)
		return;
	...
	printk("pmic_wdt: vivolog[%d], debug_mode[%d], wdtest_mode[%d], force_offline_bit[%d], is_normal_mode[%d]\n",
				is_vivolog_flag, is_debug_mode, wdt_test_mode_ctl_value, wdt_force_offline_value, is_normal_mode);
	vivo_pmic_wdt_setup(g_force_offline_init);

	if (power_off_charging_mode) {
		//vivo_pmic_wdt_start_kernel(VIVO_PMIC_WDT_BARK_TIME);
		schedule_delayed_work(&vivo_pmic_wdt_work, VIVO_PMIC_WDT_PET_TIME*HZ);
	} else if (is_normal_mode) {
		vivo_pmic_wdt_start_kernel(900);	/*900 seconds*/
		//vivo_pmic_wdt_stop();	/* temp disable*/
	}

	//vivo_pmic_wdt_stop();	/*temp disable*/
	pmic_wdt_setup_done = 1;
}
/*vivo added for pmic wdt end*/
#endif /*CONFIG_VIVO_PMIC_WDT*/
...
static struct qpnp_pon_config *qpnp_get_cfg(struct qpnp_pon *pon, u32 pon_type)
{
	...
	for (i = 0; i < pon->num_pon_config; i++) {
		printk("[%s][%d] pon_type %d\n", __FUNCTION__, __LINE__, pon->pon_cfg[i].pon_type);
...
static int qpnp_pon_input_dispatch(struct qpnp_pon *pon, u32 pon_type)
{
	...
	pr_err("[HW_KEY]keycode PMIC input: code=%d, status=0x%02X\n", cfg->key_code,
		pon_rt_sts);
...
#ifdef CONFIG_VIVO_PMIC_WDT
	vivo_pmic_wdt_prepare_and_start();
#else
...
#ifdef CONFIG_VIVO_PMIC_WDT
	if (power_off_charging_mode) {
		cancel_delayed_work(&vivo_pmic_wdt_work);
	}
#endif	/*CONFIG_VIVO_PMIC_WDT*/
