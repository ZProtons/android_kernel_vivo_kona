...
static ssize_t iris_loop_back_store(struct device *device,
...
static ssize_t iris_loop_back_show(struct device *device,
		struct device_attribute *attr, char *buf)
{
	...
	mutex_lock(&pcfg->panel->panel_lock);
	IRIS_LOGI("%s(),power mode : %d", __func__, pcfg->panel->power_mode);
	ktime0 = ktime_get();
	ret = iris_loop_back_validate();
	ktime1 = ktime_get();
	timeus = (u32)ktime_to_us(ktime1) - (u32)ktime_to_us(ktime0);
...
static DEVICE_ATTR_RW(iris_loop_back);

static struct attribute *iris_loop_back_attrs[] = {
	&dev_attr_iris_loop_back.attr,
	NULL};

static const struct attribute_group iris_loop_back_group = {
	.attrs = iris_loop_back_attrs,
};
...
int iris_loop_back_init(struct dsi_display *display)
{
	...
	if (sysfs_create_group(&display->pdev->dev.kobj, &iris_loop_back_group))
		return -EFAULT;
