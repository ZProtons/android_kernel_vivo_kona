				   ...
				   uint64_t value, struct drm_mode_object **ref)
{
	...
	if (property->flags & DRM_MODE_PROP_IMMUTABLE) {
              pr_err("travi drm_property_change_valid_get flags=%d\n", property->flags);
		return false;
       }
	*ref = NULL;
	...
	if (drm_property_type_is(property, DRM_MODE_PROP_RANGE)) {
		if (value < property->values[0] || value > property->values[1]) {
                     pr_err("travi drm_property_change_valid_get value=%d,%d,%d\n", 
                     value,property->values[0],property->values[1]);
			return false;
              }
		return true;
		...
		if (svalue < U642I64(property->values[0]) ||
				svalue > U642I64(property->values[1])){
                     pr_err("travi drm_property_change_valid_get value=%d,%d,%d\n", 
                     svalue,property->values[0],property->values[1]);
			return false;
              }
		return true;
		...
		for (i = 0; i < property->num_values; i++)
			valid_mask |= (1ULL << property->values[i]);
              //pr_err("travi drm_property_change_valid_get return1=%d\n", !(value & ~valid_mask));
		...
		if (blob) {
			*ref = &blob->base;
			return true;
		       ...
		       pr_err("travi drm_property_change_valid_get return false\n");
			return false;
		}
	...
	for (i = 0; i < property->num_values; i++)
		if (property->values[i] == value)
			return true;
       pr_err("travi drm_property_change_valid_get exit false\n");
	return false;
}
