...
static bool _iris_is_clock_switched(const struct dsi_mode_info *new_timing)
{
	...
	if (cur_timing->clk_rate_hz != new_timing->clk_rate_hz)
		return false; // for dynamic clk switch
		//return true;
...
bool iris_same_resolution_diff_refresh(int32_t type)
{
	uint32_t tm_index = 0;
	struct dsi_mode_info *cur_timing = &panel_tm_arry[cur_tm_index];
	...
	if (tm_cmd_map_arry[type] == IRIS_DTSI_NONE)
		return false;

	tm_index = tm_cmd_map_arry[type];
