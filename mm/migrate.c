...
out:
	if (nr_succeeded) {
		...
		if (reason == MR_COMPACTION) {
			count_mbd_events(COMPACT_MIGRATE_SUCCESS, nr_succeeded);
		} else if (reason == MR_CONTIG_RANGE) {
			count_mbd_events(CMA_MIGRATE_SUCCESS, nr_succeeded);
		}
#endif
	}
	if (nr_failed) {
...
#ifdef CONFIG_MEM_BIGDATA
		if (reason == MR_COMPACTION) {
			count_mbd_events(COMPACT_MIGRATE_FAIL, nr_failed);
		} else if (reason == MR_CONTIG_RANGE) {
			count_mbd_events(CMA_MIGRATE_FAIL, nr_failed);
		}
#endif
}
