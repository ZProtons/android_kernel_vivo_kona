...
#if 0	//PD1824/PD1916/PD1922 BQ27750 IIC error issue: use for keep charging current and charging UI.
...
static void cms_external_power_changed(struct power_supply *psy)
{
	...
	if(!chip->online){
		...
		chip->last_step = 0;
		chip->usbid_mv = 1800;
		chip->normal_charge_batt_scale = 0;
		chip->ai_disable_charge = 0;
		chip->ai_disable_charge_state = false;
		chip->force_bypass_to_direct_charging = false;
...
static bool cms_check_fastchg_current(struct cms *chip)
{
	...
	if (vbat_uv.intval > 5000000)	//dual battery project
		vbat_uv.intval /= 2;
	...
	if (!(usb_type.intval == POWER_SUPPLY_TYPE_USB_DCP ||
		...
		usb_type.intval == POWER_SUPPLY_TYPE_USB_PD)) {
		cms_print(chip,PR_ERROR,"usb_type=%d,!DCP || !HVDCP || !PD\n",usb_type.intval);
		return false;
	}

	if (chip->force_bypass_to_direct_charging) {
		power_supply_get_property(chip->mcu_psy,
			POWER_SUPPLY_PROP_DCHG_ENABLED, &dchg_enable);

		if (dchg_enable.intval == 0) {
			cms_print(chip,PR_ERROR,"bypass test: but dchg_enable=0, skip it...\n");
			return false;
		}
	}
...
#else
inline static int get_usbid_adc(void) { return 1800; };
#endif
...
usb_id:
	...
	if (chip->usb_connecter_protect_enable & USB_CONN_TEMP_HEAT_PROTECT) {
		...
		if (chip->usb_psy) {
			power_supply_get_property(chip->usb_psy,
				POWER_SUPPLY_PROP_USB_CONN_TEMP, &usb_conn_temp_ap);
		}
		if(chip->mcu_psy) {
			power_supply_get_property(chip->mcu_psy,
	...
	cms_print(chip, PR_WARN, "usb_connecter_protect_enable=0x%x, usbid_mv=%dmV, batt_temp=%d, usb_conn_temp=%d[%d,%d]. (batt_temp=[%d,%d],usb_conn_temp=[%d,%d]). usb_conn_heat_protect_count=%d\n",
		chip->usb_connecter_protect_enable, chip->usbid_mv, batt_temp, usb_conn_temp.intval, usb_conn_temp_ap.intval, usb_conn_temp_mcu.intval,
	...
	if (0 != batt_scale && chip->usb_conn_heat_protect_count)
...
static int cms_check_dchg_ntc_protect(struct cms *chip)
{
	...
	chip->usb_connector_temp = usb_conn_temp_ap.intval;
...
//	}
	...
	cms_print(chip,PR_INFO," bat_board_temp=%d, usb_conn_temp=%d(ap=%d, mcu=%d), master_bat_conn_temp=%d, bat_conn_temp=%d. count=[%d,%d,%d,%d]. NTC %s.\n",
		bat_board_temp.intval, usb_conn_temp.intval, usb_conn_temp_ap.intval, usb_conn_temp_mcu.intval, master_bat_conn_temp.intval, bat_conn_temp.intval,
...
static int cms_check_dchg_adapter_cooldown(struct cms *chip)
{
	...
	if(chip->adapter_power_derate_enable){
		...
		power_supply_get_property(chip->mcu_psy,
			POWER_SUPPLY_PROP_ADAPTER_IBUS_MAX, &adapter_ibus_max_mA);

		power_supply_get_property(chip->mcu_psy,
			POWER_SUPPLY_PROP_DIRECT_CHG_BYPASS_TO_DIRECT_CHARGING, &bypass_support);

		adapter_current = adapter_ibus_max_mA.intval;
		if (adapter_current <= 0)
			adapter_current = 3001;

		if(adapter_power_derate.intval != 0) {
				trigger_temp = ((int)(adapter_power_derate.intval)&0xff) * 10;
				active_current = (((int)((adapter_power_derate.intval >> 8)&0xff)) * adapter_current * IBUS_TO_IBAT_RATE(bypass_support.intval))/100;
				release_temp = ((int)(adapter_power_derate.intval >> 16)&0xff) * 10;
				cms_print(chip, PR_INFO, "notice: trigger %d, release %d, active %d adapter_current %d. ibus_to_ibat_rate=%d\n", trigger_temp, release_temp, active_current, adapter_current, IBUS_TO_IBAT_RATE(bypass_support.intval));
		}else {
			//adapter_powerderate param read fail
			if(adapter_current == 3000){
				//33W adapter
				trigger_temp = 800;
				release_temp = trigger_temp - 50;
				active_current = 1700 * IBUS_TO_IBAT_RATE(bypass_support.intval);	//ibat
			}else if(adapter_current == 2250){
				//22.5 adapter
				trigger_temp = 900;
				release_temp = trigger_temp - 50;
				active_current = 750 * IBUS_TO_IBAT_RATE(bypass_support.intval);
			}else if(adapter_current >= 4000){
				//44W/55W adapter
				trigger_temp = 1000;
				release_temp = trigger_temp - 50;
				active_current = 2250 * IBUS_TO_IBAT_RATE(bypass_support.intval);
			} else {
				trigger_temp = 900;
				release_temp = trigger_temp - 50;
				active_current = 1700 * IBUS_TO_IBAT_RATE(bypass_support.intval);
			}
		}
		...
		cms_print(chip,PR_INFO,"power_derate=0x%x adapter_temp=%d[%d, %d, %d], adapter_ibus_max_mA=%d, batt_full_mah=%d, batt_scale=%d(%dmA).  Dchg Adapter is %s.\n",
			adapter_power_derate.intval, adapter_temp.intval, trigger_temp, release_temp, active_current, adapter_ibus_max_mA.intval,
...
static int cms_get_intell_charge_current(struct cms *chip,int fb_state,int pri_temp,int par_temp, int bat_temp,
						int *primary_fastchg_ma, int *parallel_fastchg_ma, int batt_cap_mah, int fast_chg_state)
{
	struct tc_data_item *pri_data,*par_data, *bat_data;
	int pri_len,par_len,bat_len,batt_scale,current_ma, current_ma2;
	...
	switch(fb_state){
		case FB_ON:
			if (chip->ex_intell_charge_enable && !fast_chg_state) {
				...
				bat_data = chip->ex_batt_fbon_tc_data;
				bat_len = chip->ex_batt_fbon_tc_rc[ROW];
			} else if (chip->bypass_intell_charge_enable && fast_chg_state == fast_chg_state_VFCP_bypass) {
				pri_data = chip->bypass_primary_fbon_tc_data;
				pri_len = chip->bypass_primary_fbon_tc_rc[ROW];
				par_data = chip->bypass_parallel_fbon_tc_data;
				par_len = chip->bypass_parallel_fbon_tc_rc[ROW];
				bat_data = chip->bypass_batt_fbon_tc_data;
				bat_len = chip->bypass_batt_fbon_tc_rc[ROW];
			} else {
				if(chip->eq_intell_charge_enable && chip->switch_state) {
					pri_data = chip->eq_primary_fbon_tc_data;
					pri_len = chip->eq_primary_fbon_tc_rc[ROW];
					par_data = chip->eq_parallel_fbon_tc_data;
					par_len = chip->eq_parallel_fbon_tc_rc[ROW];
					bat_data = chip->eq_batt_fbon_tc_data;
					bat_len = chip->eq_batt_fbon_tc_rc[ROW];
				} else {
					...
					bat_data = chip->batt_fbon_tc_data;
					bat_len = chip->batt_fbon_tc_rc[ROW];
				}
			}
			break;
		case FB_OFF:
			if (chip->ex_intell_charge_enable && !fast_chg_state) {
				...
				bat_data = chip->ex_batt_fboff_tc_data;
				bat_len = chip->ex_batt_fboff_tc_rc[ROW];
			} else if (chip->bypass_intell_charge_enable && fast_chg_state == fast_chg_state_VFCP_bypass) {
				pri_data = chip->bypass_primary_fboff_tc_data;
				pri_len = chip->bypass_primary_fboff_tc_rc[ROW];
				par_data = chip->bypass_parallel_fboff_tc_data;
				par_len = chip->bypass_parallel_fboff_tc_rc[ROW];
				bat_data = chip->bypass_batt_fboff_tc_data;
				bat_len = chip->bypass_batt_fboff_tc_rc[ROW];
			} else {
				if(chip->eq_intell_charge_enable && chip->switch_state) {
					pri_data = chip->eq_primary_fboff_tc_data;
					pri_len = chip->eq_primary_fboff_tc_rc[ROW];
					par_data = chip->eq_parallel_fboff_tc_data;
					par_len = chip->eq_parallel_fboff_tc_rc[ROW];
					bat_data = chip->eq_batt_fboff_tc_data;
					bat_len = chip->eq_batt_fboff_tc_rc[ROW];
				} else {
					...
					bat_data = chip->batt_fboff_tc_data;
					bat_len = chip->batt_fboff_tc_rc[ROW];
				}
			}
			break;
		default:
	...
	*primary_fastchg_ma = min(*primary_fastchg_ma, current_ma);
	...
	current_ma2 = cms_get_current_by_board_temp(chip,bat_data, bat_len,
									bat_temp, fast_chg_state, "batt_temp");
	if(chip->direct_charger_enable) {
		*parallel_fastchg_ma = 0;
		if(current_ma > 0)
			*primary_fastchg_ma = min(current_ma, *primary_fastchg_ma);
		if(current_ma2 > 0)
			*primary_fastchg_ma = min(current_ma2, *primary_fastchg_ma);
	...
	cms_print(chip,PR_INFO,"temp=(%d,%d,%d) primary_ma=%d,parallel_ma=%d,current_ma=%d,current_ma2=%d,batt_scale=%d,batt_cap_mah=%d,eq_intell_charge_enable=%d,switch_state=%d, fast_chg_state=%d\n",
		pri_temp, par_temp, bat_temp, *primary_fastchg_ma, *parallel_fastchg_ma, current_ma, current_ma2, batt_scale, batt_cap_mah,
		chip->eq_intell_charge_enable, chip->switch_state, fast_chg_state);
	return batt_scale;
}
...
static int cms_check_normal_charge_current(struct cms *chip,int batt_temp)
{
	...
	chip->normal_charge_batt_scale = batt_scale;
...
static int cms_check_intell_charge_current(struct cms *chip, int batt_cap_mah, int fast_chg_state, int bat_temp)
{
	bool system_start = ((u64)(local_clock()/1000000000)) > 10;//s: 10s
	...
	if(chip->fb_on){
		//if (!power_off_charging_mode) {
		if (1) {
			batt_scale = cms_get_intell_charge_current(chip,FB_ON,primary_board_temp_now.intval,parallel_board_temp_now.intval, bat_temp,
							&chip->intell_charge_primary_result_ma, &chip->intell_charge_parallel_result_ma, batt_cap_mah, fast_chg_state);
		}
	}else{
		batt_scale = cms_get_intell_charge_current(chip,FB_OFF,primary_board_temp_now.intval,parallel_board_temp_now.intval, bat_temp,
							&chip->intell_charge_primary_result_ma, &chip->intell_charge_parallel_result_ma, batt_cap_mah, fast_chg_state);
	}
							...
							int primary_fastchg_max_ma,int parallel_fastchg_max_ma)
{
	...
	batt_scale = min((primary_fastchg_max_ma + parallel_fastchg_max_ma)*100/batt_cap_mah+1, chip->normal_charge_batt_scale);
	cms_print(chip,PR_INFO,"In factory mode,set max ibat current, batt_scale=%d, normal_charge_batt_scale=%d\n", batt_scale, chip->normal_charge_batt_scale);
...
static void cms_main_work(struct work_struct *work)
{
	...
	if(chip->direct_charger_enable && chip->mcu_psy) {
		...
		fast_chg_state = (dchg_support.intval > 0) ? fast_chg_state_VFCP : fast_chg_state_NONE;	//vivo Dchg

		power_supply_get_property(chip->mcu_psy,
			POWER_SUPPLY_PROP_DIRECT_CHG_BYPASS_TO_DIRECT_CHARGING, &bypass_support);
		if (dchg_support.intval == 1 && bypass_support.intval > 0)
			fast_chg_state = fast_chg_state_VFCP_bypass;	//bypass
	} else
	...
	ntc_high_temp_checking |= cms_check_usb_connecter_protect(chip, batt_cap_mah.intval, batt_temp_now.intval);
	...
	if (chip->continue_count >= 4 || fast_chg_state)	//wait fast_chg_state update normal
		cms_check_intell_charge_current(chip, batt_cap_mah.intval, fast_chg_state, batt_temp_now.intval);
...
out:
	...
	if (ntc_high_temp_checking)	//quickly for NTC high temp checking process...
		work_period_ms = msecs_to_jiffies(1000);
					...
					struct class_attribute *attr, char *buf)
{
	...
	if (chip->charge_param_judge_threshold[1] < 1000) { //current unit = A
		return scnprintf(buf, PAGE_SIZE, "charge test param threshold:%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", chip->charge_param_judge_threshold[0], chip->charge_param_judge_threshold[1], chip->charge_param_judge_threshold[2],
		...
		chip->charge_param_judge_threshold[8], chip->charge_param_judge_threshold[9]);
	} else	// current unit = mA
		return scnprintf(buf, PAGE_SIZE,"charge test param threshold:%d,%d.%d,%d,%d,%d,%d,%d,%d,%d,%d\n", chip->charge_param_judge_threshold[0], chip->charge_param_judge_threshold[1]/1000, chip->charge_param_judge_threshold[1]%1000/100, chip->charge_param_judge_threshold[2],
		chip->charge_param_judge_threshold[3], chip->charge_param_judge_threshold[4], chip->charge_param_judge_threshold[5], chip->charge_param_judge_threshold[6], chip->charge_param_judge_threshold[7],
		chip->charge_param_judge_threshold[8], chip->charge_param_judge_threshold[9]);
}
					...
					const char *buf, size_t count)
{
	...
	if(!strncmp(buf, "recovery-fast-charge", 20)) {
		if (supported_adapter.intval == 5) {
			value.intval = 0;
			power_supply_set_property(chip->mcu_psy,
				POWER_SUPPLY_PROP_DIRECT_CHG_BYPASS_TO_DIRECT_CHARGING, &value);
			chip->force_bypass_to_direct_charging = false;
		}
	...
	if(!strncmp(buf, "bypass-to-dchg", 14)) {
		if (supported_adapter.intval == 5) {
			value.intval = 1;
			power_supply_set_property(chip->mcu_psy,
				POWER_SUPPLY_PROP_DIRECT_CHG_BYPASS_TO_DIRECT_CHARGING, &value);
			msleep(500);	//wait MCU prepare action.
			chip->force_bypass_to_direct_charging = true;
		}
	}
					...
					const char *buf, size_t count)
{
	...
	if(!strncmp(buf, "health-warm", 11)){
		...
		schedule_delayed_work__on_system_unbound_wq(&chip->debugfs_write_work, msecs_to_jiffies(600));	//if vbus insert, we wait 500ms later and EN_HIZ(bq25790), otherwise bq25790 will error(no PG_STAT and no BC1.2)
...
static const struct file_operations power_type_fs_debug_fops = {
...
static const char * const charge_test_item_title[] = {
	...
	"M_BOARD_NTC",
	"SUB_BOARD_NTC",
	"USB_CON_NTC",	//connect in AP  side USB NTC
	"USB_CON_NTC1",	//connect in MCU side USB NTC
	"MAIN_BAT_CON_NTC",
	"SUB_BAT_CON_NTC",
	"BAT_BOARD_NTC",

	"Vbattt",	//dual battery voltage
	"Vbatt",	//single battery voltage = Vph voltage
	"Unkown test item",
};
...
static void cms_check_chg_error_state(struct cms *chip)
{
	...
	union power_supply_propval usb_connector_temp1 = {0,};
	union power_supply_propval vbattt = {0,};	//FG IC vbat
	union power_supply_propval vbatt = {0,};	//PMIC Vbat=Vsys voltage.
	int health_state = 0;
	int index = 0;
	int value = 0, min_value = 0, max_value = 0;
	int len = 0;
	...
	cms_ensure_psy_available(chip);
	if(!chip->batt_psy || !chip->bms_psy) {
		cms_print(chip,PR_ERROR,"bat_psy or bms_psy not registed\n");
		return;
	}
	power_supply_get_property(chip->bms_psy,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbatt);	//get from PMIC
	power_supply_get_property(chip->batt_psy,
		POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbattt);	//get from FG
	power_supply_get_property(chip->batt_psy,
		POWER_SUPPLY_PROP_SUPPORTED_ADAPTER, &supported_adapter);
	power_supply_get_property(chip->batt_psy,
	...
	if(chip->direct_charger_enable && chip->mcu_psy) {
		...
		power_supply_get_property(chip->mcu_psy,
			POWER_SUPPLY_PROP_USB_CONN_TEMP, &usb_connector_temp1);
	}
	...
	for(index = 0; index < len; index++) {
			if(index>=0 && index < Unkown_test_item) {
				 if(chip->charge_test_param_data[index].enable_switch) {
				 	min_value = chip->charge_test_param_data[index].min_value;
					max_value = chip->charge_test_param_data[index].max_value;
					switch(index) {
						case cableR:
							value = calbe_r.intval; /* cable resistance */
							break;
						case M_ibus:
							value = master_ibus.intval/* - slave_ibus.intval*/; /* master ibus */	//sxs need chec other project
							if (chip->force_bypass_to_direct_charging) {
								if (slave_ibus.intval == -1) {
									min_value = 2500;
									max_value = 5000;
								} else {
									min_value = 1000;
						...
						case S_ibus:
							...
							if (value > 0 && chip->force_bypass_to_direct_charging) {
								min_value = 1000;
								max_value = 4000;
							}
							break;
						...
						case USB_CON_NTC:
							...
							break;
						case USB_CON_NTC1:
							value = usb_connector_temp1.intval / 10;
							break;
						...
						case BAT_BOARD_NTC:
							...
							break;
						case VBATTT:
							value = vbattt.intval / 1000;	//return mV
							break;
						case VBATT:
							value = vbatt.intval / 1000;	//return mV
							break;
						default:
							value = 0;
					}
					pos += scnprintf(chip->chg_state + pos, sizeof(chip->chg_state) -pos, "%s:%d[%d, %d],", charge_test_item_title[index], value, min_value, max_value);
				}
				else {
...
static const struct file_operations charge_test_fs_debug_fops = {
...
static int cms_charge_test_param_fs_show(struct seq_file *s, void *unused)
{
	...
	if (chip->charge_param_judge_threshold[1] < 1000) {	//current unit = A
		seq_printf(s, "charge test param threshold:%d,%d,%d,%d,%d,%d,%d,%d,%d,%d\n", chip->charge_param_judge_threshold[0], chip->charge_param_judge_threshold[1], chip->charge_param_judge_threshold[2],
		chip->charge_param_judge_threshold[3], chip->charge_param_judge_threshold[4], chip->charge_param_judge_threshold[5], chip->charge_param_judge_threshold[6], chip->charge_param_judge_threshold[7],
		chip->charge_param_judge_threshold[8], chip->charge_param_judge_threshold[9]);
	} else	// current unit = mA
		seq_printf(s, "charge test param threshold:%d,%d.%d,%d,%d,%d,%d,%d,%d,%d,%d\n", chip->charge_param_judge_threshold[0], chip->charge_param_judge_threshold[1]/1000, chip->charge_param_judge_threshold[1]%1000/100, chip->charge_param_judge_threshold[2],
		chip->charge_param_judge_threshold[3], chip->charge_param_judge_threshold[4], chip->charge_param_judge_threshold[5], chip->charge_param_judge_threshold[6], chip->charge_param_judge_threshold[7],
		chip->charge_param_judge_threshold[8], chip->charge_param_judge_threshold[9]);
...
static const struct file_operations charge_test_param_fs_debug_fops = {
...
static struct device_node *cms_find_support_board_node(struct cms *chip)
{
	...
	if (match_node == NULL)
		cms_print(chip, PR_ERROR, "Can not match the child_node in cms_v3..\n");

	return match_node;
}
static int cms_parse_equalizing_charge_param(struct cms *chip, struct device_node *node)
{
	/* fbon battery param */
	chip->eq_batt_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
			"vivo,eq-batt-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,eq-batt-fbon-tc-row-col");
	if(chip->eq_batt_fbon_tc_rc) {
		chip->eq_batt_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
					"vivo,eq-batt-fbon-tc-data",chip->eq_batt_fbon_tc_rc[ROW],
					chip->eq_batt_fbon_tc_rc[COL], TWO_DIMENSION, "vivo,eq-batt-fbon-tc-data");
	}

	chip->eq_primary_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
			"vivo,eq-primary-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "eq-primary-fbon-tc-row-col");
	if(chip->eq_primary_fbon_tc_rc) {
		chip->eq_primary_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
					"vivo,eq-primary-fbon-tc-data",chip->eq_primary_fbon_tc_rc[ROW],
					chip->eq_primary_fbon_tc_rc[COL], TWO_DIMENSION, "eq-primary_fbon_tc_data");
	}
	if(!chip->eq_primary_fbon_tc_rc || !chip->eq_primary_fbon_tc_data) {
		chip->eq_primary_fbon_tc_data = default_primary_fbon_tc_data;
		chip->eq_primary_fbon_tc_rc[ROW] = ARRAY_SIZE(default_primary_fbon_tc_data);
		chip->eq_primary_fbon_tc_rc[COL] = 3;

	} else {

	}

	chip->eq_parallel_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
			"vivo,eq-parallel-fbon-tc-row-col", 1, 2, ONE_DIMENSION, NULL);
	if(chip->eq_parallel_fbon_tc_rc) {
		chip->eq_parallel_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
					"vivo,eq-parallel-fbon-tc-data",chip->eq_parallel_fbon_tc_rc[ROW],
					chip->eq_parallel_fbon_tc_rc[COL], TWO_DIMENSION, "eq-parallel_fbon_tc_data");
	}
	if(!chip->eq_parallel_fbon_tc_rc || !chip->eq_parallel_fbon_tc_data){
		chip->eq_parallel_fbon_tc_data = default_parallel_fbon_tc_data;
		chip->eq_parallel_fbon_tc_rc[ROW] = ARRAY_SIZE(default_parallel_fbon_tc_data);
		chip->eq_parallel_fbon_tc_rc[COL] = 3;
	...
	/* fboff battery param */
	chip->eq_batt_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
			"vivo,eq-batt-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,eq-batt-fboff-tc-row-col");
	if(chip->eq_batt_fboff_tc_rc) {
		chip->eq_batt_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
					"vivo,eq-batt-fboff-tc-data",chip->eq_batt_fboff_tc_rc[ROW],
					chip->eq_batt_fboff_tc_rc[COL], TWO_DIMENSION, "vivo,eq-batt-fboff-tc-data");
	}

	
	chip->eq_primary_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
			"vivo,eq-primary-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "eq-primary-fboff-tc-row-col");
	if(chip->eq_primary_fboff_tc_rc) {
		chip->eq_primary_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
					"vivo,eq-primary-fboff-tc-data", chip->eq_primary_fboff_tc_rc[ROW],
					chip->eq_primary_fboff_tc_rc[COL], TWO_DIMENSION, "eq-primary_fboff_tc_data");
	}
	if(!chip->eq_primary_fboff_tc_rc || !chip->eq_primary_fboff_tc_data){
		chip->eq_primary_fboff_tc_data = default_primary_fboff_tc_data;
		chip->eq_primary_fboff_tc_rc[ROW] = ARRAY_SIZE(default_primary_fboff_tc_data);
		chip->eq_primary_fboff_tc_rc[COL] = 3;

	} else {

	}

	chip->eq_parallel_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
			"vivo,eq-parallel-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "eq-parallel-fboff-tc-row-col");
	if(chip->eq_parallel_fboff_tc_rc) {
		chip->eq_parallel_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
					"vivo,eq-parallel-fboff-tc-data",chip->eq_parallel_fboff_tc_rc[ROW],
					chip->eq_parallel_fboff_tc_rc[COL], TWO_DIMENSION, "eq-parallel_fboff_tc_data");
	}
	if(!chip->eq_parallel_fboff_tc_rc || !chip->eq_parallel_fboff_tc_data){
		chip->eq_parallel_fboff_tc_data = default_parallel_fboff_tc_data;
		chip->eq_parallel_fboff_tc_rc[ROW] = ARRAY_SIZE(default_parallel_fboff_tc_data);
		chip->eq_parallel_fboff_tc_rc[COL] = 3;
...
static int cms_parse_dt(struct cms *chip)
{
	...
	if(chip->intell_charge_enable) {
		...
		/* fbon battery param */
		chip->batt_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,batt-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,batt-fbon-tc-row-col");
		if(chip->batt_fbon_tc_rc) {
			chip->batt_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,batt-fbon-tc-data",chip->batt_fbon_tc_rc[ROW],
						chip->batt_fbon_tc_rc[COL], TWO_DIMENSION, "vivo,batt-fbon-tc-data");
		}
		...
		/* fboff battery param */
		chip->batt_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,batt-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,batt-fboff-tc-row-col");
		if(chip->batt_fboff_tc_rc) {
			chip->batt_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,batt-fboff-tc-data",chip->batt_fboff_tc_rc[ROW],
						chip->batt_fboff_tc_rc[COL], TWO_DIMENSION, "vivo,batt-fboff-tc-data");
		}
		...
		/* add for equalizing_charge */
		chip->eq_intell_charge_enable = of_property_read_bool(node, "vivo,eq-intell-charge-enable");

		if(chip->eq_intell_charge_enable)
			rc = cms_parse_equalizing_charge_param(chip, node);
	}
	...
	if (chip->ex_intell_charge_enable) {
		...
		/* fbon battery param */
		chip->ex_batt_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,ex-batt-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,ex-batt-fbon-tc-row-col");
		if(chip->ex_batt_fbon_tc_rc) {
			chip->ex_batt_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,ex-batt-fbon-tc-data",chip->ex_batt_fbon_tc_rc[ROW],
						chip->ex_batt_fbon_tc_rc[COL], TWO_DIMENSION, "vivo,ex-batt-fbon-tc-data");
		}
		...
		chip->ex_parallel_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,ex-parallel-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "ex-parallel-fbon-tc-row-col");
		...
		/* fboff battery param */
		chip->ex_batt_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,ex-batt-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,ex-batt-fboff-tc-row-col");
		if(chip->ex_batt_fboff_tc_rc) {
			chip->ex_batt_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,ex-batt-fboff-tc-data",chip->ex_batt_fboff_tc_rc[ROW],
						chip->ex_batt_fboff_tc_rc[COL], TWO_DIMENSION, "vivo,ex-batt-fboff-tc-data");
		}
	...
	/* bypass intell-charge battery param */
	chip->bypass_intell_charge_enable = of_property_read_bool(node, "vivo,bypass-intell-charge-enable");
	if (chip->bypass_intell_charge_enable) {
		/* fbon battery param */
		chip->bypass_batt_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,bypass-batt-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,bypass-batt-fbon-tc-row-col");
		if(chip->bypass_batt_fbon_tc_rc) {
			chip->bypass_batt_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,bypass-batt-fbon-tc-data",chip->bypass_batt_fbon_tc_rc[ROW],
						chip->bypass_batt_fbon_tc_rc[COL], TWO_DIMENSION, "vivo,bypass-batt-fbon-tc-data");
		}
	
		chip->bypass_primary_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,bypass-primary-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "bypass-primary-fbon-tc-row-col");
		if (chip->bypass_primary_fbon_tc_rc) {
			chip->bypass_primary_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,bypass-primary-fbon-tc-data", chip->bypass_primary_fbon_tc_rc[ROW],
						chip->bypass_primary_fbon_tc_rc[COL], TWO_DIMENSION, "bypass-primary_fbon_tc_data");
		}
		if (!chip->bypass_primary_fbon_tc_rc || !chip->bypass_primary_fbon_tc_data) {
			chip->bypass_primary_fbon_tc_data = default_primary_fbon_tc_data;
			chip->bypass_primary_fbon_tc_rc[ROW] = ARRAY_SIZE(default_primary_fbon_tc_data);
			chip->bypass_primary_fbon_tc_rc[COL] = 3;
		}

		chip->bypass_parallel_fbon_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,bypass-parallel-fbon-tc-row-col", 1, 2, ONE_DIMENSION, "bypass-parallel-fbon-tc-row-col");
		if (chip->bypass_parallel_fbon_tc_rc) {
			chip->bypass_parallel_fbon_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,bypass-parallel-fbon-tc-data", chip->bypass_parallel_fbon_tc_rc[ROW],
						chip->bypass_parallel_fbon_tc_rc[COL], TWO_DIMENSION, "bypass-parallel_fbon_tc_data");
		}
		if (!chip->bypass_parallel_fbon_tc_rc || !chip->bypass_parallel_fbon_tc_data) {
			chip->bypass_parallel_fbon_tc_data = default_parallel_fbon_tc_data;
			chip->bypass_parallel_fbon_tc_rc[ROW] = ARRAY_SIZE(default_parallel_fbon_tc_data);
			chip->bypass_parallel_fbon_tc_rc[COL] = 3;
		}

		/* fboff battery param */
		chip->bypass_batt_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,bypass-batt-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "vivo,bypass-batt-fboff-tc-row-col");
		if(chip->bypass_batt_fboff_tc_rc) {
			chip->bypass_batt_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,bypass-batt-fboff-tc-data",chip->bypass_batt_fboff_tc_rc[ROW],
						chip->bypass_batt_fboff_tc_rc[COL], TWO_DIMENSION, "vivo,bypass-batt-fboff-tc-data");
		}
		chip->bypass_primary_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,bypass-primary-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "bypass-primary-fboff-tc-row-col");
		if (chip->bypass_primary_fboff_tc_rc) {
			chip->bypass_primary_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,bypass-primary-fboff-tc-data", chip->bypass_primary_fboff_tc_rc[ROW],
						chip->bypass_primary_fboff_tc_rc[COL], TWO_DIMENSION, "bypass-primary_fboff_tc_data");
		}
		if (!chip->bypass_primary_fboff_tc_rc || !chip->bypass_primary_fboff_tc_data) {
			chip->bypass_primary_fboff_tc_data = default_primary_fboff_tc_data;
			chip->bypass_primary_fboff_tc_rc[ROW] = ARRAY_SIZE(default_primary_fboff_tc_data);
			chip->bypass_primary_fboff_tc_rc[COL] = 3;
		}

		chip->bypass_parallel_fboff_tc_rc = (int*)cms_of_get_property_data(chip, node,
				"vivo,bypass-parallel-fboff-tc-row-col", 1, 2, ONE_DIMENSION, "bypass-parallel-fboff-tc-row-col");
		if (chip->bypass_parallel_fboff_tc_rc) {
			chip->bypass_parallel_fboff_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,bypass-parallel-fboff-tc-data", chip->bypass_parallel_fboff_tc_rc[ROW],
						chip->bypass_parallel_fboff_tc_rc[COL], TWO_DIMENSION, "bypass-parallel_fboff_tc_data");
		}
		if (!chip->bypass_parallel_fboff_tc_rc || !chip->bypass_parallel_fboff_tc_data) {
			chip->bypass_parallel_fboff_tc_data = default_parallel_fboff_tc_data;
			chip->bypass_parallel_fboff_tc_rc[ROW] = ARRAY_SIZE(default_parallel_fboff_tc_data);
			chip->bypass_parallel_fboff_tc_rc[COL] = 3;
		}
	}
	...
	chip->charge_param_judge_threshold = (int *)cms_of_get_property_data(chip, node,
				"vivo,charge-param-judge-threshold", 1, 10, ONE_DIMENSION, "charge-param-judge-threshold");
	if (!chip->charge_param_judge_threshold) {
		chip->charge_param_judge_threshold = default_charge_param_judge_threshold;
	}
	cms_print(chip,PR_WARN,"charge_param_judge_threshold[%d, %d, %d, %d, %d, %d, %d, %d, %d, %d]\n",chip->charge_param_judge_threshold[0], chip->charge_param_judge_threshold[1], chip->charge_param_judge_threshold[2],
	chip->charge_param_judge_threshold[3], chip->charge_param_judge_threshold[4], chip->charge_param_judge_threshold[5], chip->charge_param_judge_threshold[6], chip->charge_param_judge_threshold[7],
	chip->charge_param_judge_threshold[8], chip->charge_param_judge_threshold[9]);
...
static void cms_release_property_data(struct cms *chip)
{
	...
	if(chip->intell_charge_enable){
		...
		if(chip->eq_intell_charge_enable){
			if(chip->eq_primary_fboff_tc_data != default_primary_fboff_tc_data){
				kfree(chip->eq_primary_fboff_tc_data);
				kfree(chip->eq_primary_fboff_tc_rc);
			}
			if(chip->eq_parallel_fboff_tc_data != default_parallel_fboff_tc_data){
				kfree(chip->eq_parallel_fboff_tc_data);
				kfree(chip->eq_parallel_fboff_tc_rc);
			}
			if(chip->eq_primary_fbon_tc_data != default_primary_fbon_tc_data){
				kfree(chip->eq_primary_fbon_tc_data);
				kfree(chip->eq_primary_fbon_tc_rc);
			}
			if(chip->eq_parallel_fbon_tc_data != default_parallel_fbon_tc_data){
				kfree(chip->eq_parallel_fbon_tc_data);
				kfree(chip->eq_parallel_fbon_tc_rc);
			}
		}
	}
...
static int cms_probe(struct platform_device *pdev)
{
	...
	cms_create_debugfs_entries(chip);	//NOTE: android11 delete debugfs in user version.  so we use the cms_class
...
static const struct of_device_id cms_match_table[] = {
	{ .compatible = "vivo,charger-monitor-v3", },
	{ },
};

static struct platform_driver cms_driver = {
	.driver = {
		.name = "charger-monitor-v3",
		.owner = THIS_MODULE,
