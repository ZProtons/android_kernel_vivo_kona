...
#define  ANTIFAKE_SHOOTER_SHOW_BIT    0x40
void sde_notify_fingershooter_event(struct drm_device *dev, int status)
{
	...
	if ((status & ANTIFAKE_SHOOTER_SHOW_BIT) == ANTIFAKE_SHOOTER_SHOW_BIT) {
		envp[i++] = "ANTIFAKE_SHOOTER_SHOW=1";
		pr_err("%s:set antifake", __func__);
	}

	if (vdisp->ud_shooter_perf) {
		if(((status & SHOOTER_SHOW_BIT) || (status & ANTIFAKE_SHOOTER_SHOW_BIT))
			&& (atomic_read(&after_hbm_count) >= 3))
...
void sde_fingerprint_notify_work(struct work_struct *work)
{
	...
	pr_info("start notify fingershooter state: %d\n", sde_crtc->fingershooter_status);
...
int sde_notify_fingerprint_status(struct drm_crtc *crtc)
{
	...
	pr_debug("%s:after_hbm_count=%d,after_shooter_count=%d", __func__, atomic_read(&after_hbm_count),
		atomic_read(&after_shooter_count));
	if (!vdisp->ud_shooter_perf) {
	...
	if (sde_crtc->fingershooter_status & ANTIFAKE_SHOOTER_SHOW_BIT) {
		need_notify = (sde_crtc->fingershooter_status & ANTIFAKE_SHOOTER_SHOW_BIT) ?
	...
	if (need_notify) {
		...
		pr_info("prepare notify fingershooter state: %d\n", sde_crtc->fingershooter_status);
...
EXPORT_SYMBOL(sde_notify_fingerprint_status);
/* zwl add end for fingerprint */
