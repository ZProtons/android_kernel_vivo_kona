...
#ifndef _SYNAPTICS_DSX_RMI4_INCELL_H_
#define _SYNAPTICS_DSX_RMI4_INCELL_H_
...
struct synaptics_rmi4_data {
	...
	struct syna_reg_addr *reg_addr_ptr;
	struct input_dev *input_dev;
	...
	struct mutex vivo_mutex;
	struct mutex geture_mutex;
	...
	int (*reset_device)(struct synaptics_rmi4_data *rmi4_data,
			bool rebuild);
	int (*reset_device_for_vivo)(struct synaptics_rmi4_data *rmi4_data,
			bool rebuild);
...
#if defined(CONFIG_SECURE_TOUCH)
extern void touchscreen_secure_touch_notify(void);
#endif
...
void synaptics_3706_rmi4_new_function(struct synaptics_rmi4_exp_fn *exp_fn_module,
		bool insert);
		...
		unsigned short len)
{
	...
	while(i2c_bus_state > 0) {
		VTI("warning:iic is on suspend.delay %dms.", (retry_count+1)*10);
		retry_count++;
		msleep(10);
		if(retry_count > 50) {
			VTI("error:after 500ms delay , fts iic is still  on suspend.");
			return -1;
		}
	}
		...
		unsigned short len)
{
	...
	while(i2c_bus_state > 0) {
		...
		msleep(10);
		if(retry_count > 50) {
			VTI("error:after 60ms delay , fts iic is still  on suspend.");
			return -1;
		}
	}
