...
#define EVENT_BUF_SIZE 7
static int event_counter;
static short event_buf[EVENT_BUF_SIZE];
/***********************************************************/
...
static unsigned int is_at_boot_dl;
/***********************************************************/
...
static int m1120_i2c_read(struct i2c_client *client, u8 reg, u8 *rdata, u8 len)
{
	...
	if (rc < 0) {
		mxerr(&client->dev, "i2c_transfer %02x was failed(%d)", reg, rc);
		return rc;
	}
...
static void m1120_set_enable(struct device *dev, int enable)
{
	...
	mxinfo(dev, "m1120_set_enable %d", enable);
...
static void m1120_set_delay(struct device *dev, int delay)
{
	...
	mxinfo(dev, "m1120_set_delay %d", delay);
	...
	if (m1120_get_enable(dev)) {
		if (!(p_data->reg.map.intsrs & M1120_DETECTION_MODE_INTERRUPT)) {
			...
			mxerr(dev, "schedule_delayed_work");
		}
	}
...
static int m1120_set_operation_mode(struct device *dev, int mode)
{
	...
	mxinfo(&client->dev, "m1120_set_operation_mode %d", mode);
	switch (mode) {
	...
	case OPERATION_MODE_MEASUREMENT:
		...
		opf |= M1120_VAL_OPF_HSSON_ON|M1120_VAL_OPF_FREQ_80HZ;
		err = m1120_i2c_set_reg(client, M1120_REG_OPF, opf);
		if (p_data->reg.map.intsrs & M1120_DETECTION_MODE_INTERRUPT) {
			if (!p_data->irq_enabled) {
				/*enable irq*/
...
static int m1120_measure(m1120_data_t *p_data, short *raw)
{
	...
	m1120_i2c_read(client, M1120_REG_OPF, &opfbuf, 1);
	// (1) read data
	err = m1120_i2c_read(client, M1120_REG_ST1, buf, 1);
	if (err)
		return err;
	...
	event_counter %= EVENT_BUF_SIZE;
	event_buf[event_counter] = *raw;
	event_counter++;

	if (event_counter == EVENT_BUF_SIZE) {
		for (i = 0; i < EVENT_BUF_SIZE; i++) {
			snprintf(log_buf, 100, "%s %d", log_buf, event_buf[i]);
		}
		mxinfo(&client->dev, "m1120_measure st1 0x%02x opfbuf 0x%02x raw data (%s)\n", buf[0], opfbuf, log_buf);
	}
...
static int m1120_power_ctl(m1120_data_t *data, bool on)
{
	...
	mxinfo(&data->client->dev, "m1120_power_ctl on %d enabled %d\n", on, data->power_enabled);
...
static int m1120_i2c_drv_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	hall_dev->hall_type = RISING_HALL;
...
static const struct of_device_id m1120_of_match[] = {
	{ .compatible = "magnachip,sensor_mxm1120,down", },
	{ },
};
...
static int __init m1120_driver_init_down(void)
{
	printk(KERN_INFO "%s\n", __func__);
	if (is_at_boot_dl == 1) {
		printk(KERN_INFO "%s now is at mode, not load mxm1120_down driver\n", __func__);
		return 0;
	}
