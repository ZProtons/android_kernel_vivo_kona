...
static int iris_frc_no_fallback = 0;
...
void iris_frc_mif_reg_set(void)
{
	...
	case 30:
		if (pcfg->frc_low_latency) {
			...
			three_metagen_en = 0;
			dedicate_01phase_en = 1;
		} else if (frc_setting->out_fps == 60 && iris_game_mode) {
			three_mvc_en = 1;
			three_metagen_en = 0;
	...
	case 45:
		if (pcfg->frc_low_latency) {
			...
			frc_ratio = 0x801;
			frcc_teadj_th = 0x288080;
		} else if (frc_setting->out_fps == 90 && iris_game_mode) {
			three_mvc_en = 1;
			three_metagen_en = 0;
			frcc_teadj_th = 0x288080;
			phase_teadj_en = 0;
	...
	if (pcfg->frc_setting.no_fallback == 1 || iris_frc_no_fallback == 1) {
	...
	if(iris_frc_fallback_disable || pcfg->frc_setting.no_fallback == 1 || iris_frc_no_fallback == 1)
...
void iris_mvc_reg_set(void)
{
	...
	//iris_frc_reg_add(IRIS_MVC_ADDR + MVC_MVSELECT_0, 0x01447c03, 0);
	...
	if (iris_game_mode &&
		...
		(frc_setting->in_fps == 60 && frc_setting->out_fps == 120))) {
...
static int iris_get_mode(void)
{
	...
	if ((pcfg->dport_output_mode == 0) && (pwil_mode == FRC_MODE || pwil_mode == PT_MODE))
...
int iris_dbgfs_ms_init(struct dsi_display *display)
{
	...
	debugfs_create_u32("iris_frc_no_fallback", 0644, pcfg->dbg_root,
		(u32 *)&iris_frc_no_fallback);
