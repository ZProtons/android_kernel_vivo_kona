...
static void gpio_keys_gpio_report_event(struct gpio_button_data *bdata)
{
	...
	printk(KERN_ERR "[HW_KEY]keycode PMIC input: code=%d, sts=0x%hhx\n", button->code, state);
	input_sync(input);
}
...
static int __maybe_unused gpio_keys_resume(struct device *dev)
{
	...
	int error = 0;
	int i = 0;
	int state;/*vivo sensor team adds*/
	if (device_may_wakeup(dev)) {
	...
	/*vivo sensor team adds start*/
	for (i = 0; i < ddata->pdata->nbuttons; i++) {
		struct gpio_button_data *bdata = &ddata->data[i];
		state = gpiod_get_value_cansleep(bdata->gpiod);
		printk(KERN_ERR "gpio_keys_resume: code=%d, sts=0x%hhx\n", bdata->button->code, state);
	}
	/*vivo sensor team adds end*/
	/*gpio_keys_report_state(ddata);*//*vivo sensor team deletes*/
	return 0;
}
