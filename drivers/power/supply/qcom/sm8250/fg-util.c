...
/* vivo add start*/
static int msoc_raw_mapping(struct fg_dev *fg, int msoc)
{
	int msoc_map = msoc;

	if(!fg->msoc_mapping_enable){
		//fg_dbg(fg, FG_POWER_SUPPLY, "msoc_raw_mapping isn't enabled\n");
		return msoc;
	}

	if(msoc < 0 || msoc >= MSOC_MAPPING_ROW ){
		fg_dbg(fg, FG_POWER_SUPPLY, "msoc:%d, out of range", msoc);
	}else{
		if (fg->charge_status == POWER_SUPPLY_STATUS_DISCHARGING)
			msoc_map = (fg->msoc_mapping_table)[msoc].msoc_mapping_discharging;
		else
			msoc_map = (fg->msoc_mapping_table)[msoc].msoc_mapping_charging;
	}
	return msoc_map;
}
/* vivo end */
...
int fg_get_msoc_raw(struct fg_dev *fg, int *val)
{
	u8 cap[2];
	int rc, tries = 0;
	int msoc_raw_map = 0;	//vivo add
	static int last_msoc_raw = 0, last_msoc_map = 0;
	...
	/* vivo change */
	*val = cap[0];
	msoc_raw_map = msoc_raw_mapping(fg, cap[0]);
	if (last_msoc_map != msoc_raw_map || last_msoc_raw != cap[0])
		fg_dbg(fg, FG_POWER_SUPPLY, "raw: 0x%02x -> map: 0x%02x. (%s). msoc_mapping_enable=%d.\n", cap[0], msoc_raw_map, (fg->charge_status == POWER_SUPPLY_STATUS_DISCHARGING) ? "dis-chg" : "chg", fg->msoc_mapping_enable);
	*val = msoc_raw_map;
	last_msoc_raw = cap[0];
	last_msoc_map = msoc_raw_map;
	/* vivo end */
	return 0;
}

//vivo add comment: below is qcom origin map data, caculation from below func.
#if 0
msoc	soc
0	0
1	1
...
int fg_get_battery_resistance(struct fg_dev *fg, int *val)
{
	...
	*val = esr_uohms + rslow_uohms;
	/* vivo add start */
	pr_err("bms Resistance: esr=%d mohms,  rslow=%d mohms, total=%d mohms.\n",
		esr_uohms/1000, rslow_uohms/1000, *val/1000);
	/* vivo add end */
	return 0;
}
