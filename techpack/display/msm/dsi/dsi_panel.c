...
int dsi_panel_trigger_esd_attack(struct dsi_panel *panel)
{
	...
	if (gpio_is_valid(r_config->reset_gpio)) {
		...
		int ret = gpio_request_one(r_config->reset_gpio, GPIOF_OUT_INIT_LOW, NULL);
		if (ret < 0)
			DSI_ERR("gpio_request_one fail %d, %d\n", ret, r_config->reset_gpio);
		gpio_free(r_config->reset_gpio);
#else
...
/* vivo display team add for pixelworks memc end */
	...
	run_list(panel->panel_of_node, "vivo_panel_reset");
...
#ifdef PANEL_BOARD_POWER_CTRL
atomic_t power_on_refcount;

int vivo_panel_power_on(struct dsi_panel *panel)
{
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
	if (iris_is_dual_supported() && panel->is_secondary) {
		dsi_pwr_enable_regulator(&panel->power_info, true);
/* #endif */
/* vivo display team add for pixelworks memc end */
	} else if (atomic_read(&power_on_refcount) == 0) {
		LCD_INFO("+\n");
		run_list(panel->panel_of_node, "vivo_panel_power_on");
		atomic_set(&power_on_refcount, 1);
		LCD_INFO("-\n");
	} else {
		LCD_ERR("power_on_refcount=%d, no need power on\n", atomic_read(&power_on_refcount));
	}

	return 0;
}

int vivo_panel_power_off(struct dsi_panel *panel)
{
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
	if (iris_is_dual_supported() && panel->is_secondary) {
...
/* vivo display team add for pixelworks memc end */
		...
		LCD_ERR("power_on_refcount=%d, no need power off\n", atomic_read(&power_on_refcount));
	} else {
		LCD_INFO("+\n");
		if (is_shutdown && is_power_list_exist(panel->panel_of_node, "vivo_panel_shutdown"))
			run_list(panel->panel_of_node, "vivo_panel_shutdown");
		else
			run_list(panel->panel_of_node, "vivo_panel_power_off");

		atomic_set(&power_on_refcount, 0);
		LCD_INFO("-\n");
	}
...
extern void lcm_bias_set_avdd_n_avee(int level);
static int dsi_panel_power_on(struct dsi_panel *panel)
{
	int rc = 0;
	struct vivo_display *vdisp = get_vivo_vdisp();

	vivo_panel_power_on(panel);

	if (vdisp->bias_config.sm5109_support)
		lcm_bias_set_avdd_n_avee(vdisp->bias_config.bias_regulator);
	rc = dsi_panel_set_pinctrl_state(panel, true);
	if (rc) {
		DSI_ERR("[%s] failed to set pinctrl state, rc=%d\n", panel->name, rc);
		goto error_disable_vregs;
	}

	/*restore the level state of ELVDD after pinctrl*/
	if (vdisp && vdisp->elvdd_mos_ctrl.is_elvdd_ctrl && gpio_is_valid(vdisp->elvdd_mos_ctrl.elvdd_ctrl_gpio))
		gpio_direction_output(vdisp->elvdd_mos_ctrl.elvdd_ctrl_gpio, vdisp->elvdd_mos_ctrl.elvdd_ctrl_gpio_level);
...
error_disable_gpio:
	dsi_panel_set_pinctrl_state(panel, false);

error_disable_vregs:
	vivo_panel_power_off(panel);
...
static int dsi_panel_power_off(struct dsi_panel *panel)
{
	...
	//wait mipi_data lane from LP down to 0V
	...
	if (!strcmp(vdisp->ddic_name, "ea8079"))
		usleep_range(15*1000, (15*1000)+100);
	else
...
enum reset_and_power_ctrl_tp_type {
	POWER_CTRL_AVDDOFF_AVEEOFF_RST_LOW = 0,
	POWER_CTRL_AVDDON_AVEEON_RST_HIGH,
	POWER_CTRL_AVDDOFF_AVEEOFF_RST_HIGH,
	POWER_CTRL_AVDD_ON,
	POWER_CTRL_AVDD_OFF,
	POWER_CTRL_VDDR_OFF,	/* VDDR power off for NT36672C to enter guesture mode */
	POWER_CTRL_DISP_PANEL_POWER_ON_SEQ,
	POWER_CTRL_UNKNOWN
};

/*
 * dsi_panel_reset_and_powerctl -- control panel power by touchscreen
 * @enable: enum reset_and_power_ctrl_tp_type
 */
int mdss_dsi_panel_reset_and_powerctl(int enable)
{
	...
	if (!vdisp->tddi.is_power_off) {
		mutex_unlock(&panel->panel_lock);
		LCD_ERR("failed to operate reset and power panel state %d!\n", vdisp->tddi.is_power_off);
		return rc;
	}

	LCD_INFO("dsi panel power and reset called by tp,enable=%d\n", enable);
	/* TODO */
	switch (enable) {
	case POWER_CTRL_AVDDOFF_AVEEOFF_RST_LOW: /* 0 */
		dsi_panel_power_off(panel);
		break;
	case POWER_CTRL_AVDDON_AVEEON_RST_HIGH: /* 1 */
		dsi_panel_power_on(panel);
		break;
	case POWER_CTRL_AVDDOFF_AVEEOFF_RST_HIGH: /* 2 */
		run_list(panel->panel_of_node, "vivo_panel_reset_high");
		break;
	case POWER_CTRL_AVDD_ON: /* 3 */
		run_list(panel->panel_of_node, "vivo_panel_avdd_on");
		break;
	case POWER_CTRL_AVDD_OFF: /* 4 */
		run_list(panel->panel_of_node, "vivo_panel_avdd_off");
		break;
	case POWER_CTRL_VDDR_OFF: /* 5 */
		run_list(panel->panel_of_node, "vivo_panel_vddr_off");
		break;
	case POWER_CTRL_DISP_PANEL_POWER_ON_SEQ: /* 6 */
		run_list(panel->panel_of_node, "vivo_panel_power_on_seq");
		break;
	default:
		LCD_ERR("unkown power ctrl type\n");
		break;
	}
				...
				enum dsi_cmd_set_type type)
{
	...
	//vivo keli add for vdisp begin
	struct vivo_display *vdisp = NULL;
	//vivo keli add for vdisp end

	if (!panel || !panel->cur_mode)
		return -EINVAL;
	//vivo keli add for no panel begin
	vdisp = panel->panel_private;
	if (vdisp && (!strcmp(vdisp->project_name, "no_panel_video")))
		return rc;
	//vivo keli add for no panel end
	...
	pr_err("%s:cmds[%d]=%s,count=%d", __func__, type, cmd_set_prop_map[type], count);
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
...
extern int vivo_adjust_cabc(struct vivo_display *vdisp, u32 bl_lvl);
extern void lm3697_enable_control_a_pwm(bool enable);
int dsi_panel_set_backlight(struct dsi_panel *panel, u32 bl_lvl)
{
	int rc = 0;
	struct dsi_backlight_config *bl = &panel->bl_config;
	static u32 pre_bl_lvl;
	//vivo keli add for vdisp data begin
	struct vivo_display *vdisp = NULL;
	//vivo keli add for vdisp data end
	...
	//vivo zwl add for dc dimming dither begin
	if (vdisp)
		vdisp->userspace_bl_lvl = bl_lvl;
	//vivo zwl add for dc dimming dither end
	...
	case DSI_BACKLIGHT_DCS:
		//vivo keli modify for backlight begin
		if (!strcmp(vdisp->project_name, "pd1981") || !strcmp(vdisp->project_name, "pd2073")) {
			if (pre_bl_lvl > 0 && (bl_lvl == 0)) {
				//send display off to panel by diret mode
				vdisp->dsi_cmd_send_type = DSI_CMD_SEND_BY_DIRECT;
				rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ALL_PIXELS_OFF);
				if (rc)
					DSI_ERR("[%s] failed to send DSI_CMD_SET_ALL_PIXELS_OFF cmds, rc=%d\n",
						   panel->name, rc);
				vdisp->dsi_cmd_send_type = DSI_CMD_SEND_BY_DEFAULT;
				vdisp->is_all_pixel_off = 1;
			} else if (pre_bl_lvl == 0 && (bl_lvl > 0) && vdisp->is_all_pixel_off) {
				vdisp->is_all_pixel_off = 0;
				vdisp->dsi_cmd_send_type = DSI_CMD_SEND_BY_DIRECT;
				rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_ALL_PIXELS_ON);
				if (rc)
					DSI_ERR("[%s] failed to send DSI_CMD_SET_ALL_PIXELS_ON cmds, rc=%d\n",
						   panel->name, rc);
				vdisp->dsi_cmd_send_type = DSI_CMD_SEND_BY_DEFAULT;
				if (!IS_ERR_OR_NULL(vdisp->panel_ops.dsi_collection_exception))
					vdisp->panel_ops.dsi_collection_exception(vdisp);
			}
			pr_info("%s: pre_bl_lvl = %d, current_bl_lvl=%d", __func__, pre_bl_lvl, bl_lvl);
			lm3697_brightness_set(bl_lvl);
			vivo_adjust_cabc(vdisp, bl_lvl);
			pre_bl_lvl = bl_lvl;
		} else if (!strcmp(vdisp->project_name, "pd1963") || !strcmp(vdisp->project_name, "pd2012")) {
			lm3697_brightness_set(bl_lvl);
		} else {
			if (vdisp && (!IS_ERR_OR_NULL(vdisp->panel_ops.set_backlight)))
				rc = vdisp->panel_ops.set_backlight(vdisp, bl_lvl);
		}
		unset_backlight = 0;
		break;
...
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
	iris_set_panel_dynamic_clk_cap(panel);
/* #endif */
/* vivo display team add for pixelworks memc end */
...
/* vivo display team add for pixelworks memc end */
	...
	panel->reset_config.disp_bkg_gpio = utils->get_named_gpio(utils->data,
				"qcom,platform-bkg-en-gpio", 0);
	if (!gpio_is_valid(panel->reset_config.disp_bkg_gpio))
		DSI_INFO("[%s] failed get disp_bkg_gpio, rc=%d\n", panel->name, rc);
...
int qcom_dsi_write_cmdlist(struct vivo_display *vdisp, enum dsi_cmd_set_type type)
{
	int rc = 0;
	struct dsi_panel *panel = NULL;
	...
	if (!panel) {
		LCD_ERR("qcom_dsi_write_cmdlist panel is NULL\n");
		return -EINVAL;
	}
...
int qcom_panel_update_backlight(struct vivo_display *vdisp, u32 bl_lvl)
{
	int rc = 0;
	struct dsi_panel *panel = NULL;
...
/* vivo display team add for pixelworks memc end */
	...
	if (!panel) {
		LCD_ERR("qcom_panel_update_backlight panel is NULL\n");
		return -EINVAL;
	}
...
/* vivo display team add for pixelworks memc end */
	...
	vdisp->dc.last_bl_lvl = bl_lvl;
	vdisp->last_bl_timestamp = ktime_get();

	return rc;
}

int qcom_panel_dsi_write_dcs_cmd(struct vivo_display *vdisp, u8 cmd,
...
/* vivo display team add for pixelworks memc end */
	...
	rc = mipi_dsi_dcs_write(dsi, cmd, data, len);

	if (rc < 0)
		LCD_ERR("failed to qcom_panel_dsi_write_dcs_cmd\n");

	return rc;
}
static int dsi_panel_read_panel_reg(struct dsi_display_ctrl *ctrl, struct dsi_panel *panel,
		u8 cmd, void *rbuf,  size_t len)
{
	int rc = 0;
	...
	cmdsreq.msg.flags |= BIT(3);
	flags |= (DSI_CTRL_CMD_FETCH_MEMORY | DSI_CTRL_CMD_READ);
	rc = dsi_ctrl_cmd_transfer(ctrl->ctrl, &cmdsreq.msg, &flags);
	if (rc <= 0) {
		LCD_ERR("dsi_display_read_panel_reg rx cmd transfer failed rc=%d\n", rc);
		goto error;
	}
error:
...
static dsi_display_read_panel_reg(struct dsi_display *display, u8 cmd, void *data, size_t len)
{
	int rc = 0;
...
void qcom_vivo_panel_ops_init(struct vivo_display *vdisp)
{
	LCD_INFO("\n");
	vdisp->panel_ops.dsi_write_cmdlist = qcom_dsi_write_cmdlist;
	vdisp->panel_ops.dsi_write_dcs_cmd = qcom_panel_dsi_write_dcs_cmd;
	vdisp->panel_ops.update_backlight = qcom_panel_update_backlight;
	vdisp->panel_ops.dsi_read_cmd = dsi_display_read_panel_reg;
}
...
/* vivo display team add for pixelworks memc end */
	...
	vdisp = get_vivo_vdisp();
	if (vdisp) {
		vdisp->panel_ops.ops_init = qcom_vivo_panel_ops_init;
	}
	vivo_display_init(panel);
...
/* vivo display team add for pixelworks memc end */
	...
	if (panel->lp11_init) {
#ifdef PANEL_BOARD_POWER_CTRL
		if (vdisp && vdisp->tddi.is_support) {
			vivo_panel_power_on(panel);
			if (!strcmp(vdisp->ddic_name, "nt36672"))
				run_list(panel->panel_of_node, "vivo_panel_vddr_on");
		}
#endif
		goto error;
	}
...
int dsi_panel_set_lp1(struct dsi_panel *panel)
{
	 ...
	 */
	 /* vivo ouenhui delete below code
	...
	*/
	vdisp = panel->panel_private;
	if (vdisp->doze_mode_state == 1) {
		LCD_ERR("[0x%02x] do not need to set aod on again\n", vdisp->panel_id);
	} else {
		if (vdisp->oled_hbm_state == PANEL_HBM_AOD_UD_ON && panel->bl_config.bl_level == 0 && vdisp->ud_hbm_state == 1) {
			LCD_ERR("[0x%02x] aod off but hbm on cannot enter aod mode\n", vdisp->panel_id);
		} else {
			if (vdisp->oled_hbm_state == PANEL_HBM_POWER_OFF) {
				vdisp->oled_hbm_state = PANEL_HBM_AOD_UD_ON;
				dsi_panel_update_hbm_aod_ud_fingerprint(panel, vdisp->oled_hbm_state);
			} else {
				vdisp->dsi_cmd_send_type = DSI_CMD_SEND_BY_DIRECT;
				...
				if (rc)
					pr_err("[%s] failed to send DSI_CMD_SET_LP1 cmd, rc=%d\n",
						   panel->name, rc);
				/* ea8079: wait two te signal to ensure elvss ready before 0x29*/
				if (!strcmp(vdisp->ddic_name, "ea8079")) {
					ctl_display_te_irq_enable(true);
					dsi_wait_one_vblank(vdisp);
					dsi_wait_one_vblank(vdisp);
					ctl_display_te_irq_enable(false);
				}
				vdisp->doze_mode_state = 1;

				if ((!strcmp(vdisp->project_name, "pd2001") || !strcmp(vdisp->project_name, "pd2005")) &&
				    (vdisp->panel_id3 == 0x0 || vdisp->panel_id3 == 0x1)) {
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_DIFF_SET);
					if (rc)
						LCD_ERR("[%s] failed to send AOD_DIFF_SET cmd, rc=%d\n", panel->name, rc);
				}

				if (panel->power_mode != SDE_MODE_DPMS_OFF) {
					LCD_INFO("set hlpmmode oled_hbm_state = %d; bl_level = %d\n",
						vdisp->oled_hbm_state, panel->bl_config.bl_level);
					rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_ON_DISPLAY_ON);
					if (rc)
						LCD_ERR("[%s] failed to send SET_HLPM_HIGHT cmd, rc=%d\n", panel->name, rc);
					vdisp->oled_hlpm_state = 1;
					panel->bl_config.bl_level = 0;
				} else {
					vdisp->ud_hbm_sync.wait_pp_done = 1;
				}
				vdisp->dsi_cmd_send_type = DSI_CMD_SEND_BY_DEFAULT;
			}
		}
	}
	pr_err("[0x%02x]dsi_panel_set_lp1\n", vdisp->panel_id);
exit:
	return rc;
}
...
int dsi_panel_set_lp2(struct dsi_panel *panel)
{
	...
	if (!panel->panel_initialized)
		goto exit;
	vdisp = panel->panel_private;
	if (vdisp && vdisp->doze_mode_state != 1) {
		if (vdisp->oled_hbm_state == PANEL_HBM_AOD_UD_ON && panel->bl_config.bl_level == 0 && vdisp->ud_hbm_state == 1) {
			LCD_ERR("[0x%02x] aod off but hbm on cannot enter aod mode\n", vdisp->panel_id);
		} else {
			if (vdisp->oled_hbm_state == PANEL_HBM_POWER_OFF) {
...
int dsi_panel_set_nolp(struct dsi_panel *panel)
{
	...
	*/
	...
	vdisp = panel->panel_private;
	if (vdisp->doze_mode_state == 0) {
		LCD_ERR("[0x%02x] do not need to set aod off again\n", vdisp->panel_id);
	} else {
		rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_AOD_OFF);/*DSI_CMD_SET_NOLP*/
		if (rc)
			LCD_ERR("[0x%02x] failed to send DSI_CMD_SET_NOLP cmd, rc=%d\n",
			       vdisp->panel_id, rc);
	...
	pr_err("[0x%02x]dsi_panel_set_nolp,oled_hbm_state=%d\n", vdisp->panel_id, vdisp->oled_hbm_state);
exit:
	return rc;
}
...
/* vivo display team add for pixelworks memc end */
	...
	if (rc)
		DSI_ERR("[%s] failed to send DSI_CMD_SET_TIMING_SWITCH cmds, rc=%d\n",
		       panel->name, rc);
	//vivo ouenhui add for ea8079 gamma set begin
	if (vdisp->ea8079_dynamic_gamma.is_support && vdisp->ea8079_dynamic_gamma.gamma_90hz_valid
		&& vdisp->ea8079_dynamic_gamma.gamma_120hz_valid) {
		if (panel->cur_mode->timing.refresh_rate == 90)
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_90HZ_GAMMA);
		else if (panel->cur_mode->timing.refresh_rate == 120)
			rc = dsi_panel_tx_cmd_set(panel, DSI_CMD_SET_120HZ_GAMMA);
		if (rc)
			DSI_ERR("[%s] failed to send GAMMA cmds, rc=%d\n",
			       panel->name, rc);
	}
	//vivo ouenhui add for ea8079 gamma set end
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
extern int bbk_slsi_expect_mode(void);
...
/* vivo display team add for pixelworks memc end */
	...
	vdisp->tsp_sync_enable = true; /* include tsp sync enable in init code */
	/*add for rewrite feature value*/
	feature_value_rebuild_when_resume(panel);

	//vivo keli add for log print begin
	pr_err("dsi_panel_enable\n");
	//vivo keli add for log print end
	...
	/* set TSP sync disable when TP is in sleep mode */
	if (vdisp && bbk_slsi_expect_mode() == 1)
		dsi_panel_set_tsp_sync(false);
	return rc;
}
...
/* vivo display team add for pixelworks memc end */
	...
	panel->power_mode = SDE_MODE_DPMS_OFF;
	//vivo keli add for log print begin
	pr_err("dsi_panel_disable\n");
	vdisp->tddi.is_power_off = true;
	vdisp->hdr10p_bl.enable = 0;
	vdisp->oled_hbm_state = 0;
	vdisp->ud_hbm_state = 0;
	//vivo keli add for log print end
	vdisp->tsp_sync_enable = false;
	mutex_unlock(&panel->panel_lock);
	return rc;
}
...
static bool is_allow_power_off(struct dsi_panel *panel)
{
	struct vivo_display *vdisp = NULL;
	...
	if (vdisp && vdisp->tddi.is_support) {
		/*
		 * While touchscreen FW is downloaded in AT mode
		 * so power off by touchscreen to fix TP enter deep sleep failed in AT mode
		 */
		if (vdisp->panel_id == 0xFF || is_shutdown) { /* no panel connected or shutdown should power off */
			return true;
		} else if (esd_detect_fail_recovery) { /* ESD check failed should power off */
			esd_detect_fail_recovery = 0;
			return true;
		} else { /* power off by touchscreen for TDDI panel */
			return false;
		}
	}
