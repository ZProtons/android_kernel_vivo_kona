...
static void cms_battery_det_done(struct cms *chip)
{
	...
	fuelsummary_collect_value(ID_BYTE__BATID, chip->battery_rc.bat_id);
	if(chip->complete)
			       ...
			       union power_supply_propval *val)
{
	...
	switch(psp){
		...
		case POWER_SUPPLY_PROP_CALLING_STATE:
			...
			break;
		case POWER_SUPPLY_PROP_LIVE_STATE:
			val->intval = chip->live_state;
			break;
		case POWER_SUPPLY_PROP_CAMERA_STATE:
			val->intval = chip->camera_state;
			break;
		...
		*/
		...
		case POWER_SUPPLY_PROP_CHARGING_TECHNOLOGY:
			...
			break;
		case POWER_SUPPLY_PROP_SCENE_RECOGNITION:
			val->intval = chip->intell_scene;
			break;
			       ...
			       const union power_supply_propval *val)
{
	...
	switch(psp){
		...
		case POWER_SUPPLY_PROP_CALLING_STATE:
			...
			fuelsummary_collect_value(ID_BIT__CALLING, chip->calling_state);
			cms_print(chip,PR_INFO,"chip->calling_state = %d\n",chip->calling_state);
			break;
		case POWER_SUPPLY_PROP_LIVE_STATE:
			chip->live_state = val->intval;
			cms_print(chip,PR_INFO,"chip->live_state = %d\n",chip->live_state);
			break;
		case POWER_SUPPLY_PROP_CAMERA_STATE:
			chip->camera_state = val->intval;
			cms_print(chip, PR_INFO, "chip->camera_state = %d\n", chip->camera_state);
			break;
		case POWER_SUPPLY_PROP_WEIXIN_CALLING_STATE:
		...
		case POWER_SUPPLY_PROP_BSPTEST_STOP_CHG_SOC:
			...
			break;
		case POWER_SUPPLY_PROP_SCENE_RECOGNITION:
			chip->intell_scene = val->intval;
			cms_print(chip, PR_INFO, "#### vivo intell_scene = %d ###\n", chip->intell_scene);
			break;
		case POWER_SUPPLY_PROP_EXHIBITION_MODE:
			chip->exhibition_mode = val->intval;
			cms_print(chip, PR_INFO, "### %s vivo exhibition mode = %d ###\n", chip->exhibition_mode);
			break;
		default:
			ret = -EINVAL;
	}
...
static enum power_supply_property cms_power_props[] = {
	...
	POWER_SUPPLY_PROP_SCENE_RECOGNITION,
...
static void cms_external_power_changed(struct power_supply *psy)
{
    ...
    if(chip->enabled && chip->online && !chip->cms_work_started) {
		...
		if (chip->direct_charger_enable && chip->calling_dchg_disable && chip->live_dchg_disable) {
			...
			if (chip->dchg_disable_votable) {
				...
				vote(chip->dchg_disable_votable, LIVE_EVENT, false, 0);
			}
		}
    }
    mutex_unlock(&chip->status_lock);
	...
	switch(fb_state){
		case FB_ON:
			if (chip->ex_intell_charge_enable && !fast_chg_state) {
				...
				if (chip->scene_recognition_enable && chip->intell_scene) {
					cms_print(chip, PR_INFO, "scene %d recognized\n", chip->intell_scene);
					switch (chip->intell_scene) {
					case SPECIAL_SCENE_1:
						pri_data = chip->primary_scene_1_tc_data;
						pri_len = chip->primary_scene_1_tc_rc[ROW];
						par_data = chip->parallel_scene_1_tc_data;
						par_len = chip->parallel_scene_1_tc_rc[ROW];
						break;
					case SPECIAL_SCENE_2:
						pri_data = chip->primary_scene_2_tc_data;
						pri_len = chip->primary_scene_2_tc_rc[ROW];
						par_data = chip->parallel_scene_2_tc_data;
						par_len = chip->parallel_scene_2_tc_rc[ROW];
						break;
					case SPECIAL_SCENE_3:
						pri_data = chip->primary_scene_3_tc_data;
						pri_len = chip->primary_scene_3_tc_rc[ROW];
						par_data = chip->parallel_scene_3_tc_data;
						par_len = chip->parallel_scene_3_tc_rc[ROW];
						break;
					case NORMAL_SCENE:
					default:
...
{
	bool system_start = ((u64)(local_clock()/1000000000)) > 60;/*wait 1min*/
	int batt_scale = 0;
...
static int cms_check_live_charge_current(struct cms *chip)
{
	int batt_scale = 0;
	...
	if (!chip->live_scale_enable) {
		cms_print(chip,PR_WARN,"live scale isn't enabled!\n");
		return 0;
	}
	if (power_off_charging_mode) {
		cms_print(chip, PR_WARN, "power off charing mode,not live scale\n");
		return 0;
	}
	if(cms_is_voter_priority(chip)){
		cms_print(chip, PR_WARN, "live scale isn't priority!\n");
		return 0;
	}
	if (chip->direct_charger_enable && chip->live_dchg_disable) {
		if (!chip->dchg_disable_votable)
			chip->dchg_disable_votable = find_votable("DCHG_DISABLE");
	...
	if(chip->live_state){
		cms_print(chip, PR_INFO, "start live scale\n");
		if (chip->dchg_disable_votable) {
			if(chip->mcu_psy) {
					power_supply_set_property(chip->mcu_psy,
						POWER_SUPPLY_PROP_DCHG_LIVE_START, &pval);
			}
			vote(chip->dchg_disable_votable, LIVE_EVENT, true, 0);
		} else {
			vote(chip->user_ibat_votable, LIVE_EVENT, true, chip->live_scale_data[LIVE_IBAT]);
			vote(chip->user_input_votable, LIVE_EVENT, true, chip->live_scale_data[LIVE_INPUT]);
			batt_scale = chip->live_scale_data[LIVE_IBAT];
		}
	} else {
		cms_print(chip, PR_INFO, "stop live scale\n");
		if (chip->dchg_disable_votable) {
			vote(chip->dchg_disable_votable, LIVE_EVENT, false, 0);
		} else {
			vote(chip->user_ibat_votable, LIVE_EVENT, false, 0);
			vote(chip->user_input_votable, LIVE_EVENT, false, 0);
		}
	}
	return batt_scale;
}
...
static int cms_check_camera_charge_current(struct cms *chip)
{
	int batt_scale = 0;
	union power_supply_propval primary_board_temp_now = {0,};
	if (!chip->online) {
	...
	if (!chip->camera_scale_enable) {
		cms_print(chip, PR_WARN, "camera scale isn't enabled!\n");
		return 0;
	}
	if (power_off_charging_mode) {
		cms_print(chip, PR_WARN, "power off charing mode,not camera scale\n");
		return 0;
	}
	if (cms_is_voter_priority(chip)) {
		cms_print(chip, PR_WARN, "camera scale isn't priority!\n");
		return 0;
	}
	power_supply_get_property(chip->batt_psy,
		POWER_SUPPLY_PROP_PRIMARY_BOARD_TEMP, &primary_board_temp_now);
	if (chip->camera_state && primary_board_temp_now.intval > chip->camera_scale_data[CAMERA_TEMP_LIMIT]) {
		cms_print(chip, PR_INFO, "camera scale..\n");
		vote(chip->user_ibat_votable, CAMERA_EVENT, true, chip->camera_scale_data[CAMERA_IBAT]);
		vote(chip->user_input_votable, CAMERA_EVENT, true, chip->camera_scale_data[CAMERA_INPUT]);
		batt_scale = chip->camera_scale_data[CAMERA_IBAT];
	} else {
		vote(chip->user_ibat_votable, CAMERA_EVENT, false, 0);
		vote(chip->user_input_votable, CAMERA_EVENT, false, 0);
	}
	return batt_scale;
}
...
static bool cms_select_normal_data(struct cms *chip)
{
	int batid = 0;
	int *tmp_rc;
	struct tc_data_item_ex *tmp_data;
	if (!chip->dual_normal_config)
		return true;
	batid = cms_get_prop_batt_id(chip);
	if (batid <= 0) {
		cms_print(chip, PR_INFO, "get batid=%d <= 0, will check again\n", batid);
		return false;
	}
	if (batid == chip->sec_normal_data_id) {
		tmp_rc = chip->normal_tc_rc;
		chip->normal_tc_rc = chip->normal_tc_rc_sec;
		chip->normal_tc_rc_sec = tmp_rc;
		tmp_data = chip->normal_tc_data;
		chip->normal_tc_data = chip->normal_tc_data_sec;
		chip->normal_tc_data_sec = tmp_data;
	}
	cms_print(chip, PR_INFO, "normal data %schanged, batid=%d, second_normal_data_id=%d\n",
			batid == chip->sec_normal_data_id ? "" : "not ", batid, chip->sec_normal_data_id);
	return true;
}
...
static void cms_main_work(struct work_struct *work)
{
	...
	static bool select_normal_data_done;
	...
	if (!select_normal_data_done)
		select_normal_data_done = cms_select_normal_data(chip);
	...
	cms_check_camera_charge_current(chip);
	cms_check_live_charge_current(chip);
...
out:
	...
	fuelsummary_collect_value(ID_MARK__FBON, chip->fb_on);
	fuelsummary_collect_value(ID_BIT__BSPTEST, bsp_test_mode);
	fuelsummary_collect_value(ID_BIT__EXHIBITION, chip->exhibition_mode);
	fuelsummary_collect_value(ID_BIT__FACTORY, chip->factory_mode_state);
	fuelsummary_collect_value(ID_BYTE__INPUT_SCALE, get_effective_result(chip->user_input_votable));
	fuelsummary_collect_value(ID_BYTE__INBAT_SCALE, get_effective_result(chip->user_ibat_votable));

	fuelsummary_collect_data(ID_LONG__HEALTH, &(chip->health_status));
	if (chip->fuelsummary_psy)
					...
					struct class_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE, "charge test param threshold:%d,%d,%d,%d,%d,%d,%d,%d\n",
			...
			chip->charge_param_judge_threshold[3], chip->charge_param_judge_threshold[4], chip->charge_param_judge_threshold[5],
...
static int cms_parse_dt(struct cms *chip)
{
	...
	if (ex_fg_support) {
		...
		cms_print(chip,PR_WARN,"change bat-det-method to EX_FG\n");
	}
	...
	chip->live_scale_enable = of_property_read_bool(node, "vivo,live-scale-enable");
	if(chip->live_scale_enable) {
		chip->live_scale_data = (int*)cms_of_get_property_data(chip, node,
				"vivo,live-scale-data", 1, 2, ONE_DIMENSION, NULL);
		if(!chip->live_scale_data){
			chip->live_scale_enable = false;
		}
		chip->live_dchg_disable = of_property_read_bool(node, "vivo,live-dchg-disable");
	}
	chip->camera_scale_enable = of_property_read_bool(node, "vivo,camera-scale-enable");
	fuelsummary_of_property_put("vivo,camera-scale-enable", PARAMS_TYPE_BOOL, &(chip->camera_scale_enable));
	if(chip->camera_scale_enable) {
		chip->camera_scale_data = (int*)cms_of_get_property_data(chip, node,
				"vivo,camera-scale-data", 1, 3, ONE_DIMENSION, NULL);
		if(!chip->camera_scale_data){
			chip->camera_scale_enable = false;
		}
	}
	fuelsummary_of_property_puts("vivo,camera-scale-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->camera_scale_data, 1, 3);
	...
	fuelsummary_of_property_put("vivo,exhibition-mode-current-primary", PARAMS_TYPE_INT, &(chip->exhibition_mode_current_primary));
	...
	fuelsummary_of_property_put("vivo,exhibition-mode-current-parallel", PARAMS_TYPE_INT, &(chip->exhibition_mode_current_parallel));
	...
	chip->dual_normal_config = of_property_read_bool(node, "vivo,dual-normal-tc-data");
	if (chip->dual_normal_config) {
		chip->normal_tc_rc_sec = (int*)cms_of_get_property_data(chip, node,
					"vivo,normal-tc-row-col-second", 1, 2, ONE_DIMENSION, NULL);
		if (chip->normal_tc_rc_sec) {
			chip->normal_tc_data_sec = (struct tc_data_item_ex *)cms_of_get_property_data(chip, node,
						"vivo,normal-tc-data-second",chip->normal_tc_rc_sec[ROW],
						chip->normal_tc_rc_sec[COL], THREE_DIMENSION, "normal_tc_data_sec");
		}
		if (!chip->normal_tc_rc_sec || !chip->normal_tc_data_sec) {
			chip->normal_tc_data_sec = default_normal_tc_data;
			chip->normal_tc_rc_sec = default_normal_tc_data_rc;
			fuelsummary_of_property_puts("vivo,normal-tc-data-second", STATIC_FIXED, PARAMS_TYPE_INT, chip->normal_tc_data_sec,
					chip->normal_tc_rc_sec[ROW], chip->normal_tc_rc_sec[COL]);
		} else {
			fuelsummary_of_property_puts("vivo,normal-tc-row-col-second", STATIC_FIXED, PARAMS_TYPE_INT, chip->normal_tc_rc_sec, 1, 2);
			fuelsummary_of_property_puts("vivo,normal-tc-data-second", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->normal_tc_data_sec),
					chip->normal_tc_rc_sec[ROW], chip->normal_tc_rc_sec[COL]);
		}
		rc = of_property_read_u32(node, "vivo,sec-normal-data-batid", &chip->sec_normal_data_id);
		if (rc)
			chip->sec_normal_data_id = 0;
		cms_print(chip, PR_INFO, "Dual normal-tc-data config, rc%sgot, data%sgot, use sec-normal-data for batid %d\n",
				chip->normal_tc_rc_sec == default_normal_tc_data_rc ? " not " : " ",
				chip->normal_tc_data_sec == default_normal_tc_data ? " not " : " ", chip->sec_normal_data_id);
	} else
		cms_print(chip, PR_INFO, "Single normal-tc-data config\n");
	...
	if(chip->intell_charge_enable) {
		...
		chip->scene_recognition_enable = of_property_read_bool(node, "vivo,scene-recognition-enable");
		if (chip->scene_recognition_enable) {
			chip->primary_scene_1_tc_rc = (int *)cms_of_get_property_data(chip, node,
					"vivo,primary-scene-1-tc-row-col", 1, 2, ONE_DIMENSION, "primary-scene-1-tc-row-col");
			if (chip->primary_scene_1_tc_rc) {
				chip->primary_scene_1_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,primary-scene-1-tc-data", chip->primary_scene_1_tc_rc[ROW],
						chip->primary_scene_1_tc_rc[COL], TWO_DIMENSION, "primary_scene_1_tc_data");
			}
			if (!chip->primary_scene_1_tc_rc || !chip->primary_scene_1_tc_data) {
				chip->primary_scene_1_tc_data = default_scene_tc_data;
				if (!chip->primary_scene_1_tc_rc)
					chip->primary_scene_1_tc_rc = (int *)kzalloc(sizeof(int) * 2, GFP_KERNEL);
				chip->primary_scene_1_tc_rc[ROW] = ARRAY_SIZE(default_scene_tc_data);
				chip->primary_scene_1_tc_rc[COL] = 3;
				fuelsummary_of_property_puts("vivo,primary-scene-1-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_scene_1_tc_data,
						chip->primary_scene_1_tc_rc[ROW], chip->primary_scene_1_tc_rc[COL]);
			} else {
				fuelsummary_of_property_puts("vivo,primary-scene-1-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_scene_1_tc_rc, 1, 2);
				fuelsummary_of_property_puts("vivo,primary-scene-1-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->primary_scene_1_tc_data),
						chip->primary_scene_1_tc_rc[ROW], chip->primary_scene_1_tc_rc[COL]);
			}
			chip->parallel_scene_1_tc_rc = (int *)cms_of_get_property_data(chip, node,
					"vivo,parallel-scene-1-tc-row-col", 1, 2, ONE_DIMENSION, "parallel-scene-1-tc-row-col");
			if (chip->parallel_scene_1_tc_rc) {
				chip->parallel_scene_1_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,parallel-scene-1-tc-data", chip->parallel_scene_1_tc_rc[ROW],
						chip->parallel_scene_1_tc_rc[COL], TWO_DIMENSION, "parallel_scene_1_tc_data");
			}
			if (!chip->parallel_scene_1_tc_rc || !chip->parallel_scene_1_tc_data) {
				chip->parallel_scene_1_tc_data = default_scene_tc_data;
				if (!chip->parallel_scene_1_tc_rc)
					chip->parallel_scene_1_tc_rc = (int *)kzalloc(sizeof(int) * 2, GFP_KERNEL);
				chip->parallel_scene_1_tc_rc[ROW] = ARRAY_SIZE(default_scene_tc_data);
				chip->parallel_scene_1_tc_rc[COL] = 3;
				fuelsummary_of_property_puts("vivo,parallel-scene-1-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_scene_1_tc_data,
						chip->parallel_scene_1_tc_rc[ROW], chip->parallel_scene_1_tc_rc[COL]);
			} else {
				fuelsummary_of_property_puts("vivo,parallel-scene-1-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_scene_1_tc_rc, 1, 2);
				fuelsummary_of_property_puts("vivo,parallel-scene-1-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->parallel_scene_1_tc_data),
						chip->parallel_scene_1_tc_rc[ROW], chip->parallel_scene_1_tc_rc[COL]);
			}
			chip->primary_scene_2_tc_rc = (int *)cms_of_get_property_data(chip, node,
					"vivo,primary-scene-2-tc-row-col", 1, 2, ONE_DIMENSION, "primary-scene-2-tc-row-col");
			if (chip->primary_scene_2_tc_rc) {
				chip->primary_scene_2_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,primary-scene-2-tc-data", chip->primary_scene_2_tc_rc[ROW],
						chip->primary_scene_2_tc_rc[COL], TWO_DIMENSION, "primary_scene_2_tc_data");
			}
			if (!chip->primary_scene_2_tc_rc || !chip->primary_scene_2_tc_data) {
				chip->primary_scene_2_tc_data = default_scene_tc_data;
				if (!chip->primary_scene_2_tc_rc)
					chip->primary_scene_2_tc_rc = (int *)kzalloc(sizeof(int) * 2, GFP_KERNEL);
				chip->primary_scene_2_tc_rc[ROW] = ARRAY_SIZE(default_scene_tc_data);
				chip->primary_scene_2_tc_rc[COL] = 3;
				fuelsummary_of_property_puts("vivo,primary-scene-2-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_scene_2_tc_data,
						chip->primary_scene_2_tc_rc[ROW], chip->primary_scene_2_tc_rc[COL]);
			} else {
				fuelsummary_of_property_puts("vivo,primary-scene-2-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_scene_2_tc_rc, 1, 2);
				fuelsummary_of_property_puts("vivo,primary-scene-2-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->primary_scene_2_tc_data),
						chip->primary_scene_2_tc_rc[ROW], chip->primary_scene_2_tc_rc[COL]);
			}
			chip->parallel_scene_2_tc_rc = (int *)cms_of_get_property_data(chip, node,
					"vivo,parallel-scene-2-tc-row-col", 1, 2, ONE_DIMENSION, "parallel-scene-2-tc-row-col");
			if (chip->parallel_scene_2_tc_rc) {
				chip->parallel_scene_2_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,parallel-scene-2-tc-data", chip->parallel_scene_2_tc_rc[ROW],
						chip->parallel_scene_2_tc_rc[COL], TWO_DIMENSION, "parallel_scene_2_tc_data");
			}
			if (!chip->parallel_scene_2_tc_rc || !chip->parallel_scene_2_tc_data) {
				chip->parallel_scene_2_tc_data = default_scene_tc_data;
				if (!chip->parallel_scene_2_tc_rc)
					chip->parallel_scene_2_tc_rc = (int *)kzalloc(sizeof(int) * 2, GFP_KERNEL);
				chip->parallel_scene_2_tc_rc[ROW] = ARRAY_SIZE(default_scene_tc_data);
				chip->parallel_scene_2_tc_rc[COL] = 3;
				fuelsummary_of_property_puts("vivo,parallel-scene-2-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_scene_2_tc_data,
						chip->parallel_scene_2_tc_rc[ROW], chip->parallel_scene_2_tc_rc[COL]);
			} else {
				fuelsummary_of_property_puts("vivo,parallel-scene-2-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_scene_2_tc_rc, 1, 2);
				fuelsummary_of_property_puts("vivo,parallel-scene-2-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->parallel_scene_2_tc_data),
						chip->parallel_scene_2_tc_rc[ROW], chip->parallel_scene_2_tc_rc[COL]);
			}
			chip->primary_scene_3_tc_rc = (int *)cms_of_get_property_data(chip, node,
					"vivo,primary-scene-3-tc-row-col", 1, 2, ONE_DIMENSION, "primary-scene-3-tc-row-col");
			if (chip->primary_scene_3_tc_rc) {
				chip->primary_scene_3_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,primary-scene-3-tc-data", chip->primary_scene_3_tc_rc[ROW],
						chip->primary_scene_3_tc_rc[COL], TWO_DIMENSION, "primary_scene_3_tc_data");
			}
			if (!chip->primary_scene_3_tc_rc || !chip->primary_scene_3_tc_data) {
				chip->primary_scene_3_tc_data = default_scene_tc_data;
				if (!chip->primary_scene_3_tc_rc)
					chip->primary_scene_3_tc_rc = (int *)kzalloc(sizeof(int) * 2, GFP_KERNEL);
				chip->primary_scene_3_tc_rc[ROW] = ARRAY_SIZE(default_scene_tc_data);
				chip->primary_scene_3_tc_rc[COL] = 3;
				fuelsummary_of_property_puts("vivo,primary-scene-3-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_scene_3_tc_data,
						chip->primary_scene_3_tc_rc[ROW], chip->primary_scene_3_tc_rc[COL]);
			} else {
				fuelsummary_of_property_puts("vivo,primary-scene-3-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->primary_scene_3_tc_rc, 1, 2);
				fuelsummary_of_property_puts("vivo,primary-scene-3-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->primary_scene_3_tc_data),
						chip->primary_scene_3_tc_rc[ROW], chip->primary_scene_3_tc_rc[COL]);
			}
			chip->parallel_scene_3_tc_rc = (int *)cms_of_get_property_data(chip, node,
					"vivo,parallel-scene-3-tc-row-col", 1, 2, ONE_DIMENSION, "parallel-scene-3-tc-row-col");
			if (chip->parallel_scene_3_tc_rc) {
				chip->parallel_scene_3_tc_data = (struct tc_data_item *)cms_of_get_property_data(chip, node,
						"vivo,parallel-scene-3-tc-data", chip->parallel_scene_3_tc_rc[ROW],
						chip->parallel_scene_3_tc_rc[COL], TWO_DIMENSION, "parallel_scene_3_tc_data");
			}
			if (!chip->parallel_scene_3_tc_rc || !chip->parallel_scene_3_tc_data) {
				chip->parallel_scene_3_tc_data = default_scene_tc_data;
				if (!chip->parallel_scene_3_tc_rc)
					chip->parallel_scene_3_tc_rc = (int *)kzalloc(sizeof(int) * 2, GFP_KERNEL);
				chip->parallel_scene_3_tc_rc[ROW] = ARRAY_SIZE(default_scene_tc_data);
				chip->parallel_scene_3_tc_rc[COL] = 3;
				fuelsummary_of_property_puts("vivo,parallel-scene-3-tc-data", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_scene_3_tc_data,
						chip->parallel_scene_3_tc_rc[ROW], chip->parallel_scene_3_tc_rc[COL]);
			} else {
				fuelsummary_of_property_puts("vivo,parallel-scene-3-tc-row-col", STATIC_FIXED, PARAMS_TYPE_INT, chip->parallel_scene_3_tc_rc, 1, 2);
				fuelsummary_of_property_puts("vivo,parallel-scene-3-tc-data", EXTENSIBLE, PARAMS_TYPE_INT, &(chip->parallel_scene_3_tc_data),
						chip->parallel_scene_3_tc_rc[ROW], chip->parallel_scene_3_tc_rc[COL]);
			}
			cms_print(chip, PR_INFO, "scene recognition enabled:[primary, parallel] scene_1[%d, %d], scene_2[%d, %d], scene_3[%d, %d]\n",
					chip->primary_scene_1_tc_data != default_scene_tc_data, chip->parallel_scene_1_tc_data != default_scene_tc_data,
					chip->primary_scene_2_tc_data != default_scene_tc_data, chip->parallel_scene_2_tc_data != default_scene_tc_data,
					chip->primary_scene_3_tc_data != default_scene_tc_data,  chip->parallel_scene_3_tc_data != default_scene_tc_data);
		}
...
static void cms_release_property_data(struct cms *chip)
{
	...
	if (chip->dual_normal_config) {
		if (chip->normal_tc_data_sec && chip->normal_tc_data_sec != default_normal_tc_data) {
			kfree(chip->normal_tc_rc_sec);
			kfree(chip->normal_tc_data_sec);
		}
	}
	...
	if(chip->intell_charge_enable){
		...
		if (chip->scene_recognition_enable) {
			if (chip->primary_scene_1_tc_data != default_scene_tc_data)
				kfree(chip->primary_scene_1_tc_data);
			if (chip->primary_scene_1_tc_rc)
				kfree(chip->primary_scene_1_tc_rc);
			if (chip->parallel_scene_1_tc_data != default_scene_tc_data)
				kfree(chip->parallel_scene_1_tc_data);
			if (chip->parallel_scene_1_tc_rc)
				kfree(chip->parallel_scene_1_tc_rc);
			if (chip->primary_scene_2_tc_data != default_scene_tc_data)
				kfree(chip->primary_scene_2_tc_data);
			if (chip->primary_scene_2_tc_rc)
				kfree(chip->primary_scene_2_tc_rc);
			if (chip->parallel_scene_2_tc_data != default_scene_tc_data)
				kfree(chip->parallel_scene_2_tc_data);
			if (chip->parallel_scene_2_tc_rc)
				kfree(chip->parallel_scene_2_tc_rc);
			if (chip->primary_scene_3_tc_data != default_scene_tc_data)
				kfree(chip->primary_scene_3_tc_data);
			if (chip->primary_scene_3_tc_rc)
				kfree(chip->primary_scene_3_tc_rc);
			if (chip->parallel_scene_3_tc_data != default_scene_tc_data)
				kfree(chip->parallel_scene_3_tc_data);
			if (chip->parallel_scene_3_tc_rc)
				kfree(chip->parallel_scene_3_tc_rc);
		}
	}
...
static int cms_probe(struct platform_device *pdev)
{
	...
	chip->intell_scene = NORMAL_SCENE;
