					...
					struct drm_device *dev)
{
	...
	if (vdisp->bias_config.sm5109_support || !strcmp(vdisp->project_name, "pd1981")) {
		/*lm3697 exponential backlight change, default brightness need set higher*/
		props.brightness = 1600; /*same to UEFI*/
	} else
			...
			size_t *len)
{
	...
	*cfg = msm_property_get_blob(&c_conn->property_info,
	...
	} else {//if first out dc,restore bitdepth table
		if (dc_state == true) {
			dc_state = false;
...
static u32 get_status_work_interval(struct sde_connector *c_conn)
{
	...
	struct vivo_display *vdisp = display->panel->panel_private;;

	if (vdisp->esd.status_mode == ESD_MODE_REG_READ) {
...
static void sde_connector_check_status_work(struct work_struct *work)
{
	...
	if (rc > 0) {
		...
		if(display)
			vdisp = display->panel->panel_private;
		if (vdisp && !vdisp->esd.is_check_more)
			return;
