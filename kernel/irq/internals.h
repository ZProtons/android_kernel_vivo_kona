...
static inline void kstat_irq_count(struct irq_desc *desc)
{
	if (!(desc->action->flags & IRQF_TIMER) &&
			!(desc->action->flags & IRQF_SKIP)) {
		if (!test_bit(IRQ_DETECT_WORK_PENDING, &desc->flags)) {
			int cpu;
			unsigned int sum = 0;

			set_bit(IRQ_DETECT_WORK_PENDING, &desc->flags);
			...
			desc->num = sum;
			desc->atime = jiffies;
			queue_delayed_work(irq_detect_wq, &desc->dwork, 5*HZ);
		}
	}
}
#endif
