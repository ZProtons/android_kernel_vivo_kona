...
static int _iris_configure(u32 display, u32 type, u32 value)
{
	...
	case IRIS_MEMC_LEVEL:
		pcfg->frc_setting.no_fallback = (value >> 8)&0xFF;
		value &= 0xFF;
		if (value <= 3) {
	...
	case IRIS_SET_METADATA_LOCK:	// lock by panel_lock
		...
		break;
	case IRIS_SET_DSI_MODE_INFO:
		if (value > 1)
			goto error;
		iris_config_mipi_clk(pcfg->clk_rate[value]);
		break;
	default:
		goto error;
	}
...
static int _iris_configure_ex(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DBG_TARGET_REGADDR_VALUE_SET:
		...
		case I2C:
			...
			break;
		default:
			IRIS_LOGW("Iris not support communication method");
			break;
		}
		break;
...
int iris_configure_get(u32 display, u32 type, u32 count, u32 *values)
{
	...
	case IRIS_DBG_TARGET_REGADDR_VALUE_GET:
		...
		case I2C:
			reg_addr = *values;
			iris_i2c_conver_ocp_read(values, 1, false);
			IRIS_LOGD("addr = %x, value = %x\n", reg_addr, *values);
			break;
		default:
			IRIS_LOGW("Iris not support communication method");
	...
	case IRIS_GET_METADATA:
		*values = pcfg->metadata;
		break;
	case IRIS_SET_DSI_MODE_INFO:
		*values = atomic_read(&pcfg->set_dsi_mode);
		break;
	default:
		return -EFAULT;
	}
