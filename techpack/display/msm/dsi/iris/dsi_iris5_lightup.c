...
#include "dsi_iris5_clk_switch.h"
...
void iris_init(struct dsi_display *display, struct dsi_panel *panel)
{
	...
	pcfg->clk_rate[0] = 864000000;
	pcfg->clk_rate[1] = 874000000;
}
...
void iris_reset(void)
{
	...
	if (pcfg->ext_clk) {
		ret = clk_prepare_enable(pcfg->ext_clk);
		if (ret)
			IRIS_LOGI("%s(), failed enable ext clk", __func__);
		else
...
struct iris_ip_opt *iris_find_specific_ip_opt(uint8_t ip, uint8_t opt_id, int32_t type)
{
	...
	IRIS_LOGV("%s(), ip: %#x, opt: %#x, type: %d", __func__, ip, opt_id, type);
...
int32_t iris_parse_dtsi_cmd(const struct device_node *lightup_node, uint32_t cmd_index)
{
	...
	uint8_t key[str_len] = "pxlw,iris-cmd-list";
...
int iris_parse_param(struct dsi_display *display)
{
	...
	rc = iris_parse_dynamic_clk_info(lightup_node);
	if (rc)
		IRIS_LOGI("%s, [optional] have not dynamic clk switch info", __func__);
		...
		int32_t pos, uint32_t value)
{
	...
	for (type = IRIS_DTSI_PIP_IDX_START; type <= IRIS_DTSI_PIP_IDX_END; type++) {
		if (type == iris_get_cmd_list_index())
			continue;
		if (!iris_same_resolution_diff_refresh(type))
			continue;
		...
		pvalue[0] = value;
		IRIS_LOGD("sync timing: %d,  ip: %d, opt_id: %x, pos: %d, value: %x", type, ip, opt_id, pos, value);
	}
}
		...
		const char __user *user_buf, size_t count, loff_t *ppos)
{
	...
	IRIS_LOGW("%s(), for ip: 0x%02X opt: 0x%02X, payload[%d] is: 0x%08X",
