					...
					struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if (allGestureSwitchIntoBitmap() == 0 && (vivo_ts_data->exportSwitch & 0x01) == 0 && 0==vivo_ts_data->hasScreenClock) {
...
static ssize_t touchscreen_screen_clock_support_show(struct kobject *kobj,
					struct kobj_attribute *attr, char *buf)
{	
	if(0) {
	    if(vivo_ts_data->readScrenClockEdge) {
	    	  vivo_ts_data->readScrenClockEdge();
	    }
	}
	return snprintf(buf, 128, "%d", vivo_ts_data->supportsCreenclock);
}			
static ssize_t touchscreen_screen_clock_support_store(struct kobject *kobj,
					struct kobj_attribute *attr,  const char *buf, size_t count)
{	
    int val;
	if (sscanf(buf, "%d", &val) != 1) {
	...
	if (val == 0) {
		vivo_ts_data->hasScreenClock = 0;
	} else if (val == 1) {
		vivo_ts_data->hasScreenClock = 1;
	} else {
		VTI("invalide parameter:%d", val);
		return -EINVAL;
	}
...
static ssize_t touchscreen_screen_clock_point_show(struct kobject *kobj,
					struct kobj_attribute *attr, char *buf)
{	   
	if(vivo_ts_data->screenClockPoint) {
		int ret = 0;
		mutex_lock(&vivo_ts_data->screenClockPoint->scrclMutex);
		ret = snprintf(buf, PAGE_SIZE, "%04d%04d\n", vivo_ts_data->screenClockPoint->pointX, vivo_ts_data->screenClockPoint->pointY);
		mutex_unlock(&vivo_ts_data->screenClockPoint->scrclMutex);
		return ret;
	}
	else
			return -1;
...
static ssize_t touchscreen_high_scan_rate_support_store(struct kobject *kobj,
					struct kobj_attribute *attr,  const char *buf, size_t count)
{
	int val = 0;
	sscanf(buf, "%d", &val);
	...
	if (val == 180) {	
		vivo_ts_data->hight_scan_rate = 180;
		VTI("game mode scan rate is 180hz");
	} else if (val == 120) {
		vivo_ts_data->hight_scan_rate = 120;
		VTI("game mode scan rate is 120hz");
	} 
	else {
		VTI("hight_scan_rate set error");	
	}
	
	return count;
}

static ssize_t touchscreen_high_scan_rate_support_show(struct kobject *kobj,
					struct kobj_attribute *attr, char *buf)	
{	
	return snprintf(buf, 255, "%d,%d", vivo_ts_data->hight_scan_support, vivo_ts_data->hight_scan_rate);
}
					...
					struct kobj_attribute *attr,  const char *buf, size_t count)
{
	...
	cmd_type = VivoGetScreenCmdType(vivo_ts_data->app_name);
	...
	if (vivo_ts_data->processByPackage) {
		vivo_ts_data->processByPackage(vivo_ts_data->app_name, cmd_type);
	}
		...
		if (val < 0x80) {
			if (val == 2 || val == 3) {
				...
				if (val == 6 || val == 8) {
					...
					VTI("The Fingerprint icon is extinguish response Fingerprint");
					atomic_set(&vivo_ts_data->fingerIcoState, 1);
					vivoTsScanRate(4);
				}
			}
...
static ssize_t touchscreen_secure_touch_enable_show(struct kobject *kobj,
					struct kobj_attribute *attr, char *buf)
{
	return scnprintf(buf, PAGE_SIZE, "%d", vivo_ts_data->get_st_enable());
}
static ssize_t touchscreen_secure_touch_enable_store(struct kobject *kobj,
					struct kobj_attribute *attr,  const char *buf, size_t count)
{
	unsigned long value;
	int err = 0;
	...
	return vivo_ts_data->set_st_enable(value);
}
static ssize_t touchscreen_secure_touch_show(struct kobject *kobj,
					struct kobj_attribute *attr, char *buf)
{
	int ret;

	ret = vivo_ts_data->get_st_state();
	if (ret < 0)
		return ret;
	
	return scnprintf(buf, PAGE_SIZE, "%u", ret);
}

void touchscreen_secure_touch_notify(void)
{
	sysfs_notify(&vivo_ts_data->kobjectDebug, NULL, "secure_touch");
}
...
static struct touchscreen_driver_sysfs_entry touchscreen_screen_clock_support =
	__ATTR(screen_clock_support, 0644,
			touchscreen_screen_clock_support_show, touchscreen_screen_clock_support_store);

static struct touchscreen_driver_sysfs_entry touchscreen_screen_clock_point =
	__ATTR(screen_clock_point, 0644,
			touchscreen_screen_clock_point_show, touchscreen_null_store);

static struct touchscreen_driver_sysfs_entry touchscreen_high_scan_rate_support =
	__ATTR(high_scan_rate_support, 0644,
			touchscreen_high_scan_rate_support_show, touchscreen_high_scan_rate_support_store);

#if defined(CONFIG_SECURE_TOUCH)
static struct touchscreen_driver_sysfs_entry touchscreen_secure_touch_enable =
	__ATTR(secure_touch_enable, (S_IRUGO | S_IWUSR | S_IWGRP),
			touchscreen_secure_touch_enable_show, touchscreen_secure_touch_enable_store);
static struct touchscreen_driver_sysfs_entry touchscreen_secure_touch =
	__ATTR(secure_touch, S_IRUGO,
			touchscreen_secure_touch_show, NULL);
#endif
...
#if defined(CONFIG_SECURE_TOUCH)
	&touchscreen_secure_touch_enable.attr,
	&touchscreen_secure_touch.attr,
#endif
	/*
...
*/ 
	&touchscreen_screen_clock_support.attr,
	&touchscreen_screen_clock_point.attr,
	&touchscreen_high_scan_rate_support.attr,
	NULL
};
