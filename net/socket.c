 ...
 *	the protocol.
 */
//vivo wanwenyin add for udp begin
static kuid_t g_uid;
static int g_port;
static unsigned long long uid_ts;
//vivo wanwenyin add for udp end
		 ...
		 struct sockaddr __user *addr,  int addr_len)
{
	...
	if (addr) {
		...
		//vivo wanwenyin add for udp begin
		u_addr = (struct sockaddr_in *)msg.msg_name;
		port = ntohs(u_addr->sin_port);
		if (uid.val > 9999 && port != 0) {
			if (uid.val == g_uid.val && port == g_port) {
				if (sched_clock() - uid_ts > 30000000000UL) {
					printk(KERN_INFO "sendto uid:%d,port:%d\n", uid, port);
					uid_ts = sched_clock();
				}
			} else {
				printk(KERN_INFO "sendto uid:%d,port:%d\n", uid, port);
