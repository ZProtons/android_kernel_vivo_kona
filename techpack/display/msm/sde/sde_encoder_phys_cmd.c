...
static void sde_encoder_phys_cmd_pp_tx_done_irq(void *arg, int irq_idx)
{
	...
	if (ud_hbm_sync != NULL && ud_hbm_sync->wait_pp_done) {
...
/* vivo display team add for pixelworks memc begin */
/* #if defined(PXLW_IRIS_DUAL) */
	if (iris_is_dual_supported() && iris_is_virtual_encoder_phys(phys_enc)) {
		...
		bool recovery_events)
{
	...
	lcm_error_code_report(VIVO_PINGPONG_TIMEOUT);
	/*Add for vivo LCM driver exception event*/
	atomic_inc(&vivo_exception_count[VIVO_PINGPONG_TIMEOUT]);
	/*Add for vivo LCM driver exception event*/
