...
#ifdef CONFIG_HIGHATOMIC_POOL
static bool if_permit_show(int mtype)
{
		if ((current_uid().val == 0) || (current_uid().val == 1000) ||
			!is_highatomic_pool(mtype))
			return true;
		else
			return false;
}
#else
static bool if_permit_show(int mtype) {return true;}
#endif
...
#ifdef CONFIG_HIGHATOMIC_POOL
		if ((current_uid().val == 0) || (current_uid().val == 1000) ||
			!is_highatomig_pool_stat_item(i))
			goto next;
		else
			continue;
next:
...
#ifdef CONFIG_HIGHATOMIC_POOL
	if ((current_uid().val == 0) || (current_uid().val == 1000) ||
		!is_highatomig_pool_stat_item(off))
		goto next;
	else
		return 0;
next:
