...
static int fg_parse_msoc_mapping_table(struct fg_gen4_chip *chip,
	...
	if (MSOC_MAPPING_ROW != fg->msoc_mapping_row || MSOC_MAPPING_COL!= fg->msoc_mapping_col) {
		...
		fuelsummary_of_property_puts("vivo,msoc-mapping-data", STATIC_FIXED, PARAMS_TYPE_INT, fg->msoc_mapping_table, fg->msoc_mapping_row, fg->msoc_mapping_col);
	}
...
static int fg_gen4_get_batt_profile(struct fg_dev *fg)
{
	...
	fuelsummary_of_property_puts("qcom,fg-profile-data", STATIC_FIXED, PARAMS_TYPE_BYTE, chip->batt_profile, (PROFILE_LEN / 4), 4);
	fuelsummary_init_func_profile_update(fuelsummary_batt_profile_running_update);
...
static bool fg_check_is_chg_est_vbat_diff(int ocv, int soc)
{
...
static void fg_monitor_work(struct work_struct *work)
{
	...
	if (chip->profile_loaded && temp_now >= 100) {
		chip->chg_est_vbat_diff = fg_check_is_chg_est_vbat_diff(ocv_now, soc_now);
	} else {
	...
	if (!chip->bq_fg) {
		...
		fuelsummary_collect_value(ID_INT__FCC, chip->cl->learned_cap_uah / 1000);
		fuelsummary_collect_value(ID_INT__SOH, chip->soh);
		fuelsummary_collect_value(ID_BYTE__FG_DELTA_DIFF, chip->chg_est_vbat_diff);
	}
...
static int fg_gen4_parse_dt(struct fg_gen4_chip *chip)
{
	...
	fuelsummary_of_property_put("qcom,fg-cutoff-voltage", PARAMS_TYPE_INT, &(chip->custom_cutoff_volt_mv));
...
#if 1
	if (is_project("PD1955") || is_project("PD2073"))
	{
