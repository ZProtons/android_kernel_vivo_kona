...
/*Begin leiweiqiang for fbe debug log 2019-01-30*/
#define ABSOLUTE_PATH_MAX	(512)
void vivo_get_path(struct path *pwd, struct dentry *dentry, int res, bool set_flag)
{
	char *start = NULL;
	char pathbuf[ABSOLUTE_PATH_MAX] = {0};
	char cmdline[256] = {0};

	get_cmdline(current, cmdline, 255);
	//get the path
	if (pwd != NULL) {
		start = d_absolute_path(pwd, pathbuf, ABSOLUTE_PATH_MAX);
	} else if (dentry != NULL) {
		/*relative path*/
		start = dentry_path_raw(dentry, pathbuf, ABSOLUTE_PATH_MAX);
	}
	printk(KERN_ERR "CZLBIN bin_path:%s(%d) policy result:%d set policy:%d operation_path:%s\n",
		cmdline, current->tgid, res, set_flag, start);
}
/*End leiweiqiang for fbe debug log  2019-01-30*/
