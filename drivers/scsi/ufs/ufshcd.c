...
int ufshcd_wb_toggle_flush_during_h8(struct ufs_hba *hba, bool set, 
	enum wb_flush_reason reason, bool need_check);
static enum wb_flush_ctrl ufshcd_is_wb_flush_need(struct ufs_hba *hba);
void ufshcd_wb_flush_during_h8_policy(struct ufs_hba *hba, bool need_check);

#ifdef CONFIG_UFS_IRQ_AFFINITY
static int ufs_irq = -1;

int get_ufshcd_irq_num(void)
{
	if (ufs_irq <= 0)
		pr_err("ERR: %s: ufs_irq=%d\n", __func__, ufs_irq);
	return ufs_irq;
}
EXPORT_SYMBOL_GPL(get_ufshcd_irq_num);

int set_ufshcd_irq_affinity(int cpu)
{
	pr_err("%s: CPU(%d), IRQ%d\n", __func__, cpu, ufs_irq);

	if (cpu < NR_CPUS && cpu >= 0) {
		struct cpumask ccpu;
		cpumask_clear(&ccpu);
		cpumask_set_cpu(cpu, &ccpu);
		if (cpumask_subset(&ccpu, cpu_isolated_mask) ||
				!cpumask_intersects(&ccpu, cpu_online_mask)) {
			pr_err("can't set irq affinity to cpu %d\n", cpu);
			return -EINVAL;
		}
		irq_modify_status(ufs_irq, IRQ_NO_BALANCING, 0);
		if (irq_set_affinity(ufs_irq, &ccpu)) {
			pr_err("failed to set irq affinity to cpu %d\n", cpu);
			return -EINVAL;
		}
		irq_modify_status(ufs_irq, 0, IRQ_NO_BALANCING);
	} else {
		irq_modify_status(ufs_irq, IRQ_NO_BALANCING, 0);
	}
...
EXPORT_SYMBOL_GPL(set_ufshcd_irq_affinity);

static ssize_t ufshcd_irq_affinity_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	set_ufshcd_irq_affinity(value);

	return count;
}

static ssize_t ufshcd_irq_affinity_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	...
	curr_len = snprintf(buf, PAGE_SIZE,
			"irqd_can_balance=%d, "
			"cpumask=[%*pbl]\n",
			irqd_can_balance(&irq_to_desc(hba->irq)->irq_data),
			cpumask_pr_args(irq_get_affinity_mask(hba->irq)));
	return curr_len;
}

static void ufshcd_add_irq_affinity_sysfs_nodes(struct ufs_hba *hba)
{
	hba->irq_affinity_attr.show = ufshcd_irq_affinity_show;
	hba->irq_affinity_attr.store = ufshcd_irq_affinity_store;
	sysfs_attr_init(&hba->irq_affinity_attr.attr);
	hba->irq_affinity_attr.attr.name = "irq_affinity";
	hba->irq_affinity_attr.attr.mode = 0644;
	if (device_create_file(hba->dev, &hba->irq_affinity_attr))
		dev_err(hba->dev, "Failed to create sysfs for irq_affinity\n");
}

static void ufshcd_remove_irq_affinity_sysfs_nodes(struct ufs_hba *hba)
{
	device_remove_file(hba->dev, &hba->irq_affinity_attr);
}
#endif
...
static inline void ufshcd_wb_config(struct ufs_hba *hba)
{
	...
	ufshcd_wb_flush_during_h8_policy(hba, false);
}
...
static int ufshcd_devfreq_scale(struct ufs_hba *hba, bool scale_up)
{
	...
	ufshcd_wb_flush_during_h8_policy(hba, true);
	down_write(&hba->lock);
	goto clk_scaling_unprepare;
		...
		if (cmd) {
			...
			if (cmd->request) {
				...
				ufshcd_vops_pm_qos_req_end(hba, cmd->request,
					true);
#ifdef CONFIG_BLK_ENHANCEMENT
				trigger_cmd_abort(cmd->request, lrbp->compl_time_stamp);
#endif
			}
			clear_bit_unlock(index, &hba->lrb_in_use);
...
int ufshcd_wb_toggle_flush_during_h8(
	struct ufs_hba *hba, bool set, 
	enum wb_flush_reason reason, bool need_check)
{
	int val;
	int ret;

	if (!(set ^ hba->wb_h8_buf_flush_enabled) && need_check)
			return 0;
	...
	ret = ufshcd_query_flag_retry(hba, val,
			       QUERY_FLAG_IDN_WB_BUFF_FLUSH_DURING_HIBERN8,
				       NULL);
	if (!ret) {
		dev_err(hba->dev, "%s: %d %s wb flush\n",
				__func__, __LINE__, set ? "Eable" : "Disable");
		hba->wb_h8_buf_flush_enabled = !!set;
		hba->wb_h8_buf_flush_reason = reason;
		if (hba->wb_h8_buf_flush_enabled && reason == BUF_THER_FLUSH)
			hba->wb_h8_buf_flush_count_thr++;
		else if (hba->wb_h8_buf_flush_enabled && reason == USB_PLUG_FLUSH)
			hba->wb_h8_buf_flush_count_usb++;
	}
...
void ufshcd_wb_flush_during_h8_policy(struct ufs_hba *hba, bool need_check)
{
	...
	if (ufshcd_is_wb_flush_need(hba) == NEED_FLUSH) {
		ufshcd_wb_toggle_flush_during_h8(hba, true, BUF_THER_FLUSH, need_check);
	} else if (ufshcd_is_wb_flush_need(hba) == NO_NEED_FLUSH
				&& hba->wb_h8_buf_flush_reason != USB_PLUG_FLUSH) {
		ufshcd_wb_toggle_flush_during_h8(hba, false, DISABLE_FLUSH, need_check);
	}
}
...
static enum wb_flush_ctrl ufshcd_is_wb_flush_need(struct ufs_hba *hba)
{
	...
	if (avail_buf == UFS_WB_100_PERCENT_BUF_REMAIN)
		return NO_NEED_FLUSH;
	else if (avail_buf < UFS_WB_60_PERCENT_BUF_REMAIN)
		return NEED_FLUSH;
...
void ufshcd_remove(struct ufs_hba *hba)
{
	...
	ufshcd_io_stats_remove(hba);
#ifdef CONFIG_UFS_IRQ_AFFINITY
	ufshcd_remove_irq_affinity_sysfs_nodes(hba);
#endif
	ufs_sysfs_remove_nodes(hba->dev);
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	/* IRQ registration */
	err = devm_request_irq(dev, irq, ufshcd_intr, IRQF_SHARED | IRQF_SKIP,
				dev_name(dev), hba);
	if (err) {
...
#ifdef CONFIG_UFS_IRQ_AFFINITY
	ufs_irq = irq;
	ufshcd_add_irq_affinity_sysfs_nodes(hba);
#endif
