...
/* vivo luoguizhao add for factory test begin */
static int cnss_fw_ready;
module_param(cnss_fw_ready, int, 0644);
MODULE_PARM_DESC(cnss_fw_ready, "Added for wcnss ready");

char *fix_chain = "MIMO";
EXPORT_SYMBOL(fix_chain);
module_param(fix_chain, charp, 0664);
MODULE_PARM_DESC(fix_chain, "Added for wcnss ready");

int cnss_driver_nopowersave;
EXPORT_SYMBOL(cnss_driver_nopowersave);
module_param(cnss_driver_nopowersave, int, 0644);
MODULE_PARM_DESC(cnss_driver_nopowersave, "Added for wcnss ready");
/* vivo luoguizhao add for factory test end */
...
static int cnss_fw_ready_hdlr(struct cnss_plat_data *plat_priv)
{
	...
	if (test_bit(ENABLE_WALTEST, &plat_priv->ctrl_params.quirks)) {
		...
		cnss_fw_ready = 1; /* vivo luoguizhao add for factory test */
...
extern unsigned int is_atboot2; /* vivo luoguizhao add for factory current test */
			      ...
			      const char *buf, size_t count)
{
	...
	/* vivo luoguizhao add for factory current test begin */
	if (is_atboot2 == 1) {
		cnss_pr_dbg("special AT mode, do not load wifi\n");
		return -EINVAL;
	}
	/* vivo luoguizhao add for factory current test end */
