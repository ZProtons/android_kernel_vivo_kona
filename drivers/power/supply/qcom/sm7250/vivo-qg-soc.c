...
static int qg_delta_soc_cold_interval_ms = 20000;/*vivo modify to 20s,before 4000*/
...
int qg_adjust_sys_soc(struct qpnp_qg *chip)
{
	...
	if (chip->sys_soc == QG_MAX_SOC) {
		soc = FULL_SOC;
	} else if (chip->sys_soc >= (QG_MAX_SOC - chip->full_soc_drop_pct)) {/*vivo modify,qcom default:100, vivo default:50*/
