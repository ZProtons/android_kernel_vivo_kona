...
enum f54_report_types {
	...
	F54_TRANS_RX_SHORT_USING_CBC_VARIATION = 133,
	F54_FD_DELTA = 200,
...
DECLARE_COMPLETION(s3706_test_remove_complete);
...
static bool test_report_type_valid(enum f54_report_types report_type)
{
	...
	case F54_TRANS_RX_SHORT_USING_CBC_VARIATION:
...
static int test_wait_for_command_completion(void)
{
	...
	do {
		retval = synaptics_rmi4_reg_read(rmi4_data,
					...
					sizeof(value));
		if (retval < 0) {
			VTE("Failed to read command register");
			return retval;
		}
	...
	if (timeout_count == COMMAND_TIMEOUT_100MS) {
		VTE("Timed out waiting for command completion");
		return -ETIMEDOUT;
	}
...
int test_do_command(unsigned char command)
{
	int retval;
	struct synaptics_rmi4_data *rmi4_data;
	
	if (!f54) {
		VTI("f54 is NULL just return!");
		return 0;
	}
...
static int test_do_preparation(void)
{
	...
	case F54_AMP_RAW_ADC:
			if (f54->query_49.has_ctrl188) {
				...
				if (retval < 0) {
					VTE("Failed to set start production test");
					return retval;
				}
	...
	default:
			...
			if ((f54->query.has_query15) &&
				...
				if (retval < 0) {
					VTE("Failed to disable global CBC");
					return retval;
				}
			}
			...
			if (f54->query.has_signal_clarity) {
				retval = synaptics_rmi4_reg_read(rmi4_data,
							...
							sizeof(f54->control.reg_41->data));
				if (retval < 0) {
					VTE("Failed to disable signal clarity");
					return retval;
				}
				value |= 0x01;
				retval = synaptics_rmi4_reg_write(rmi4_data,
...
static int test_do_afe_calibration(enum f54_afe_cal mode)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(f54->control.reg_188->data));
	if (retval < 0) {
		VTE("Failed to start calibration");
		return retval;
	}
	...
	do {
		retval = synaptics_rmi4_reg_read(rmi4_data,
					...
					sizeof(f54->control.reg_188->data));
		if (retval < 0) {
			VTE("Failed to complete calibration");
			return retval;
		}
		...
		if (timeout_count == timeout) {
			VTE("Timed out waiting for calibration completion");
			return -EBUSY;
		}
	...
	if (retval < 0) {
		VTE("Failed to read calibration CRC");
		return retval;
	}
...
static int test_check_for_idle_status(void)
{
	...
	case STATUS_ERROR:
			...
			break;
	default:
			VTE("Invalid status (%d)", f54->status);
			retval = -EINVAL;
	}
...
static void test_timeout_work(struct work_struct *work)
{
	...
	if (f54->status == STATUS_BUSY) {
		...
		if (retval < 0) {
			...
			VTE("Report type not supported by FW");
		} else {
...
static ssize_t test_sysfs_rt200_report_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	...
	ret = test_sysfs_read_report_store(NULL, NULL, "200", 1);
	if (ret < 0) {
		return -EINVAL;
	}
	report_data_16 = (short *)f54->report_data;
	if (NULL == report_data_16) {
		VTI("report_data_16 is NULL pointer");
		return -EINVAL;
	}

	for (iRow = 0; iRow < TP_TX; iRow++) {
		for (iCol = 0; iCol < TP_RX; iCol++) {
			//data[iRow*TP_RX+iCol] = (int)(*report_data_16);
			ret += snprintf(&buf[ret], 255, "%5d ", (signed short)(*report_data_16));
			report_data_16++;
		}
...
static ssize_t test_sysfs_get_report_polling(void)
{
	...
	if (f54->data_buffer_size < f54->report_size) {
		...
		if (!f54->report_data) {
			VTE("Failed to alloc mem for data buffer");
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
				...
				sizeof(report_index));
	if (retval < 0) {
		VTE("Failed to write report data index");
		retval = -EIO;
		f54->status = STATUS_ERROR;
		return retval;
	}
	...
	if (retval < 0) {
		VTE("Failed to read report data");
		retval = -EIO;
		f54->status = STATUS_ERROR;
		return retval;
	}
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
				...
				sizeof(command));
	if (retval < 0) {
		VTE("Failed to write get report command");
		goto exit;
	}

	VTE("set GET_REPORT command");

#ifdef F54_POLLING_GET_REPORT

	retval = test_sysfs_get_report_polling();
	if (retval < 0) {
		VTE("Failed to get report image");
		goto exit;
	}
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(device_ctrl));
	if (retval < 0) {
		VTE("Failed to restore no sleep setting");
		return retval;
	}
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!f54->query_49.has_ctrl188) {
		VTE("F54_ANALOG_Ctrl188 not foundVTE(");
		return -EINVAL;
	}
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (!test_report_type_valid((enum f54_report_types)setting)) {
		VTE("Report type not supported by driver");
		retval = -EINVAL;
		goto exit;
	}
	...
	VTE("f54->report_type = %d (%d)", data, f54->report_type);
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
				...
				sizeof(data));
	if (retval < 0) {
		VTE("Failed to write report type");
		goto exit;
	}
			...
			struct device_attribute *attr, char *buf)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(data));
	if (retval < 0) {
		VTE("Failed to read report index");
		return retval;
	}
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
				...
				sizeof(data));
	if (retval < 0) {
		VTE("Failed to write report indexVTE(");
		return retval;
	}
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(data));
	if (retval < 0) {
		VTE("Failed to read no auto cal setting");
		return retval;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
				...
				sizeof(data));
	if (retval < 0) {
		VTE("Failed to write no auto cal setting");
		return retval;
	}
...
int tx_max;
int pixel_min;
static short rawdata_tmp[3072];
static int rawdata_tx;
static int rawdata_rx;
			...
			struct device_attribute *attr, char *buf)
{
	...
	case F54_FD_DELTA:
			report_data_16 = (short *)f54->report_data;
			memcpy((char *)rawdata_tmp, (char *)f54->report_data, rx_num * tx_num * 2);
			rawdata_tx = tx_num;
			rawdata_rx = rx_num;
			cnt = snprintf(buf, PAGE_SIZE - count, "tx = %d\nrx = %d\n",
						tx_num, rx_num);
			...
			for (ii = 0; ii < tx_num; ii++) {
				cnt = snprintf(buf, PAGE_SIZE - count, "tx[%2d] ", ii);
				buf += cnt;
				count += cnt;
	...
	case F54_FULL_RAW_CAP_MIN_MAX:
			report_data_16 = (short *)f54->report_data;
			tx_max = report_data_16[0];
			rx_max = report_data_16[1];
			pixel_min = report_data_16[2];
	...
	case F54_TRANS_RX_SHORT_USING_CBC_VARIATION:
			report_data_16 = (short *)f54->report_data;
			cnt = snprintf(buf, PAGE_SIZE - count, "rx = %d\n",
						rx_num);
			buf += cnt;
			count += cnt;
			cnt = snprintf(buf, PAGE_SIZE - count, " rx ");
			...
			struct device_attribute *attr, const char *buf, size_t count)
{
	...
	VTI("the count is:%ld", (long int)count);

	is_present_26100 = false;

	if (F54_EXTEND_TRX_SHORT_INCELL == setting) {
		unsigned char data;
		VTE("26100 setting enter numberofRows:%d numberofColums:%d", numberOfRows, numberOfColums);
		...
		for (i = 0; i < f54->report_size; i++) {
			...
			if (p_data_rt26[i] != 0) {
				if ((i == 0) && (p_data_rt26[i] & 0x01)) {
					...
					VTE("RT26 test fail!!!  data byte %d - data 0x%x", i, p_data_rt26[i]);
					result_26100 = 1;
				}
			}
		...
		if (retval < 0) {
			VTE("fail to set all F54 control_96 registers to 0");
			goto exit_26100_type;
		}
		/*do force update*/
		retval = test_do_command(COMMAND_FORCE_UPDATE);
		...
		if (!p_data_baseline1) {
			VTE("Failed to do alloc baseline1 buf");
			goto exit_26100_type;
		}
		...
		if (!p_data_delta) {
			VTE("Failed to do alloc delta buf");
			goto exit_26100_type;
		}
		goto exit_26100_type;
		...
		if (retval < 0) {
			...
			VTE("ERROR: fail to read f54_read_report_image 100");
			goto exit_26100_type;
		}
		...
		for (i = 0; i < 4; i++) {
			...
			VTE("\ninfo: RT26 pin %d, logical pin %d ", ExtendRT26_pin[i], logical_pin);
			...
			retval = synaptics_rmi4_reg_write(rmi4_data, f54->control.reg_96->address, &temp_data[0], f54->rx_assigned);
			if (retval < 0) {
				VTE("fail to set all F54 control_96 register after changing the cbc");
				goto exit_26100_type;
			}
			...
			if (retval < 0) {
				/*save result to p_data_rt100*/
				VTE("fail to read f54_read_report_image 100");
				goto exit_26100_type;
			}
			...
			for (rxIndex = 0; rxIndex < f54->rx_assigned; rxIndex++) {
				if (rxIndex == logical_pin) {
					if (minRX[rxIndex] < limit_1) {
						VTE("test failed: minRX[%-2d] = %4d when test pin %d (RX Logical pin [%d])",
							rxIndex, minRX[rxIndex], ExtendRT26_pin[i], logical_pin);
						result_26100 = 1;
					}
				} else {
...
exit:
	...
	VTI("the return val:%ld", (long int)retval);
	return retval;
}
			...
			char *buf, loff_t pos, size_t count)
{
	...
	if (!f54->report_data) {
		VTE("Report type %d data not available", f54->report_type);
		retval = -EINVAL;
		goto exit;
	}
	...
	if (retval < 0) {
		VTE("Failed to copy report data");
		goto exit;
	}
...
static int test_set_sysfs(void)
{
	...
	if (!f54->sysfs_dir) {
		VTE("Failed to create sysfs directory");
		goto exit_directory;
	}

	retval = sysfs_create_bin_file(f54->sysfs_dir, &test_report_data);
	if (retval < 0) {
		VTE("Failed to create sysfs bin file");
		goto exit_bin_file;
	}
...
static int test_set_controls(void)
{
	...
	if (f54->query_13.has_ctrl86) {
		...
		if (retval < 0) {
			VTE("Failed to read sense display ratio");
			return retval;
		}
...
exit_no_mem:
	VTE("Failed to alloc mem for control registers");
	return -ENOMEM;
}
...
static void test_f55_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		VTE("Failed to read F55 query registers");
		return;
	}
	...
	if (retval < 0) {
		VTE("Failed to set up F55 control registers");
		return;
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				tx_electrodes);
	if (retval < 0) {
		VTE("Failed to read F55 tx assignment");
		return;
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				rx_electrodes);
	if (retval < 0) {
		VTE("Failed to read F55 rx assignment");
		return;
	}
	...
	if (f55->extended_amp) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
					...
					sizeof(ctrl_43.data));
		if (retval < 0) {
			VTE("Failed to read F55 AFE mux sizes");
			return;
		}
	...
	if (f55->has_force) {
		...
		retval = synaptics_rmi4_reg_read(rmi4_data,
					...
					tx_electrodes);
		if (retval < 0) {
			VTE("Failed to read F55 force tx assignment");
			return;
		}
		...
		retval = synaptics_rmi4_reg_read(rmi4_data,
					...
					rx_electrodes);
		if (retval < 0) {
			VTE("Failed to read F55 force rx assignment");
			return;
		}
			...
			unsigned char page)
{
	...
	if (!f55) {
		VTE("Failed to alloc mem for F55");
		return;
	}
...
static void test_f21_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (!query_2) {
		VTE("Failed to alloc mem for query_2");
		goto exit;
	}

	query_5 = kzalloc(sizeof(*query_5), GFP_KERNEL);
	if (!query_5) {
		VTE("Failed to alloc mem for query_5");
		goto exit;
	}

	query_11 = kzalloc(sizeof(*query_11), GFP_KERNEL);
	if (!query_11) {
		VTE("Failed to alloc mem for query_11");
		goto exit;
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				f21->max_num_of_txrx);
	if (retval < 0) {
		VTE("Failed to read F21 force txrx assignment");
		goto exit;
	}
			...
			unsigned char page)
{
	...
	if (!f21) {
		VTE("Failed to alloc mem for F21");
		return;
	}
...
static int synaptics_rmi4_test_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (f54) {
		VTD("Handle already exists");
		return 0;
	}

	f54 = kzalloc(sizeof(*f54), GFP_KERNEL);
	if (!f54) {
		VTE("Failed to alloc mem for F54");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (retval < 0) {
		VTE("Failed to read F54 query registers");
		goto exit_free_mem;
	}
	...
	if (retval < 0) {
		VTE("Failed to set up F54 control registers");
	...
	if (retval < 0) {
		VTE("Failed to create sysfs entries");
		goto exit_sysfs;
	}
...
exit:
	complete(&s3706_test_remove_complete);
...
#define RX_MAX_THREASHOLD 450
#define TX_MAX_THREASHOLD 450
#define PIXEL_MIN_THREASHOLD (-400)
static int high_resistance_test(void)
{
	char buf[256] = {"\0"};

	test_sysfs_read_report_store(NULL, NULL, "4", 2);
	test_sysfs_read_report_show(NULL, NULL, buf);

	VTI("high resistance:%d %d %d", rx_max, tx_max, pixel_min);

	if (rx_max > RX_MAX_THREASHOLD || tx_max >= TX_MAX_THREASHOLD || pixel_min < PIXEL_MIN_THREASHOLD) {
		return MINUS_ONE;
	}
...
static int rawdata_test(void)
{
	char *buf = NULL;
	int i, j;
	int *rawdata_limit = NULL;
	int *rawdata_limit_min = NULL;
	int *rawdata_limit_max = NULL;
	int limit_size = 0;
	int error_tag = 0;
	buf = (char *)kzalloc(1024*10, GFP_KERNEL);
	if (!buf) {
		VTE("kzalloc fail");
		return MINUS_TWO;
	}
	test_sysfs_read_report_store(NULL, NULL, "20", 3);
	test_sysfs_read_report_show(NULL, NULL, buf);
	kfree(buf);
	/*compare rawdata limit*/
	VTI("rawdata show:");
	for (i = 0; i < rawdata_tx; i++) {
		printk("VIVO_TS:");
		for (j = 0; j < rawdata_rx; j++) {
			printk("%d ", rawdata_tmp[j+i*rawdata_rx]);
		}
		printk("\n");
	}

	rawdata_limit = (int *)vivoTsGetFw(VTS_FW_TYPE_LIMIT, &limit_size);
	rawdata_limit_max = rawdata_limit;
	rawdata_limit_min = rawdata_limit + limit_size/(2*sizeof(int));
	if (rawdata_limit == NULL) {
		VTE("vivoTsGetFw fail");
		return MINUS_THREE;
	}

	VTI("rawdata compare with limit:");
	/*compare rawdata limit*/
	for (i = 0; i < rawdata_tx; i++) {
		for (j = 0; j < rawdata_rx; j++) {
			if (rawdata_tmp[j+i*rawdata_rx] > rawdata_limit_max[j+i*rawdata_rx] || rawdata_tmp[j+i*rawdata_rx] < rawdata_limit_min[j+i*rawdata_rx]) {
				VTE("tx:%d rx:%d rawdata:%d rawdata_min:%d rawdata_max:%d", i, j, rawdata_tmp[j+i*rawdata_rx], rawdata_limit_min[j+i*rawdata_rx], rawdata_limit_max[j+i*rawdata_rx]);
				error_tag = 1;
			}
		}
	}
...
#define ADC_MAX 230
#define ADC_MIN 25
static int adc_range_test(void)
{
	char *buf = NULL;
	int i, j;
	int error_tag = 0;
	...
	test_sysfs_read_report_store(NULL, NULL, "23", 3);
	test_sysfs_read_report_show(NULL, NULL, buf);
	kfree(buf);
	/*compare rawdata limit*/
	VTI("adc_data show:");
	...
	VTI("adc_data compare with limit:");
	/*compare adc_data limit*/
	for (i = 0; i < rawdata_tx; i++) {
		for (j = 0; j < rawdata_rx; j++) {
			if (rawdata_tmp[j+i*rawdata_rx] > ADC_MAX || rawdata_tmp[j+i*rawdata_rx] < ADC_MIN) {
				VTE("tx:%d rx:%d adc_data:%d rawdata_min:%d rawdata_max:%d", i, j, rawdata_tmp[j+i*rawdata_rx], ADC_MIN, ADC_MAX);
				error_tag = 1;
			}
		}
	}
...
static int extended_trx_short_test(void)
{
	test_sysfs_read_report_store(NULL, NULL, "26100", 6);
	if (is_present_26100) {
		if (result_26100 == 0) {
			VTI("test pass");
			return 0; 	/*pass*/
...
int s3706_syna_sensor_test(char *buf, void *pdata, int tmp)
{
	int ret = 0;
	int count = 0;
	int test_result = 0;
	...
	p_buf = &buf[ret];
if (0) {
	VTI("high resistance test");
	ret += snprintf(&buf[ret], 30, "high resistance test:");
	if (high_resistance_test() < 0) {
		test_result = 1;
		count = snprintf(&buf[ret], 10, "fail\n");
	} else {
		count = snprintf(&buf[ret], 10, "pass\n");
	}
	ret += count;
	VTI("%s", p_buf);
	p_buf = &buf[ret];
	...
	VTI("rawdata test");
	ret += snprintf(&buf[ret], 20, "rawdata test:");
	if (rawdata_test() < 0) {
		test_result = 1;
		count = snprintf(&buf[ret], 10, "fail\n");
	} else {
	...
	VTI("adc range test");
	ret += snprintf(&buf[ret], 20, "adc range test:");
	if (adc_range_test() < 0) {
		test_result = 1;
		count = snprintf(&buf[ret], 10, "fail\n");
	} else {
	...
	VTI("extended trx short test");
	ret += snprintf(&buf[ret], 30, "extended trx short test:");
	if (extended_trx_short_test() < 0) {
		test_result = 1;
		count = snprintf(&buf[ret], 10, "fail\n");
	} else {
		count = snprintf(&buf[ret], 10, "pass\n");
...
int s3706_syna_get_rawordiff_data(int which, int *data)
{
	int TP_TX = 0xff & vivoTsGetTxRxNum();
	int iRow = 0;
	int TP_RX = 0xff & (vivoTsGetTxRxNum()>>8);
	...
	if (which == 0) {	/*rawdata*/
		ret = test_sysfs_read_report_store(NULL, NULL, "3", 1);
		if (ret < 0) {
			return -EINVAL;
		}
		report_data_16 = (short *)f54->report_data;
	...
	if (which == 1) {	/*diffdata	*/
		ret = test_sysfs_read_report_store(NULL, NULL, "2", 1);
		if (ret < 0) {
			return -EINVAL;
		}
		report_data_16 = (short *)f54->report_data;
...
{
	synaptics_3706_rmi4_new_function(&test_module, true);
...
{
	synaptics_3706_rmi4_new_function(&test_module, false);

	wait_for_completion(&s3706_test_remove_complete);
