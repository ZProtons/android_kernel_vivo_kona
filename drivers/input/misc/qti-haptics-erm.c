...
static int qti_haptics_play(struct qti_hap_chip *chip, bool play)
{
	...
	/*vivo sensor add for [B200401-3906] begin*/
	struct qti_hap_play_info *play_info = &chip->play;
	pr_err("qpnp_haptics_play: enable %d, %d ms, pid is %d\n", play, (int)(play_info->length_us/1000), current->pid);
	/*vivo sensor add for [B200401-3906] end*/
...
static int qti_haptics_module_en(struct qti_hap_chip *chip, bool en)
{
	...
	dev_err(chip->dev, "enter qti haptics module en");
		...
		struct ff_effect *effect, struct ff_effect *old)
{
	...
	if (chip->vdd_supply && !chip->vdd_enabled) {
		...
		if (rc)
			dev_err(chip->dev, "failed to set load vdd vol\n");
...
static enum hrtimer_restart qti_hap_stop_timer(struct hrtimer *timer)
{
	...
	schedule_work(&chip->vibrator_work);
	return HRTIMER_NORESTART;
}

static void factory_vibrator_work_routine(struct work_struct *work)
{
	struct qti_hap_chip *chip = container_of(work, struct qti_hap_chip, vibrator_work);
	int rc = 0;
	if (chip->vdd_supply && chip->vdd_enabled) {
		rc = regulator_disable(chip->vdd_supply);
		if (rc < 0) {
...
static enum hrtimer_restart factory_hap_stop_timer(struct hrtimer *timer)
{
	struct qti_hap_chip *chip = container_of(timer, struct qti_hap_chip,
			factory_timer);
	int rc;
	...
	chip->play.length_us = 0;
	dev_err(chip->dev, "factory_hap_stop_timer enter");
	rc = qti_haptics_play(chip, false);
	if (rc < 0)
		dev_err(chip->dev, "Stop playing failed, rc=%d\n", rc);
...
static int qti_haptics_activate(struct qti_hap_chip *chip, int val)
{
	...
	if (!!val) {
		...
		if (play->playing_pattern) {
			...
			HAPTIC_LOG("constant mode play, %d\n", play->length_us);
			secs = play->length_us / USEC_PER_SEC;
			nsecs = (play->length_us % USEC_PER_SEC) *
				NSEC_PER_USEC;
			hrtimer_start(&chip->factory_timer, ktime_set(secs, nsecs),
					HRTIMER_MODE_REL);
		}
	} else {
		play->length_us = 0;
...
static int qti_haptics_probe(struct platform_device *pdev)
{
	...
	hrtimer_init(&chip->factory_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	chip->factory_timer.function = factory_hap_stop_timer;
	INIT_WORK(&chip->vibrator_work, factory_vibrator_work_routine);
