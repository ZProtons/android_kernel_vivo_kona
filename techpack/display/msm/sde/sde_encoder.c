...
static bool sde_encoder_input_filter(struct input_handle *handle,
		...
		struct sde_encoder_virt *sde_enc)
{
	...
	input_handler->filter = sde_encoder_input_filter;
		...
		struct sde_encoder_phys *phy_enc)
{
	...
	/*Add for vivo LCM driver cloud data collection*/
	atomic_inc(&vivo_exception_count[VIVO_ENCODE_PHY_UNDERRUN]);
	/*Add for vivo LCM driver cloud data collection*/

	SDE_ATRACE_BEGIN("encoder_underrun_callback");
	atomic_inc(&phy_enc->underrun_cnt);
	//lcm_error_code_report(VIVO_ENCODE_PHY_UNDERRUN);
