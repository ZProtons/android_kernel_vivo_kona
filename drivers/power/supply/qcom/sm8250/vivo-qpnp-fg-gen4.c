 ...
 */
// VIVO_CHG: [20200408]: last update from qpnp-fg-gen4.c 
...
#define _GET_DEFINE_STRING(define)	#define
#define GET_DEFINE_STRING(define)	_GET_DEFINE_STRING(define)
//vivo add end

extern unsigned int is_atboot;
extern bool ex_fg_ffc_support;	//vivo add
extern unsigned int bsp_test_mode;
static int is_profiles_same;
bool fg_do_reset = false;
...
extern int fg_soc_really;	//vivo add
...
struct fg_dt_props {
	...
	//vivo add:
	bool	ext_sense;
	bool	sense_r_2mO;	//default 5mO

	bool disable_side_loading;	//vivo add
	int last_sys_term_curr_ma;
};
...
struct fg_gen4_chip {
	...
	bool check_battery_id_enable;
	struct delayed_work check_battery_id_work;
	int check_battid_count;
	int user_chg_term_curr_ma;

	/*monitor work*/
	struct delayed_work fg_monitor_work;

	/*fake battery full flag*/
	struct delayed_work fake_battery_full_flag_work;
	bool fake_battery_full_flag;
	int chg_est_vbat_diff;
	bool	profile_loaded;
	int float_volt_mv;
	bool bq_fg;
	struct power_supply *fuelsummary_psy;
	bool hold_on_99_enable;
	int hold_on_99_ibat_ua;
	bool hold_on_99;	//hold on 99% during charging when Ibat too high.
	bool fake_batt_therm_during_power_on;

	struct regulator	*vivo_s8c_ctrl;
	bool profile_running_update;
	...
	bool skip_fg_restart;
	bool rslow_parameter_used;
	/* vivo add end */
};
/* vivo add start */
static int fg_get_batt_id_from_cms(struct fg_gen4_chip *chip);
...
static int fg_gen4_debug_mask = FG_IRQ | FG_POWER_SUPPLY| FG_STATUS | FG_FVSS; /* vivo add */
...
static int fg_sram_dump_period_ms = 30000;
...
static int fg_gen4_get_prop_capacity(struct fg_dev *fg, int *val)
{
	...
	if (chip->vbatt_low) {
		//vivo add start
		fg_get_battery_voltage(fg, &vbatt_mv);
		vbatt_mv /= 1000;
		if (vbatt_mv > chip->dt.cutoff_volt_mv) {
			chip->vbatt_low = false;
			pr_err("vbatt_mv=%d, force vbatt_low=false.\n", vbatt_mv);
		} else {
			//vivo add end
			*val = EMPTY_SOC;
			return 0;
	...
	fg_soc_really |= BMS_REALLY;	//vivo add

	if (fg->charge_full) {
		chip->hold_on_99 = false;
		*val = FULL_CAPACITY;
		return 0;
	}
	/*vivo add*/
	if (chip->fake_battery_full_flag) {
		chip->hold_on_99 = false;
		*val = FULL_CAPACITY;
		pr_err("fake_battery_full_flag report 100\n");
		return 0;
	}
	/*vivo end*/
	if (chip->soc_scale_mode) {
		...
		*val = chip->soc_scale_msoc;
		...
		if (rc < 0)
			return rc;
		if (chip->hold_on_99 && msoc == 100) {
			pr_err("hold_on_99=true, charge_status=%d, keep %d -> 99...\n", fg->charge_status, msoc);
			msoc = 99;
		}
...
/* vivo add start */
static int fg_parse_msoc_mapping_table(struct fg_gen4_chip *chip,
	...
	fg->msoc_mapping_enable = of_property_read_bool(batt_node, "vivo,msoc-mapping-enable");
	if(!fg->msoc_mapping_enable){
		pr_info("not support msoc mapping!\n");
		return rc;
	}

	rc = of_property_read_u32(batt_node, "vivo,msoc-mapping-col-legend",
						&fg->msoc_mapping_col);
	if (rc){
		pr_err("prop 'vivo,msoc-mapping-col-legend' not found\n");
		goto err;
	}

	rc = of_property_read_u32(batt_node, "vivo,msoc-mapping-row-legend",
						&fg->msoc_mapping_row);
	if (rc){
		pr_err("prop 'vivo,msoc-mapping-row-legend' not found\n");
		goto err;
	}

	if (MSOC_MAPPING_ROW != fg->msoc_mapping_row || MSOC_MAPPING_COL!= fg->msoc_mapping_col) {
		pr_err("row or col value error\n");
		goto err;
	} else {
		prop = of_find_property(batt_node, "vivo,msoc-mapping-data", NULL);
		if (!prop) {
			pr_err("prop 'vivo,msoc-mapping-data' not found\n");
			goto err;
		}
		data = prop->value;
		size = prop->length/sizeof(int);
		if (size != fg->msoc_mapping_row * fg->msoc_mapping_col) {
			pr_err("%s: data size mismatch, %d x %d != %d\n",
					batt_node->name, fg->msoc_mapping_col, fg->msoc_mapping_row, size);
			goto err;
		}
		pr_info("row:%d, col:%d\n", fg->msoc_mapping_row, fg->msoc_mapping_col);
		for (i = 0; i < fg->msoc_mapping_row; i++) {
			(fg->msoc_mapping_table)[i].msoc_raw = be32_to_cpup(data++);
			(fg->msoc_mapping_table)[i].msoc_mapping_charging = be32_to_cpup(data++);
			(fg->msoc_mapping_table)[i].msoc_mapping_discharging = be32_to_cpup(data++);
			//pr_info("msoc = %d, msoc_mapping_charging = %d, msoc_mapping_discharging = %d.\n", (fg->msoc_mapping_table)[i].msoc_raw, (fg->msoc_mapping_table)[i].msoc_mapping_charging, (fg->msoc_mapping_table)[i].msoc_mapping_discharging);	//sxs need delete log
		}
	}

	pr_info("msoc mapping paramater load ok\n");
	return rc;
err:
	pr_info("msoc mapping paramater load fail\n");
	fg->msoc_mapping_enable = false;
	return rc;
}
					...
					struct device_node *profile_node)
{
	...
	rc = fg_parse_msoc_mapping_table(chip, profile_node);
	if (rc < 0) {
		pr_err("parse msoc mapping table err, rc:%d\n", rc);
	}
...
/*
static void fuelsummary_batt_profile_running_update(void)
{
	...
	if (chip) {
		fg = &(chip->fg);
		if (chip->profile_loaded && fg->profile_available) {
			chip->profile_running_update = true;
...
static int fg_gen4_get_batt_profile(struct fg_dev *fg)
{
	...
	/* vivo modify*/
	int batt_id;
	pr_err("chip->batt_id_ohms= %d\n", fg->batt_id_ohms);

	batt_id = fg->batt_id_ohms / 1000;

	if (chip->check_battery_id_enable) {
		batt_id = fg_get_batt_id_from_cms(chip) * 100;
		pr_err("check batt_id = %d\n", batt_id);
	}
	batt_node = of_find_node_by_name(node, "vivo,battery-data");
	if (!batt_node) {
...
static bool is_profile_load_required(struct fg_gen4_chip *chip)
{
	...
	if (val & PROFILE_LOAD_BIT) {
		...
		profiles_same = memcmp(chip->batt_profile, buf,
					PROFILE_COMP_LEN) == 0;
		is_profiles_same = profiles_same;
		if (profiles_same) {
			if(chip->rslow_parameter_used) {
				rc = fg_sram_read(fg, 110, 0, rslow_buf, 8, FG_IMA_DEFAULT);
				if (rc < 0) {
					pr_err("Error in reading battery profile, rc:%d\n", rc);
					fg->profile_load_status = PROFILE_SKIPPED;
					return false;
				}
				rslow_param_same = memcmp(&chip->batt_profile[90], rslow_buf, 8) == 0;
...
static int qpnp_fg_gen4_load_profile(struct fg_gen4_chip *chip)
{
	...
	pr_err("qpnp_fg_gen4_load_profile normal=%d, multi=%d, batt_age=%d\n", \
			normal_profile_load, chip->dt.multi_profile_load, chip->batt_age_level);
	...
	if (normal_profile_load) {
		if (!chip->dt.disable_side_loading) {
...
static void profile_load_work(struct work_struct *work)
{
	...
	u8 val, buf[2];
	int rc, ibatt_now, vbatt_now, ocv_now, soc_now, temp_now;

	pr_err("start..\n");
	schedule_delayed_work(&fg->sram_dump_work, msecs_to_jiffies(1));
	msleep(50);
	vote(fg->awake_votable, PROFILE_LOAD, true, 0);

        if (!chip->profile_running_update) {
	...
	if (fg->profile_load_status == PROFILE_LOADED) {
		...
		chip->profile_running_update = false;
        }
	/* vivo end */
...
out:
	...
	if(is_profiles_same){
		rc = fg_get_battery_current(&chip->fg, &ibatt_now);
		if (rc < 0) {
			pr_err("failed to get battery current, rc=%d\n", rc);
			ibatt_now = 0;
		}

		rc = fg_get_battery_voltage(&chip->fg, &vbatt_now);
		if (rc < 0) {
			pr_err("failed to get battery voltage, rc=%d\n", rc);
			vbatt_now = 0;
		}

		rc = fg_get_sram_prop(&chip->fg, FG_SRAM_OCV, &ocv_now);
		if (rc < 0) {
			pr_err("failed to get battery voltage, rc=%d\n", rc);
			ocv_now = 0;
		}

		rc = fg_gen4_get_prop_capacity(&chip->fg, &soc_now);
		if (rc < 0) {
			pr_err("failed to get battery soc, rc=%d\n", rc);
			soc_now = 50;
		}

		rc = fg_gen4_get_battery_temp(&chip->fg, &temp_now);
		if (rc < 0) {
			pr_err("failed to get battery soc, rc=%d\n", rc);
			temp_now = 0;
		}

		pr_err("ibatt_now=%d, vbatt_now=%d, ocv_now=%d, soc_now=%d, temp_now=%d\n", \
					ibatt_now, vbatt_now, ocv_now, soc_now, temp_now);
		if (!chip->skip_fg_restart) {
			if(((temp_now >= 100) && (ibatt_now > 30000) && (vbatt_now - ocv_now > 300000)) || \
				((temp_now >= 100) && (ibatt_now > 30000) && vbatt_now >= 3750000 && soc_now <= 20) || \
				((temp_now >= 100) && (ibatt_now > 30000) && vbatt_now >= 4000000 && soc_now <= 50) || \
				((temp_now >= 100) && (ibatt_now > 30000) && vbatt_now >= 3850000 && soc_now <= 30)){
				rc = fg_restart(&chip->fg, SOC_READY_WAIT_TIME_MS);
				msleep(500);
				fg_do_reset = true;
				if (rc < 0) {
					pr_err("Error in restarting FG, rc=%d\n", rc);
...
static void get_batt_psy_props(struct fg_dev *fg)
{
	...
	/*vivo add*/
	if (is_input_present(fg)) {
		rc = power_supply_get_property(fg->batt_psy, POWER_SUPPLY_PROP_FFC_CHARGE_FULL,
			&prop);
		if (rc < 0) {
			pr_err("Error in getting battery FFC charge FULL, rc=%d\n", rc);
			return;
		}
		fg->ffc_charge_full = prop.intval;
	} else
		fg->ffc_charge_full = 0;
...
static int fg_gen4_charge_full_update(struct fg_dev *fg)
{
	...
	fg_dbg(fg, FG_STATUS, "msoc: %d bsoc: %x health: %d status: %d full: %d, ffc_charge_full: %d\n",
		msoc, bsoc, fg->health, fg->charge_status,
		fg->charge_full, fg->ffc_charge_full);
	if ((fg->charge_done || fg->ffc_charge_full) && !fg->charge_full) {	//vivo change
		/* vivo modify */
		if (msoc >= 95 && (fg->health == POWER_SUPPLY_HEALTH_GOOD ||
			fg->health == POWER_SUPPLY_HEALTH_DUAL_ENGINE)) {
		/* vivo end */
		...
		if (rc < 0)
			goto out;
		/*vivo add*/
		chip->fake_battery_full_flag = true;
		cancel_delayed_work(&chip->fake_battery_full_flag_work);
		schedule_delayed_work(&chip->fake_battery_full_flag_work,
...
static irqreturn_t fg_batt_missing_irq_handler(int irq, void *data)
{
	...
	if (fg->battery_missing) {
		...
		chip->profile_loaded = false;/* vivo add */
...
/* vivo add */
static bool is_cms_available(struct fg_gen4_chip *chip)
{
	if (!chip->cms_psy)
...
static int fg_get_batt_id_from_cms(struct fg_gen4_chip *chip)
{
	...
	if (!is_cms_available(chip)) {
		...
		power_supply_get_property(chip->cms_psy, POWER_SUPPLY_PROP_BATTERY_ID, &val);
		if (val.intval <= B_UNKOWN || val.intval >= B_MAX) {
			batt_id = 0;
			if(!chip->dt.ext_sense)
				batt_id = 1;
		} else{
			batt_id = val.intval;
		}
...
#define CHECK_BATTERY_ID_COUNT_MAX	10
#define CHECK_BATTERY_ID_PERIOD_MS	200
#define CHECK_BATTERY_ID_LONG_PERIOD_MS	3000
static void check_battery_id_work(struct work_struct *work)
{
	...
	int batt_id = fg_get_batt_id_from_cms(chip);

	pr_warn("profile_loaded:%d,batt_id:%d,check_battid_count:%d, profile_available=%d\n", chip->profile_loaded, batt_id, chip->check_battid_count, fg->profile_available);

	if(chip->profile_loaded || chip->check_battid_count >= CHECK_BATTERY_ID_COUNT_MAX){
		pr_warn("check work is done! batt_id=%d, profile_loaded=%d\n", batt_id, chip->profile_loaded);
		fg->soc_reporting_ready = true;
		chip->check_battid_count = 0;
#if 1
		if (fg_sram_dump)
		schedule_delayed_work(&fg->sram_dump_work,
			msecs_to_jiffies(fg_sram_dump_period_ms * 10));
#endif
		return;
	}
	...
	if (batt_id) {
		...
		if (fg->profile_available) {
			schedule_delayed_work(&fg->profile_load_work, 0);
			delay_time = CHECK_BATTERY_ID_LONG_PERIOD_MS;
		}
	}
...
static void fake_battery_full_flag_work(struct work_struct *work)
{
	struct fg_gen4_chip *chip = container_of(work, struct fg_gen4_chip,
					    fake_battery_full_flag_work.work);

	chip->fake_battery_full_flag = false;
	pr_err("clear fake battery full flag!\n");
}
...
static bool fg_check_is_chg_est_vbat_diff(int vbat, int ocv, int soc)
{
	...
	if((vbat >= 3750000 && soc <= 20) || (vbat >= 4000000 && soc <= 50) || (vbat >= 3850000 && soc <= 30)){
		est_vbat_diff = true;
	}

	if(vbat <= 3700000 && soc > 65){
		est_vbat_diff = true;
	}
...
static void fg_update_cutoff_volt_mv(struct fg_gen4_chip *chip, int temp_now)
{
	bool update = false;
	uint8_t buf[4];
	union power_supply_propval val = {3400000,};
	int rc, cutoff_mv = chip->dt.cutoff_volt_mv;
	struct fg_dev *fg = &chip->fg;

	pr_warn("temperature:[%d,%d] cutoff_volt_mv:[%d,%d,%d]\n",
			chip->custom_cutoff_temperature, temp_now,
			chip->custom_cutoff_volt_mv, chip->dt.cutoff_volt_mv, chip->last_cutoff_volt_mv);

	if (temp_now < chip->custom_cutoff_temperature) {
		if (chip->custom_cutoff_volt_mv != chip->last_cutoff_volt_mv) {
			update = true;
			cutoff_mv = chip->custom_cutoff_volt_mv;
		}
	} else if (temp_now > chip->custom_cutoff_temperature) {
		if (chip->dt.cutoff_volt_mv != chip->last_cutoff_volt_mv) {
			update = true;
			cutoff_mv = chip->dt.cutoff_volt_mv;
		}
	}

	if (update) {
		chip->last_cutoff_volt_mv = cutoff_mv;
		fg_encode(fg->sp, FG_SRAM_CUTOFF_VOLT, cutoff_mv, buf);
		...
		if (fg->batt_psy) {
			val.intval = cutoff_mv * 1000;
			rc = power_supply_set_property(fg->batt_psy,
					POWER_SUPPLY_PROP_VOLTAGE_MIN, &val);
			pr_err("set batt_psy prop cutoff_volt=%d\n", cutoff_mv);
		}
	}
}

#if 0
//FG_SRAM_SYS_TERM_CURR: [log] 022 00 fe ff 45 --> 0xFE00 
static void fg_update_sys_term_curr(struct fg_gen4_chip *chip, int current_ma)	//current_ma < 0
{
	int rc;
	u8 buf[4];
	struct fg_dev *fg = &chip->fg;

	if (chip->last_sys_term_curr_ma != current_ma && current_ma < 0 && current_ma > -1000) {
		fg_encode(fg->sp, FG_SRAM_SYS_TERM_CURR, current_ma,
			buf);
		...
		if (rc < 0) {
			...
			chip->last_sys_term_curr_ma = current_ma;
			pr_err("update sys_term_curr_ma=%dmA\n", chip->last_sys_term_curr_ma);
		}
	} else
		pr_err("try to update sys_term_curr_ma=%dmA. but skip...\n", current_ma);

}
#endif
static void fg_monitor_work(struct work_struct *work)
{
	struct fg_gen4_chip *chip = container_of(work, struct fg_gen4_chip,
					    fg_monitor_work.work);
	int rc,  ibatt_now, vbatt_now, temp_now, soc_now, ocv_now;
	static int pre_soc = -1;
	static int count = 0;
	static int ibat_count = 0;
	static int do_reset_status_count = 0;
	static int do_reset_count = 0;
	int batt_capacity_mah;
	int soh = 100;
	#ifdef FG_DUMP_ENABLE
	u8 data_buf[500];
	int i;
	#endif
	//int coul;
	...
	get_votable_info(chip->fg.awake_votable);
	...
	if (rc < 0) {
		...
		fg_update_cutoff_volt_mv(chip, temp_now);
	}
	...
	if (chip->profile_loaded && temp_now >= 100 && ibatt_now >30000) {
		chip->chg_est_vbat_diff = fg_check_is_chg_est_vbat_diff(vbatt_now, ocv_now, soc_now);
	} else {
	...
	/*For Charging process: >450mA -> 100%*/
	if (chip->hold_on_99_enable) {
		if ((chip->fg).charge_status == POWER_SUPPLY_STATUS_CHARGING || (chip->fg).charge_status == POWER_SUPPLY_STATUS_FULL) {
			if (ibatt_now < (chip->hold_on_99_ibat_ua - 50000) && soc_now < 100) {
				chip->hold_on_99 = true;
				ibat_count = 0;
			} else if (ibatt_now >= chip->hold_on_99_ibat_ua) {
				if (ibat_count++ >= 3) {
					ibat_count = 3;
					chip->hold_on_99 = false;
				}
		...
		if (chip->fg.charge_full) {
			chip->hold_on_99 = false;
		}
	}

	if (count > 2)
		chip->fake_batt_therm_during_power_on = false;
	if (++count > 3600) {
		count = 0;
	}

	pr_warn("charge_status=%d,current=%d,volt=%d,ocv=%d,temp=%d,soc=%d,hold_on_99=%d(en=%d, ibat_thr=%d, count=%d),ext_sense=%d\n",
		(chip->fg).charge_status, ibatt_now, vbatt_now, ocv_now, temp_now, soc_now, chip->hold_on_99, chip->hold_on_99_enable, chip->hold_on_99_ibat_ua, ibat_count, chip->dt.ext_sense);

	if (batt_psy_initialized(&chip->fg) && (pre_soc != soc_now || 0 == soc_now)) {
		pre_soc = soc_now;
		power_supply_changed((chip->fg).batt_psy);
	}

	if (!chip->fuelsummary_psy) {
		chip->fuelsummary_psy = power_supply_get_by_name("fuelsummary");
		if (power_supply_get_by_name("bq_bms"))
			chip->bq_fg = true;
		else
			chip->bq_fg = false;
	}
	...
	if (!chip->bq_fg) {
		if (chip->cl->learned_cap_uah > 0) {
			if (chip->profile_loaded) {
				batt_capacity_mah = (chip->fg).bp.batt_capacity_mah;
			} else{
				batt_capacity_mah = 4500;
			}
			soh = (chip->cl->learned_cap_uah / 10 / batt_capacity_mah);
			if (soh > 100)
					soh = 100;
		} else {
			soh = chip->soh;
		}
		//fg_gen4_get_charge_raw(chip, &coul);
		//fuelsummary_collect_value(ID_COUL, (coul / 1000));
	}

	if (chip->fuelsummary_psy)
		power_supply_set_property(chip->fuelsummary_psy,
				POWER_SUPPLY_PROP_FEX_RUNNING, &scene);
	pr_err("profile_loaded=%d, do_reset_count=%d, do_reset_status_count=%d, skip_fg_restart=%d", \
		chip->profile_loaded, do_reset_count, do_reset_status_count, chip->skip_fg_restart);
	if (!chip->skip_fg_restart) {
		if(chip->profile_loaded && do_reset_count <= 3 && temp_now > 100){
			if((ibatt_now > 30000 && ((vbatt_now - ocv_now) > 300000)) || ((ibatt_now > 30000) && vbatt_now >= 3750000 && soc_now < 20) || ((ibatt_now > 30000) && vbatt_now >= 4000000 && soc_now < 50)){
				do_reset_status_count++;
			}else{
				do_reset_status_count = 0;
			}

			if(do_reset_status_count >= 3){
				rc = fg_restart(&chip->fg, SOC_READY_WAIT_TIME_MS);
				msleep(500);
				fg_do_reset = true;
				do_reset_count++;
	...
	schedule_delayed_work(&chip->fg_monitor_work,
...
static void sram_dump_work(struct work_struct *work)
{
	...
	fg_dbg(fg, FG_STATUS, "SRAM Dump done at %lld.%d\n",
		quotient, remainder);
resched:
	cancel_delayed_work(&fg->sram_dump_work);	//vivo add
	schedule_delayed_work(&fg->sram_dump_work,
				       ...
				       union power_supply_propval *pval)
{
	...
	int rc = 0, val;
	int64_t temp;
	int rcc =0, ibatt_now = 0, vbatt_now = 0, ocv_now = 0, soc_now = 0, temp_now = 0;
	...
	case POWER_SUPPLY_PROP_CHARGE_TERM_CURRENT:
		pval->intval = chip->user_chg_term_curr_ma;
		break;
	#endif
	case POWER_SUPPLY_PROP_CHG_EST_VBAT_DIFF:
		rcc = fg_get_battery_current(&chip->fg, &ibatt_now);
		if (rcc < 0) {
			pr_err("failed to get battery current, rcc=%d\n", rcc);
			ibatt_now = 0;
		}

		rcc = fg_get_battery_voltage(&chip->fg, &vbatt_now);
		if (rcc < 0) {
			pr_err("failed to get battery voltage, rcc=%d\n", rcc);
			vbatt_now = 0;
		}

		rcc = fg_get_sram_prop(&chip->fg, FG_SRAM_OCV, &ocv_now);
		if (rc < 0) {
			pr_err("failed to get battery voltage, rcc=%d\n", rcc);
			ocv_now = 0;
		}

		rcc = fg_gen4_get_battery_temp(&chip->fg, &temp_now);
		if (rcc < 0) {
			pr_err("failed to get battery soc, rcc=%d\n", rcc);
			temp_now = 0;
		}

		rcc = fg_gen4_get_prop_capacity(&chip->fg, &soc_now);
		if (rcc < 0) {
			pr_err("failed to get battery soc, rcc=%d\n", rcc);
			soc_now = 50;
		}
		if (chip->profile_loaded && temp_now >= 100 && ibatt_now >20000) {
			chip->chg_est_vbat_diff = fg_check_is_chg_est_vbat_diff(vbatt_now, ocv_now, soc_now);
		} else {
			pr_warn("chg_est_vbat_diff status %d, profile_loaded %d or temp too low, set to 2\n", \
				chip->chg_est_vbat_diff, chip->profile_loaded);
		...
		pr_err("ibatt_now=%d, vbatt_now=%d, ocv_now=%d, soc_now=%d, temp_now=%d\n", \
				ibatt_now, vbatt_now, ocv_now, soc_now, temp_now);
		pr_info("chg_est_vbat_diff status %d\n", pval->intval);
		break;
	case POWER_SUPPLY_PROP_AGING_SUPPORT:
		pval->intval = (chip->aging_support ? 1 : 0);
		break;
	/*vivo end*/
	default:
				  ...
				  const union power_supply_propval *pval)
{
	...
	case POWER_SUPPLY_PROP_CALIBRATE:
		...
		break;
	//vivo add start
	case POWER_SUPPLY_PROP_FG_MONITOR_WORK:
		if (!pval->intval) {
			cancel_delayed_work(&chip->fg_monitor_work);
		} else {
			cancel_delayed_work(&chip->fg_monitor_work);
			schedule_delayed_work(&chip->fg_monitor_work, 3);
		}
		fg_dbg(fg, FG_STATUS, "FG_MONITOR_WORK %d ...\n", pval->intval);
		break;
#if 0
	case POWER_SUPPLY_PROP_FG_SYS_TERM_CURR:
		if (pval->intval == 0)
			fg_update_sys_term_curr(chip, chip->dt.sys_term_curr_ma);	//reset to default value.
		else
			fg_update_sys_term_curr(chip, pval->intval);
		break;
#endif
	//vivo add end
	default:
...
//vivo add
static int fg_gen4_esr_timer_dischg_config(struct fg_gen4_chip *chip, bool sleep)
{
	struct fg_dev *fg = &chip->fg;
	int esr_timer_dischg_init, esr_timer_dischg_max;
	int rc;
	if (chip->esr_fast_calib)
		return 0;
	if (chip->dt.esr_timer_dischg_slow[TIMER_RETRY] >= 0 &&
			chip->dt.esr_timer_dischg_slow[TIMER_MAX] >= 0 &&
			chip->dt.esr_timer_dischg_fast[TIMER_RETRY] >= 0 &&
			chip->dt.esr_timer_dischg_fast[TIMER_MAX] >= 0) {
		esr_timer_dischg_init = sleep ? chip->dt.esr_timer_dischg_slow[TIMER_RETRY] : 
			chip->dt.esr_timer_dischg_fast[TIMER_RETRY];
		esr_timer_dischg_max = sleep ? chip->dt.esr_timer_dischg_slow[TIMER_MAX] : 
			chip->dt.esr_timer_dischg_fast[TIMER_MAX];
		rc = fg_set_esr_timer(fg, esr_timer_dischg_init,
				esr_timer_dischg_max, false,
...
static int fg_gen4_esr_calib_config(struct fg_gen4_chip *chip)
{
	...
	if (chip->esr_fast_calib) {
		...
		//vivo modify
		rc = fg_gen4_esr_timer_dischg_config(chip, false);
		if (rc < 0)
			return rc;
		...
		if (chip->dt.esr_calib_dischg) {
			...
			pr_err("Allow ESR calibration only during discharging.\n");
		} else {
			/*
			 * Disable ESR discharging timer and ESR pulsing during
...
static int fg_gen4_hw_init(struct fg_gen4_chip *chip)
{
	...
	//0x305e:  bit7:1=use_ext_sns; bit(1:0):0=2mO,1=3.3mO,2=5mO(default),3=10mO
	//内部采样时，adb实测 0x305E = 0x02.
	//外部采样时，adb实测 0x305E = 0x82. (外部采样+5mO)
	//[发现即使配置成外部采样，每次开机时读出来都是 0x02].

	/*
	 * 注：AT模式下，不要配置成外部采样（外部采样电阻可能没有贴），容易引起Ibat过大，触发到BCL相关过流保护/AFP功能.
	 *
	 * [以下patch: disable BCL trigger AFP]:
	 * @@/sm8150_boot/boot_images/QcomPkg/Library/PmicLib/target/sdm855_pm855_pm855b/psi/pm_config_target_sbl_sequence.c
	 *    --{ 2,	 0x48,	 0x7515,	 0xFF,	 PM_SBL_WRITE,	 0,	 0},
	 *    ++{ 2,	 0x8,	 0x7515,	 0xFF,	 PM_SBL_WRITE,	 0,	 0},
	 */
	#if 1
	if (chip->dt.ext_sense && !is_atboot) {
		u8 temp_val = 0x82;
		int ibatt_now = 0;

		if (chip->dt.sense_r_2mO)
			temp_val = 0x80;

		rc = fg_write(fg, 0x305E, &temp_val, 1);
		if (rc < 0) {
			pr_err("Couldn't write to 0x305E rc=%d\n", rc);
		}

		#if 0
		if (chip->dt.sense_r_2mO) {
			// CR#2568867 : The both registers are trim values for 2mohm sensing resistor, it鈥檚 to adjust the battery current reading data.
			// boot_images/QcomPkg/Library/PmicLib/target/sdm8250_pm855_pm855b/psi/pm_config_target_pbs_ram.c::PMIC PSI Version: Kona-SBL-b0x00_v0x3E: after PSI>0x3E, no need below code.
			temp_val = 0xB0;
			rc |= fg_write(fg, 0x30F7, &temp_val, 1);
			temp_val = 0x01;
			rc |= fg_write(fg, 0x30F6, &temp_val, 1);
		}
		#endif
		fg_get_battery_current(&chip->fg, &ibatt_now);
		pr_err("fg_gen4_hw_init: ext_sense %s configured success!!! ibat_now_ua=%d.\n", chip->dt.sense_r_2mO ? "2mO" : "5mO", ibatt_now);
		#if 0
		msleep(800);
		fg_get_battery_current(&chip->fg, &ibatt_now);
		pr_err("fg_gen4_hw_init: ext_sense configured success!!! ibat_now_ua=%d. 2\n", ibatt_now);
		msleep(1100);
		fg_get_battery_current(&chip->fg, &ibatt_now);
		pr_err("fg_gen4_hw_init: ext_sense configured success!!! ibat_now_ua=%d. 3\n", ibatt_now);
		#endif
	}
	#endif
	...
	if (rc < 0) {
		...
		chip->last_sys_term_curr_ma = chip->dt.sys_term_curr_ma;
	}
	...
	if (chip->dt.rconn_uohms) {
		...
		if (!buf[0] && !buf[1]) {
			...
			pr_err("Skipping configuring Rconn [0x%x 0x%x]\n",
				buf[0], buf[1]);
		}
	}
...
static int fg_parse_esr_cal_params(struct fg_dev *fg)
{
	...
	if (chip->dt.esr_timer_dischg_slow[TIMER_RETRY] >= 0 &&
		...
		if (chip->dt.esr_calib_dischg) {
			pr_err("read qcom,fg-esr-calib-dischg, return.\n");
			return 0;
		}
	}
...
static int fg_gen4_parse_dt(struct fg_gen4_chip *chip)
{
	...
	chip->custom_cutoff_temperature = 100;
	chip->custom_cutoff_volt_mv = chip->dt.cutoff_volt_mv;
	chip->last_cutoff_volt_mv = chip->dt.cutoff_volt_mv;
	...
	//vivo add
	fg_sram_dump = of_property_read_bool(node, "vivo,sram-dump-enable");

	chip->dt.ext_sense = of_property_read_bool(node, "qcom,ext-sense");
	chip->dt.sense_r_2mO = of_property_read_bool(node, "qcom,sense-r-2mO");

#if 1
	if (is_project("PD1955"))
	{
		board_version= get_bbk_board_version();
		if (!board_version) {
			pr_err("failed to board_version\n");
		} else {
			/*QCOM FG: "xx0xxxx"*/
			pr_info("PD1955: board_version=%s\n", board_version);
			if(board_version[2] == '0') {
				pr_info("PD1955 Detect QCOM FG. use the ext_sense=true.\n");
				chip->dt.ext_sense = true;
				chip->dt.sense_r_2mO = true;
				fg_sram_dump = true;
			} else {
				pr_info("PD1955 Detect TI FG. use the ext_sense=false.\n");
				chip->dt.ext_sense = false;
				chip->dt.sense_r_2mO = false;
			}
		}
	}
#endif
	...
	chip->skip_fg_restart = of_property_read_bool(node, "vivo,skip-fg-restart");
	pr_info("skip_fg_restart=%d\n", chip->skip_fg_restart);

	chip->rslow_parameter_used = of_property_read_bool(node, "vivo,rslow-parameter-used");
	pr_info("rslow_parameter_used=%d\n", chip->rslow_parameter_used);

	chip->hold_on_99_enable = of_property_read_bool(node, "vivo,hold-on-99-chg-enable");
	if (chip->hold_on_99_enable) {
		rc = of_property_read_u32(node, "vivo,hold-on-99-chg-ibat", &temp);
		if (rc < 0) {
			chip->hold_on_99_ibat_ua = -500000;
			//chip->hold_on_99_enable = false;
		} else
			chip->hold_on_99_ibat_ua = temp;
	}

	pr_info("hold_on_99_enable=%d, hold_on_99_ibat_ua=%d\n", chip->hold_on_99_enable, chip->hold_on_99_ibat_ua);

	chip->vivo_s8c_ctrl = devm_regulator_get(fg->dev, "vivo_s8c_ctrl");
	if (IS_ERR(chip->vivo_s8c_ctrl)) {
		pr_err("unable to get vivo_s8c_ctrl supply..\n");
	}

	chip->dt.disable_side_loading = of_property_read_bool(node,
					"vivo,disable-side-loading");
	pr_err("disable_side_loading=%d..\n", chip->dt.disable_side_loading);
...
static int fg_gen4_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&chip->fg_monitor_work, fg_monitor_work);
	INIT_DELAYED_WORK(&chip->fake_battery_full_flag_work, fake_battery_full_flag_work);
	chip->fake_battery_full_flag = false;

	chip->fg.msoc_mapping_enable = false;
	memcpy(chip->fg.msoc_mapping_table, default_msoc_mapping_table, sizeof(default_msoc_mapping_table));
	chip->fg.msoc_mapping_col = 0;
	chip->fg.msoc_mapping_row = 0;
	...
	*/
	...
	chip->check_battery_id_enable = true;
	schedule_delayed_work(&chip->check_battery_id_work, 0);
	schedule_delayed_work(&chip->fg_monitor_work, 0);
	/* vivo end */
...
static void fg_gen4_shutdown(struct platform_device *pdev)
{
	...
	if (fg->charge_full) {
		pr_info("charge full,write 0xFF.\n"); /* vivo add */
		/* We need 2 most significant bytes here */
		bsoc = (u32)bsoc >> 16;
...
static int fg_gen4_suspend(struct device *dev)
{
	...
	fg_gen4_esr_timer_dischg_config(chip, true);
	...
	if (!IS_ERR(chip->vivo_s8c_ctrl)) {
		pr_err("set 0.\n");
		regulator_set_load(chip->vivo_s8c_ctrl, 0);	//allow to keep auto_LPM
	}
	//vivo add end
	return 0;
}
...
static int fg_gen4_resume(struct device *dev)
{
	...
	if (!IS_ERR(chip->vivo_s8c_ctrl)) {
		pr_err("set >0.\n");
		regulator_set_load(chip->vivo_s8c_ctrl, 1000000);	//allow to keep force_NPM
	}
	//vivo add end

	fg_gen4_esr_timer_dischg_config(chip, false);
