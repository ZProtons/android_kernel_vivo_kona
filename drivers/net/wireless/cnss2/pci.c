...
/* vivo luoguizhao add for runtime suspend begin */
static int cnss_runtimepm_state = 1;
module_param(cnss_runtimepm_state, int, 0644);
MODULE_PARM_DESC(cnss_runtimepm_state, "Added for RuntimePM control");
/* vivo luoguizhao add for runtime suspend end */

/* vivo luoguizhao modify for adding delay before 3rd training retry begin */
#define LINK_TRAINING_RETRY_DELAY_TIME_MS           1000
int cnss_sleep_before_driver_time = LINK_TRAINING_RETRY_DELAY_TIME_MS;
EXPORT_SYMBOL(cnss_sleep_before_driver_time);
module_param(cnss_sleep_before_driver_time, int, 0644);
MODULE_PARM_DESC(cnss_sleep_before_driver_time, "Added for adding delay before 3rd training retry");
/* vivo luoguizhao modify for adding delay before 3rd training retry end */

static int cnss_ftm_mode = 0;
module_param(cnss_ftm_mode, int, 0644);
MODULE_PARM_DESC(cnss_ftm_mode, "Added for cnss_ftm_mode control");
...
retry:
	...
	if (ret) {
		...
		if (link_up && retry++ < LINK_TRAINING_RETRY_MAX_TIMES) {
			/* vivo luoguizhao modify for adding delay before 3rd training begin */
			cnss_pr_dbg("sleep %dms before retry to training\n", cnss_sleep_before_driver_time);
			msleep(cnss_sleep_before_driver_time);
			/* vivo luoguizhao modify for adding delay before 3rd training end */
...
// vivo luoguizhao add 1000ms delay for PCIe resume before suspend begin
#define SUSPEND_RESUME_INTERVAL_US  ((u64)(1ULL * 1000ULL * 1000ULL))
#define SUSPEND_RESUME_INTERVAL_MS  ((u64)(1ULL * 1000ULL))
static u64 last_suspend_timestamp;
// vivo luoguizhao add 1000ms delay for PCIe resume before suspend end
...
static int cnss_qca6290_powerup(struct cnss_pci_data *pci_priv)
{
	...
	int retry = 0;
// vivo luoguizhao add 1000ms delay for PCIe resume before suspend begin
	u64 current_timestamp = cnss_get_host_timestamp(plat_priv);
	u64 timestamp_delta, sleep_time_ms;

	cnss_pr_vdbg("%s: current_timestamp=%lluus last_suspend_timestamp=%lluus\n", __func__, current_timestamp, last_suspend_timestamp);

	if (!cnss_ftm_mode)
	{
		cnss_pr_vdbg("cnss_ftm_mode = 0, need sleep");
		timestamp_delta = current_timestamp - last_suspend_timestamp;
		if (timestamp_delta < SUSPEND_RESUME_INTERVAL_US) {
			sleep_time_ms = (SUSPEND_RESUME_INTERVAL_US - timestamp_delta) / 1000ULL;
			if (sleep_time_ms > SUSPEND_RESUME_INTERVAL_MS)
				sleep_time_ms = SUSPEND_RESUME_INTERVAL_MS;
			cnss_pr_vdbg("no enough time(%lluus) to wait for suspending, sleep %llums\n", SUSPEND_RESUME_INTERVAL_US, sleep_time_ms);
			msleep(sleep_time_ms);
		}
	}
	else
		cnss_pr_vdbg("cnss_ftm_mode  = 1, no need sleep\n");
...
// vivo luoguizhao add 1000ms delay for PCIe resume before suspend end
	...
	cnss_power_off_device(plat_priv);
// vivo luoguizhao add 1000ms delay for PCIe power on before off begin
	current_timestamp = cnss_get_host_timestamp(plat_priv);
	cnss_pr_vdbg("%s: current_timestamp=%lluus last_suspend_timestamp=%lluus\n", __func__, current_timestamp, last_suspend_timestamp);
	last_suspend_timestamp = current_timestamp;
// vivo luoguizhao add 1000ms delay for PCIe power on before off end
...
register_driver:
	...
	/* vivo luoguizhao modify for adding delay before 3rd training begin */
	cnss_pr_dbg("sleep %dms before probe driver\n", cnss_sleep_before_driver_time);
	msleep(cnss_sleep_before_driver_time);
	cnss_pr_dbg("begin probe driver\n");
	/* vivo luoguizhao modify for adding delay before 3rd training end */
...
static int cnss_pci_runtime_suspend(struct device *dev)
{
	...
	if (!cnss_runtimepm_state) {
		cnss_pr_dbg("RuntimePM disabled, reject runtime suspend\n");
		return -EAGAIN;
	}
...
extern char *vivo_get_projeck_name(void);
...
int cnss_pci_force_fw_assert_hdlr(struct cnss_pci_data *pci_priv)
{
	...
	project_name = vivo_get_projeck_name();
	...
	if (subsys_info && subsys_info->subsys_device) {
		restart_level = subsys_get_restart_level(subsys_info->subsys_device);
	}
	...
	if (ret) {
		...
		if (project_name != NULL
				&& ((!strncmp(project_name, "PD1950", strlen("PD1950")))
				|| (!strncmp(project_name, "PD1955", strlen("PD1955")))
				|| (!strncmp(project_name, "PD1981", strlen("PD1981")))
				|| (!strncmp(project_name, "PD2011", strlen("PD2011")))
				|| (!strncmp(project_name, "PD2024", strlen("PD2024")))
				|| (!strncmp(project_name, "PD2025", strlen("PD2025")))
				|| (!strncmp(project_name, "PD1955F_EX", strlen("PD1955F_EX")))
				|| (!strncmp(project_name, "PD2024F_EX", strlen("PD2024F_EX"))))
				&& restart_level == 1) {
			cnss_fatal_err("do not trigger crash for user\n");
		} else {
			panic("Subsystem WLAN crashed for Failed to trigger RDDM!");
		}
		cnss_schedule_recovery(&pci_priv->pci_dev->dev,
				       CNSS_REASON_DEFAULT);
		return ret;
...
retry:
	...
	if (ret) {
		...
		if (retry++ < LINK_TRAINING_RETRY_MAX_TIMES) {
			/* vivo luoguizhao modify for adding delay before 3rd training begin */
			cnss_pr_dbg("sleep %dms before PCI link training in cnss_pci_init\n", cnss_sleep_before_driver_time);
			msleep(cnss_sleep_before_driver_time);
			/* vivo luoguizhao modify for adding delay before 3rd training end */
			cnss_pr_dbg("Retry PCI link training #%d\n", retry);
			goto retry;
