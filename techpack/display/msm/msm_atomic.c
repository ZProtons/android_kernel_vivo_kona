...
msm_disable_outputs(struct drm_device *dev, struct drm_atomic_state *old_state)
{
	...
	for_each_old_connector_in_state(old_state, connector,
		...
		if ((vdisp && (!strcmp(vdisp->project_name, "pd1981") || !strcmp(vdisp->project_name, "pd2012")) && is_atboot)){
...
/* vivo zwl add for dc dimming/color matrix/hbm ud flag set begin */
static void vivo_ud_flag_commit(struct drm_atomic_state *state)
{
	struct vivo_display *vdisp  = get_vivo_vdisp();
	...
	/* vivo add for color matrix */
	if (state->ud_flag & UD_FLAG_COLOR_MATRIX_MASK) {
		SDE_ATRACE_BEGIN("dsi_panel_set_color_matrix");
		if ((state->ud_flag & UD_FLAG_COLOR_MATRIX_CLOSE) && (state->ud_flag & UD_FLAG_DC_DIMMING_OPEN)) {
			dsi_panel_set_color_matrix(state->dc_dimming_threshold, state->ud_flag & UD_FLAG_COLOR_MATRIX_MASK);
			vdisp->dc.dc_dimming = state->dc_dimming_threshold;
		} else {
			dsi_panel_set_color_matrix(state->color_matrix_brightness, state->ud_flag & UD_FLAG_COLOR_MATRIX_MASK);
		}
		SDE_ATRACE_END("dsi_panel_set_color_matrix");
		state->ud_flag = 0;
	}

	/* vivo add for dc dimming */
	...
	if (vdisp->smart_aod.is_support) {
		if (state->ud_hbm_flag == 1) {
			SDE_ATRACE_BEGIN("dsi_panel_set_hbm");
			dsi_panel_hbm_ud_ctrl(true);
			SDE_ATRACE_END("dsi_panel_set_hbm");
			state->ud_hbm_flag = 0;
		} else if (state->ud_hbm_flag == 2) {
			SDE_ATRACE_BEGIN("dsi_panel_close_hbm");
			dsi_panel_hbm_ud_ctrl(false);
			SDE_ATRACE_END("dsi_panel_close_hbm");
			state->ud_hbm_flag = 0;
		}

		/* vivo add for smart aod */
		if (vdisp->doze_mode_state)
			vivo_smart_aod_update(vdisp);
	} else {
		if (state->ud_hbm_flag == 1) {
			SDE_ATRACE_BEGIN("dsi_panel_set_hbm");
			dsi_panel_set_fod_state(true);
			SDE_ATRACE_END("dsi_panel_set_hbm");
			state->ud_hbm_flag = 0;
		}
		if (state->ud_hbm_flag == 2) {
			SDE_ATRACE_BEGIN("dsi_panel_close_hbm");
			dsi_panel_set_fod_state(false);
			SDE_ATRACE_END("dsi_panel_close_hbm");
			state->ud_hbm_flag = 0;
		}
	}
}
/* vivo zwl add for dc dimming/color matrix/hbm ud flag set end */
		...
		struct drm_atomic_state *state, bool nonblock)
{
	...
	if (!priv || priv->shutdown_in_progress) {
		DRM_INFO("priv is null or shutdwon is in-progress\n");
		return -EINVAL;
	}
