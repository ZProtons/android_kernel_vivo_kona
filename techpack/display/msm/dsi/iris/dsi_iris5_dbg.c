...
static ssize_t display_mode_show(struct device *device,
...
static DEVICE_ATTR_RO(display_mode);
...
int iris_dbgfs_status_init(struct dsi_display *display)
{
	...
	if (display->pdev)
		sysfs_create_group(
			&display->pdev->dev.kobj, &iris_attr_group);
	else
		IRIS_LOGE("display->pdev is null");
