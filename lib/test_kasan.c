...
#if 1
static void (*kasan_test_func[])(void) = {
	kmalloc_oob_right,
	kmalloc_oob_left,
...
static int kasan_param_set(const char *val, const struct kernel_param *kp)
{
	unsigned long i = 0;
	int ret;

	ret = kstrtoul(val, 10, &i);
	if (ret)
		return ret;

	if (i < ARRAY_SIZE(kasan_test_func))
		kasan_test_func[i]();

	return 0;
}

static const struct kernel_param_ops kasan_param_ops = {
		.set = kasan_param_set,
};

module_param_cb(test, &kasan_param_ops, NULL, 0600);
#else
