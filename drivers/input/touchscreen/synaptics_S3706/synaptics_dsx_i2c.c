		...
		unsigned short addr)
{
	...
	memset(rmi4_data->data_buf, 0, PAGE_SELECT_LEN);
	
	msg[0].addr = hw_if.board_data->i2c_addr;
	msg[0].flags = 0;
	msg[0].len = PAGE_SELECT_LEN;
	msg[0].buf = rmi4_data->data_buf;

	page = ((addr >> 8) & MASK_8BIT);
	rmi4_data->data_buf[0] = MASK_8BIT;
	rmi4_data->data_buf[1] = page;

	if (page != rmi4_data->current_page) {
		for (retry = 0; retry < SYN_I2C_RETRY_TIMES; retry++) {
			mutex_lock(&(vivoTsGetVtsData()->lcmResumeAndProximityMutex));
			if (rmi4_data->has_power_on) {
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	if (atomic_read(&rmi4_data->st_enabled) == 1) {
		VTE("secure touch enable, deny i2c read");
		retval = -EPERM;
		goto exit;
	}
#endif
	...
	memset(rmi4_data->data_buf, 0, remaining_length + 1);

	msg[rd_msgs].addr = hw_if.board_data->i2c_addr;
	msg[rd_msgs].flags = I2C_M_RD;
	msg[rd_msgs].len = remaining_length;
	msg[rd_msgs].buf = &rmi4_data->data_buf[data_offset + 1];

	rmi4_data->data_buf[0] = addr & MASK_8BIT;
	...
	memcpy(data, &rmi4_data->data_buf[1], length);
	retval = length;
...
#if defined(CONFIG_SECURE_TOUCH)
	if (atomic_read(&rmi4_data->st_enabled) == 1) {
		VTE("secure touch enable, deny i2c write");
		retval = -EPERM;
		goto exit;
	}
#endif
