...
static int qg_debug_mask = QG_DEBUG_PON | QG_DEBUG_PROFILE | QG_DEBUG_SOC |
						QG_DEBUG_IRQ | QG_DEBUG_STATUS | QG_DEBUG_PM | QG_DEBUG_ESR;
...
static int qg_charge_full_update(struct qpnp_qg *chip)
{
	...
	if (chip->charge_done && !chip->charge_full) {
		if (chip->msoc >= 95 && health == POWER_SUPPLY_HEALTH_GOOD) {/*vivo modify*/
...
static void qg_status_change_work(struct work_struct *work)
{
	...
	/*vivo modify*/
	rc = power_supply_get_property(chip->batt_psy,
			POWER_SUPPLY_PROP_STATUS_FOR_QG, &prop);/*vivo end*/
	if (rc < 0)
...
static int qg_parse_dt(struct qpnp_qg *chip)
{
	...
	rc = of_property_read_u32(node, "vivo,full-soc-drop-pct", &chip->full_soc_drop_pct);
	if (rc < 0)
		chip->full_soc_drop_pct = 50;
	qg_dbg(chip, QG_DEBUG_PON, "full soc drop percentage = %d\n", chip->full_soc_drop_pct);
	/*vivo end*/
...
static void qg_monitor_work(struct work_struct *work)
{
	...
	//fuelsummary_collect_value(ID_BOTH__STEP_COUL, (cc_uah / 1000));
