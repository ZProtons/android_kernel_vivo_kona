...
static int dload_set(const char *val, const struct kernel_param *kp)
{
	...
	if (bsp_test_mode) {
		printk("bsp_test_mode force set download_mode to 1\n");
		download_mode = 1;
	}
...
static int msm_restart_probe(struct platform_device *pdev)
{
	...
	if (bsp_test_mode || is_debug_mode)
		download_mode = 1;
	printk(KERN_ERR "%s %d bsp_test_mode:%d is_debug_mode:%d download_mode:%d\n", __func__, __LINE__, 
				bsp_test_mode, is_debug_mode, download_mode);
