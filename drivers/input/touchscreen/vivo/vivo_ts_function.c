...
int vivoHighScanRateInit(void)
{
	vivoTsData->hight_scan_support = vivo_touchscreen_is_support(FS_TS_HIGH_SCAN_SUPPORT);
	vivoTsData->hight_scan_rate = vivo_touchscreen_is_support(FS_TS_HIGH_SCAN_RATE);

	return 0;
}

int vivoVtsScreenClockint(void)
{
	//struct VtsScreenClockPoint  *screenClockPoint;
	//struct VtsScreenClockRect   *screenClockRect;
	
	int retval =0;
	VTI("main probe init screenClockPoint enter ");
	vivoTsData->screenClockPoint = (struct VtsScreenClockPoint *)kzalloc(sizeof(struct VtsScreenClockPoint), GFP_KERNEL);
 
	if (!vivoTsData->screenClockPoint) {
		VTI("Failed to alloc mem for screenClockPoint");
		retval = -ENOMEM;
		goto exit;
	}

	vivoTsData->screenClockRect =  (struct VtsScreenClockRect *)kzalloc(sizeof(struct VtsScreenClockRect), GFP_KERNEL);
	if (!vivoTsData->screenClockRect) {
		VTI("Failed to alloc mem for screenClockRect");
		retval = -ENOMEM;
		goto freeClockPoint;
	}

	vivoTsData->screenClockPoint->pointX = 0;
	vivoTsData->screenClockPoint->pointY = 0;
	mutex_init(&(vivoTsData->screenClockPoint->scrclMutex));
	mutex_init(&(vivoTsData->screenClockRect->ClockRectMutex));
	vivoTsData->hasScreenClock = 0;//default  is 0
  
	vivoTsData->screenClockRect->startPoint.pointX = 0;
	vivoTsData->screenClockRect->startPoint.pointY = 0;
	vivoTsData->screenClockRect->width = 0; //vivoTsData->tsDimensionX;
	vivoTsData->screenClockRect->height = 0; //vivoTsData->tsDimensionY;
	vivoTsData->screenClockRect->enable= 0;    //enable default 
	vivoTsData->supportsCreenclock = vivo_touchscreen_is_support(FS_TS_SCREEN_CLOCK);
	VTI("main probe init screenClockPoint exit ");
	return retval;
 
freeClockPoint:	
	 kfree(vivoTsData->screenClockPoint);
	 vivoTsData->screenClockPoint = NULL;
exit:
	VTI("main probe init screenClockPoint exit ");
	return retval;
...
void vivoVtsScreenClockfree(void)
{
	//struct VtsScreenClockPoint  *screenClockPoint;
	//struct VtsScreenClockRect   *screenClockRect;
	...
	if(vivoTsData->screenClockPoint) {
 	...
 	if(vivoTsData->screenClockRect) {
 		kfree(vivoTsData->screenClockRect);
		vivoTsData->screenClockRect = NULL;
 	}
}

void vivoScreenPointSync(int x,int y)
{
	VTI("enter vivoScreenPointSync");
	mutex_lock(&vivoTsData->screenClockPoint->scrclMutex);
	vivoTsData->screenClockPoint->pointX = x;
	vivoTsData->screenClockPoint->pointY = y;
	//vivoTsData->pointX = x;
	//vivoTsData->pointY = y;

	mutex_unlock(&vivoTsData->screenClockPoint->scrclMutex);
	VTI("exit vivoScreenPointSync");
}
...
static char* GetClockEdgeCmd(struct vts_screen_clock_cmd *buf, char *cmd) {
...
int VivoGetScreenCmdType(char *app_name) 
{
	struct vts_screen_clock_cmd clockCmd;
	int ret = 0;
	char *tmp = (char *)app_name;
	char *tag;

	VTI("app_name is :%s\n", app_name);
	if(!vivoTsData->supportsCreenclock) {
    	VTI("vts do not support screen clock!!");
		return ret;
	}
	...
	if(strstr(tmp, "vts_clock_area_cmd")) {
		...
		if (IS_ERR(GetClockEdgeCmd(&clockCmd, tmp))) {
			VTE("get clock edge cmd error\n");
			return -EINVAL;
		}
   
		VTI("the  clock_edge_cmd is x:%d, y:%d, width:%d, height:%d\n", clockCmd.x, clockCmd.y, clockCmd.width, clockCmd.height);
		
		mutex_lock(&vivoTsData->screenClockRect->ClockRectMutex);
	    vivoTsData->screenClockRect->startPoint.pointX = clockCmd.x;
	    vivoTsData->screenClockRect->startPoint.pointY = clockCmd.y;
	    vivoTsData->screenClockRect->width = clockCmd.width;
	    vivoTsData->screenClockRect->height = clockCmd.height;
		mutex_unlock(&vivoTsData->screenClockRect->ClockRectMutex);

		if (vivoTsGetState() == TOUCHSCREEN_SLEEP) {
			VTI("in SLEEP mode, could not write app name");
			return -EINVAL;
		}
		
		if (vivoTsData->screenClockRect->enable) {
			return VTS_SCREEN_SET_EDGE_AND_DCLICK_AREA;
		}
		else {
			//VTI(" screen clock report abs not enable, no need to send to IC");
			return VTS_SCREEN_SET_DCLICK_AREA;
		}
	
    }
	
    if (strstr(tmp, "SCREEN_CLOCK_SWITCH_OFF")) {
		vivoTsData->hasScreenClock = 0;
		VTI("screen clock gesture off\n");
		return VTS_SCREEN_SET_NO_CMD;
    }
   
    if (strstr(tmp, "SCREEN_CLOCK_SWITCH_ON")) {
		vivoTsData->hasScreenClock = 1;
	    VTI("screen clock gesture on\n");
	    return VTS_SCREEN_SET_NO_CMD;
    }
   
    if (strstr(tmp, "SCREEN_CLOCK_REPORT_SWITCH_ON")) {
  		vivoTsData->screenClockRect->enable= 1;
		return VTS_SCREEN_SET_REPORT;
    }
  
    if (strstr(tmp, "SCREEN_CLOCK_REPORT_SWITCH_OFF")) {
  		vivoTsData->screenClockRect->enable= 0;
		return VTS_SCREEN_SET_REPORT;
    }
...
*/
		...
		if(KEY_SCREEN_CLOCK_WAKE_UP == touchIdOrGestureEvent)
			VTI("ic report gesture:KEY_SCREEN_CLOCK_WAKE_UP");
	...
	if (event == vivoTsData->suspendEventBlank) {	/* wake_p */
		...
		if (*blank == FB_BLANK_POWERDOWN && fb_suspend_flag == 0) {
			...
			if (vivoTsData->icNum == VTS_IC_TD4322 || vivoTsData->icNum == VTS_IC_TD3320 || 
				...
				vivoTsData->icNum == VTS_IC_NT36525 || vivoTsData->icNum == VTS_IC_TD4330 || 
				vivoTsData->icNum == VTS_IC_FT8719 || vivoTsData->icNum == VTS_IC_SEC_TDDI
				|| vivoTsData->icNum == VTS_IC_NT36670) {
				VTI("wait tp suspend done");
				ret = wait_event_interruptible_timeout(vivoTsData->lcmTpSyncWaitQueue, vivoTsData->lcmTpSyncFlag != VTS_RESUME_WAIT, 2000);
				if (ret == 0) {
					VTI("wait timeout");
...
static int vivoTsInputDeviceInit(struct i2c_client *client, void *tmpP, int tmp)
{
	...
	input_set_capability(inputDev, EV_KEY, KEY_SCREEN_CLOCK_WAKE_UP);
...
static int gestureAndSleepModeExchange(int needChangeToGestureMode)
{
	...
	if (needChangeToGestureMode) {
		...
		/****sleep to gesture mode need check screen clock cmd******/
		if (vivoTsData->screenClockRect && vivoTsData->supportsCreenclock) {
			if (vivoTsData->screenClockRect->enable) {
				vivoTsData->screenClockRect->enable = 0;
				ret = vivoTsSendReportAbs();
				VTI("vivoTs clear report abs cmd !!!!!!!!!!!!!!!!!!!!!!!!!");
				if (-FUN_NOT_REG == ret) {
					VTI("vivoTsSendReportAbs no define.");
				} else if (ret < 0) {
					VTI("vivoTsSendReportAbs fail.");
				}
			}
		}
 ...
 */
unsigned int allGestureSwitchIntoBitmap(void)
{
...
static int vivoTsKthreadResumeSupend(void *vivoTsData, int which)
{
	...
	if (which) {	/* resume */
		...
		/*******clear screen clock zone  cmd in normal *********/
		if (vtsData->screenClockRect && vtsData->supportsCreenclock) //NOT NULL
			ret = vivoClearScreenClockCmd();
...
int vivoTsInit(struct i2c_client *client, void *tmpP, int tmp)
{
	...
	vivoVtsScreenClockint();
	/* ts info init */
	
	vivoHighScanRateInit();
