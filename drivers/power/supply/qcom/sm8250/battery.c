...
static bool is_main_available(struct pl_data *chip)
{
	...
	if (chip->main_psy) {
		rc = power_supply_get_property(chip->main_psy,
				       POWER_SUPPLY_PROP_ALTERNATE_CHARGER, &pval);
		if (rc < 0) {
			pr_err("Couldn't get main charge : alternate_charger_enable rc=%d\n", rc);
		} else {
			chip->alternate_charger_enable = !!pval.intval;
			pr_err("alternate_charger_enable = %d\n", chip->alternate_charger_enable);
		}
	}
	/*vivo add end*/
			...
			int icl_ua, const char *client)
{
	...
	if (!chip->alternate_charger_enable) {	//vivo add
...
int qcom_batt_init(struct charger_param *chg_param)
{
	...
	chip->alternate_charger_enable = false;	/*vivo add*/
