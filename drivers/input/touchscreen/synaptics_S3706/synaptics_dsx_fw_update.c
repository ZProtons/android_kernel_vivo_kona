...
#endif
*/
/*MAX_WRITE_SIZE must define in s3706 chip, else the byte size(64bytes) will will >= i2c transfer max size(64bytes), cause fw update fail.*/
...
#define MAX_OEM_DATA_SIZE 256  /*avoid memory leak */
...
static ssize_t fwu_sysfs_oem_data_show(struct device *dev,
		struct device_attribute *attr, char *buf);

static ssize_t fwu_sysfs_write_oem_data_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count);
...
static struct device_attribute attrs[] = {
	...
	__ATTR(oemdata, (S_IWUSR | S_IWGRP | S_IRUGO),
			fwu_sysfs_oem_data_show,
			fwu_sysfs_write_oem_data_store),
};
...
static int fwu_f51_force_data_init(void)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(offset));
	if (retval < 0) {
		VTE("Failed to read force data offset");
		return retval;
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(query_count));
	if (retval < 0) {
		VTE("Failed to read number of F51 query registers");
		return retval;
	}
	...
	if (query_count >= 10) {
		retval = synaptics_rmi4_reg_read(rmi4_data,
				...
				sizeof(packet_info));
		if (retval < 0) {
			VTE("Failed to read F51 packet register info");
			return retval;
		}
	...
	if (fwu->cal_data_size > fwu->cal_data_buf_size) {
		...
		if (!fwu->cal_data) {
			VTE("Failed to alloc mem for fwu->cal_data");
...
static int fwu_allocate_read_config_buf(unsigned int count)
{
	if (count > fwu->read_config_buf_size) {
		...
		if (!fwu->read_config_buf) {
			VTE("Failed to alloc mem for fwu->read_config_buf");
		...
		struct block_count *blkcount, struct physical_address *phyaddr)
{
	...
	for (ii = 0; ii < fwu->partitions; ii++) {
		...
		VTD("Partition entry %d", ii);
		for (offset = 0; offset < 8; offset++) {
			VTD("0x%02x", partition_table[index + offset]);
		}
		switch (ptable->partition_id) {
		case CORE_CODE_PARTITION:
			blkcount->ui_firmware = partition_length;
			phyaddr->ui_firmware = physical_address;
			VTD("Core code block count: %d", blkcount->ui_firmware);
			blkcount->total_count += partition_length;
			break;
		case CORE_CONFIG_PARTITION:
			blkcount->ui_config = partition_length;
			phyaddr->ui_config = physical_address;
			VTD("Core config block count: %d", blkcount->ui_config);
			blkcount->total_count += partition_length;
			break;
		case BOOTLOADER_PARTITION:
			blkcount->bl_image = partition_length;
			phyaddr->bl_image = physical_address;
			VTD("Bootloader block count: %d", blkcount->bl_image);
			blkcount->total_count += partition_length;
			break;
		case UTILITY_PARAMETER_PARTITION:
			blkcount->utility_param = partition_length;
			phyaddr->utility_param = physical_address;
			VTD("Utility parameter block count: %d", blkcount->utility_param);
			blkcount->total_count += partition_length;
			break;
		case DISPLAY_CONFIG_PARTITION:
			blkcount->dp_config = partition_length;
			phyaddr->dp_config = physical_address;
			VTD("Display config block count: %d", blkcount->dp_config);
			blkcount->total_count += partition_length;
			break;
		case FLASH_CONFIG_PARTITION:
			blkcount->fl_config = partition_length;
			phyaddr->fl_config = physical_address;
			VTD("Flash config block count: %d", blkcount->fl_config);
			blkcount->total_count += partition_length;
			break;
		case GUEST_CODE_PARTITION:
			blkcount->guest_code = partition_length;
			phyaddr->guest_code = physical_address;
			VTD("Guest code block count: %d", blkcount->guest_code);
			blkcount->total_count += partition_length;
			break;
		case GUEST_SERIALIZATION_PARTITION:
			blkcount->pm_config = partition_length;
			phyaddr->pm_config = physical_address;
			VTD("Guest serialization block count: %d", blkcount->pm_config);
			blkcount->total_count += partition_length;
			break;
		case GLOBAL_PARAMETERS_PARTITION:
			blkcount->bl_config = partition_length;
			phyaddr->bl_config = physical_address;
			VTD("Global parameters block count: %d", blkcount->bl_config);
			blkcount->total_count += partition_length;
			break;
		case DEVICE_CONFIG_PARTITION:
			blkcount->lockdown = partition_length;
			phyaddr->lockdown = physical_address;
			VTD("Device config block count: %d", blkcount->lockdown);
			blkcount->total_count += partition_length;
			break;
		};
	}
...
static void fwu_parse_image_header_05_06(void)
{
	...
	if (fwu->img.contains_disp_config) {
		...
		retval = secure_memcpy(fwu->img.cstmr_product_id,
				...
				PRODUCT_ID_SIZE);
		if (retval < 0) {
			VTE("Failed to copy custom product ID string");
		}
	...
	retval = secure_memcpy(fwu->img.product_id,
			...
			PRODUCT_ID_SIZE);
	if (retval < 0) {
		VTE("Failed to copy product ID string");
	}
...
static int fwu_parse_image_info(void)
{
	...
	case IMAGE_HEADER_VERSION_06:
		...
		break;
	default:
		VTE("Unsupported image file format (0x%02x)", header->major_header_version);
		return -EINVAL;
	}

	if (fwu->bl_version == BL_V7 || fwu->bl_version == BL_V8) {
		if (!fwu->img.contains_flash_config) {
			VTE("No flash config found in firmware image");
			return -EINVAL;
		}
...
static int fwu_read_flash_status(void)
{
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(status));
	if (retval < 0) {
		VTE("Failed to read flash status");
		return retval;
	}
	...
	if (fwu->flash_status != 0x00) {
		VTE("Flash status = %d, command = 0x%02x", fwu->flash_status, fwu->command);
	}
	...
	retval = synaptics_rmi4_reg_read(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		VTE("Failed to read flash command");
		return retval;
	}
...
static int fwu_wait_for_idle(int timeout_ms, bool poll)
{
	...
	VTI("Timed out waiting for idle status");
...
static int fwu_write_f34_v7_command_single_transaction(unsigned char cmd)
{
	...
	if (retval < 0) {
		VTE("Failed to write single transaction command");
		return retval;
	}
...
static int fwu_write_f34_v7_command(unsigned char cmd)
{
	...
	case CMD_ENABLE_FLASH_PROG:
		...
		break;
	default:
		VTE("Invalid command 0x%02x", cmd);
		return -EINVAL;
	};
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		VTE("Failed to write flash command");
		return retval;
	}
...
static int fwu_write_f34_v5v6_command(unsigned char cmd)
{
	...
	case CMD_ENABLE_FLASH_PROG:
		...
		if (retval < 0) {
			VTE("Failed to write bootloader ID");
			return retval;
		}
		break;
	default:
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(command));
	if (retval < 0) {
		VTE("Failed to write command 0x%02x", command);
		return retval;
	}
...
static int fwu_write_f34_v7_partition_id(unsigned char cmd)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(partition));
	if (retval < 0) {
		VTE("Failed to write partition ID");
		return retval;
	}
...
static int fwu_read_f34_v7_partition_table(unsigned char *partition_table)
{
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(block_number));
	if (retval < 0) {
		VTE("Failed to write block number");
		return retval;
	}
	...
	retval = synaptics_rmi4_reg_write(rmi4_data,
			...
			sizeof(length));
	if (retval < 0) {
		VTE("Failed to write transfer length");
		return retval;
	}

	retval = fwu_write_f34_command(CMD_READ_CONFIG);
	if (retval < 0) {
		VTE("Failed to write command");
		return retval;
	}

	retval = fwu_wait_for_idle(WRITE_WAIT_MS, true);
	if (retval < 0) {
		VTE("Failed to wait for idle status");
		return retval;
	}
...
static int fwu_read_f34_v7_queries(void)
{
	...
	if (retval < 0) {
		VTE("Failed to read queries 1 to 7");
		return retval;
	}
	...
	for (offset = 0; offset < V7_PARTITION_SUPPORT_BYTES; offset++) {
		...
		VTD("Supported partitions: 0x%02x", query_1_7.data[index + offset]);
	}
		...
		unsigned short block_cnt, unsigned char command)
{
	...
	do {
		...
		if (retval < 0) {
			VTI("Failed to wait for idle status (remaining = %d)", remaining);
			return retval;
		}
...
int s3706_bbk_get_header_file_fworconfig_version(int which, unsigned char *fw)
{
	int ret = 0;
	if (which == FW_VERSION) {
	...
	if (which == CONFIG_VERSION) {
		fwu->image = fw;
		ret = fwu_parse_image_info();
		if (ret < 0) {
			VTI("fwu_parse_image_info fail");
			goto exit;
		}

		if (fwu->img.contains_firmware_id) {
			return fwu->img.firmware_id;
		}
...
static enum flash_area fwu_go_nogo(void)
{
	...
	VTI("Device firmware ID = %d", device_fw_id);
	...
	VTI("Image firmware ID = %d", image_fw_id);

	if (image_fw_id != device_fw_id) {
		flash_area = UI_FIRMWARE;
		goto exit;
	} else {
		VTI("Image firmware ID(0x%x) == device firmware ID(0x%x)", image_fw_id, device_fw_id);
/*		flash_area = NONE; */
/*		goto exit; */
	}
	...
	if (retval < 0) {
		VTI("Failed to read device config ID");
		flash_area = NONE;
		goto exit;
	}
	...
	for (ii = 0; ii < config_id_size; ii++) {
		VTI("img.ui_config.data[%d] = %d fwu->config_id[%d] = %d", ii, fwu->img.ui_config.data[ii], ii, fwu->config_id[ii]);
		if (fwu->img.ui_config.data[ii] != fwu->config_id[ii]) {
			VTI("config id not same, update fw");
			flash_area = UI_CONFIG;
			goto exit;
		}
	}

	VTI("Image config ID == device config ID");
	flash_area = NONE;
...
static int fwu_do_reflash(void)
{
	...
	if (retval < 0)
		return retval;
	VTI("Firmware programmed");

	fwu->config_area = UI_CONFIG_AREA;
	retval = fwu_write_ui_configuration();
	if (retval < 0)
		return retval;
	VTI("Configuration programmed");
	...
	if (fwu->flash_properties.has_disp_config &&
		...
		if (retval < 0)
			return retval;
		VTI("Display configuration programmed");
	}
...
static char imei_buf[16] = {0};
static int fwu_do_get_oem_data(void)
{
	int retval = 0;
	int block_count = 0;
	int i = 0, j = 0, tag_count = 0, valid_offset = 0;
	struct synaptics_rmi4_data *rmi4_data;
	if (!fwu) {
		VTE("fwu is NULL");
		return -ENOMEM;
	...
	VTI("Start of oem data aquirement process");

		retval = fwu_read_flash_status();
		if (retval < 0) {
			VTE("fwu_read_flash_status fail");
			goto exit;
		}

			retval = fwu_enter_flash_prog();
			if (retval < 0) {
				VTE("fwu_enter_flash_prog fail");
				goto exit;
			}
			...
			block_count = fwu->blkcount.pm_config;
			fwu->config_size = fwu->block_size * block_count;
			VTI("permanent config size = %d", fwu->config_size);

			retval = fwu_allocate_read_config_buf(fwu->config_size);
			if (retval < 0) {
				VTE("fwu_allocate_read_config_buf fail");
				rmi4_data->reset_device(rmi4_data, false);
				goto exit;
			}
			...
			retval = fwu_read_f34_blocks(block_count,
					CMD_READ_CONFIG);
			if (retval < 0) {
				VTE("fwu_read_f34_blocks fail");
				rmi4_data->reset_device(rmi4_data, false);
				goto exit;
			}

	VTI("config show:");
	for (i = 0; i < fwu->config_size; i++) {
		if (i%0x10 == 0) {
			printk("\n");
			printk("VIVO_TS:");
		}
		printk("%d ", fwu->read_config_buf[i]);
	}
	rmi4_data->reset_device(rmi4_data, false);

	/*check how many times has been write*/
	for (i = 0; i < fwu->config_size/0x10; i++) {
		for (j = 0; j < 0x10; j++) {
			if (fwu->read_config_buf[i*0x10+j] != 0) {
				continue;
			} else {
				tag_count++;
				if (tag_count > 5) {
					valid_offset = i * 0x10;
					VTI("valid_offset:0x%x has been used times:%d", valid_offset, i);
					break;
				}
			}
		}
	...
	if (valid_offset == 0) {
		VTI("no valid imei has been write to chip.");
	} else {
		if (valid_offset >= 0x1E0) {
			valid_offset = 0x1E0;
			VTI("full flash memory");
		}
		memcpy(imei_buf, &(fwu->read_config_buf[valid_offset-0x10]), 15);
	}


exit:

	VTI("End of oem data acquirement process");
...
int fwu_do_set_oem_data(const char *oem_data, size_t count)
{
	int retval = 0;
	int block_count = 0;
	int i = 0, j = 0, tag_count = 0, valid_offset = 0;
	...
	VTI("Start of permanent config data store process");
	...
	block_count = fwu->blkcount.pm_config;
	fwu->config_size = fwu->block_size * block_count;
	VTI("permanent config size = %d, fwu->block_size = %d, block_count = %d", fwu->config_size, fwu->block_size, block_count);
	...
	if (valid_offset > 0) {
		if (valid_offset >= 0x1E0) {	/*30 times have been writed, could not write*/
			retval = -10;
			VTI("no more time to wirte.");
			rmi4_data->reset_device(rmi4_data, false);
			goto exit;
		}
		if (!strncmp(oem_data, &(fwu->read_config_buf[valid_offset-0x10]), 15)) {
			VTI("write the same imei, no need to write");
			rmi4_data->reset_device(rmi4_data, false);
			goto exit;
		}
	}

	retval = secure_memcpy(&(fwu->read_config_buf[valid_offset]), fwu->config_size,
			oem_data,
			fwu->config_size, 15);
	if (retval < 0) {
		VTE("Failed to save permanent config data");
		rmi4_data->reset_device(rmi4_data, false);
		goto exit;
	}
	...
	if (retval < 0) {
		VTE("Failed to write permanent config data");
		rmi4_data->reset_device(rmi4_data, false);
		goto exit;
...
exit:
	...
	VTI("End of permanent config data store process");
...
static int fwu_start_reflash(void)
{
	...
	if (fwu->image == NULL) {
		...
		if (retval < 0) {
			VTI("Failed to copy image file name");
			goto exit;
		}
		VTI("Requesting firmware image %s", fwu->image_name);

		retval = request_firmware(&fw_entry, fwu->image_name,
				rmi4_data->pdev->dev.parent);
		if (retval != 0) {
			VTI("Firmware image %s not available", fwu->image_name);
			retval = -EINVAL;
			goto exit;
		}

		VTI("Firmware image size = %d", (unsigned int)fw_entry->size);

		fwu->image = fw_entry->data;
	}

	VTI("fwu_parse_image_info");
	retval = fwu_parse_image_info();
	if (retval < 0) {
		VTI("fwu_parse_image_info fail");
		goto exit;
	...
	if (fwu->blkcount.total_count != fwu->img.blkcount.total_count) {
		VTI("Flash size mismatch");
		retval = -EINVAL;
		goto exit;
	}

	if (fwu->bl_version != fwu->img.bl_version) {
		VTI("Bootloader version mismatch");
		retval = -EINVAL;
		goto exit;
	}

	VTI("fwu_read_flash_status");
	retval = fwu_read_flash_status();
	if (retval < 0)
		goto exit;
	...
	if (fwu->in_bl_mode) {
		...
		VTI("Device in bootloader mode");
	} else {
		fwu->bl_mode_device = false;
	}

	VTI("check firmware version");
	...
	if (flash_area != NONE && !fwu->bl_mode_device) {
		...
		if (retval < 0) {
			VTI("Failed to save calibration data");
			rmi4_data->reset_device(rmi4_data, false);
			goto exit;
		}
	}
	...
	VTI("flash_area = %d", flash_area);
	switch (flash_area) {
	case UI_FIRMWARE:
		VTI("flash_area is UI_FIRMWARE");
		do_rebuild = true;
		...
		if (fwu->bl_mode_device || fwu->in_bl_mode) {
			VTI("Device in bootloader mode, skipping calibration data restoration");
			break;
		}
...
int s3706_bbk_syna_get_config_id(void)
{
	int ret = 0;
	int ver = 0;
	...
	if (ret < 0)
		return ret;
	VTI("0x%x 0x%x 0x%x 0x%x ", fwu->config_id[0], fwu->config_id[1], fwu->config_id[2], fwu->config_id[3]);
	ver = (int)fwu->config_id[3];
/*	return *((unsigned int *)fwu->config_id); */
	return ver;
}
...
int s3706_synaptics_incell_fw_updater(const unsigned char *fw_data)
{
	int retval;
	VTI("enter");
	...
	if (!fwu->initialized) {
		VTI("fwu->initialized is 0");
		return -ENODEV;
	}

	if (fwu->in_ub_mode) {
		VTI("in_ub_mode");
		fwu->image = vivoTsGetFw(VTS_FW_TYPE_IHEX, &(fwu->image_size));
		if (fwu->image == NULL) {
			VTI("vivoTsGetFw fail");
			return MINUS_ONE;
		}
...
EXPORT_SYMBOL(s3706_synaptics_incell_fw_updater);
...
static void fwu_startup_fw_update_work(struct work_struct *work)
{
	...
	/*chenpeng add, ensure main probe complete, because fwu dependence interrupt*/
	while (vivoTsGetVtsData()->isProbeComplete == 0) {
		VTI("core probe not complete, wait...");
		msleep(10);
	}

	VTI("auto update fw start");
	/*must set this flag, avoid usb charger to write during fwu*/
	vivoTsGetVtsData()->fwUpdatingFlag = 1;
	ret = s3706_synaptics_incell_fw_updater(vivoTsGetFw(VTS_FW_TYPE_FW, &fw_size));
	if (ret < 0) {
		VTI("update fw fail");
	}
	vivoTsGetVtsData()->fwUpdatingFlag = 0;

#ifdef TOUCH_DATA_COLLECT
	vivoTsGetVtsData()->fw_version = (u64)s3706_bbk_syna_get_config_id();
	lcm_id = mdss_report_lcm_id();
	get_touch_info(lcm_id, vivoTsGetVtsData()->fw_version);
#endif
	VTI("auto update fw end");
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	retval = s3706_synaptics_incell_fw_updater(fwu->image);
	if (retval < 0) {
		dev_err(rmi4_data->pdev->dev.parent,
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (count > MAX_OEM_DATA_SIZE) {
	dev_err(rmi4_data->pdev->dev.parent,
			"%s: Input data over size\n",
			__func__);
	return -ENODEV;
	...
	if (count < 15) {
		VTI("str length to short");
		retval = -EINVAL;
		goto exit;
	}

	fwu_do_set_oem_data(buf, 15);
...
static int synaptics_rmi4_fwu_init(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (fwu) {
		VTI("Handle already exists");
		return 0;
	}

	fwu = kzalloc(sizeof(*fwu), GFP_KERNEL);
	if (!fwu) {
		VTI("Failed to alloc mem for fwu");
		retval = -ENOMEM;
		goto exit;
	}
	...
	if (!fwu->image_name) {
		VTI("Failed to alloc mem for image name");
		retval = -ENOMEM;
		goto exit_free_fwu;
	}
	...
	if (retval < 0) {
		VTI("Failed to read PDT properties, assuming 0x00");
	...
	retval = sysfs_create_bin_file(&rmi4_data->input_dev->dev.kobj,
			&dev_attr_data);
	if (retval < 0) {
		VTI("Failed to create sysfs bin file");
		goto exit_free_mem;
	}
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(attrs); attr_count++) {
		...
		if (retval < 0) {
			VTI("Failed to create sysfs attributes");
			retval = -ENODEV;
...
int s3706_bbk_syna_fwu_interface(const struct firmware *firmware)
{
	/*get firmware info*/
	/*size_t fw_size = firmware->size; */
	/*unsigned char *fw_data = (unsigned char *)firmware->data; */
	int ret = 0;

	fwu->image = (const unsigned char *)firmware->data;
	fwu->image_size = (unsigned int)firmware->size;
	...
	if (ret < 0) {
		VTI("updata fw fail");
			...
			collectTouchException(1, 2);
#endif

	} else {
		VTI("updata fw success");
		ret = 0;
	}
	return ret;
}
...
{
	fwu_do_get_oem_data();
	return snprintf(imei, 16, "%s\n", imei_buf); 	/*nprontf*/
}
int s3706_bbk_syna_imei_write(unsigned char *imei)
{
	int ret = 0;
	ret = fwu_do_set_oem_data(imei, 15);
...
{
	VTI("enter");
	synaptics_3706_rmi4_new_function(&fwu_module, true);
...
{
	synaptics_3706_rmi4_new_function(&fwu_module, false);
