...
int msm_get_i2c_gpio_value(struct i2c_adapter *adap)
{
	...
	if (strcmp(dev_name(gi2c->dev), "a8c000.i2c")) {
...
int msm_i2c_gpio_pull_down(struct i2c_adapter *adap)
{
...
#else
	struct geni_i2c_dev *gi2c = i2c_get_adapdata(adap);
	int clk_gpio_value = -1;
	int data_gpio_value = -1;
	...
	pr_err("Bus busy cleared after 2.5s pull down, clk_gpio_value=%d, data_gpio_value=%d\n", clk_gpio_value, data_gpio_value);
	return 0;
#endif
}
