...
static int m1120_power_ctl(m1120_data_t *data, bool on)
{
	...
	mxerr(&data->client->dev, "m1120_power_ctl on %d enabled %d\n", on, data->power_enabled);
...
static const struct of_device_id m1120_of_match[] = {
	{ .compatible = "magnachip,sensor_mxm1120,up", },
	{ },
};
...
static int __init m1120_driver_init_up(void)
{
	printk(KERN_INFO "%s\n", __func__);
	if (is_at_boot_dl == 1) {
		printk(KERN_INFO "%s now is at mode, not load mxm1120_up driver\n", __func__);
		return 0;
	}
