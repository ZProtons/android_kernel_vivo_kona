			 ...
			 const struct input_value *vals, unsigned int count, ktime_t kt)
{
	...
	ktime_t ev_time[INPUT_CLK_MAX];

	ev_time[INPUT_CLK_MONO] = kt;
	ev_time[INPUT_CLK_REAL] = ktime_mono_to_real(ev_time[INPUT_CLK_MONO]);
	ev_time[INPUT_CLK_BOOT] = ktime_mono_to_any(ev_time[INPUT_CLK_MONO],
						 TK_OFFS_BOOT);
