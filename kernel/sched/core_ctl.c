...
struct cluster_data {
	...
	unsigned int offline_cpus;
	unsigned int affinity_ctl;
...
#ifdef CONFIG_UFS_IRQ_AFFINITY
static void do_affinity_ctl(struct cluster_data *cluster);
static void restore_cluster_affinity(struct cluster_data *cluster);

extern int get_ufshcd_irq_num(void);
extern int set_ufshcd_irq_affinity(int cpu);
#endif
...
#ifdef CONFIG_UFS_IRQ_AFFINITY
static ssize_t store_affinity_en(struct cluster_data *state,
				const char *buf, size_t count)
{
	unsigned int val;
	...
	if (state != &cluster_state[1])
		return -EINVAL;

	spin_lock(&state->affinity_lock);

	if (val == 0) {
		state->affinity_en = 0;
		restore_cluster_affinity(state);
	} else if (val == 1) {
		state->affinity_en = 1;
	}

	spin_unlock(&state->affinity_lock);
	return count;
}

static ssize_t show_affinity_en(const struct cluster_data *state, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", state->affinity_en);
}

static ssize_t store_affinity_ctl(struct cluster_data *state,
				const char *buf, size_t count)
{
	unsigned int val;
	struct cluster_data *cluster = &cluster_state[1];

	if (sscanf(buf, "%u\n", &val) != 1)
		return -EINVAL;

	state->affinity_ctl = val;

	if (val == 1)
		do_affinity_ctl(cluster);

	return count;
}

static ssize_t show_affinity_ctl(const struct cluster_data *state, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%u\n", state->affinity_ctl);
}
#endif
...
#ifdef CONFIG_UFS_IRQ_AFFINITY
core_ctl_attr_rw(affinity_en);
core_ctl_attr_rw(affinity_ctl);
#endif
...
#ifdef CONFIG_UFS_IRQ_AFFINITY
		if (cluster == &cluster_state[1]) {
			spin_lock(&cluster->affinity_lock);
			if (!cluster->offline_cpus && cluster->affinity_en) {
				if (need > 1) {
					if (cpumask_equal(
								irq_get_affinity_mask(get_ufshcd_irq_num()),
								cpumask_of(c->cpu))) {
						pr_debug("skip CPU%d\n", c->cpu);
						spin_unlock(&cluster->affinity_lock);
						continue;
					}
				}
			}
...
static void restore_cluster_affinity(struct cluster_data *cluster)
{
	if (cpumask_subset(
				irq_get_affinity_mask(get_ufshcd_irq_num()),
				&cluster->cpu_mask)) {
		if (set_ufshcd_irq_affinity(cpumask_first(&cluster->last_mask)))
			set_ufshcd_irq_affinity(0);
		set_ufshcd_irq_affinity(-1);
	}
}
...
static void do_affinity_ctl(struct cluster_data *cluster)
{
	...
	do {
		...
		pr_debug("is all cpu online? %s. (%d)\n",
			cpumask_subset(&cluster->cpu_mask, cpu_online_mask) ? "Yes" : "No",
			cluster->offline_cpus);
		...
		if (!cpumask_subset(
					irq_get_affinity_mask(get_ufshcd_irq_num()),
					&cluster->cpu_mask)) {
			pr_debug("Set irq affinity to big, last cpumask=[%*pbl]\n",
					cpumask_pr_args(irq_get_affinity_mask(
							get_ufshcd_irq_num())));

			cpumask_copy(&cluster->last_mask,
					irq_get_affinity_mask(get_ufshcd_irq_num()));

			if (!set_ufshcd_irq_affinity(
						cpumask_first(&cluster->cpu_mask)))
				schedule_delayed_work(&cluster->affinity_work,
						msecs_to_jiffies(IRQ_DURATION));
		}
	...
	if (adjustment_possible(cluster, need)) {
		...
		if (cluster->active_cpus > need) {
			...
			if (cluster == &cluster_state[1]) {
				...
				if (!cluster->offline_cpus && cluster->affinity_en) {
					if (need <= 1)
						restore_cluster_affinity(cluster);
				}
				spin_unlock(&cluster->affinity_lock);
			}
#endif
...
static int core_ctl_isolation_online_cpu(unsigned int cpu)
{
	...
	if (cluster == &cluster_state[1]) {
		...
		cluster->offline_cpus--;
		spin_unlock(&cluster->affinity_lock);
		WARN_ON_ONCE(cluster->offline_cpus > cluster->num_cpus);
	}
#endif
...
static int core_ctl_isolation_offline_cpu(unsigned int cpu)
{
#ifdef CONFIG_UFS_IRQ_AFFINITY
	struct cpu_data *state = &per_cpu(cpu_state, cpu);
	struct cluster_data *cluster = state->cluster;
	...
	if (cluster == &cluster_state[1]) {
		spin_lock(&cluster->affinity_lock);
		if (++cluster->offline_cpus == 1) {
			if (cluster->affinity_en)
				restore_cluster_affinity(cluster);
		}
		spin_unlock(&cluster->affinity_lock);
		WARN_ON_ONCE(cluster->offline_cpus > cluster->num_cpus);
...
#ifdef CONFIG_UFS_IRQ_AFFINITY
static void cluster_affinity_work(struct work_struct *work)
{
	struct cluster_data *cluster =
		container_of(work, struct cluster_data, affinity_work.work);

	if (unlikely(!cluster || !cluster->inited))
		return;

	if (cluster == &cluster_state[1]) {
		unsigned int cpu0freq, curfreq;

		cpu0freq = cpufreq_quick_get_max(0);
		curfreq = cpufreq_quick_get_min(cluster->first_cpu);

		spin_lock(&cluster->affinity_lock);

		pr_debug("offline_cpus=%d, affinity_en=%d, "
				"boost=%d, cpu0freq=%u, curfreq=%u, "
				"irqmask=[%*pbl]\n",
				cluster->offline_cpus,
				cluster->affinity_en,
				cluster->boost,
		...
		if (!cluster->offline_cpus && cluster->affinity_en) {
			if (curfreq > cpu0freq - 200000) {
				if (cpumask_subset(
					...
					pr_err("WARNING: offline_cpus=%d, affinity_en=%d, "
							"boost=%d, cpu0freq=%u, curfreq=%u, "
							"irqmask=[%*pbl]\n",
							cluster->offline_cpus,
							cluster->affinity_en,
...
#ifdef CONFIG_UFS_IRQ_AFFINITY
	cluster->affinity_en = (get_ufshcd_irq_num() > 0) ? 1 : 0;
	cluster->offline_cpus = 0;
	cpumask_copy(&cluster->last_mask, mask);
	spin_lock_init(&cluster->affinity_lock);
	INIT_DELAYED_WORK(&cluster->affinity_work, cluster_affinity_work);
#endif
...
static int __init core_ctl_init(void)
{
	...
	cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN,
			...
			core_ctl_isolation_online_cpu,
			core_ctl_isolation_offline_cpu);
