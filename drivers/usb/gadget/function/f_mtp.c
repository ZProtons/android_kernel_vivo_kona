...
void *mtp_pre_buf_rx[RX_REQ_MAX];
void *mtp_pre_buf_tx[MTP_TX_REQ_MAX];
...
static struct usb_request *mtp_request_new(struct usb_ep *ep, int buffer_size)
{
	...
	req->buf = kmalloc(buffer_size, GFP_KERNEL | __GFP_NOMONITOR);
#else
...
static struct usb_request *mtp_request_new_buf(struct usb_ep *ep, void *buffer)
{
	struct usb_request *req = usb_ep_alloc_request(ep, GFP_KERNEL);
...
static void mtp_request_free_buf(struct usb_request *req, struct usb_ep *ep)
{
	if (req)
		usb_ep_free_request(ep, req);
}
		...
		if (!mtp_pre_buf_tx[i]) {
			...
			if (!req) {
				...
				while ((req = mtp_req_get(dev, &dev->tx_idle))) {
					for (j = 0; j < MTP_TX_REQ_MAX; j++) {
						if (mtp_pre_buf_tx[j] == req->buf) {
							use_pre_buf_tx = true;
							break;
						}
					}
					...
					if (use_pre_buf_tx) {
						mtp_request_free_buf(req, dev->ep_in);
						use_pre_buf_tx = false;
					} else
			...
			req = mtp_request_new_buf(dev->ep_in, mtp_pre_buf_tx[i]);
			if (!req)
				goto fail;
		}
			...
			if (!req) {
				...
				for (--i; i >= 0; i--) {
					if (!mtp_pre_buf_rx[i])
						...
						mtp_request_free_buf(dev->rx_req[i], dev->ep_out);
				}
				mtp_rx_req_len = MTP_BULK_BUFFER_SIZE;
				goto retry_rx_alloc;
			}
		} else {
			req = mtp_request_new_buf(dev->ep_out, mtp_pre_buf_rx[i]);
			if (!req)
				goto fail;
		}
...
static int mtp_init(void)
{
	...
	for (i = 0; i < RX_REQ_MAX; i++) {
#ifdef CONFIG_BIG_ORDER_MONITOR
		mtp_pre_buf_rx[i] = kmalloc(MTP_RX_BUFFER_INIT_SIZE, GFP_KERNEL | __GFP_NOMONITOR);
#else
		mtp_pre_buf_rx[i] = kmalloc(MTP_RX_BUFFER_INIT_SIZE, GFP_KERNEL);
#endif
		if (!mtp_pre_buf_rx[i]) {
			pr_err("%s: %d: rx(%d) pre alloc fail\n", __func__, __LINE__, i);
			for (--i; i >= 0; i--) {
				mtp_pre_buf_rx[i] = NULL;
				kfree(mtp_pre_buf_rx[i]);
			}
			break;
		} else
			pr_info("%s: %d: rx(%d) pre alloc success\n", __func__, __LINE__, i);
	}

	for (i = 0; i < MTP_TX_REQ_MAX; i++) {
#ifdef CONFIG_BIG_ORDER_MONITOR
		mtp_pre_buf_tx[i] = kmalloc(MTP_TX_BUFFER_INIT_SIZE, GFP_KERNEL | __GFP_NOMONITOR);
#else
		mtp_pre_buf_tx[i] = kmalloc(MTP_TX_BUFFER_INIT_SIZE, GFP_KERNEL);
#endif
		if (!mtp_pre_buf_tx[i]) {
			pr_err("%s: %d: tx(%d) pre alloc fail\n", __func__, __LINE__, i);
			for (--i; i >= 0; i--) {
				mtp_pre_buf_tx[i] = NULL;
				kfree(mtp_pre_buf_tx[i]);
			}
			break;
		} else
			pr_info("%s: %d: tx(%d) pre alloc success\n", __func__, __LINE__, i);
	}
