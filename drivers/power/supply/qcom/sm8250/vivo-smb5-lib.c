...
#include <linux/timer.h>	//vivo add
#include <linux/sched/clock.h> //vivo add
#include <linux/soc/qcom/fsa4480-i2c.h>	//vivo add
...
extern int fg_soc_really;

//vivo comment:  copy from vivo-qpnp-smb5.c
struct smb_dt_props {
	int			usb_icl_ua;
...
static const struct apsd_result *smblib_get_apsd_result(struct smb_charger *chg)
{
	...
	static const struct apsd_result *last_result = &smblib_apsd_results[UNKNOWN];

	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		/*vivo add: get alternate charger BC1.2 result to PMI*/
		rc = power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_REAL_TYPE, &val);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get real_type prop from alternate_charger_psy rc=%d\n", rc);
			return result;
		}

		for (i = 0; i < ARRAY_SIZE(smblib_apsd_results); i++) {
			if (smblib_apsd_results[i].pst == val.intval) {
				result = &smblib_apsd_results[i];
				break;
			}
		}
		if (chg->usb_present && val.intval == POWER_SUPPLY_TYPE_USB_OCP) {
			result = &smblib_apsd_results[OCP];
		}

		#if 0
		/*below func maybe trigger CPU usb enum. this action will influence the D+ voltage */
		if (chg->usb_present && result->pst == POWER_SUPPLY_TYPE_UNKNOWN) {
			result = &smblib_apsd_results[FLOAT];
			smblib_err(chg, "alternate_charger can not detect BC1.2 result normally when usb_present, force cable type to Float!!\n");
		}
		#endif
	} else {
	...
	if (last_result != result)
		smblib_dbg(chg, PR_REGISTER, "detect charger type = %s(%d), alternate_charger_enable=%d\n", result->name, result->pst, chg->alternate_charger_enable);
...
//vivo add
void smblib_set_alternate_charger_suspend(struct smb_charger *chg, bool suspend)
{
	int rc = 0;
	union power_supply_propval pval = {0,};
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		pval.intval = suspend;
		rc = power_supply_set_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_INPUT_SUSPEND, &pval);
		smblib_dbg(chg, PR_INTERRUPT, "suspend=%d, alternate_charger_enable=%d. rc=%d\n", suspend, chg->alternate_charger_enable, rc);
	}
}
...
int smblib_set_alternate_charger_icl(struct smb_charger *chg, int icl_ma)
{
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		smblib_dbg(chg, PR_MISC, "vote_client=%s, typec_mode=%d, icl_ma=%d\n",
				get_effective_client_locked(chg->usb_icl_votable), chg->typec_mode, icl_ma);

		if (icl_ma <= 0) {
			smblib_dbg(chg, PR_INTERRUPT, "icl_ma=%d, force suspend !!!\n", icl_ma);
			smblib_set_alternate_charger_suspend(chg, true);
			return 1;
		}
		pval.intval = icl_ma;
		rc = power_supply_set_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_CURRENT_MAX, &pval);
		smblib_dbg(chg, PR_INTERRUPT, "icl_ma=%d, rc=%d\n", icl_ma, rc);

		smblib_set_alternate_charger_suspend(chg, false);
...
int smblib_set_alternate_charger_fcc(struct smb_charger *chg, int fcc_ma)
{
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		smblib_dbg(chg, PR_MISC, "vote_client=%s, typec_mode=%d, fcc_ma=%d\n",
				get_effective_client_locked(chg->fcc_votable), chg->typec_mode, fcc_ma);
		pval.intval = fcc_ma;
		rc = power_supply_set_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX, &pval);
		smblib_dbg(chg, PR_INTERRUPT, "fcc_ma=%d, rc=%d\n", fcc_ma, rc);
...
int smblib_set_usb_suspend(struct smb_charger *chg, bool suspend)
{
	int rc = 0;
	/* vivo add start*/
	if (chg->alternate_charger_enable) {
		suspend = true;	//force PMI suspend
	}
	smblib_dbg(chg, PR_INTERRUPT, "suspend=%d, alternate_charger_enable=%d.\n", suspend, chg->alternate_charger_enable);
	/* vivo add end */
	if (suspend)
					...
					int min_allowed_uv, int max_allowed_uv)
{
	...
	//vivo_chg add log
	smblib_dbg(chg, PR_MISC, "typec log: pd_active=%d, vbus_allowance=%d, min_allowed_uv=%d, max_allowed_uv=%d.\n", chg->pd_active, vbus_allowance, min_allowed_uv, max_allowed_uv);
	...
	//vivo add start
	if (aicl_threshold < chg->default_aicl_cont_threshold_mv)
		aicl_threshold = chg->default_aicl_cont_threshold_mv;
	//vivo add end
...
int smblib_set_prop_to_qcom_fg(struct smb_charger *chg,
				enum power_supply_property psp,
				union power_supply_propval *val)
{
	int rc;
	...
	rc = power_supply_set_property(qc_bms_psy, psp, val);
	if (rc < 0) {
		smblib_err(chg, "Couldn't set qcom bms psp %d,rc=%d\n", psp, rc);
		return -EINVAL;
	}
	return rc;
}
					...
					union power_supply_propval *val)
{
	...
	#else
	val->intval = 3900;//DEFAULT_BATT_CAPACITY_MAH;
	#endif
	return rc;
}
					...
					union power_supply_propval *val)
{
	...
	#else
	if(chg->non_use_smb_therm_adc){
		smb_temp = DEFAULT_BOARD_TEMP;
	}else{
		...
		union power_supply_propval *val)
{
	...
	if(chg->enable_usb_conn_temp_from_bq25970){
		...
		union power_supply_propval *val)
{
	...
	if(chg->bms_psy && !strcmp(chg->bms_psy->desc->name, "bq_bms")) {
		if (is_atboot) {
			val->intval = DEFAULT_BOARD_TEMP;
			return 0;
		}
		...
		if (rc < 0) {
			...
			val->intval = DEFAULT_BOARD_TEMP;
			bat_board_temp = val->intval;
		} else {
			bat_board_temp = pval.intval;
			val->intval = bat_board_temp;
		}
	}else {
		if(chg->enable_bat_board_temp_from_bq25970){
			//not should call this, bat board temp must get from mcu phy;
			val->intval = DEFAULT_BOARD_TEMP;
			pr_err("not should call bat phy, bat board temp must get from mcu phy\n");
		} else {
...
int smblib_get_handshake_status(struct smb_charger *chg)
{
	...
	struct power_supply *mcu_psy = get_mcu_psy(chg);
	static int last_handshake_status = 0;
	...
	if (last_handshake_status > 0  && (chg->fake_cable_connect || (chg->vbus_smpl_detect && (((jiffies - chg->last_cable_disconnect_start_time) * 1000)/HZ < 5000)))) {
		smblib_err(chg,"keep last_handshake_status=%d !! because fake_cable_connect=%d, vbus_smpl_detect=%d, time=%dms\n", last_handshake_status, chg->fake_cable_connect, chg->vbus_smpl_detect, ((jiffies - chg->last_cable_disconnect_start_time) * 1000)/HZ);
	} else {
		power_supply_get_property(chg->mcu_psy,
		POWER_SUPPLY_PROP_HANDSHAKE_STATUS,&handshake_status);
		last_handshake_status = handshake_status.intval;
	}
	if(chg->apsd_result_not_dcp && (last_handshake_status != VIVO_FLASH_CHARGE_STATUS_NONE)) {
		chg->apsd_result_error |= DCHG_HANDSHAKE_SUCCESS_NOT_DCP_ERROR;
		if (chg->usb_present)
			;
		smblib_err(chg, "apsd_result_error=%d\n", chg->apsd_result_error);
	}
	return last_handshake_status;
}
/*dchg function end*/
...
int smblib_get_prop_batt_fcc(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_BAT_FCC, val);
	if (rc < 0) {
...
int smblib_get_prop_batt_rm(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	rc = power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_BAT_RM, val);
	if (rc < 0) {
		val->intval = -1;
		smblib_err(chg, "Couldn't get remain_mah rc=%d\n", rc);
	}

	return rc;
}
extern bool fg_do_reset;
int smblib_get_prop_batt_engine(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	bool system_start = ((u64)(local_clock()/1000000000)) > 120;//15s
	int ibat_ma = 0;
	...
	if (!chg->direct_charger_enable &&
		...
		if (smblib_get_handshake_status(chg) == VIVO_FLASH_CHARGE_STATUS_SUPER) {
			...
			if(chg->soc_decimal_enable) {
				...
				if(chg->battery_capacity_is_exception
					|| ((chg->real_soc >= 95 || chg->ui_soc >= 95) && chg->charge_monitor_count >= 2)
					|| ((chg->real_soc < 95 && chg->ui_soc < 95) && chg->charge_monitor_count >= 6)
					|| chg->real_soc < 1 || (abs(chg->ui_soc - chg->real_soc) >= 3)
					|| !system_start || chg_status.intval == POWER_SUPPLY_STATUS_FULL || chg->ffc_charge_full
					|| (ibat_ma > -1500 && chg->charge_monitor_count >= 2)
					|| (fg_do_reset && is_factory_mode) || power_off_charging_mode || is_atboot || chg->fake_cable_connect)
					engine = POWER_SUPPLY_ENGINE_DUAL;
				smblib_err(chg, "battery_capacity_is_exception=%d,real_soc=%d,ui_soc=%d,ffc_charge_full=%d,system_start=%d,chg_status=%d,ibat_ma=%d,fake_cable_connect=%d\n",
					chg->battery_capacity_is_exception, chg->real_soc, chg->ui_soc, chg->ffc_charge_full, system_start, chg_status.intval, ibat_ma, chg->fake_cable_connect);
			}
		}else
			engine = POWER_SUPPLY_ENGINE_DUAL;
...
int smblib_get_prop_batt_soc_decimal(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	if(chg->soc_decimal_enable) {
		...
		smblib_err(chg, "batt_soc=%d\n", batt_soc.intval);
		val->intval = batt_soc.intval * 100;
	...
	if(chg->battery_capacity_is_exception || batt_soc.intval  >=98 ||batt_soc.intval  <= 3) {
		...
		if(batt_soc.intval != ui_soc) {
			decimal = chg->soc_decimal_from_user % 100;
			chg->soc_decimal = batt_soc.intval * 100 + decimal;
		}else{
			chg->soc_decimal = chg->soc_decimal_from_user;
		}
		smblib_err(chg,"bat_soc=%d,ui_soc=%d,decimal=%d,chg->soc_decimal=%d\n", batt_soc.intval, ui_soc, decimal, chg->soc_decimal);
	}else {
...
int smblib_set_prop_batt_soc_decimal(struct smb_charger *chg, int soc_decimal)
{
	...
	if(!chg->soc_decimal_enable) {
		...
		if (chg->engine != POWER_SUPPLY_ENGINE_SUPER && chg->engine != POWER_SUPPLY_ENGINE_DUAL) {
			smblib_err(chg, "It's not flashcharge\n");
			return rc;
		}
	}
	if (!fg_soc_really) {
		smblib_err(chg, "fg_soc_really=0x%x, skip..\n", fg_soc_really);
		return rc;
	}
	#endif
	if(chg->soc_decimal_enable) {
		ui_soc = (u8) (soc_decimal / 100);
		...
		if(abs(ui_soc - chg->ui_soc) >= 3 || (ui_soc < chg->ui_soc)) {
			smblib_err(chg, "ui soc is exception!\n");
			return rc;
		}
...
int smblib_get_equalizing_chg_temp_state(struct smb_charger *chg, union power_supply_propval *val)
{
	...
	if (chg->equalizing_chg_tbat_limit < EQUALIZING_CHG_TEMP_LOWER_LIMIT ||
			chg->equalizing_chg_tbat_limit > EQUALIZING_CHG_TEMP_UPPER_LIMIT) {
		chg->equalizing_chg_tbat_limit = EQUALIZING_CHG_TEMP_LIMIT_DEF;
	}
	if (chg->equalizing_chg_tboard_limit < EQUALIZING_CHG_TEMP_LOWER_LIMIT ||
			chg->equalizing_chg_tboard_limit > EQUALIZING_CHG_TEMP_UPPER_LIMIT) {
		chg->equalizing_chg_tboard_limit = EQUALIZING_CHG_TEMP_LIMIT_DEF;
	}
	if (chg->battery_temprature >= chg->equalizing_chg_tbat_limit &&
			chg->main_board_temprature >= chg->equalizing_chg_tboard_limit) {
		val->intval = 1;
	} else {
		val->intval = 0;
	}

	pr_err("system ui get board temperature status=%d,%d,%d,%d,%d\n", val->intval,
			chg->main_board_temprature, chg->battery_temprature,
			chg->equalizing_chg_tbat_limit, chg->equalizing_chg_tboard_limit);
	return ret;
}
static int vivo_battery_handle_ui_soc(struct smb_charger *chg, int soc, int vbat_mV)
{
	union power_supply_propval chg_status = {0,};
	...
	if(fg_do_reset && is_factory_mode){
			pr_err("factory mode and fg do reset\n");
			chg->ui_soc = soc;
			chg->soc_refer = chg->ui_soc;
			chg->last_change_record_tm = 0;
			fg_do_reset = false;
	...
	if(chg->ui_soc != soc) {
		...
		if(chg_status.intval == POWER_SUPPLY_STATUS_FULL) {
			...
			if(chg->charge_monitor_count >= 2 && chg->ui_soc < soc) {
				if (chg->ui_soc_delta_time_s >= (chg->alternate_charger_enable ? 4 : 10)) {
			...
			if (chg->ui_soc > soc && ((0 == soc && vbat_mV < 3400) || chg->real_charger_type == POWER_SUPPLY_TYPE_USB)) {
				if (chg->ui_soc_delta_time_s >= soc_smooth_time_limit) {
					chg->ui_soc = chg->ui_soc - 1;
					get_current_time(&chg->last_change_record_tm);
				}
			} else {
				if(((chg->ui_soc + 1) < soc || (100 == soc && chg->ui_soc < 100)) && (chg->alternate_charger_enable ? 1 : (chg->charge_monitor_count >= 2))) {
					if (chg->ui_soc_delta_time_s >= chg->alternate_charger_enable ? 4 : 10) {
						chg->ui_soc = chg->ui_soc + 1;
						if (chg->ui_soc > soc)
							chg->ui_soc = soc;
				...
				} else if(chg->ui_soc <= soc && (chg->alternate_charger_enable ? 1 : (chg->charge_monitor_count >= 2))) {
					chg->ui_soc = soc;
					chg->last_change_record_tm = 0;
	...
	chg->soc_refer = chg->ui_soc;
	pr_info("ui_soc_delta_time_s=%d,chg_status=%d,chg.ui_soc=%d,soc=%d, vbat=%dmV, soc_decimal_from_user=%d, fg_soc_really=0x%x\n",
			chg->ui_soc_delta_time_s, chg_status.intval, chg->ui_soc, soc, vbat_mV, chg->soc_decimal_from_user, fg_soc_really);
	mutex_unlock(&chg->ui_soc_lock);
	return chg->ui_soc;
}
static int vivo_battery_handle_ui_decimal_soc(struct smb_charger *chg, int soc, int vbat_mV)
{
	union power_supply_propval chg_status = {0,};
	int  soc_smooth_time_limit = 30; //30s
	int rc = 0;
	...
	if(chg->ui_soc != soc) {
		...
		if(chg_status.intval == POWER_SUPPLY_STATUS_FULL) {
			...
			} else if ((chg->ui_soc - soc) > 10 && vbat_mV < 4000) {
				if (chg->ui_soc_delta_time_s >= soc_smooth_time_limit) {
					chg->ui_soc = chg->ui_soc - 1;
					get_current_time(&chg->last_change_record_tm);
					pr_info("ui_soc_delta_time_s=%d, FULL status, but soc drop %d!!!\n", chg->ui_soc_delta_time_s, chg->ui_soc);
				}
			}
		}else if(chg_status.intval == POWER_SUPPLY_STATUS_CHARGING) {
			if (chg->ui_soc > soc && ((0 == soc && vbat_mV < 3400) || chg->real_charger_type == POWER_SUPPLY_TYPE_USB)) {
				...
				if(((chg->ui_soc + 1) < soc || (100 == soc && chg->ui_soc < 100))
				&& (chg->alternate_charger_enable ? 1 : (chg->charge_monitor_count >= 2))
				&& chg->engine != 3) {
					if (chg->ui_soc_delta_time_s >= chg->alternate_charger_enable ? 4 : 10) {
							chg->ui_soc = chg->ui_soc + 1;
							if (chg->ui_soc > soc)
	...
	if(abs(chg->ui_soc -soc) >= 3)
		chg->battery_capacity_is_exception = true;
	else if(chg->battery_capacity_is_exception && abs(chg->ui_soc -soc) <= 0)
		chg->battery_capacity_is_exception = false;
	chg->soc_refer = chg->ui_soc;
	pr_info("ui_soc_delta_time_s=%d,chg_status=%d,chg.ui_soc=%d,soc=%d, vbat=%dmV, soc_decimal_from_user=%d, fg_soc_really=0x%x,battery_capacity_is_exception=%d\n",
			chg->ui_soc_delta_time_s, chg_status.intval, chg->ui_soc, soc, vbat_mV, chg->soc_decimal_from_user, fg_soc_really, chg->battery_capacity_is_exception);
	mutex_unlock(&chg->ui_soc_lock);
	return chg->ui_soc;
}
/* FlashCharge end */
...
void smblib_apsd_enable(struct smb_charger *chg, bool enable)
{
	...
	if (chg->alternate_charger_enable)
	{	//we need apsd interrupt:usb_source_change_irq_handler
		smblib_err(chg, "alternate_charger_enable: force apsd_enable=%d --> 1. \n", enable);
		enable = true;
	}
...
{
	int rc;
	union power_supply_propval apsd_rerun = {0,};
	
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {	//vivo add		
		rc = power_supply_set_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_RERUN_APSD, &apsd_rerun);
		if (rc < 0) {
			smblib_err(chg, "Couldn't set RERUN_APSD prop from alternate_charger_psy rc=%d\n", rc);
		}
	} else {
		...
		unsigned long ev, void *v)
{
	...
	if (!is_atboot && !strcmp(psy->desc->name, "bq_bms")) {
		if ((!chg->bms_psy || chg->bms_psy != psy)) {
			chg->bms_psy = psy;
	...
	/*vivo add*/
	if (!strcmp(psy->desc->name, chg->alternate_charger_psy_name)) {
		if (!chg->alternate_charger_psy) {
			chg->alternate_charger_psy = psy;
			chg->alternate_charger_enable = true;
		}
		schedule_delayed_work(&chg->alternate_charger_update_work, msecs_to_jiffies(0));
	}
...
int smblib_rerun_apsd_if_required(struct smb_charger *chg)
{
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		...
		/* vivo add
			Before APSD_rerun, disable Typec-Switcher (only for max20328),
			make sure D+/D- voltage = 0, trigger HVDCP Adapter to reset to DCP.
			because HVDCP adapter will connect D+/D- again when D+/D-=0mV
		*/
		if (chg->usb_det_pin_mode)
			fsa4480_switch_mode_event(FSA_USBC_SWITCH_DISABLE);	//Typec_switch IC defalut connect to AP side.

		/* vivo modified for audio conflict*/
...
int smblib_set_icl_current(struct smb_charger *chg, int icl_ua)
{
	...
	smblib_dbg(chg, PR_MISC, "suspend=%d,icl_ua=%d, vote_client=%s,typec_mode=%d, force_usb_icl=%d\n", /* vivo add */
				suspend, icl_ua, get_effective_client_locked(chg->usb_icl_votable), chg->typec_mode, chg->force_usb_icl);
#if 0
	/*vivo copy start: from android 10.0,  not vivo func. only support android10.0=android11.0*/
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC) {
	...
	/*vivo copy end: from android 10.0,  not vivo func. only support android10.0=android11.0*/
#endif
	if (chg->chg_param.smb_version == PMI632_SUBTYPE)
	...
	if (suspend) {
		smblib_set_alternate_charger_suspend(chg, true);	//vivo add
	...
	/* configure current */
	if (chg->force_usb_icl <= 0 &&	//vivo add
		chg->real_charger_type == POWER_SUPPLY_TYPE_USB
		&& (chg->typec_legacy
...
unsuspend:
	...
	smblib_set_alternate_charger_suspend(chg, false);	//vivo add
			...
			int chg_disable, const char *client)
{
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		pval.intval = chg_disable ? 0 : 1;
		rc = power_supply_set_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_CHARGING_ENABLED, &pval);
		smblib_dbg(chg, PR_INTERRUPT, "chg_disable=%s, alternate_charger_enable=%d. rc=%d\n", chg_disable ? "disable" : "enable", chg->alternate_charger_enable, rc);
				  ...
				  union power_supply_propval *val)
{
	...
	union power_supply_propval vcell_min = {0,};
	static bool report_flag = false;
	int i = 0;
	/* init battery capacity is 5% */
	static u8 last_soc = 5;
	static unsigned long fg_really_start = 0;
	int fg_soc_really_temp = fg_soc_really;
	bool bq_bms_on = chg->bq_bms_on;
	/* vivo add end */
	...
	if(enable_slave_charger) {
		...
		rc = smblib_get_prop_from_qcom_fg(chg, POWER_SUPPLY_PROP_VOLTAGE_NOW, &batt_volt);
		rc = smblib_get_prop_from_qcom_fg(chg, POWER_SUPPLY_PROP_TEMP, &batt_temp);
	} else {
		rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CAPACITY, val);

		//power_supply_get_property(chg->bms_psy,
		//		POWER_SUPPLY_PROP_VOLTAGE_NOW, &batt_volt);
		rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_VOLTAGE_NOW, &batt_volt);
		if (batt_volt.intval < 2300000 && batt_volt.intval > 0) {
			for (i = 0; i < 3 && batt_volt.intval < 2500000; i++)
				smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_VOLTAGE_NOW, &batt_volt);
		}
		rc = smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_TEMP, &batt_temp);
	}
	smblib_get_prop_batt_status(chg, &batt_status);

	if (batt_volt.intval >= 5000000) {//add for dual battery project
		batt_volt.intval /= 2;

		smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_CELL_VOLTAGE_MIN, &vcell_min);
		if (vcell_min.intval > 0)
			batt_volt.intval = vcell_min.intval * 1000;	//uV
	}

	if ((batt_volt.intval < 2300000 && val->intval == 0) || val->intval < 0) {
		val->intval = last_soc;
		if (0 == val->intval)
			val->intval = 1;	//error case , keep soc=1%.
		smblib_dbg(chg, PR_INTERRUPT, "batt_volt=%d, report soc %d!!\n", batt_volt.intval, val->intval);
	} else if (val->intval == 0 && !report_flag && batt_volt.intval >= chg->cutoff_volt_mv && batt_temp.intval >= (bsp_test_mode ? -260 : -100)) {
		val->intval = 1;
		smblib_err_ratelimited(chg, "soc=0,batt volt=%dmV(>%d),batt_temp=%d, report soc=1!\n",
				batt_volt.intval / 1000, chg->cutoff_volt_mv / 1000, batt_temp.intval);
	} else if (val->intval == 0 && batt_volt.intval < chg->cutoff_volt_mv) {
		report_flag = true;
	...
	if (rc < 0) {
		if (0 == val->intval) {
			smblib_dbg(chg, PR_INTERRUPT, "batt_volt=%d, rc = %d, force report soc 1!\n", batt_volt.intval, rc);
			val->intval = 1;	//error case , keep soc=1%.
		}
		rc = 0;
	}
	...
	last_soc = val->intval;
	chg->real_soc = val->intval;
	//pr_info("last_soc=%d, ex_fg_soc=%d\n", last_soc, ex_fg_soc);
	if((enable_slave_charger || (chg->supported_adapter == SUPPORTED_ADAPTER_VIVO_HV_DCHG_10V_4000MA) || (chg->supported_adapter == SUPPORTED_ADAPTER_VIVO_HV_DCHG_20V_6000MA))&& !is_atboot && !power_off_charging_mode &&  \
				((0 && (fg_soc_really_temp & BQ_BMS_REALLY) && bq_bms_on) || (1 && (fg_soc_really_temp & BMS_REALLY)))) {
		if (fg_really_start == 0)
			fg_really_start = jiffies;
		if (time_is_before_jiffies(fg_really_start + 1*HZ)) {
			if (batt_volt.intval < 3000000 && val->intval == 0) {
				chg->ui_soc = 0;
				pr_info("batt_voltage=%dmV, soc=%d, keep ui_soc=0.\n", batt_volt.intval, val->intval);
			} else {
				if(chg->soc_decimal_enable)
					val->intval = vivo_battery_handle_ui_decimal_soc(chg, val->intval, batt_volt.intval /1000);
				else
				    val->intval = vivo_battery_handle_ui_soc(chg, val->intval, batt_volt.intval/1000);
			}
			val->intval = max(0, val->intval);
			val->intval = min(100, val->intval);
		}
	...
	if (is_atboot) {
		if (val->intval <= 20 && batt_volt.intval >= 3300000) {
			val->intval = 20;
			smblib_err_ratelimited(chg, "is_atboot: soc=%d,batt volt=%dmV(>3.3v), force report soc=20!!\n", val->intval, batt_volt.intval/1000);
		}
	}
	/*vivo end*/
	return rc;
...
{
	union power_supply_propval pval = {0, };
	union power_supply_propval vbat = {0, };	//vivo_chg
	union power_supply_propval alternate_charger_status = {0, };
	...
	if (!rc && pval.intval == 0 && input_present && chg->ui_soc == 0) {	//vivo_chg change
		rc = smblib_get_prop_from_bms(chg,
	...
	//vivo add
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		rc = power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_STATUS, &alternate_charger_status);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get alternate_charger_psy status, rc=%d\n", rc);
			return rc;
		}

		if (!usb_online && !dc_online && chg->sink_src_mode == UNATTACHED_MODE) {
			val->intval = POWER_SUPPLY_STATUS_DISCHARGING;
			return rc;
		}

		val->intval = alternate_charger_status.intval;

		if (val->intval == POWER_SUPPLY_STATUS_FULL)
			chg->charge_full = true;

		if (chg->fake_cable_connect || (chg->vbus_smpl_detect && (((jiffies - chg->last_cable_disconnect_start_time) * 1000)/HZ < 1500)) ||
			(usb_online && chg->alternate_charger_disable_by_vbat_too_high))	//vivo add
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
	} else {
	...
	case PAUSE_CHARGE:
		if(chg->usb_temporary_present)	/* vivo add */
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		else if (chg->fake_cable_connect || (chg->vbus_smpl_detect && (((jiffies - chg->last_cable_disconnect_start_time) * 1000)/HZ < 1500)))	//vivo add
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
		else
			val->intval = POWER_SUPPLY_STATUS_NOT_CHARGING;
		break;
	...
	if (chg->usbin_suspend_when_full_charge_enable) {
		if (chg->usbin_full_charge_suspend) {//vivo add 
			val->intval = POWER_SUPPLY_STATUS_FULL;
			chg->charge_full = true;
		}
	}
	...
	if(chg->usb_temporary_present &&
		is_client_vote_enabled_locked(chg->usb_icl_votable, USB_PSY_VOTER) &&
		get_client_vote_locked(chg->usb_icl_votable, USB_PSY_VOTER) < USBIN_100MA) {
		val->intval = POWER_SUPPLY_STATUS_CHARGING;
		return 0;
	}
	...
	if (chg->fake_cable_connect || (chg->vbus_smpl_detect && (((jiffies - chg->last_cable_disconnect_start_time) * 1000)/HZ < 1500)))	//vivo add
			val->intval = POWER_SUPPLY_STATUS_CHARGING;
	}
	return 0;
}
				...
				union power_supply_propval *val)
{
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		rc = power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_CHARGE_TYPE, val);
		smblib_err(chg, "%d\n", val->intval);
	} else {
		rc = smblib_read(chg, BATTERY_CHARGER_STATUS_1_REG, &stat);
		if (rc < 0) {
					...
					union power_supply_propval *val)
{
	...
	/*vivo add*/
	if (chg->usbin_suspend_when_full_charge_enable) {
		if (chg->usbin_full_charge_suspend && stat == DISABLE_CHARGE) {
			val->intval = true;
			smblib_err(chg, "usbin_full_charge_suspend & disable_charge, force charge_done.\n", rc);
		}
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	//smblib_set_alternate_charger_suspend(chg, !enable);
				  ...
				  const union power_supply_propval *val)
{
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		power_supply_set_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_HW_CHG_TIMEOUT, val);
	} else {
#if 0
...
int smblib_disable_hw_jeita(struct smb_charger *chg, bool disable)
{
	...
	mask = JEITA_EN_COLD_SL_FCV_BIT
		...
		| JEITA_EN_HARDLIMIT_BIT //add by vivo for disable jeita
...
static int smblib_update_thermal_readings(struct smb_charger *chg)
{
	...
	//vivo change
	if(chg->enable_usb_conn_temp_from_bq25970){
		smblib_get_usb_conn_pro_temp(chg, &pval);
	}
	else
	{
		rc = smblib_read_iio_channel(chg, chg->iio.connector_temp_chan,
				...
				union power_supply_propval *val)
{
	...
	//vivo add
	if (chg->third_party_OTG_enable) {
		smblib_err_ratelimited(chg, "third_party_OTG_enable, change present %d -> %d\n", val->intval, 0);
		val->intval = 0;
	}
	return 0;
}
...
//		   stat);
	...
	if(chg->usb_temporary_present)
		val->intval = chg->usb_temporary_present;
			...
			chg->dchg_status == DIRECT_CHARGER_IS_CHARGERING) {
			val->intval = 1;
	} else if (chg->fake_cable_connect || (chg->vbus_smpl_detect && (((jiffies - chg->last_cable_disconnect_start_time) * 1000)/HZ < 1500))) {
		val->intval = 1;
	} else if (chg->alternate_charger_enable) {
		val->intval = chg->usb_present || (chg->sink_src_mode == SINK_MODE);
	} else if (chg->usbin_suspend_when_full_charge_enable && chg->usbin_full_charge_suspend && chg->usb_present) {
		val->intval = 1;
	} else	/* vivo add end */
					...
					union power_supply_propval *val)
{
	...
	if (val->intval < 1000000) {
		if (chg->alternate_charger_enable && chg->alternate_charger_psy) {	//vivo add
			power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_VOLTAGE_NOW, val);
			smblib_err(chg, "read vbus voltage from alternate_charger, vbus=%d\n", val->intval);
		} else
...
static int smblib_get_prop_ufp_mode(struct smb_charger *chg)
{
	...
	/*	//vivo delete: because Huawei Cable use the two Rp/Rp, device will detect to SNK_DAM_500MA_BIT,  PC-USB can not setup enum.
					  ...
					  union power_supply_propval *val)
{
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
	   rc = power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED, &pval);
			       ...
			       union power_supply_propval *val)
{
	...
	smblib_err(chg, "ok_to_pd=%d\n", chg->ok_to_pd);	//vivo_chg add
	return 0;
}
				    ...
				    const union power_supply_propval *val)
{
	...
	if (chg->pd_active) {
		...
		if (get_client_vote_locked(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER) > 0 && curr_ua > get_client_vote_locked(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER)) {
				     ...
				     const union power_supply_propval *val)
{
	...
	if (!chg->usb_det_pin_mode) {
		if (!chg->drp_enable) {
			chg->power_role_backup_when_drp_disable = val->intval;
			pr_info("drp_enable=0, skip...[value=%d]\n", chg->power_role_backup_when_drp_disable);
			return 0;
		}
	}
	if (chg->connector_type == POWER_SUPPLY_CONNECTOR_MICRO_USB)
	...
	smblib_get_prop_typec_power_role(chg, &pval);	//vivo add: use for update chg->power_role
	smblib_dbg(chg, PR_MISC, "power role change: %d --> %d! usb_det_pin_mode=%d",
			chg->power_role, val->intval, chg->usb_det_pin_mode);
				...
				const union power_supply_propval *val)
{
	...
	/*vivo add */
	if (!chg->usb_det_pin_mode) {
		if (chg->drp_enable && chg->pd_active && chg->drp_disable_timer_on) {
			smblib_dbg(chg, PR_INTERRUPT, "typec switch on(drp enable), del the drp disable timer when pd_active!!\n");
			del_timer(&chg->drp_disable_timer);
			chg->drp_disable_timer_on = false;
		}
	}
	/*vivo end */
	...
	if (chg->pd_active) {
		 ...
		 * pd_current_max
		 */
		vote(chg->usb_icl_votable, PD_VOTER, true, USBIN_500MA);	//vivo_chg change
...
//	smblib_dbg(chg, PR_INTERRUPT, "IRQ: %s\n", irq_data->name);
	...
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: %s, stat=0x%02x\n", irq_data->name, stat);
...
#if 0
	if (stat == TERMINATE_CHARGE && chg->usbin_suspend_when_full_charge_enable) {
		smblib_dbg(chg, PR_INTERRUPT, "Charge TERMINATE & usbin_suspend_when_full_charge_enable, suspend usbin...\n");
		chg->usbin_full_charge_suspend = true;
		vote(chg->usb_icl_votable, FULL_CHARGE_SUSPEND, chg->usbin_full_charge_suspend, 0);
	}

	if (chg->lcm_consume_mos_enable) {
		if (chg->lcm_consume_mos_gpio > 0) {
			if (stat == TERMINATE_CHARGE && chg->usb_present) {	//拉高GPIO
				chg->lcm_consume_mos_gpio_setting_value = 1;
				gpio_direction_output(chg->lcm_consume_mos_gpio, chg->lcm_consume_mos_gpio_setting_value);

				msleep(15);
				chg->lcm_consume_mos_gpio_read_value = gpio_get_value(chg->lcm_consume_mos_gpio);
				//if (chg->lcm_consume_mos_gpio_setting_value != chg->lcm_consume_mos_gpio_read_value) {
				//	chg->lcm_consume_mos_gpio_control_error_count = min(INT_MAX-1, chg->lcm_consume_mos_gpio_control_error_count+1);	
				//}
				smblib_dbg(chg, PR_INTERRUPT, "lcm_consume_mos_gpio_setting_value=%d, lcm_consume_mos_gpio_read_value=%d, lcm_consume_mos_gpio_control_error_count=%d.\n",
					chg->lcm_consume_mos_gpio_setting_value, chg->lcm_consume_mos_gpio_read_value, chg->lcm_consume_mos_gpio_control_error_count);
			}			
		}
	}
#endif
...
#else
inline void charger_connect_judge(char on_or_off) {return ;};
#endif

#define PL_DELAY_MS			30000
void smblib_usb_plugin_init(struct smb_charger *chg)
{
	union power_supply_propval pval = {0,};

	cancel_delayed_work_sync(&chg->cable_disconnect_confirm_work);	//vivo add
	if (chg->fake_cable_connect) {
		chg->vbus_smpl_detect = true;	//fake-cable_connect during 600ms
	} else {
		chg->charge_full = false;
		chg->vbus_smpl_detect = false;
	}
	chg->fake_cable_connect = false;

	charger_connect_judge(1);
	if(1) {
		vivo_set_chg_term_voltage(chg, smb5_chip->dt.batt_profile_fv_uv);
		vivo_set_chg_term_current(chg, smb5_chip->dt.term_current_thresh_hi_ma);
		if(chg->dchg_disable_votable)
	...
	schedule_delayed_work(&chg->split_current_monitor_work,
					msecs_to_jiffies(chg->direct_charger_enable ? 100 : 5000));

	if (chg->alternate_charger_enable) {	//alternate charger USBIN_SUSPEND(EN_HIZ) will reset after Vbus insert, so we reset vote and wait client vote it again.
		if (!is_atboot) {
			/*reset vote and wait vote again...*/
	...
	chg->ai_disable_charge_state = 0;
	chg->apsd_result_error = 0;
	chg->apsd_result_not_dcp = false;
	chg->pmic_ffc_charging_continue_count = 0;
	cancel_delayed_work(&chg->charge_monitor_work);
	schedule_delayed_work(&chg->charge_monitor_work, msecs_to_jiffies(5000));
}
void smblib_usb_plugin_deinit(struct smb_charger *chg)
{
		union power_supply_propval pval = {0,};
		int i = 0;

		if (1 /*chg->direct_charger_enable && (chg->dchg_status == DIRECT_CHARGER_PREPARE || chg->dchg_status == DIRECT_CHARGER_IS_CHARGERING)*/) {
			chg->fake_cable_connect = chg->usb_present;
			if (POWER_SUPPLY_TYPE_USB_DCP == chg->real_charger_type)
				schedule_delayed_work(&chg->cable_disconnect_confirm_work, msecs_to_jiffies(600));	//vivo add: after cable disconnect , keep ui 600ms 锛坮eport fake_cable_connect)
			else
				schedule_delayed_work(&chg->cable_disconnect_confirm_work, msecs_to_jiffies(750));	//vivo add: after cable disconnect , keep ui 750ms 锛坮eport fake_cable_connect)
		} else {
			chg->fake_cable_connect = false;
		}
		chg->vbus_smpl_detect = false;
		chg->last_cable_disconnect_start_time = jiffies;
		...
		if(1) {
			vote(chg->chg_disable_votable, FG_CTRL_SCHG_VOTER, false, 0);
		}
		chg->usbin_full_charge_suspend = false;
		vote(chg->usb_icl_votable, FULL_CHARGE_SUSPEND, false, 0);
		vote(chg->chg_disable_votable, COUT_VBAT_TOO_HIGH, false, 0);
		vote(chg->fcc_votable, COUT_VBAT_TOO_HIGH, false, 0);
		chg->alternate_charger_disable_by_vbat_too_high = false;
		

		vote(chg->chg_disable_votable, KEEP_CHG_SOC, false, 0);
		vote(chg->usb_icl_votable, KEEP_CHG_SOC, false, 0);
		vote(chg->usb_icl_votable, DCHG_LIMIT, false, 0);	//need uA
		...
		chg->recharge_flag = 0;
		//chg->charge_full = false;	//move to init.
		...
		* add 2s delay wake lock for Android LCD control*/
		...
		if (chg->lcm_consume_mos_enable) {
			if (chg->lcm_consume_mos_gpio > 0) {
				if (chg->lcm_consume_mos_gpio_read_value || chg->lcm_consume_mos_gpio_setting_value) {
					for (i = 0; i < 6; i++) {
						chg->lcm_consume_mos_gpio_setting_value = 0;	//拉低GPIO (默认情况)						
						gpio_direction_output(chg->lcm_consume_mos_gpio, chg->lcm_consume_mos_gpio_setting_value);						
						msleep(15);
						chg->lcm_consume_mos_gpio_read_value = gpio_get_value(chg->lcm_consume_mos_gpio);
		...
		if (chg->usb_det_pin_enable) {
			if (!get_effective_result_locked(chg->drp_mode_votable)) {
				...
				if (pval.intval == POWER_SUPPLY_TYPEC_PR_DUAL) {
					pr_info("usb_det_pin_mode: vote drp_enable=0. but current in PR_DUAL,  so we change it to PR_SINK\n");
...
void smblib_usb_plugin_hard_reset_locked(struct smb_charger *chg)
{
	...
	/* vivo end */
	power_supply_changed(chg->usb_psy);
	smblib_dbg(chg, PR_INTERRUPT, "IRQ: usbin-plugin %s, fake_cable_connect=%d, vbus_smpl_detect=%d\n",
					vbus_rising ? "attached" : "detached",
					chg->fake_cable_connect, chg->vbus_smpl_detect);
}
...
irqreturn_t usb_plugin_irq_handler(int irq, void *data)
{
	struct smb_irq_data *irq_data = data;
	struct smb_charger *chg = irq_data->parent_data;
	int rc = 0;	//vivo_chg add
	union power_supply_propval pval = {0,}; //vivo_chg add

	smblib_dbg(chg, PR_INTERRUPT, "IRQ: %s\n", irq_data->name);

	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {	//vivo_chg add
		rc = power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_USB_OTG, &pval);
		chg->third_party_OTG_enable = !!pval.intval;
		if (chg->third_party_OTG_enable) {
			smblib_dbg(chg, PR_INTERRUPT, "third_party_OTG_enable, skip!!!\n");
			return IRQ_HANDLED;
		}
	}
...
irqreturn_t usb_source_change_irq_handler(int irq, void *data)
{
	...
	if (chg->third_party_OTG_enable) {
		smblib_dbg(chg, PR_REGISTER, "third_party_OTG_enable, skip!!!\n");
		return IRQ_HANDLED;
	}
	...
	if (chg->pd_active) {	//vivo modify
		if (chg->alternate_charger_enable) {
			smblib_dbg(chg, PR_REGISTER, "PD active. alternate_charger_enable.\n");
			/*
			if (chg->usb_present &&
				chg->apsd_rerun_status == WORK_STATUS_IDLE &&
				!chg->usb_conndoned &&
				chg->direct_charger_enable) {
					smblib_get_apsd_result(chg);
					smblib_dbg(chg, PR_MISC, "pd_active: inserted type =%d,  enable usbsel for dchg.\n", chg->bc1p2_result);
	...
	if(is_present){
		...
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB){
			...
			if(chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP ||
				...
				if (chg->alternate_charger_enable) {
					if (!is_atboot && chg->bc1p2_result != POWER_SUPPLY_TYPE_USB_DCP) {
						smblib_start_apsd_rerun_work(chg);
						smblib_dbg(chg, PR_MISC, "bc1p2_result=%d, real_charger_type=%d, will start apsd-rerun-work\n", chg->bc1p2_result, chg->real_charger_type);
					} else
		...
		*/
#if 1	//vivo: set AICL=4.4V when SDP||CDP;  else set to 4.2V
		if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB || chg->real_charger_type == POWER_SUPPLY_TYPE_USB_CDP) {
			chg->default_aicl_5v_threshold_mv = 4400;
			chg->default_aicl_cont_threshold_mv = 4400;
			...
			chg->default_aicl_5v_threshold_mv = 4200;
			chg->default_aicl_cont_threshold_mv = 4200;
...
static void typec_sink_insertion(struct smb_charger *chg)
{
	...
	if (!chg->alternate_charger_enable)
		vote(chg->usb_icl_votable, OTG_VOTER, true, 0);/* vivo add */
...
static void typec_sink_removal(struct smb_charger *chg)
{
	int rc;
	smblib_dbg(chg, PR_INTERRUPT, "typec log\n"); /* vivo add */
	vote(chg->usb_icl_votable, OTG_VOTER, false, 0);//vivo add
...
static void typec_mode_unattached(struct smb_charger *chg)
{
	...
	vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, true, USBIN_100MA);
}
...
//	smblib_dbg(chg, PR_INTERRUPT, "IRQ: %s\n", irq_data->name);
	...
	if (attached) {
		...
		/*vivo add */
		if (chg->drp_enable && chg->sink_src_mode == SRC_MODE) {
			smblib_dbg(chg, PR_INTERRUPT, "typec switch on(drp enable), del the drp disable timer when UFP detect(device in SRC_MODE)!!\n");
			del_timer_sync(&chg->drp_disable_timer);
			chg->drp_disable_timer_on = false;
		}
		/*vivo end */
		rc = typec_partner_register(chg);
		...
		/*vivo add*/
		if (chg->drp_enable && !chg->drp_disable_timer_on) {
			mod_timer(&chg->drp_disable_timer, jiffies + DRP_DISABLE_DELAY_TIME);
			chg->drp_disable_timer_on = true;
			smblib_dbg(chg, PR_INTERRUPT, "typec switch on(drp enable), start the drp disable timer when typec remove!!\n");
		}
	}
...
int smblib_handle_soc(struct smb_charger *chg)
{
	...
	}else if(chg_status.intval== POWER_SUPPLY_STATUS_FULL){
			chg->last_change_record_tm = 0;
			chg->ui_soc = soc.intval;
			is_charging_change = false;
			smblib_dbg(chg, PR_MISC, "POWER_SUPPLY_STATUS_FULL!!\n");
	}else{
		chg->last_change_record_tm = 0;
...
static int smblib_termination_by_soft(struct smb_charger *chg, int chg_status)
{
	...
	if(chg_status == POWER_SUPPLY_STATUS_CHARGING || chg->alternate_charger_disable_by_vbat_too_high){
		smblib_get_prop_batt_capacity(chg, &batt_soc);
...
{
	int rc;
	union power_supply_propval batt_soc_raw = {255,};
	union power_supply_propval charge_done = {0,};
	union power_supply_propval alternate_charger_status = {0, };

	if (chg->usbin_suspend_when_full_charge_enable) {
		if (chg->usbin_full_charge_suspend) {
			if (chg->bms_psy)
				power_supply_get_property(chg->bms_psy,
					POWER_SUPPLY_PROP_CAPACITY_RAW, &batt_soc_raw);
		
			if (!strcmp(chg->bms_psy->desc->name, "bms")) {
				if (batt_soc_raw.intval <= 254) {
					vote(chg->usb_icl_votable, FULL_CHARGE_SUSPEND, false, 0);
					smblib_dbg(chg, PR_MISC,"usbin_full_charge_suspend: BMS: Enable USBIN... batt_soc_raw=%d.\n", batt_soc_raw.intval);
				}

				if (batt_soc_raw.intval < 252) {
					vote(chg->usb_icl_votable, FULL_CHARGE_SUSPEND, false, 0);
					chg->usbin_full_charge_suspend = false;
					smblib_dbg(chg, PR_MISC,"usbin_full_charge_suspend=0: BMS: batt_soc_raw=%d. soc too low, keep charging status.\n", batt_soc_raw.intval);
				}
					

			} else if (!strcmp(chg->bms_psy->desc->name, "bq_bms")) {
				if (chg->recharge_flag || batt_soc_raw.intval <= 99) {
					vote(chg->usb_icl_votable, FULL_CHARGE_SUSPEND, false, 0);
					smblib_dbg(chg, PR_MISC,"usbin_full_charge_suspend: bq_bms: Enable USBIN... batt_soc_raw=%d.\n", batt_soc_raw.intval);
				}

				if (batt_soc_raw.intval < 98) {
					vote(chg->usb_icl_votable, FULL_CHARGE_SUSPEND, false, 0);
					chg->usbin_full_charge_suspend = false;
					smblib_dbg(chg, PR_MISC,"usbin_full_charge_suspend=0: bq_bms: batt_soc_raw=%d. soc too low, keep charging status.\n", batt_soc_raw.intval);
				}
			}


			smblib_dbg(chg, PR_MISC,"usbin_full_charge_suspend=%d: [%s]:  chg->recharge_flag=%d, batt_soc_raw=%d.\n",
				chg->usbin_full_charge_suspend, chg->bms_psy->desc->name, chg->recharge_flag, batt_soc_raw.intval);
		}
	}
	...
	if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
		...
		if (rc < 0) {
			...
			charge_done.intval = (POWER_SUPPLY_STATUS_FULL == alternate_charger_status.intval) ? 1 : 0;

		if (POWER_SUPPLY_STATUS_CHARGING == alternate_charger_status.intval) {
			smblib_dbg(chg, PR_MISC,"already charging, skip recharge...\n");
			chg->recharge_flag = false;
		}
	} else
	...
	if(is_parallel_charger_available(chg)){
		...
		switch(pval.intval){
			...
			case PL_STATUS_I2C_ERROR:
				...
				fuelsummary_collect_value(ID_BIT__S_I2C_ERR, 1);
...
static void smblib_dump_regs(struct smb_charger *chg)
{
	...
	smblib_dump_reg(chg, "SCHG_P_CHGR_INT_LATCHED_STS", 0x1018);
	smblib_dump_reg(chg, "SCHG_P_CHGR_INT_PENDING_STS", 0x1019);
	...
	smblib_dump_reg(chg, "CHGR_ADC_ITERM_UP_THD_MSB_REG", 0x1067);
	smblib_dump_reg(chg, "CHGR_ADC_ITERM_UP_THD_LSB_REG", 0x1068);
	smblib_dump_reg(chg, "CHGR_ADC_ITERM_LO_THD_MSB_REG", 0x1069);
	smblib_dump_reg(chg, "CHGR_ADC_ITERM_LO_THD_LSB_REG", 0x106A);
	...
	smblib_dump_reg(chg, "SCHG_P_USB_INT_LATCHED_STS", 0x1318);
	smblib_dump_reg(chg, "SCHG_P_USB_INT_PENDING_STS", 0x1319);
	...
	smblib_dump_reg(chg, "LEGACY_CABLE_STATUS_REG", 0x150D);
	
	smblib_dump_reg(chg, "TYPE_C_DEBUG_ACCESS_SNK_CFG", 0x154A);
	smblib_dump_reg(chg, "TYPE_C_EXIT_STATE_CFG_REG", 0x1550);
...
/* add for ffc start */
#if 0
#define QCOM_FG_I_TERM__SYS_TERM__CURRENT_DELTA		30	//-30mA
static int vivo_set_fg_sys_term_curr(struct smb_charger *chg, int current_mA)
{
	union power_supply_propval val = {0,};

	val.intval = current_mA;
	smblib_set_prop_to_qcom_fg(chg, POWER_SUPPLY_PROP_FG_SYS_TERM_CURR, &val);
	smblib_dbg(chg, PR_MISC, "setting fg_sys_term_curr=%d ...\n", current_mA);
...
static int vivo_set_chg_term_current(struct smb_charger *chg, int term_current)
{
	...
	if (chg->term_current_thresh_hi_ma != term_current) {
		...
		if (val.intval == term_current) {
			...
			//vivo_set_fg_sys_term_curr(chg, (chg->term_current_thresh_hi_ma - QCOM_FG_I_TERM__SYS_TERM__CURRENT_DELTA));
		} else
...
static int vivo_set_chg_term_voltage(struct smb_charger *chg, int term_volt_uv)
{
	...
	chg->last_ffc_fv_max_uv = term_volt_uv;
	return rc;
}
/* add for ffc end */
#define jiffies_to_kernel_time_s(jiffies)	\
	(unsigned long)((jiffies - INITIAL_JIFFIES) / HZ)
...
void ibat_circ_buf_add(struct ibat_circ_buf *buf, int val)
{
	...
	buf->last_update_jiffies = jiffies;
	//printk("ibat_circ_buf_add: time=%ld, add data=%d, size=%d\n", jiffies_to_kernel_time_s(buf->last_update_jiffies), val, buf->size);
}
...
void ibat_circ_buf_clr(struct ibat_circ_buf *buf)
{
	buf->size = 0;
	buf->head = 0;
	buf->last_update_jiffies = 0;
	buf->last_read_jiffies = 0;
	memset(buf->arr, 0, sizeof(buf->arr));
	//printk("ibat_circ_buf_clr...\n");
}
...
int ibat_circ_buf_avg(struct ibat_circ_buf *buf, int *avg)
{
	...
	*avg = div_s64(result, buf->size);
	...
	buf->last_read_jiffies = jiffies;
	//printk("ibat_circ_buf_avg: time=%ld, avg_data=%d, size=%d.....\n", jiffies_to_kernel_time_s(buf->last_read_jiffies), *avg, buf->size);
	return 0;
}
...
static void smblib_charge_monitor_work(struct work_struct *work)
{
	...
	union power_supply_propval pval = {0, };
	union power_supply_propval usbin_uv = {0,};
	union power_supply_propval vcell1 = {0, };
	union power_supply_propval vcell2 = {0, };
	union power_supply_propval normal_charge__is_ffc_running = {0, };
	union power_supply_propval normal_charge__charge_state = {0, };
	static int cout_vbat_too_high_flag = 0;

	bool vbus_rising = chg->usb_present;
	u8 stat, stat_1 = 0, stat_2 = 0;
	int rc = 0;
	static int vbus_check_count;
	static int temp_count;
	int float_voltage = get_effective_result_locked(chg->fv_votable);
	union power_supply_propval fastchg_ma = {0,};
	union power_supply_propval vbat_uv = {0,};
	union power_supply_propval fg_vbat_uv = {0,};
	union power_supply_propval fg_capacity = {0,};
	union power_supply_propval bat_temp_value = {0,};
	union power_supply_propval alternate_charger_ibat_now = {0,};
	int alternate_charger_ibat_step_down_current = 0;

	int ffc_temp_index = 0;
	int ibat_ma = 0;
	int pmi_vbat_sum = 0;

	int ex_fg_vbat_sum = 0;
	int ex_fg_ibat_sum = 0;
	int work_period_ms = CHARGER_MONITOR_WORK_PEROID_MS;

	int i = 0;

	static int alternate_charger_wait_recharge_count = 0;

	const struct apsd_result *apsd_result = smblib_get_apsd_result(chg);

	smblib_dbg(chg, PR_MISC, "######## start charger monitor work : fake_cable_connect=%d, vbus_smpl_detect=%d#########\n", chg->fake_cable_connect, chg->vbus_smpl_detect);
	/* dump regs */
	smblib_dump_regs(chg);
	smblib_get_primary_board_temp(chg, &pval);

	smblib_get_prop_usb_voltage_now(chg, &usbin_uv);
	usbin_vbus_uv = usbin_uv.intval;

	smblib_get_prop_from_bms(chg, POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbat_voltage);
	smblib_get_prop_from_qcom_fg(chg, POWER_SUPPLY_PROP_TEMP, &bat_temp_value);
	...
	if (chg->alternate_charger_enable) {
		 if (((chg->real_charger_type == POWER_SUPPLY_TYPE_USB_DCP && !strncmp(chg->usb_type_name, "DCP", 3)) || chg->real_charger_type == POWER_SUPPLY_TYPE_USB_HVDCP) &&
		 	is_client_vote_enabled_locked(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER) &&
		 	get_client_vote_locked(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER) < chg->total_input_current_max_ua) {
				smblib_dbg(chg, PR_MISC, "alternate_charger_enable: reset the usb_icl_votable for USER_DEFAULT_INPUT_VOTER...\n");
				vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, true, chg->total_input_current_max_ua);
		 	}

		if (chg->charge_monitor_count == 0)
			cout_vbat_too_high_flag = 0;

		if (chg->alternate_charger_psy) {
			power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_IS_FFC_RUNNING, &normal_charge__is_ffc_running);
			power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_STATUS, &normal_charge__charge_state);
		}

		if (chg->bms_psy) {
			power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_CELL1_VOLTAGE, &vcell1);
			power_supply_get_property(chg->bms_psy, POWER_SUPPLY_PROP_CELL2_VOLTAGE, &vcell2);
		}

		if ((normal_charge__charge_state.intval == POWER_SUPPLY_STATUS_CHARGING) && !normal_charge__is_ffc_running.intval) {
			if (chg->alternate_charger_psy) {
				power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_CURRENT_NOW, &alternate_charger_ibat_now);
			}
			if (battery_cout_value || vcell1.intval > 4430 || vcell2.intval > 4430) {
				if (cout_vbat_too_high_flag++ >= 3) {
					cout_vbat_too_high_flag = 0;

					if (alternate_charger_ibat_now.intval > 0) {	//charging and have charging current.
						alternate_charger_ibat_step_down_current = alternate_charger_ibat_now.intval - 200;
						if (alternate_charger_ibat_step_down_current < 150) {
							alternate_charger_ibat_step_down_current = 150;
						}

						vote(chg->fcc_votable, COUT_VBAT_TOO_HIGH, true, alternate_charger_ibat_step_down_current*1000);	//uA
						smblib_dbg(chg, PR_MISC, "alternate_charger_enable: normal charging: step down ibat=%d...\n", alternate_charger_ibat_step_down_current);
					} else {
						vote(chg->fcc_votable, COUT_VBAT_TOO_HIGH, true, 400*1000);	//uA
						smblib_dbg(chg, PR_MISC, "alternate_charger_enable: normal charging but IC ichg=0: step down ibat=400mA.....\n");
					}

					if (alternate_charger_ibat_now.intval <= 150) {
						smblib_dbg(chg, PR_MISC, "alternate_charger_enable: normal charging: disable charging because ibat too low....\n");
						chg->alternate_charger_disable_by_vbat_too_high = true;
						vote(chg->chg_disable_votable, COUT_VBAT_TOO_HIGH, true, 0);
					}
				}
			} else if ((vbat_voltage.intval / 1000) >= 8850 && alternate_charger_ibat_now.intval < 120 && alternate_charger_ibat_now.intval > 20){
					if (cout_vbat_too_high_flag++ >= 12) {
						cout_vbat_too_high_flag = 0;
						smblib_dbg(chg, PR_MISC, "alternate_charger_enable: normal charging: disable charging because ibat < iterm.\n");
						chg->alternate_charger_disable_by_vbat_too_high = true;
						vote(chg->chg_disable_votable, COUT_VBAT_TOO_HIGH, true, 0);
					}
			} else {
			...
			smblib_dbg(chg, PR_MISC, "alternate_charger_enable:  normal charging: battery_cout_value=%d, Vcell=[%d,%d], vbat=%d, alternate_charger_ibat_now=%d, flag=%d... normal_charge__is_ffc_running=%d, normal_charge__charge_state=%d!\n", 
				battery_cout_value, vcell1.intval, vcell2.intval, (vbat_voltage.intval / 1000), alternate_charger_ibat_now.intval, cout_vbat_too_high_flag, normal_charge__is_ffc_running.intval, normal_charge__charge_state.intval);
		} else {
			cout_vbat_too_high_flag = 0;
			if ((!battery_cout_value && vcell1.intval <= 4425 && vcell2.intval <= 4425 && chg->recharge_flag) ||
				(!battery_cout_value && vcell1.intval < 4420 && vcell2.intval < 4420 && bat_temp_value.intval > 50 && alternate_charger_wait_recharge_count++ > 30) ||
				(!battery_cout_value && vcell1.intval < 4350 && vcell2.intval < 4350 && bat_temp_value.intval <= 50 && alternate_charger_wait_recharge_count++ > 6)) {
				alternate_charger_wait_recharge_count = 0;
				smblib_dbg(chg, PR_MISC, "alternate_charger_enable: normal charging[%d]: battery_cout_value=%d, Vcell=[%d,%d], alternate_charger_ibat_now=%dmA, ui_soc=%d, recharge_flag=%d, restart charging ****\n",
					normal_charge__charge_state.intval, battery_cout_value, vcell1.intval, vcell2.intval, alternate_charger_ibat_now.intval, chg->ui_soc, chg->recharge_flag);

				if ((vbat_voltage.intval / 1000) < 8800)
					vote(chg->fcc_votable, COUT_VBAT_TOO_HIGH, false, 0);
	...
	/* pet watch dog & all*/
	if(enable_slave_charger || 1) {
		rc = smblib_write(chg, BARK_BITE_WDOG_PET_REG, BARK_BITE_WDOG_PET_BIT);
		if (rc < 0)
			smblib_err(chg, "Couldn't pet the dog rc=%d\n", rc);
	}
	/*WA: double check Vbus status*/
	rc = smblib_read(chg, USBIN_BASE + INT_RT_STS_OFFSET, &stat);
	if (rc < 0) {
		smblib_err(chg, "Couldn't read USB_INT_RT_STS rc=%d\n", rc);
	} else {
		vbus_rising = (bool)(stat & USBIN_PLUGIN_RT_STS_BIT);
		if (vbus_rising != chg->usb_present) {
			if (++vbus_check_count > 1) {
				mutex_lock(&chg->smb_lock);
	...
	if (vbus_rising && !is_atboot) {
		if (chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC && chg->typec_mode == POWER_SUPPLY_TYPEC_NONE) {
	...
	//WA:  //LCM 驱动IC，在满充截止后，会出现闪屏问题：增加以下GPIO，拉高GPIO增加LCM驱动IC负载，规避闪屏。
	if (chg->lcm_consume_mos_enable) {
		if (chg->lcm_consume_mos_gpio > 0) {
			if (chg->lcm_consume_mos_gpio_setting_value != chg->lcm_consume_mos_gpio_read_value) {
				if (temp_count++ < 6) {
			...
			smblib_dbg(chg, PR_INTERRUPT, "lcm_consume_mos_gpio_setting_value=%d, lcm_consume_mos_gpio_read_value=%d, lcm_consume_mos_gpio_control_error_count=%d. temp_count=%d.\n",
				chg->lcm_consume_mos_gpio_setting_value, chg->lcm_consume_mos_gpio_read_value, chg->lcm_consume_mos_gpio_control_error_count, temp_count);
		}
	}
	...
	*/
	...
	//fuelsummary_collect_value(ID_VBAT_IN_ITERM, chg->vbat_in_iterm);
	...
	if (chg->ffc_charge_full || chg->alternate_charger_enable) {
		if (0 == chg->charge_monitor_count % 3)
			smblib_recharge_by_soft(chg, chg_status.intval);
	} else
		smblib_recharge_by_soft(chg, chg_status.intval);
	#endif
	/* charge full force 9v to 5v in HVDCP case*/
	/*
	...
	*/
	...
	/* when disable pl,voter input 1.4a */
	if (get_effective_result_locked(chg->pl_disable_votable) &&
				smblib_get_hvdcp_type(chg) >= HVDCP_TYPE_9V && !chg->direct_charger_enable) {
	...
	if(get_effective_result_locked(chg->fcc_votable) < 1000000){
	...
	if (2 == chg->charge_monitor_count && chg->connector_type == POWER_SUPPLY_CONNECTOR_TYPEC &&
			chg->cable_with_cc_float && usb_input_current.intval <= 500000) {
		apsd_result = smblib_get_apsd_result(chg);
	...
	/* vivo add start */
	#if 1
	/* if usb present, when CC pin short to GND, typec enter into audio mode, enable charge */
	if ((chg->typec_mode == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER ||
...
#if 1
	/* cable disconnect will setting 100mA in USER_DEFAULT_INPUT_VOTER, so we need reset it if some error occur.*/
	if (SDP_100_MA == get_client_vote_locked(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER))
		vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, false, 0);
#endif

	/* get soc from fuel gauge */
	if (chg->bms_psy)
		power_supply_get_property(chg->bms_psy,
					POWER_SUPPLY_PROP_CAPACITY, &fg_capacity);

	/* force apsd_rerun_work delay 900s for android11(soc>=5%) when upgrade system software with usbin */
	if (chg->apsd_rerun_first && !power_off_charging_mode &&
		(chg->charge_monitor_count >= 90 || fg_capacity.intval < 5) &&
		(chg->real_charger_type == POWER_SUPPLY_TYPE_USB ||
	...
	if (chg->force_usb_icl > 0 && chg->real_charger_type == POWER_SUPPLY_TYPE_USB) {
		vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, true, chg->force_usb_icl);
		vote(chg->usb_icl_votable, USB_PSY_VOTER, false, 0);
	}
	...
	if(1 && chg->bms_psy
		&& smblib_get_handshake_status(chg) && (vbat_voltage.intval / 1000) < 5000	//dual battery project skip below func.
		&& (chg->ffc_temperature_range_len > 0 && chg->ffc_full_chg_ibat_ma_len > 0 && chg->ffc_temperature_range_len == 2*chg->ffc_full_chg_ibat_ma_len && chg->ffc_temperature_range && chg->ffc_full_chg_ibat_ma)) {

		for(i = 0; i < chg->ffc_full_chg_ibat_ma_len; i++) {
			if (is_between(chg->ffc_temperature_range[i*2], chg->ffc_temperature_range[i*2+1], bat_temp_value.intval)) {
				ffc_temp_index=i;
				break;
			}
		}
		if (ffc_temp_index > chg->ffc_full_chg_ibat_ma_len - 1)
			ffc_temp_index = chg->ffc_full_chg_ibat_ma_len - 1;

		if(is_between(chg->ffc_full_chg_ibat_ma[ffc_temp_index], 0, ibat_ma)) {
			ex_fg_ibat_sum += ibat_ma;
			...
			ibat_ma = ex_fg_ibat_sum / 3;
			work_period_ms = CHARGER_MONITOR_WORK_PEROID_MS /2;
			pr_err("ibat_ma=%d,ex_fg_ibat_sum=%d\n", ibat_ma, ex_fg_ibat_sum);
		}

		/* get vbat from qcom fuel gauge */
		smblib_get_prop_from_qcom_fg(chg, POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbat_uv);
		pmi_vbat_sum = 0;
		if (vbat_uv.intval >= chg->ffc_full_chg_vbat_uv - 10000) {
			pmi_vbat_sum += vbat_uv.intval;
			msleep(1100);
			smblib_get_prop_from_qcom_fg(chg, POWER_SUPPLY_PROP_VOLTAGE_NOW, &vbat_uv);
			pmi_vbat_sum += vbat_uv.intval;
			msleep(1100);
			...
			vbat_uv.intval = pmi_vbat_sum / 3;
			work_period_ms = CHARGER_MONITOR_WORK_PEROID_MS /2;
		}
		/*get vbat from bq27750*/
		power_supply_get_property(chg->bms_psy,
			POWER_SUPPLY_PROP_VOLTAGE_NOW, &fg_vbat_uv);
		ex_fg_vbat_sum = 0;
		if (fg_vbat_uv.intval >= chg->ffc_full_chg_vbat_uv - 40000) {
			ex_fg_vbat_sum += fg_vbat_uv.intval;
			...
			fg_vbat_uv.intval = ex_fg_vbat_sum / 3;
			work_period_ms = CHARGER_MONITOR_WORK_PEROID_MS /2;
			pr_err("fg_vbat_uv=%d,ex_fg_vbat_sum=%d\n", fg_vbat_uv, ex_fg_vbat_sum);
		}

		
		smblib_dbg(chg, PR_MISC, "ibat_ma=%d,flag=%d(%dmA),capcity=%d,bat_temp=%d[%d,%d],vbat_uv=%d,fg_vbat_uv=%d,cout_gpio=%d,dchg_status=%d,charge_full=%d,ffc_charge_full=%d.pmic_ffc_charging_continue_count=%d.\n",
			ibat_ma, is_between(chg->ffc_full_chg_ibat_ma[ffc_temp_index], 0, ibat_ma), chg->ffc_full_chg_ibat_ma[ffc_temp_index], fg_capacity.intval, bat_temp_value.intval, chg->ffc_temperature_range[ffc_temp_index*2], chg->ffc_temperature_range[ffc_temp_index*2+1], vbat_uv.intval, fg_vbat_uv.intval, battery_cout_value, chg->dchg_status, chg->charge_full, chg->ffc_charge_full, chg->pmic_ffc_charging_continue_count);

		if(bat_temp_value.intval < chg->ffc_temperature_range[0] || bat_temp_value.intval > chg->ffc_temperature_range[chg->ffc_temperature_range_len-1]
			|| 0) {	/*bq27750_IIC_ERROR = BIT(1) */
				vivo_set_chg_term_voltage(chg, smb5_chip->dt.batt_profile_fv_uv);
				vivo_set_chg_term_current(chg, smb5_chip->dt.term_current_thresh_hi_ma);
				smblib_dbg(chg, PR_MISC, "charigng not in ffc mode\n");
		} else {
			if (!chg->ffc_charge_full && chg->dchg_status == DIRECT_CHARGER_UNKNOW
				 && !chg->pmic_ffc_charging_continue_count && fg_capacity.intval < 100) {
				vivo_set_chg_term_voltage(chg, chg->ffc_fv_max_uv);
				vivo_set_chg_term_current(chg, chg->ffc_chg_term_current_ma);

				chg->pmic_ffc_charging_continue_count = 1;
				smblib_dbg(chg, PR_MISC, "ffc step0\n");
			} else if(!chg->ffc_charge_full && chg->dchg_status == DIRECT_CHARGER_IS_CHARGERING
				 && is_between(-8100, 0, ibat_ma))
			{
				vivo_set_chg_term_voltage(chg, chg->ffc_fv_max_uv);
				vivo_set_chg_term_current(chg, chg->ffc_chg_term_current_ma);

				if((is_between(chg->ffc_full_chg_ibat_ma[ffc_temp_index], 0, ibat_ma) && (fg_vbat_uv.intval >= chg->ffc_full_chg_vbat_uv || battery_cout_value >= 1))
					|| (is_between(-700, 0, ibat_ma) && (fg_capacity.intval == 100 || fg_vbat_uv.intval >= chg->ffc_full_chg_vbat_uv - 40000))) {
					vivo_set_chg_term_voltage(chg, smb5_chip->dt.batt_profile_fv_uv);
					vivo_set_chg_term_current(chg, smb5_chip->dt.term_current_thresh_hi_ma);
					/* disable direct charge */
					if (!chg->dchg_disable_votable)
				...
				&& chg->pmic_ffc_charging_continue_count++ > 1
				&& ((is_between(chg->ffc_full_chg_ibat_ma[ffc_temp_index], 0, ibat_ma) && (vbat_uv.intval >= chg->ffc_full_chg_vbat_uv - 10000 || fg_vbat_uv.intval >= chg->ffc_full_chg_vbat_uv || battery_cout_value >= 1))
				|| (is_between(-700, 0, ibat_ma) && (fg_capacity.intval == 100 || fg_vbat_uv.intval >= chg->ffc_full_chg_vbat_uv - 40000)))){

				/*if (chg->last_ffc_fv_max_uv == chg->ffc_fv_max_uv) {
					chg->pmic_ffc_charging_continue_count = 1;
					vivo_set_chg_term_voltage(chg, chg->ffc_fv_max_uv-10000);
					vivo_set_chg_term_current(chg, chg->ffc_chg_term_current_ma);
					smblib_dbg(chg, PR_MISC, "ffc step2 a\n");
				} else */{
					vivo_set_chg_term_voltage(chg, smb5_chip->dt.batt_profile_fv_uv);
					vivo_set_chg_term_current(chg, smb5_chip->dt.term_current_thresh_hi_ma);
					chg->ffc_charge_full = true;
					smblib_dbg(chg, PR_MISC, "ffc step2 b\n");
				}
	...
	if (0) {
		if (chg->dchg_status == DIRECT_CHARGER_END && float_voltage == smb5_chip->dt.batt_profile_fv_uv && vbat_voltage.intval > float_voltage + 15000 && ibat_ma > 0 && (chg_status.intval == POWER_SUPPLY_STATUS_CHARGING)) {
	...
	if (chg->pmic_ffc_charging_continue_count >= INT_MAX)
		chg->pmic_ffc_charging_continue_count = 1;

	smblib_dbg(chg, PR_MISC, "usb_present=%d, count=%d,bigdata_chg_voltage_uv=%d,hvdcp_type=%d,usb_input_current=%d,usbin_uV=%d,usb_temporary_present=%d,chg_abnormal_flag=0x%02x,vbat_in_iterm=%d,vbat=%d[%d]uV,ibat=%dmA,charge_full=%d,ffc_charge_full=%d,usbin_suspend_when_full_charge[%d,%d], usb_det_pin_mode=%d.\n",
				chg->usb_present, chg->charge_monitor_count, chg->bigdata_chg_voltage_uv, chg->hvdcp_type, usb_input_current.intval, usbin_vbus_uv, chg->usb_temporary_present, chg->chg_abnormal_flag, chg->vbat_in_iterm, vbat_voltage.intval, float_voltage, ibat_ma, chg->charge_full, chg->ffc_charge_full,
				chg->usbin_suspend_when_full_charge_enable, chg->usbin_full_charge_suspend, chg->usb_det_pin_mode);
...
static void smblib_check_drp_disable_work(struct work_struct *work)
{
	...
	if (chg->usb_det_pin_mode)	//it will auto detect Device connect and check the Source/SINK in DRP mode
		return;
	...
	smblib_get_prop_typec_power_role(chg, &pval);
	if (chg->power_role_backup_when_drp_disable < 0 && !chg->drp_enable)
		chg->power_role_backup_when_drp_disable = pval.intval;	//init.

	if (pval.intval == POWER_SUPPLY_TYPEC_PR_DUAL && !chg->drp_enable && !chg->usb_present) {

		rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
					TYPEC_POWER_ROLE_CMD_MASK | TYPEC_TRY_MODE_MASK,
					EN_SNK_ONLY_BIT);
		if (rc < 0) {
			smblib_dbg(chg, PR_MISC, "Couldn't write  SINK  to TYPE_C_INTRPT_ENB_SOFTWARE_CTRL rc=%d\n", rc);
		}

		smblib_dbg(chg, PR_MISC, "Type-C switch (OTG switch) Disable(drp_disable), but PMIC also in DRP mode, retry to disable DRP again!!\n");
	}
...
static void smblib_drp_disable_function(struct timer_list *t)
{
	struct smb_charger *chg  = from_timer(chg, t, drp_disable_timer);
	...
	smblib_dbg(chg, PR_MISC, "DRP disable here because the %dmin timeout !!\n", DRP_DISABLE_DELAY_TIME/(60*HZ));
}

static void smblib_cable_disconnect_confirm_work(struct work_struct *work)
{
	struct smb_charger *chg = container_of(work, struct smb_charger, cable_disconnect_confirm_work.work);

	smblib_dbg(chg, PR_MISC, " entry!\n");
	chg->fake_cable_connect = false;
	chg->vbus_smpl_detect = false;
	chg->charge_full = false;
...
static void smblib_start_apsd_rerun_work(struct smb_charger *chg)
{
	...
	/*
	if (chg->is_audio_adapter == true) {
		smblib_dbg(chg, PR_MISC, "is_audio_adapter, skip apsd rerun work start!!\n");
		return;
	}

	if (chg->fake_usb_insertion) {
		smblib_dbg(chg, PR_MISC, "fake_usb_insertion, skip apsd rerun work start!!\n");
		return;
	}
	*/
	if((chg->usb_present || chg->apsd_done_before_usb_present) &&
		...
		if(chg->real_charger_type == POWER_SUPPLY_TYPE_USB || chg->real_charger_type == POWER_SUPPLY_TYPE_USB_CDP || chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
			work_period = APSD_RERUN_FOR_USB_MS;
			if (!is_atboot && !power_off_charging_mode) {
				if (((u64)(local_clock()/1000000000)) > 60)
					work_period = APSD_RERUN_FOR_USB_MS;
				else
					work_period = (60 - ((u64)(local_clock()/1000000000)))*1000 + APSD_RERUN_FOR_USB_MS;
			}

		} else
			work_period = is_atboot ? APSD_RERUN_MS_AT : APSD_RERUN_MS;
		smblib_dbg(chg, PR_MISC, "rerun apsd after %dms,charger_type=%d, chg->apsd_rerun_first=%d\n", work_period, chg->real_charger_type, chg->apsd_rerun_first);
		cancel_delayed_work(&chg->apsd_rerun_work);
...
static void smblib_apsd_rerun_work(struct work_struct *work)
{
	...
	if (chg->cable_with_cc_float && chg->real_charger_type == POWER_SUPPLY_TYPE_USB) {
		if (chg->apsd_rerun_retries == 0) {
			chg->apsd_rerun_retries = 1;
			smblib_dbg(chg, PR_MISC, "cable_with_cc_float detect, skip APSD_rerun first time wait USB core USB detect, will rerun apsd after %dms later.\n", APSD_RERUN_PERIOD_MS);
			schedule_delayed_work(&chg->apsd_rerun_work,
						msecs_to_jiffies(APSD_RERUN_PERIOD_MS));
		}
		/*
			chg->apsd_rerun_status = WORK_STATUS_END;
	...
	case POWER_SUPPLY_TYPE_USB:
		if(rerun_secceful){/* retry max */
			...
			if(!chg->usb_conndoned){
				...
				smblib_dbg(chg, PR_MISC, "sdp usb_conndone is false,set sdp(%dma) to dwc3\n", current_limit_ua/1000);
			}
		}
		break;
	case POWER_SUPPLY_TYPE_USB_CDP:
	...
	case POWER_SUPPLY_TYPE_USB_FLOAT:
		if (rerun_secceful) {/* retry max */
			if (chg->typec_mode == POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER) {
				current_limit_ua = DEFAULT_SDP_UA;	//audio adapter default 500mA
			} else
				current_limit_ua = DEFAULT_FLOAT_UA;
			...
			if(!chg->usb_conndoned) {
				...
				smblib_dbg(chg, PR_MISC, "float usb_conndone is false,set (%dmA) to dwc3\n", current_limit_ua);
			}
		}
		break;
	case POWER_SUPPLY_TYPE_USB_DCP:
	...
	if(rerun_secceful){
		...
		if(apsd_result->pst == POWER_SUPPLY_TYPE_USB ||
			...
			if(!chg->usb_conndoned){
				chg->apsd_result_error |= USB_CDP_ENUMERATE_ERROR;
				if (chg->usb_present)
					;
			}
			usbsel_direct = 0;
		...
		if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
			pval.intval = current_limit_ua / 1000;
			rc = power_supply_set_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_INPUT_CURRENT_LIMIT, &pval);
			smblib_dbg(chg, PR_MISC, "icl_ma=%d, rc=%d\n", pval.intval, rc);
		}
...
static void smblib_alternate_charger_update_work(struct work_struct *work)
{
	//struct smb_charger *chg = container_of(work, struct smb_charger, alternate_charger_update_work.work);
	//SXS: todo something you need notifier from the alternate_charger PSY.
	return;
}
...
static void smblib_usb_conn_pro_monitor_work(struct work_struct *work)
{
	...
	union power_supply_propval usb_conn_temp = {0,};
	union power_supply_propval usb_conn_temp_ap = {0,};		//from ap side
	union power_supply_propval usb_conn_temp_mcu = {0,};	//from mcu side
	...
	smblib_get_usb_conn_pro_temp(chg, &usb_conn_temp_ap);
	if (chg->alternate_charger_enable) {	//this project have two USB_CON_NTC
		if (mcu_psy) {
			power_supply_get_property(mcu_psy,
				POWER_SUPPLY_PROP_USB_CONN_TEMP, &usb_conn_temp_mcu);
		}
	}
	usb_conn_temp.intval = max(usb_conn_temp_ap.intval, usb_conn_temp_mcu.intval);

	usb_trigger_threshold = batt_temp_now.intval >= 260 ? 700 : 600;
	usb_release_threshold = batt_temp_now.intval >= 260 ? 620 : 520;

	smblib_dbg(chg, PR_MISC, "batt_temp=%d, usb_conn_temp=%d[%d,%d](trigger=%d, realease=%d), flag=%d, work_count=%d, usb_conn_pro_gpio=%d, cms_health_now=0x%x.\n",
			batt_temp_now.intval, usb_conn_temp.intval, usb_conn_temp_ap.intval, usb_conn_temp_mcu.intval, usb_trigger_threshold, usb_release_threshold, chg->usb_conn_pro_flag,
				...
				void *data, int enable, const char *client)
{
	...
	if (votable != NULL)
		smblib_dbg(chg, PR_MISC, "vote_client=%s(%s), typec_mode=%d. setting drp_dual_mode=%d,\n",
				get_effective_client_locked(votable), client, chg->typec_mode, enable);
	...
	rc = smblib_set_prop_typec_power_role(chg, &pval);
	if (rc < 0) {
		smblib_err(chg, "Couldn't smblib_set_prop_typec_power_role rc=%d\n", rc);
		return rc;
	}
				...
				const union power_supply_propval *val)
{
	...
	if (!chg->usb_det_pin_mode) {
		...
		if (!get_effective_result_locked(chg->drp_mode_votable)) {
			smblib_dbg(chg, PR_MISC, "usb_det_pin_mode=%d: vote drp_enable=0, skip...[value=%d]\n", chg->usb_det_pin_mode, val->intval);
			return 0;
		}
	}
	...
	if (!chg->pr_swap_in_progress) {
		...
		vote(chg->awake_votable, DETACH_DETECT_VOTER, true, 0);
		schedule_delayed_work(&chg->pr_swap_detach_work,
				msecs_to_jiffies(chg->usb_present ? 200 : DETACH_DETECT_DELAY_MS));	//vivo_chg change the time.
	}
...
static void smblib_chg_termination_work(struct work_struct *work)
{
	...
	union power_supply_propval fastchg_ma = {0,};	//vivo add
	...
	power_supply_get_property(chg->bms_psy,	POWER_SUPPLY_PROP_CURRENT_NOW, &fastchg_ma);	//vivo add
	 ...
	 * re-initialize it.
	 */
	if (pval.intval != chg->charge_full_cc || !chg->cc_soc_ref || is_between(-2000, smb5_chip->dt.term_current_thresh_hi_ma, fastchg_ma.intval/1000)) {	//vivo add
	...
	smblib_dbg(chg, PR_MISC, "Chg Term WA readings: cc_soc: %d, cc_soc_ref: %d, delay: %d vbat_now %d term_vbat %d. fastchg_ma=%d(%d,%d)\n",	//vivo add
			pval.intval, chg->cc_soc_ref, delay, vbat_now_uv,
			chg->term_vbat_uv,
			fastchg_ma.intval/1000, smb5_chip->dt.term_current_thresh_hi_ma, is_between(-2000, smb5_chip->dt.term_current_thresh_hi_ma, fastchg_ma.intval/1000));
...
int smblib_init(struct smb_charger *chg)
{
	...
	/* vivo add */
	ibat_circ_buf_clr(&chg->ibat_buf);
	...
	INIT_DELAYED_WORK(&chg->cable_disconnect_confirm_work, smblib_cable_disconnect_confirm_work);
	chg->fake_cable_connect = false;
	chg->vbus_smpl_detect = false;

	#if 1
	timer_setup(&chg->drp_disable_timer, smblib_drp_disable_function, 0);
	#else
	init_timer(&chg->drp_disable_timer);
	chg->drp_disable_timer.function = smblib_drp_disable_function;
	chg->drp_disable_timer.data = (unsigned long)chg;
	#endif
	wake_lock_init(&chg->drp_disable_wake_lock, WAKE_LOCK_SUSPEND, "smblib_drp_disable_wl");

	if (chg->usb_det_pin_enable && chg->usb_det_pin_mode) //it will auto detect Device connect and check the Source/SINK in DRP mode
		smblib_err(chg, "usb_det_pin enable, skip drp disable func and OTG timer in smblib.\n");
	else {
		chg->drp_enable = 0;
		chg->power_role_backup_when_drp_disable = -1;
	}

	INIT_DELAYED_WORK(&chg->alternate_charger_update_work, smblib_alternate_charger_update_work);
	strcpy(chg->usb_type_name,smblib_apsd_results[UNKNOWN].name);
	chg->split_current_monitor_status = false;
	chg->hvdcp_type = HVDCP_TYPE_UNKNOWN;
	chg->apsd_rerun_status = WORK_STATUS_IDLE;
...
int smblib_deinit(struct smb_charger *chg)
{
	...
	case PARALLEL_MASTER:
		...
		cancel_delayed_work_sync(&chg->alternate_charger_update_work);
		/* vivo end */
		break;
	case PARALLEL_SLAVE:
		break;
