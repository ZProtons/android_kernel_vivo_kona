...
//vivo comment: copy struct to vivo-smb5-lib.c
struct smb_dt_props {
	int			usb_icl_ua;
...
struct smb5 *smb5_chip = NULL;	//vivo add
...
static int smb5_parse_dt_misc(struct smb5 *chip, struct device_node *node)
{
	...
	chg->usbin_suspend_when_full_charge_enable = false;	//vivo add
	...
	chg->enable_bat_board_temp_from_bq25970 = of_property_read_bool(node,
						"vivo,bat-board-temp-from-bq25970");

	chg->enable_usb_conn_temp_from_bq25970 = of_property_read_bool(node,
						"vivo,usb-conn-temp-from-bq25970");

	chg->non_use_smb_therm_adc = of_property_read_bool(node,
						"vivo,non-use-smb-therm-adc");
	pr_err("enable_bat_board_temp_from_bq25970=%d, enable_usb_conn_temp_from_bq25970=%d, non_use_smb_therm_adc=%d\n", \
					chg->enable_bat_board_temp_from_bq25970, chg->enable_usb_conn_temp_from_bq25970, chg->non_use_smb_therm_adc);
	...
	/* read the alternate_charger power supply name */
	rc = of_property_read_string(node, "vivo,alternate_charger_name",
						&chg->alternate_charger_psy_name);
	if (rc) {
		chg->alternate_charger_enable = false;
		chg->alternate_charger_psy_name = "unknow";
	} else
		chg->alternate_charger_enable = true;
	pr_err("alternate_charger_psy_name=%s, alternate_charger_enable=%d.\n", chg->alternate_charger_psy_name, chg->alternate_charger_enable);
	...
	rc = of_property_read_u32(node, "vivo,equalizing-chg-tbat-limit", &chg->equalizing_chg_tbat_limit);
	if (rc < 0)
		chg->equalizing_chg_tbat_limit = EQUALIZING_CHG_TEMP_LIMIT_DEF;

	rc = of_property_read_u32(node, "vivo,equalizing-chg-tboard-limit", &chg->equalizing_chg_tboard_limit);
	if (rc < 0)
	...
	{
		chg->supported_adapter = SUPPORTED_ADAPTER_VIVO_HV_DCHG_10V_4000MA;
	}else {
		chg->supported_adapter = SUPPORTED_ADAPTER_VIVO_HV_DCHG_10V_2250MA;
		chg->current_spec_min = 3500;
		chg->current_spec_max = 4600;
	}
	scnprintf(chg->charging_info, PAGE_SIZE, "%s, spec:[%d, %d].", supprot_adapter_strings[chg->supported_adapter], chg->current_spec_min, chg->current_spec_max);
	#endif
	...
	///LCM Çý¶¯IC£¬ÔÚÂú³ä½ØÖ¹ºó£¬»á³öÏÖÉÁÆÁÎÊÌâ£ºÔö¼ÓÒÔÏÂGPIO£¬À­¸ßGPIOÔö¼ÓLCMÇý¶¯IC¸ºÔØ£¬¹æ±ÜÉÁÆÁ.
	chg->lcm_consume_mos_enable = of_property_read_bool(node, "vivo,lcm-consume-mos-enable");
	if (chg->lcm_consume_mos_enable) {
		chg->lcm_consume_mos_gpio = of_get_named_gpio(node, "vivo,lcm-consume-mos-gpio", 0);
		if (chg->lcm_consume_mos_gpio < 0) {
			pr_err("lcm_consume_mos_gpio not defined\n");
			chg->lcm_consume_mos_gpio = 0;
		} else
			chg->lcm_consume_mos_gpio_read_value = gpio_get_value(chg->lcm_consume_mos_gpio);
		pr_err("lcm_consume_mos_gpio =  %d, lcm_consume_mos_gpio_read_value = %d.\n", chg->lcm_consume_mos_gpio, chg->lcm_consume_mos_gpio_read_value);
	}

	/*
	rc = of_property_read_u32(node,"vivo,quiet-therm-chan",&chg->quiet_therm_chan);
	if (rc) {
		chg->quiet_therm_chan = 0x50;
		pr_err("Unable to read vivo,quiet-therm-chan, rc=%d.\n", rc);
	}
	*/
	...
	rc = of_property_read_u32(node, "vivo,ffc-full-chg-vbat",
				   &(chg->ffc_full_chg_vbat_uv));
	if (rc) {
		pr_err("Unable to read vivo,ffc-full-chg-vbat, ret=%d.\n", rc);
		chg->ffc_full_chg_vbat_uv = 4400000;
	}
	pr_err("ffc_chg_term_current_ma=%d, ffc_fv_max_uv=%d, ffc_full_chg_vbat_uv=%d\n", chg->ffc_chg_term_current_ma, chg->ffc_fv_max_uv, chg->ffc_full_chg_vbat_uv);

	chg->ffc_full_chg_ibat_ma_len = of_property_count_elems_of_size(node, "vivo,ffc-full-chg-ibat", sizeof(u32));
	if (chg->ffc_full_chg_ibat_ma_len <= 0) {
		pr_warn("Failed to read node of vivo,ffc-full-chg-ibat (%d)\n", chg->ffc_full_chg_ibat_ma_len);
	} else {
		chg->ffc_full_chg_ibat_ma = (int *)devm_kzalloc(chg->dev, sizeof(int) * chg->ffc_full_chg_ibat_ma_len, GFP_KERNEL);
		rc = of_property_read_u32_array(node, "vivo,ffc-full-chg-ibat",
			   chg->ffc_full_chg_ibat_ma, chg->ffc_full_chg_ibat_ma_len);
		if (rc) {
			pr_err("Unable to read vivo,ffc-full-chg-ibat, ret=%d.\n", rc);
			chg->ffc_full_chg_ibat_ma = NULL;
			chg->ffc_full_chg_ibat_ma_len = 0;
		}
	}

	chg->ffc_temperature_range_len = of_property_count_elems_of_size(node, "vivo,ffc-temperature-range", sizeof(u32));
	if (chg->ffc_temperature_range_len <= 0) {
		pr_warn("Failed to read node of vivo,ffc-temperature-range (%d)\n", chg->ffc_temperature_range_len);
	} else {
		chg->ffc_temperature_range = (int *)devm_kzalloc(chg->dev, sizeof(int) * chg->ffc_full_chg_ibat_ma_len, GFP_KERNEL);
		rc = of_property_read_u32_array(node, "vivo,ffc-temperature-range",
					chg->ffc_temperature_range, chg->ffc_temperature_range_len);
		if (rc) {
			pr_err("Unable to read vivo,ffc-temperature-range, ret=%d.\n", rc);
			chg->ffc_temperature_range = NULL;
			chg->ffc_temperature_range_len = 0;
		}
	}
	
	if (chg->ffc_temperature_range_len > 0 && chg->ffc_full_chg_ibat_ma_len > 0 && chg->ffc_temperature_range_len == 2*chg->ffc_full_chg_ibat_ma_len && chg->ffc_temperature_range && chg->ffc_full_chg_ibat_ma) {
		for (i = 0; i < chg->ffc_full_chg_ibat_ma_len; i++) {
			pr_err("FFC data: ffc_temperature_range=[%d, %d] ffc_full_chg_ibat_ma=%d \n",
				chg->ffc_temperature_range[i*2], chg->ffc_temperature_range[i*2+1], chg->ffc_full_chg_ibat_ma[i]);
		}
	} else
		pr_err("get ffc data error from dtsi!!!.\n");

	/*usb_det_pin_mode: detect id enable*/
	chg->usb_det_pin_enable = of_property_read_bool(node, "vivo,usb-det-pin-mode-enable");
	if (chg->usb_det_pin_enable) {
		...
		chg->usb_det_pin_mode = USB_DET_PIN_MODE_NONE;
	pr_err("usb_det_pin %s, usb_det_pin_mode=%d.\n", chg->usb_det_pin_enable ? "enabled" : "disabled", chg->usb_det_pin_mode);

	/* vivo add end */
	chg->soc_decimal_enable = of_property_read_bool(node,
						"vivo,soc-decimal-enable");
	/*vivo end*/
	return 0;
}
...
static int smb5_parse_dt_voltages(struct smb5 *chip, struct device_node *node)
{
	...
	/*VIVO add*/
	rc = of_property_read_u32(node,
				"vivo,Rafvc-mO", &chip->chg.Rafvc_mO);
	if (rc < 0)
		chip->chg.Rafvc_mO = -EINVAL;
...
static enum power_supply_property smb5_usb_props[] = {
	...
	POWER_SUPPLY_PROP_FORCE_USB_ICL,
	/* vivo add end */
};
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ONLINE:
		...
		if ((power_off_charging_mode && chg->usb_present) ||
			(chg->alternate_charger_enable && chg->usb_present && chg->real_charger_type == POWER_SUPPLY_TYPE_UNKNOWN)) {
			val->intval = 1;
		}
		/*vivo end*/
		break;
	...
	case POWER_SUPPLY_PROP_REAL_TYPE:
		if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
			if (chg->real_charger_type != POWER_SUPPLY_TYPE_USB_DCP || chg->real_charger_type != POWER_SUPPLY_TYPE_USB_HVDCP)
				smblib_update_usb_type(chg);	//update cable_type quickly when no DCP

			if (chg->apsd_rerun_status != WORK_STATUS_END && chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT)
				val->intval = POWER_SUPPLY_TYPE_UNKNOWN;	//before apsd_end, force Float cable to unknow for other driver(ex: usb driver), avoid USB Enum influence the D+ volt.
			else
	...
	case POWER_SUPPLY_PROP_FIXED_TEMP:
		...
		break;
	case POWER_SUPPLY_PROP_FORCE_USB_ICL:
		if (chg->force_usb_icl <= 0)
			val->intval = chg->force_usb_icl;
		else
			rc = smblib_get_charge_param(chg, &chg->param.usb_icl,
								&val->intval);
		pr_err("force_usb_icl=%d, val=%d\n", chg->force_usb_icl, val->intval);
		break;
	case POWER_SUPPLY_PROP_BC1P2_RESULT:
		val->intval = chg->bc1p2_result;
	/* vivo add end */
	default:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_TYPEC_POWER_ROLE:
		 if (chg->usb_det_pin_mode) {	//vivo add
			if (!get_effective_result_locked(chg->drp_mode_votable)) {
				pr_err("usb_det_pin_mode=%d: vote drp_enable=0, skip...[val=%d]\n", chg->usb_det_pin_mode, val->intval);
				break;
			}
		}
	...
	case POWER_SUPPLY_PROP_TYPEC_SWITCH:
		if (chg->usb_det_pin_mode) {
			...
			if (chg->drp_enable) {
				//if (chg->power_role_backup_when_drp_disable > 0) {
					value.intval = POWER_SUPPLY_TYPEC_PR_DUAL;	//chg->power_role_backup_when_drp_disable;
	...
	case POWER_SUPPLY_PROP_USB_DET_PIN_MODE:
		if (chg->usb_det_pin_enable) {
			pr_err("usb_det_pin_mode:set %d->%d...\n", chg->usb_det_pin_mode, val->intval);
			chg->usb_det_pin_mode = val->intval;
			...
			pr_err("Try setting usb_det_pin_mode[%d->%d] but pin is not enabled, skip\n", chg->usb_det_pin_mode, val->intval);
		break;
	...
	case POWER_SUPPLY_PROP_FIXED_TEMP:
		...
		break;
	case POWER_SUPPLY_PROP_FORCE_USB_ICL:
		chg->force_usb_icl = val->intval;	//vivo add:  uA
		pr_err("setting force_usb_icl=%d\n", chg->force_usb_icl);
		break;
	case POWER_SUPPLY_PROP_SETTING_USBIN_OVP:
		if (chg->alternate_charger_enable && chg->alternate_charger_psy)
			power_supply_set_property(chg->alternate_charger_psy, psp, val);
		break;
	case POWER_SUPPLY_PROP_DDPM_RESET:
		...
		break;
	case POWER_SUPPLY_PROP_PE_START:
		chg->ok_to_pd = val->intval;
		pr_err("set ok_to_pd=%d\n", chg->ok_to_pd);
		break;
	case POWER_SUPPLY_AI_CHARGE_STAE:
		chg->ai_disable_charge_state = val->intval;
		pr_err("set ai charge state=%d\n", chg->ai_disable_charge_state);
		break;
	case POWER_SUPPLY_PROP_AUDIO_ATTACHED:
		if (val->intval) {
			chg->audio_attached = true;
			rc = smblib_request_dpdm(chg, !val->intval);
			pr_debug("Audio attach report accepted\n");
			if (rc < 0)
				pr_err("Couldn't request DPDM rc=%d\n", rc);
		} else {
			chg->audio_attached = false;
			pr_debug("Audio dettach report accepted\n");
		}
		break;
	/* vivo end */
	default:
		...
		enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_APSD_RERUN:
	case POWER_SUPPLY_PROP_USB_DET_PIN_MODE:
	case POWER_SUPPLY_PROP_FORCE_USB_ICL:	//vivo add
		return 1;
	default:
		break;
	}
		...
		union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HOT_TEMP:
		...
		break;
	case POWER_SUPPLY_PROP_ALTERNATE_CHARGER:
		val->intval = (chg->alternate_charger_enable) ? 1 : 0;
		break;
	default:
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_CONSTANT_CHARGE_CURRENT_MAX:
		if (smblib_set_alternate_charger_fcc(chg, val->intval/1000))
			break;
		else {
		...
		break;
	case POWER_SUPPLY_PROP_CURRENT_MAX:
		if (smblib_set_alternate_charger_icl(chg, val->intval/1000))
			break;
		else
...
static enum power_supply_property smb5_batt_props[] = {
	...
	POWER_SUPPLY_PROP_VOLTAGE_NOW,
	POWER_SUPPLY_PROP_VOLTAGE_OCV,	//vivo add
	POWER_SUPPLY_PROP_VOLTAGE_MAX,
	...
	/* vivo add for VTS test [B200818-1852] */
		...
		union power_supply_propval *val)
{
	...
	union power_supply_propval mAh = {0, };/* vivo add */
	static int ibat_avg = 0;	//vivo add
	...
	case POWER_SUPPLY_PROP_STATUS:
		...
		if((chg->recharge_flag == true || chg->charge_full == true || chg->ffc_charge_full == true) && pval.intval == 100){
			...
			if (chg->dchg_status == DIRECT_CHARGER_PREPARE ||
				...
				|| chg->fg_disable_charging
				|| chg->ai_disable_charge_state)
				val->intval = POWER_SUPPLY_STATUS_CHARGING;
		}else if(chg->ai_disable_charge_state)
			val->intval = POWER_SUPPLY_STATUS_CHARGING;

		if (chg->lcm_consume_mos_enable) {
			if (chg->lcm_consume_mos_gpio > 0) {
				if (val->intval == POWER_SUPPLY_STATUS_FULL && chg->usb_present) {
					if (0 == chg->lcm_consume_mos_gpio_setting_value) {
						chg->lcm_consume_mos_gpio_setting_value = 1;	//À­¸ßGPIO					
						...
						//}
						pr_err("lcm_consume_mos_gpio_setting_value=%d, lcm_consume_mos_gpio_read_value=%d, lcm_consume_mos_gpio_control_error_count=%d.\n",
		...
		break;
	/* vivo add
		AT+BK_CHG_WPC=0 : ²éÑ¯ÕæÊµµÄµç³Ø³äµç×´Ì¬*/
	case POWER_SUPPLY_PROP_STATUS_EX:
		if(!bsp_test_mode && chg->ffc_charge_full) {
			val->intval = POWER_SUPPLY_STATUS_FULL;
		} else {
		...
		break;
	case POWER_SUPPLY_PROP_FFC_CHARGE_FULL:
		val->intval = chg->ffc_charge_full;
		break;
	/* vivo end */
	case POWER_SUPPLY_PROP_HEALTH:
	...
	case POWER_SUPPLY_PROP_SW_JEITA_ENABLED:
		...
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_NOW:
		val->intval = 0;	//vivo add 
		if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
			if (chg->bms_psy && !strcmp(chg->bms_psy->desc->name, chg->bms_psy_name)) {
				rc = smblib_get_prop_from_bms(chg,
					POWER_SUPPLY_PROP_VOLTAGE_NOW, val);
			} else {
				rc = power_supply_get_property(chg->alternate_charger_psy, POWER_SUPPLY_PROP_CHG_VOLTAGE, val);
				pr_err("get voltage_now from alternate_charger_psy. val->intval=%d.\n", val->intval);
			}
		...
		chg->vbat_uV = val->intval;
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MAX:
		if (chg->alternate_charger_enable && chg->alternate_charger_psy)
			rc = power_supply_get_property(chg->alternate_charger_psy, psp, val);
		else
	...
	case POWER_SUPPLY_PROP_CURRENT_NOW:
		...
		//vivo add
		if ((jiffies - chg->ibat_buf.last_update_jiffies)/HZ > 100)	//if > 100s no update Ibat, reset.
			ibat_circ_buf_clr(&chg->ibat_buf);

		if ((jiffies - chg->ibat_buf.last_update_jiffies)/HZ >= 1) //>1s: because FG side SRAM update every 1s
			ibat_circ_buf_add(&chg->ibat_buf, val->intval);
		break;
	//vivo add start
	case POWER_SUPPLY_PROP_DEBUG_INFO:	//PD2024 temp use for get Ibat_avg.
	//case POWER_SUPPLY_PROP_CURRENT_AVG:
		...
		if ((jiffies - chg->ibat_buf.last_update_jiffies)/HZ > 1) {
			rc = smblib_get_batt_current_now(chg, &pval);
			ibat_circ_buf_add(&chg->ibat_buf, pval.intval);
		}

		if ((jiffies - chg->ibat_buf.last_read_jiffies)/HZ >= IBAT_CURRENT_AVG_BUFFER_SIZE)	//update to UI side.
			ibat_circ_buf_avg(&chg->ibat_buf, &ibat_avg);

		scnprintf(chg->debug_info, PAGE_SIZE, "Current_Average: %d.%d", ibat_avg/1000, abs(ibat_avg)%1000);	//mA
		val->strval = chg->debug_info;
		break;
	//vivo add end
	...
	case POWER_SUPPLY_PROP_TEMP:
		if (chg->typec_mode == POWER_SUPPLY_TYPEC_SINK_DEBUG_ACCESSORY)
			...
			if (!bbk_fixed_temp)
				...
				chg->battery_temprature = val->intval;
			/* vivo modify end */
		}
		/*rc = smblib_get_prop_from_bms(chg,
		...
		if (chg->qnovo_disable_votable)
			val->intval =
				!get_effective_result(
					chg->qnovo_disable_votable);
		break;
	case POWER_SUPPLY_PROP_CHARGE_FULL_DESIGN:/* vivo add */
	...
	case POWER_SUPPLY_PROP_PRIMARY_BOARD_TEMP:
		...
		chg->main_board_temprature = val->intval;
		break;
	...
	case POWER_SUPPLY_PROP_VOLTAGE_MAX_DESIGN:
		...
		if (chg->vbat_uV > 5000000 &&  val->intval < 5000000) {
			val->intval = 9000000;
		}
		break;
	case POWER_SUPPLY_PROP_CHARGE_NOW_RAW:
		rc = smblib_get_prop_from_bms(chg,
				POWER_SUPPLY_PROP_CHARGE_NOW_RAW, val);
		break;
	/* add for vivo super FlashCharge */
	...
	case POWER_SUPPLY_PROP_CHARGING_SPEED:
		...
		break;
	case POWER_SUPPLY_PROP_BOARD_TEMP_STATE:
		rc = smblib_get_equalizing_chg_temp_state(chg, val);
		break;
	case POWER_SUPPLY_PROP_BAT_FCC:
		rc = smblib_get_prop_batt_fcc(chg, &mAh);
		if (rc < 0) {
			val->intval = 0;
			pr_err("Couldn't get fcc rc=%d\n", rc);
		} else {
			val->intval = mAh.intval;
			pr_err("get fcc mAh=%d\n", mAh.intval);
		}
		break;
	case POWER_SUPPLY_PROP_BAT_RM:
		rc = smblib_get_prop_batt_rm(chg, &mAh);
		if (rc < 0) {
			val->intval = rc;
			pr_err("Couldn't get rm capacity rc=%d\n", rc);
		} else {
			val->intval = mAh.intval;
			pr_err("get rm mAh=%d\n", mAh.intval);
		}
	...
	case POWER_SUPPLY_CURRENT_SPEC_MAX:
		...
		break;
	case POWER_SUPPLY_PROP_EX_FG_STATE:
		val->intval = ex_fg_state;
		break;
	#endif
	case POWER_SUPPLY_PROP_TIME_TO_FULL_NOW:
		val->intval = 100; /* add fixed return value for VTS test */
		break;
	case POWER_SUPPLY_PROP_AGING_SUPPORT:
		rc = smblib_get_prop_from_qcom_fg(chg, POWER_SUPPLY_PROP_AGING_SUPPORT, val);
	/*vivo end*/
	default:
		...
		const union power_supply_propval *val)
{
	...
	*/
	...
	case POWER_SUPPLY_PROP_DCHG_LIMIT_MA:
		pr_err("### POWER_SUPPLY_PROP_DCHG_LIMIT_MA=%d mA ###\n", val->intval);
		vote(chg->usb_icl_votable, DCHG_LIMIT, (val->intval > 0), val->intval*1000);	//need uA
		break;
	case POWER_SUPPLY_PROP_KEEP_CHG_SOC:
	...
	case POWER_SUPPLY_PROP_FFC_CHARGE_FULL:
		if (chg->usb_present)
			chg->ffc_charge_full = val->intval;
		break;
	case POWER_SUPPLY_PROP_DIRECT_CHG_BYPASS_TO_SWITCH_CHG:	//settint mA
		if (chg->alternate_charger_enable && chg->alternate_charger_psy) {
			rc = power_supply_set_property(chg->alternate_charger_psy, prop, val);
		}
		rc |= vote(chg->usb_icl_votable, USER_DEFAULT_INPUT_VOTER, true, val->intval*1000);	//need uA
		break;
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		if (val->intval >= 3000000 && val->intval <= 3450000)
			chg->cutoff_volt_mv = val->intval;
		else
			chg->cutoff_volt_mv = 3400000;
		break;
	/*vivo end*/
	default:
		rc = -EINVAL;
...
static int smb5_init_hw(struct smb5 *chip)
{
	...
	if (chg->alternate_charger_enable) {
		...
		if (rc < 0)
			pr_err("Couldn't write USBIN_SUSPEND_BIT rc=%d\n", rc);
	}
	
	if (chip->chg.Rafvc_mO >= 0)	{//vivo add: 4450mV Battery Project.
		rc = smblib_write(chg, DCDC_AFVC_GAIN_CFG, chip->chg.Rafvc_mO/2);	//16mO
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't config DCDC_AFVC_GAIN_CFG rc=%d\n",	rc);
		}
	}
	/* vivo add end */
...
#if 1	//vivo_chg add: set AICL 4.2V default
	dev_err(chg->dev,
		"default_aicl_5v_threshold_mv=%d, default_aicl_cont_threshold_mv=%d. will set it to 4.2V default\n", chg->default_aicl_5v_threshold_mv, chg->default_aicl_cont_threshold_mv);

	chg->default_aicl_5v_threshold_mv = 4200;
	chg->aicl_5v_threshold_mv = 4200;
	chg->default_aicl_cont_threshold_mv = 4200;
	chg->aicl_cont_threshold_mv = 4200;
	...
	/*vivo add start*/
	if (chip->dt.batt_profile_fcc_ua > 0) {
		rc = smblib_set_charge_param(chg, &chg->param.fcc, chip->dt.batt_profile_fcc_ua);
		if (rc < 0) {
			pr_err("Couldn't set fcc current  rc=%d\n", rc);
		}
	}
	/*vivo add end*/
	...
	/* vivo modify */
	vote(chg->fv_votable,
		BATT_PROFILE_VOTER, false,
		chg->batt_profile_fv_uv);

	vote(chg->fcc_main_votable, HW_LIMIT_VOTER, chip->dt.batt_profile_fcc_ua > 0, chip->dt.batt_profile_fcc_ua);
	/* vivo end */
	...
	/*vivo change:  HW AICL rerun  time config in AT boot */
	if (is_atboot)
		/*set to 3s*/
		rc = smblib_write(chg, AICL_RERUN_TIME_CFG_REG,
...
static int smb5_post_init(struct smb5 *chip)
{
	...
	//VIVO_add: keep default SINK mode
	if (!chg->usb_det_pin_mode) {
		if (!chg->drp_enable) {
			rc = smblib_masked_write(chg, TYPE_C_MODE_CFG_REG,
						...
						EN_SNK_ONLY_BIT);
			if (rc < 0) {
				dev_err(chg->dev, "Couldn't write  SINK  to TYPE_C_INTRPT_ENB_SOFTWARE_CTRL rc=%d\n", rc);
			}
		}
	}
	//VIVO_add end
	...
	/*vivo modify*/
	//if (chg->usb_det_pin_mode && chg->usb_present)
	//	pval.intval = POWER_SUPPLY_TYPEC_PR_DUAL;
	//else
...
static struct smb_irq_info smb5_irqs[] = {
	...
	[AICL_DONE_IRQ] = {
		.name		= "aicl-done",
		.handler	= smblib_handle_aicl_done,/*default_irq_handler,*/
	},
	[SMB_EN_IRQ] = {
		.name		= "smb-en",
...
static void smb5_shutdown(struct platform_device *pdev)
{
	...
	if (chg->usbin_suspend_when_full_charge_enable)	{//VIVO add
		chg->usbin_full_charge_suspend = false;
		smblib_masked_write(chg, USBIN_CMD_IL_REG, USBIN_SUSPEND_BIT, 0);	//enable USBIN
	}
