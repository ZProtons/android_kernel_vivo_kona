	...
	if (!bl_update) {
		...
		if (retry_time < 3) {
			...
			if ((ts->ic_info[6] != tBuff[2]) || (ts->ic_info[7] != tBuff[3])) {
				...
				if (ts->ic_info[3] == 0x50) {
					ret = sec_ts_force_calibration_after_update(ts);
					if (ret < 0)
						VTE("fail to write OFFSET CAL SEC!");
					ts->sec_ts_i2c_read(ts, SEC_TS_READ_CALIBRATION_REPORT, tBuff, 8);
					memcpy(ts->ic_info, tBuff, 8);
					VTE("second force calibrate statue %d, %d, %d, %d, %d, %d, %d, %d", ts->ic_info[0], ts->ic_info[1], ts->ic_info[2], ts->ic_info[3],
						ts->ic_info[4], ts->ic_info[5], ts->ic_info[6], ts->ic_info[7]);
				}
			} else {
