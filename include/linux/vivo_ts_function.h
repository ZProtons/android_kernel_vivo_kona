...
enum vts_screen_cmd_type {
	VTS_SCREEN_SET_NO_CMD,
	VTS_SCREEN_SET_EDGE_AND_DCLICK_AREA,
	VTS_SCREEN_SET_REPORT,
	VTS_SCREEN_SET_DCLICK_AREA,
	VTS_SCREEN_CMD_COUNT
};
...
struct VtsScreenClockRect {
	struct mutex  ClockRectMutex;
	struct VtsScreenClockPoint startPoint;
	int width;
	int height;
	int enable;
	};
...
struct vivo_ts_struct {
	...
	int (*processByPackage)(unsigned char *package_name, int type);
	...
	int (*get_st_enable)(void);
	int (*set_st_enable)(int state);
	int (*get_st_state)(void);
	...
	/*add for gesture report point state */
	struct VtsScreenClockPoint  *screenClockPoint;
	struct VtsScreenClockRect   *screenClockRect;
	unsigned int hasScreenClock;
	unsigned int supportsCreenclock; // config in dtsi 
	unsigned int pointX;
	unsigned int pointY;
	unsigned int hight_scan_support;
	unsigned int hight_scan_rate;
	int (*readScrenClockEdge)(void);
	int (*sendScreenReportCmd)(void);
...
extern void vivoScreenPointSync(int x,int y);
extern int  vivoVtsScreenClockint(void);
extern void vivoVtsScreenClockfree(void);
extern int VivoGetScreenCmdType(char *app_name);
