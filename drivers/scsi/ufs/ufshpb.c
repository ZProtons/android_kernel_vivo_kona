...
static void ufshpb_pre_req_compl_fn(struct request *req, blk_status_t error)
{
	...
	if (error) {
		...
		if (req->q->mq_ops)
			scsi_normalize_sense(scsi_req(req)->sense, SCSI_SENSE_BUFFERSIZE,
					&sshdr);
		else
...
extern int scsi_mq_prep(struct request *req);

static void ufshpb_pre_req_done_mq(struct scsi_cmnd *cmd)
{
	blk_mq_complete_request(cmd->request);
}

static int ufshpb_mimic_scsi_request_mq(struct ufshpb_lu *hpb,
					struct request *req,
					struct ufshpb_req *pre_req)
{
	...
	cmd->scsi_done = ufshpb_pre_req_done_mq;
	atomic_inc(&cmd->device->iorequest_cnt);
	scsi_log_send(cmd);
			      ...
			      struct ufshpb_req *pre_req, int hpb_ctx_id)
{
	...
	if (q->mq_ops) {
		req = blk_get_request(q, REQ_OP_SCSI_OUT,
				BLK_MQ_REQ_NOWAIT | BLK_MQ_REQ_DRIVER | BLK_MQ_REQ_PREEMPT);
		if (IS_ERR(req)) {
			pr_debug("%s: can't get request %d\n", __func__, PTR_ERR(req));
			return -EIO;
		}
		pre_req->req_mq = req;
	} else {
	...
	if (q->mq_ops)
		ret = ufshpb_mimic_scsi_request_mq(hpb, req, pre_req);
	else
...
static int ufshpb_map_req_error(struct ufshpb_req *map_req, struct request *req)
{
	struct ufshpb_lu *hpb = map_req->hpb;
	struct ufshpb_region *rgn;
	struct ufshpb_subregion *srgn;
	...
	if (req->q->mq_ops)
		scsi_normalize_sense(scsi_req(req)->sense, SCSI_SENSE_BUFFERSIZE, &sshdr);
	else
...
static void ufshpb_map_req_compl_fn(struct request *req, blk_status_t error)
{
	...
	if (error) {
		...
		ret = ufshpb_map_req_error(map_req, req);
		if (ret)
			goto retry_map_req;
	} else
