...
struct usbpd *pd_chip = NULL;	//vivo_chg add
...
/*vivo_chg add start */
int pd_check_pdo(int request_vbus_mv)
{
	struct usbpd *pd = pd_chip;
	int i;
	int cnt = 0;
	char buf[1024] = {0,};
	int match_pdo_index = 1;
	...
	for (i = 0; i < ARRAY_SIZE(pd->received_pdos); i++) {
		...
		cnt += scnprintf(&buf[cnt], PAGE_SIZE - cnt, "PDO%d: ", i + 1);

		if (PD_SRC_PDO_TYPE(pdo) == PD_SRC_PDO_TYPE_FIXED) {
			if (request_vbus_mv == PD_SRC_PDO_FIXED_VOLTAGE(pdo) * 50) {
				match_pdo_index = i+1;
			}
			cnt += scnprintf(&buf[cnt], PAGE_SIZE - cnt,
					"Fixed supply:"
					"DRP:%d,"
					"USB_Suspend_Supported:%d,"
	...
	usbpd_err(&pd->dev, "request_vbus_mv=%d,match_pdo=%d.. %s\n", request_vbus_mv, match_pdo_index, buf);
	return match_pdo_index;
}

int  pd_select_pdo_for_external_charger(int pdo)	//min = 1
{
	struct usbpd *pd = pd_chip;
	int ret = 0;
	...
	if (!val.intval && !pd->pd_connected &&
		...
		if (val.intval == POWER_SUPPLY_TYPE_USB ||
			...
			if (val.intval != POWER_SUPPLY_TYPE_USB_FLOAT) {
				...
				usbpd_info(&pd->dev, "ignore float type notify to usb\n");
		}
