...
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
#include "iris/dsi_iris5_api.h"
#include <video/mipi_display.h>
/* #endif */
/* vivo display team add for pixelworks memc end */
...
#ifdef PANEL_BOARD_POWER_CTRL
extern int vivo_panel_power_on(struct dsi_panel *panel);
#endif
...
static irqreturn_t dsi_display_panel_te_irq_handler(int irq, void *data)
{
	...
	struct vivo_display *vdisp = NULL;
	...
	if (!display)
		return IRQ_HANDLED;
	//vivo keli add for ud begin
	vdisp = display->panel->panel_private;
	if (atomic_read(&vdisp->hbm_wait_te_status) == DSI_PANEL_WAIT_ONE_TE) {
		atomic_set(&vdisp->hbm_wait_te_status, DSI_PANEL_NO_WAIT_TE);
		pr_err("%s:set hbm_wait_te_status = %d", __func__, atomic_read(&vdisp->hbm_wait_te_status));
		wake_up_all(&vdisp->ud_hbm_sync.te_wq);
	}

	if (vdisp && (!IS_ERR_OR_NULL(vdisp->panel_ops.te_irq_handler)))
		vdisp->panel_ops.te_irq_handler(vdisp);
	//vivo keli add for ud end

	if (vdisp->ud_shooter_perf && atomic_read(&after_hbm_count) > 0)
	{
		if (atomic_read(&after_hbm_count) < 3) {
				atomic_inc(&after_hbm_count);
				pr_info("%s:after_hbm_count=%d", __func__, atomic_read(&after_hbm_count));
		}
		if (display->drm_conn && display->drm_conn->encoder && display->drm_conn->encoder->crtc)
			sde_notify_fingerprint_status(display->drm_conn->encoder->crtc);
		else
			pr_info("%s: detect invaild state\n", __func__);
	}
					...
					bool enable)
{
	...
	/* user vivo customer irq enabel/disable function to avoid irq can't enable in te check case */
	ctl_display_te_irq_enable(enable);
}
...
static void dsi_display_parse_te_data(struct dsi_display *display)
{
	...
	pr_err("dsi_display_parse_te_data %d\n", display->disp_te_gpio);
		...
		struct dsi_panel *panel)
{
	...
	pr_info("%s:read esd reg value = 0x%x\n", __func__, config->return_buf[0]);
	return rc;
}
...
/* #if defined(CONFIG_PXLW_IRIS) */
	...
	panel = display->panel;
	if ((!strcmp(panel->name, "dsi_pd2055_amb662_ea8079_fhd_plus_cmd panel")) &&
		iris_is_chip_supported()) {
		memset(register1, 0, sizeof(register1));
		if (iris_is_pt_mode(panel)) {
			rc = iris_get_status();
			if (rc <= 0) {
				DSI_ERR("Iris ESD error\n");
				goto exit;
			}

			cmds = mode->priv_info->cmd_sets[DSI_CMD_SET_PANEL_ID1].cmds;
		...
		pr_info("%s:read esd reg value = 0x%x\n", __func__, register1[0]);
		if (register1[0] != 0x9c) {
			DSI_ERR("ESD error and reg value = 0x%x \n", register1[0]);
			rc = -1;
		} else {
			rc = 1;
		}
...
exit:
	...
	vdisp = display->panel->panel_private;
	if ((rc <= 0 && vdisp && !vdisp->esd.is_check_more)
		|| vdisp->panel_power_mode_state != SDE_MODE_DPMS_ON) {
		DSI_DEBUG("rc=%d,is_check_more=%d,panel_power_mode_state=%d", rc, vdisp->esd.is_check_more, vdisp->panel_power_mode_state);
		rc = 1;//do not need report panel dead
	}
	return rc;
}
...
static int dsi_display_status_check_te(struct dsi_display *display)
{
	...
	if (!wait_for_completion_timeout(&display->esd_te_gate,
		...
		/*Add for vivo LCM driver exception event*/
		atomic_inc(&vivo_exception_count[VIVO_TE_CHECK_FAIL]);
		/*Add for vivo LCM driver exception event*/
		rc = -EINVAL;
	}
					...
					bool te_check_override)
{
	...
	if (!te_check_override) {
		if (vdisp && !vdisp->esd.esd_check_screendown &&
		    !vdisp->esd.last_screendown) {
			DSI_DEBUG("Panel not face down\n");
			if (vdisp->esd.is_power_state_check)
				DSI_DEBUG("Panel resume first check power state,need alow..\n");
			else
				goto release_panel_lock;
		}
		/* add check one more time */
		if (vdisp->esd.esd_check_screendown)
			vdisp->esd.last_screendown = 3;
		else if (vdisp->esd.last_screendown > 0)
			vdisp->esd.last_screendown--;
	}
...
exit:
	...
	if (rc <=0) {
		...
		if (status_mode == ESD_MODE_REG_READ)
			;//lcm_error_code_report(VIVO_IC_POWER_CHECK_FAIL);
		else if (status_mode == ESD_MODE_PANEL_TE)
			lcm_error_code_report(VIVO_TE_CHECK_FAIL);
	}

release_panel_lock:
	if(vdisp)
		vdisp->esd.is_power_state_check = false;
		...
		int power_mode, void *disp)
{
	...
	if (!rc) {
		...
		vdisp->panel_power_mode_state = power_mode;
	}
				 ...
				 const struct mipi_dsi_msg *msg)
{
	...
	if (display->ctrl_count > 1 && !(msg->flags & MIPI_DSI_MSG_UNICAST)) {
		...
		/* add by ouenhui for send some command by direct way for sync,start*/
		if (vdisp->dsi_cmd_send_type == DSI_CMD_SEND_BY_DIRECT) {
			cmd_flags &= ~DSI_CTRL_CMD_ASYNC_WAIT;
		}
		/* add by ouenhui for send some command by direct way for sync,end*/
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
		if (iris_is_chip_supported()) {
...
#ifdef PANEL_BOARD_POWER_CTRL
	rc = vivo_panel_power_on(display->panel);
#else
...
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
	if (index == DSI_PRIMARY && (!strcmp(boot_displays[0].name, "qcom,mdss_dsi_pd2055_amb656_ea8079_fhd_plus_cmd")))
...
/*vivo ouenhui add for dynamic gamma function start*/
int dsi_display_find_mode_by_refresh_rate(struct dsi_display *display,
		...
		struct dsi_display_mode **out_mode)
{
	...
	*out_mode = NULL;
	...
	for (i = 0; i < count; i++) {
		...
		if (refresh_rate == m->timing.refresh_rate) {
			*out_mode = m;
			rc = 0;
			pr_info("%s:get out mode successfule index=%d/%d", __func__, i, count);
			break;
		}
	}
	mutex_unlock(&display->display_lock);
	...
	if (!*out_mode) {
		DSI_ERR("[%s] failed to find mode for refresh_rate = %u\n",
				display->name, refresh_rate);
		rc = -ENOENT;
	}
	return rc;
}
/*vivo ouenhui add for dynamic gamma function end*/
...
/*vivo ouenhui add for dynamic gamma function start*/
#define EA8079_GAMMA_WRITE_CMD_NUM		7
#define EA8079_90HZ_CHECKSUM_PART1		344
#define EA8079_90HZ_CHECKSUM_PART2		345
#define EA8079_120HZ_CHECKSUM_PART1		347
#define EA8079_120HZ_CHECKSUM_PART2		348
#define EA8079_90HZ_OTP_GAMMA_PART1		200
#define EA8079_90HZ_OTP_GAMMA_PART2		149
#define EA8079_90HZ_OTP_GAMMA_LENGTH	(EA8079_90HZ_OTP_GAMMA_PART1 + EA8079_90HZ_OTP_GAMMA_PART2)
#define EA8079_120HZ_OTP_GAMMA_PART1_ALL	231
#define EA8079_120HZ_OTP_GAMMA_PART1	44
#define EA8079_120HZ_OTP_GAMMA_PART2	237
#define EA8079_120HZ_OTP_GAMMA_PART3	63
#define EA8079_120HZ_OTP_GAMMA_LENGTH 	(EA8079_120HZ_OTP_GAMMA_PART1 + EA8079_120HZ_OTP_GAMMA_PART2 + EA8079_120HZ_OTP_GAMMA_PART3 )

static int dsi_panel_check_gamma_cmd(struct dsi_panel_cmd_set cmd_set)
{
	struct dsi_cmd_desc *cmds;
	int rc = 0;
	if (!cmd_set.count || cmd_set.count != EA8079_GAMMA_WRITE_CMD_NUM ) {
		DSI_ERR("command count %d error\n", cmd_set.count);
		rc = -EINVAL;
		goto check_error;
	}
	...
	/* B8 cmd need 44 payload */
	if (cmds[2].msg.tx_len != EA8079_B8_GAMMA_LENGTH + 1) {
		DSI_ERR("B8 cmd parameters  setting error\n");
		return -EINVAL;
		goto check_error;
	}

	/* B9 cmd need 237 payload */
	if (cmds[3].msg.tx_len != EA8079_B9_GAMMA_LENGTH + 1) {
		DSI_ERR("B9 cmd parameters setting error\n");
		return -EINVAL;
		goto check_error;
	}

	/* BA cmd need 63 payload */
	if (cmds[4].msg.tx_len != EA8079_BA_GAMMA_LENGTH + 1) {
		DSI_ERR("BA cmd parameters setting error\n");
		return -EINVAL;
		goto check_error;
	}
...
void dsi_panel_update_ea8079_dynamic_gamma(struct dsi_display *display)
{
	struct vivo_display *vdisp = NULL;
	u8 reg_90hz = 0xF6, reg_120hz_part1 = 0xB8, reg_120hz_part2 = 0xB9, reg_120hz_part3 = 0xBA;
	u32 checksum = 0x00;
	int index,rc;
	u8 *buf = NULL;
	struct ea8079_dynamic_gamma_cfg *gamma_cfg = NULL;

	if (IS_ERR_OR_NULL(display)) {
		DSI_ERR("%s:display is null", __func__);
		return ;
	}
	vdisp = display->panel->panel_private;
	if (IS_ERR_OR_NULL(vdisp)) {
		DSI_ERR("%s:vdisp is null", __func__);
		return ;
	}
	buf = vdisp->ea8079_dynamic_gamma.temp_buf;
	gamma_cfg = &vdisp->ea8079_dynamic_gamma;
	gamma_cfg->gamma_90hz_valid = false;
	gamma_cfg->gamma_120hz_valid = false;
	if (IS_ERR_OR_NULL(buf) || IS_ERR_OR_NULL(gamma_cfg)
		|| IS_ERR_OR_NULL(vdisp->panel_ops.dsi_write_dcs_cmd)
		|| IS_ERR_OR_NULL(vdisp->panel_ops.dsi_read_cmd)) {
		DSI_ERR("%s:error(%p,%p,%p,%p)", __func__, buf, gamma_cfg, vdisp->panel_ops.dsi_write_dcs_cmd, vdisp->panel_ops.dsi_read_cmd);
		return ;
	}
	//read 90hz gamma part1
	//level3 key access enable & flash read enable
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFC, (u8[]){ 0x5A, 0x5A }, 2);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xB0, (u8[]){ 0x10 }, 1);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFF, (u8[]){ 0x02, 0x70 }, 2);
	//read part1 200 bytes
	rc = vdisp->panel_ops.dsi_read_cmd(display, reg_90hz, gamma_cfg->temp_buf, EA8079_90HZ_OTP_GAMMA_PART1);
	if (rc < 0) {
		DSI_ERR("dsi read 90hz gamma part1 error,rc=%d", rc);
		goto read_90hz_gamma_error;
	} else {
		DSI_INFO("read 90hz gamma part1:rc=%d.data=", rc);
		for (index = 0; index < EA8079_90HZ_OTP_GAMMA_PART1; index++) {
			pr_cont("0x%02X ", gamma_cfg->temp_buf[index]);
		}
	}
	//flash read disable & level3 key access disable
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xB0, (u8[]){ 0x10 }, 1);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFF, (u8[]){ 0x00, 0x70 }, 2);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFC, (u8[]){ 0xA5, 0xA5 }, 2);

	//read 90hz gamma part2
	//level3 key access enable
	//flash read enable
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFC, (u8[]){ 0x5A, 0x5A }, 2);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xB0, (u8[]){ 0x10 }, 1);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFF, (u8[]){ 0x02, 0x71 }, 2);
	//read 149 bytes
	rc = vdisp->panel_ops.dsi_read_cmd(display, reg_90hz, &gamma_cfg->temp_buf[EA8079_90HZ_OTP_GAMMA_PART1], EA8079_90HZ_OTP_GAMMA_PART2);
	if (rc < 0) {
		DSI_ERR("dsi read 90hz gamma part2 error,rc=%d", rc);
		goto read_90hz_gamma_error;
	} else {
		DSI_INFO("read 90hz gamma part2:rc=%d.data=", rc);
		for (index = EA8079_90HZ_OTP_GAMMA_PART1; index < EA8079_90HZ_OTP_GAMMA_LENGTH; index++) {
			pr_cont("0x%02X ", gamma_cfg->temp_buf[index]);
		}
		gamma_cfg->gamma_90hz_checksum = gamma_cfg->temp_buf[EA8079_90HZ_CHECKSUM_PART1] << 8 | gamma_cfg->temp_buf[EA8079_90HZ_CHECKSUM_PART2];
		gamma_cfg->gamma_120hz_checksum = gamma_cfg->temp_buf[EA8079_120HZ_CHECKSUM_PART1] << 8 | gamma_cfg->temp_buf[EA8079_120HZ_CHECKSUM_PART2];
		DSI_INFO("gamma_90hz_checksum=0x%X, gamma_120hz_checksum = 0x%X\n", gamma_cfg->gamma_90hz_checksum, gamma_cfg->gamma_120hz_checksum);
	}
	//flash read disable
	//level3 key access disable
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xB0, (u8[]){ 0x10 }, 1);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFF, (u8[]){ 0x00, 0x70 }, 2);
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xFC, (u8[]){ 0xA5, 0xA5 }, 2);
	//compare checksum
	checksum = 0;
	for(index = 0;index < EA8079_90HZ_OTP_GAMMA_LENGTH - 5;index++) {
		checksum += gamma_cfg->temp_buf[index];
	}
	DSI_INFO("cal 90hz checksum is 0x%X\n", checksum);
	if (gamma_cfg->gamma_90hz_checksum == checksum)
		DSI_INFO("90Hz gamma parameters check successuful\n");
	else {
		DSI_INFO("90Hz gamma parameters check fail\n");
		goto read_90hz_gamma_error;
	}
	//
	gamma_cfg->gamma_90hz_valid = true;

	//fill in cmd sets
	if (gamma_cfg->gamma_90hz_valid)
	{
		struct dsi_display_mode *mode;
		struct dsi_cmd_desc *cmds;
		u8 *tx_buf = NULL;
		rc = dsi_display_find_mode_by_refresh_rate(display, 90, &mode);
		if (rc) {
			DSI_ERR("can't find 90hz mode.");
			goto read_90hz_gamma_error;
		}
		if (dsi_panel_check_gamma_cmd(mode->priv_info->cmd_sets[DSI_CMD_SET_90HZ_GAMMA]))
			goto read_90hz_gamma_error;
		cmds = mode->priv_info->cmd_sets[DSI_CMD_SET_90HZ_GAMMA].cmds;
		tx_buf = (u8 *)cmds[2].msg.tx_buf;
		memcpy(&tx_buf[1], gamma_cfg->temp_buf, EA8079_B8_GAMMA_LENGTH);
		tx_buf = (u8 *)cmds[3].msg.tx_buf;
		memcpy(&tx_buf[1], &gamma_cfg->temp_buf[EA8079_B8_GAMMA_LENGTH], EA8079_B9_GAMMA_LENGTH);
		tx_buf = (u8 *)cmds[4].msg.tx_buf;
		memcpy(&tx_buf[1], &gamma_cfg->temp_buf[EA8079_B8_GAMMA_LENGTH + EA8079_B9_GAMMA_LENGTH], EA8079_BA_GAMMA_LENGTH);
	}

	//read 120hz gamma
	//level2 key access enable
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xF0, (u8[]){ 0x5A, 0x5A }, 2);
	//vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,	0xB0, (u8[]){ 0xBB }, 1);
	//read 120hz part1
	rc = vdisp->panel_ops.dsi_read_cmd(display, reg_120hz_part1, gamma_cfg->temp_buf, EA8079_120HZ_OTP_GAMMA_PART1_ALL);
	if (rc < 0) {
		DSI_ERR("dsi read 120hz gamma part1 error rc=%d", rc);
		goto read_120hz_gamma_error;
	} else {
		DSI_INFO("read 120hz gamma part1:rc=%d.data=", rc);
		for (index = 0; index < EA8079_120HZ_OTP_GAMMA_PART1; index++) {
			gamma_cfg->temp_buf[index] = gamma_cfg->temp_buf[index + EA8079_120HZ_OTP_GAMMA_PART1_ALL - EA8079_120HZ_OTP_GAMMA_PART1];
			pr_cont("0x%02X ", gamma_cfg->temp_buf[index]);
		}
	}
	//read 120hz part2
	rc = vdisp->panel_ops.dsi_read_cmd(display, reg_120hz_part2, &gamma_cfg->temp_buf[EA8079_120HZ_OTP_GAMMA_PART1], EA8079_120HZ_OTP_GAMMA_PART2);
	if (rc < 0) {
		DSI_ERR("dsi read 120hz gamma part2 error rc=%d", rc);
		goto read_120hz_gamma_error;
	} else {
		DSI_INFO("read 120hz gamma part2:rc=%d.data=", rc);
		for (index = EA8079_120HZ_OTP_GAMMA_PART1; index < EA8079_120HZ_OTP_GAMMA_PART1 + EA8079_120HZ_OTP_GAMMA_PART2; index++) {
			pr_cont("0x%02X ", gamma_cfg->temp_buf[index]);
		}
	}
	//read 120hz part3
	rc = vdisp->panel_ops.dsi_read_cmd(display, reg_120hz_part3, &gamma_cfg->temp_buf[EA8079_120HZ_OTP_GAMMA_PART1 + EA8079_120HZ_OTP_GAMMA_PART2], EA8079_120HZ_OTP_GAMMA_PART3);
	if (rc < 0) {
		DSI_ERR("dsi read 120hz gamma part3 error rc=%d", rc);
		goto read_120hz_gamma_error;
	} else {
		DSI_INFO("read 120hz gamma part3:rc=%d.data=", rc);
		for (index = EA8079_120HZ_OTP_GAMMA_PART1 + EA8079_120HZ_OTP_GAMMA_PART2; index < EA8079_120HZ_OTP_GAMMA_PART1 + EA8079_120HZ_OTP_GAMMA_PART2 + EA8079_120HZ_OTP_GAMMA_PART3; index++) {
			pr_cont("0x%02X ", gamma_cfg->temp_buf[index]);
		}
	}
	...
	vdisp->panel_ops.dsi_write_dcs_cmd(vdisp, 0xF0, (u8[]){ 0xA5, 0xA5 }, 2);

	//compare checksum
	checksum = 0;
	for(index = 0;index < EA8079_120HZ_OTP_GAMMA_LENGTH;index++) {
		checksum += gamma_cfg->temp_buf[index];
	}
	DSI_INFO("cal 120hz checksum is 0x%X\n", checksum);
	if (gamma_cfg->gamma_120hz_checksum == checksum)
		DSI_INFO("120Hz gamma parameters check successuful\n");
	else {
		DSI_INFO("120Hz gamma parameters check fail\n");
		goto read_120hz_gamma_error;
	}
	//valid
	gamma_cfg->gamma_120hz_valid = true;

	//fill in cmd sets
	if (gamma_cfg->gamma_120hz_valid)
	{
		struct dsi_display_mode *mode;
		struct dsi_cmd_desc *cmds;
		u8 *tx_buf = NULL;
		rc = dsi_display_find_mode_by_refresh_rate(display, 120, &mode);
		if (rc) {
			DSI_ERR("can't find 120hz mode.");
			goto read_120hz_gamma_error;
		}
		if (dsi_panel_check_gamma_cmd(mode->priv_info->cmd_sets[DSI_CMD_SET_120HZ_GAMMA]))
			goto read_120hz_gamma_error;
		cmds = mode->priv_info->cmd_sets[DSI_CMD_SET_120HZ_GAMMA].cmds;
		tx_buf = (u8 *)cmds[2].msg.tx_buf;
		memcpy(&tx_buf[1], gamma_cfg->temp_buf, EA8079_B8_GAMMA_LENGTH);
		tx_buf = (u8 *)cmds[3].msg.tx_buf;
		memcpy(&tx_buf[1], &gamma_cfg->temp_buf[EA8079_B8_GAMMA_LENGTH], EA8079_B9_GAMMA_LENGTH);
	...
	DSI_INFO("get ea8079 gamma param successful.");
	return;
read_90hz_gamma_error:
	gamma_cfg->gamma_90hz_valid = false;
read_120hz_gamma_error:
	gamma_cfg->gamma_120hz_valid = false;

	DSI_ERR("error");
	return;
}
EXPORT_SYMBOL(dsi_panel_update_ea8079_dynamic_gamma);
/*vivo ouenhui add for dynamic gamma function end*/

extern unsigned int recoverymode;
extern unsigned int survivalmode;
...
/* vivo display team add for pixelworks memc begin */
/* #if defined(CONFIG_PXLW_IRIS) */
	if ((!strcmp(boot_displays[0].name, "qcom,mdss_dsi_pd2055_amb656_ea8079_fhd_plus_cmd"))) {
		if ((recoverymode || survivalmode) && vdisp && vdisp->panel_ops.dsi_write_dcs_cmd) {
			vdisp->panel_ops.dsi_write_dcs_cmd(vdisp,       0x51, (u8[]){ 0x00, 0x00 }, 2);
			mdelay(500);
		}
...
/* vivo display team add for pixelworks memc end */
		...
		*/
		...
		if (!strcmp(vdisp->project_name, "pd2001") || !strcmp(vdisp->project_name, "pd2005")) {
			char reg[PANEL_TX_MAX_BUF] = { 0 };

			/* default panel_id3 is 0x02 */
			vdisp->panel_id3 = 0x02;

			if (vdisp->panel_ops.dsi_read_cmd) {
				vdisp->panel_ops.dsi_read_cmd(display, 0xDC, reg, 1);
				if (!rc)
					vdisp->panel_id3 = reg[0];
			} else {
				vdisp->panel_id3 = 0x02;
			}
			pr_info("dsi panel id3 = 0x%x\n", vdisp->panel_id3);
		}

		dsi_panel_acquire_panel_lock(display->panel);
		if (vdisp->colour_gamut_state && !IS_ERR_OR_NULL(vdisp->panel_ops.update_seed_crc)) {
			vdisp->panel_ops.update_seed_crc(vdisp, vdisp->colour_gamut_state);
		}
		dsi_panel_release_panel_lock(display->panel);

		/*read ea8079 90hz/120hz gamma param*/
		if (vdisp->ea8079_dynamic_gamma.is_support)
			dsi_panel_update_ea8079_dynamic_gamma(display);
	...
	vdisp = display->panel->panel_private;
	if(vdisp)
		vdisp->esd.is_power_state_check = true;
	goto error;
