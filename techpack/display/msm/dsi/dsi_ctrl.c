				...
				unsigned long error)
{
	...
	if (error & 0xF0000) {
		...
		if (cb_info.event_cb && !(mask & 0xf0000)) {
			...
			DSI_CTRL_ERR(dsi_ctrl, "dsi FIFO OVERFLOW error: 0x%lx\n",
					error);
			//lcm_error_code_report(VIVO_DSI_FIFO_OVERFLOW);
			/*Add for vivo LCM driver exception event*/
			atomic_inc(&vivo_exception_count[VIVO_DSI_FIFO_OVERFLOW]);
			/*Add for vivo LCM driver exception event*/
		}
	}
	...
	if (error & 0xF00000) {
		...
		DSI_CTRL_ERR(dsi_ctrl, "dsi FIFO UNDERFLOW error: 0x%lx\n",
				error);
		//lcm_error_code_report(VIVO_DSI_FIFO_UNDERFLOW);
		/*Add for vivo LCM driver exception event*/
		atomic_inc(&vivo_exception_count[VIVO_DSI_FIFO_UNDERFLOW]);
		/*Add for vivo LCM driver exception event*/
	}
	...
	if (error & BIT(8)) {
		...
		lcm_error_code_report(VIVO_DSI_PLL_UNLOCK);
		/*Add for vivo LCM driver exception event*/
		atomic_inc(&vivo_exception_count[VIVO_DSI_PLL_UNLOCK]);
		/*Add for vivo LCM driver exception event*/
	}
...
static irqreturn_t dsi_ctrl_isr(int irq, void *ptr)
{
	...
	if (status & DSI_CMD_MODE_DMA_DONE) {
		/*add by ouenhui for ud perf start*/
		struct vivo_display *vdd  = get_vivo_vdisp();
		if (atomic_read(&vdd->hbm_wait_te_status) == DSI_PANEL_WAIT_HBM_CMD_DONE) {
			atomic_set(&vdd->hbm_wait_te_status, DSI_PANEL_WAIT_ONE_TE);
			pr_err("%s:set hbm_wait_te_status = %d", __func__, atomic_read(&vdd->hbm_wait_te_status));
		}
		/*add by ouenhui for ud perf end*/
	...
	if (*flags & DSI_CTRL_CMD_READ) {
		...
		if (rc) {
			DSI_CTRL_ERR(dsi_ctrl, "command msg transfer failed, rc = %d\n",
					rc);
			/*Add for vivo LCM driver exception event*/
			atomic_inc(&vivo_exception_count[VIVO_COMMAND_TRANSFER_FAIL]);
			/*Add for vivo LCM driver exception event*/
		}
	}
