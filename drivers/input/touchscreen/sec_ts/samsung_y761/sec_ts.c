...
int sec_ts_i2c_write(struct sec_ts_data *ts, u8 reg, u8 *data, int len)
{
	...
	for (retry = 0; retry < SEC_TS_I2C_RETRY_CNT; retry++) {
		...
		if (retry > 1) {
			...
			data_collect_increase(TOUCH_VCODE_I2C_EVENT);
			#endif
		}
	}
	...
	do {
		...
		case SEC_TS_STATUS_EVENT:
			...
			if (channel_comp) {
				...
				if(Tx_num || Rx_num) {
					;
					//vts_report_channel_comp_data(Tx_num, Rx_num, Tx_bit, Rx_bit);
				}
			}
...
#ifdef TOUCH_DATA_COLLECT
				data_collect_increase(TOUCH_VCODE_UNEXPECTED_RESET_EVENT);
				...
				if ((p_event_status->stype == TYPE_STATUS_EVENT_VENDOR_INFO) &&
					(p_event_status->status_id == SEC_TS_VENDOR_ACK_HSYNC_IMMIGRATE)) {
						...
						if (hsync_freq < hsync_lower || (hsync_freq > hsync_upper && hsync_freq < 30000))
							data_collect_increase(TOUCH_VCODE_HSYNC_EVENT);
				}
			}				
#endif
...
{	
	...
	if (0 != is_ng_panel[0]) {
		VTI("is_ng_panel: %X \n", is_ng_panel[0]);		
		data_collect_increase(TOUCH_VCODE_NG_PANEL_DETECT);
	}
...
static int bbk_slsi_set_input_method(struct vts_device *vtsdev, int state)
{
	...
	para = state ? 1 : 0;
	ret = ts->sec_ts_i2c_write(ts, 0x48, &para, 1);
	if (ret < 0)
...
int sec_ts_dump_fw_data(struct vts_device *vtsdev, int state)
{
	...
	memset(read_event_buff, 0, MAX_EVENT_COUNT * SEC_TS_EVENT_BUFF_SIZE);
	ret = sec_ts_i2c_read(ts, SEC_TS_READ_ONE_EVENT, (u8 *)read_event_buff[0], SEC_TS_EVENT_BUFF_SIZE);
	if (ret < 0) {
	...
	for (i = 0; i < (left_event_count + 1); i++) {
		VTI("%02x %02x %02x %02x %02x %02x %02x %02x", read_event_buff[i][0], read_event_buff[i][1], read_event_buff[i][2],
					read_event_buff[i][3], read_event_buff[i][4], read_event_buff[i][5], read_event_buff[i][6], read_event_buff[i][7]);
	}
