...
void reg_dmav1_setup_dspp_pccv4(struct sde_hw_dspp *ctx, void *cfg)
{
	...
	u32 reg = 0;
	u32 num_of_mixers, blk = 0;
	/*vivo ouenhui modify for trace color_matrix start*/
	static u32 pre_data;
	/*vivo ouenhui modify for trace color_matrix end*/
	...
	/*{
		u32 temp_i = 0;
		pr_err("%s [color_matrix]:dump_data", __func__);
		for(temp_i = 0 ; temp_i < PCC_LEN/sizeof(u32) ; temp_i++){
			pr_cont("%d ", data[temp_i]);
		}
		pr_err("\n%s [color_matrix]:end", __func__);
	}*/
	/*Tracking color_ Matrix change information*/
	if ((data[3] == 0 && pre_data != 0) ||
		(data[3] != 0 && pre_data == 0))
		pr_info("%s:pre_data=%d,cur_data=%d", __func__, pre_data, data[3]);
	pre_data = data[3];
	/*vivo ouenhui modify for trace color_matrix end*/
